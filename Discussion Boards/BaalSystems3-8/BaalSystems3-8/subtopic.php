...
if (session_is_registered("whossession")) {
    if (($_SESSION['who']) == "admin" || $_SESSION['who'] == "moderator") {
        $query = "select views from {$tableprefix}tblforum where forumid=\"" . intval($_GET['fid']) . "\";";
        ...
        $debut = ($page - 1) * $nb;
        $fid = $_GET['fid'];
        $result1 = db_query("select count(*) as count1 from {$tableprefix}tblsubforum where forumid=" . intval($fid));
  ...
  <tr>
    <td colSpan="2"><table border="0" cellPadding="0" cellSpacing="0" width="100%">
          ...
          <td class="q"><table border="0" cellPadding="5" cellSpacing="1" width="100%">
              ...
              <tr class="c">
                <td colSpan="6"><table cellPadding="2" cellSpacing="0" width="100%">
                    <tbody>
                      <tr>
                        <td noWrap><span class="c"><a href="<?php if ($_SESSION['who'] == 'admin') {
            echo "admin";
        } else {
            echo "mod";
        } 
        ...
        ?>.php" id="lnk" style="COLOR: #ffffff; TEXT-DECORATION: none" title="Click here to refresh the latest topics...">Index</a></span></td>
                        ...
                        <td align="right"><b><a href="addpost.php?fid=<?php echo($fid);
...
&nbsp;Subject&nbsp;</td>
                ...
                <td class="f" noWrap width="53%">&nbsp;Post&nbsp;</td>
                <td class="f" noWrap width="21%">&nbsp;Date / Time&nbsp;</td>
                <td class="f" noWrap width="7%">&nbsp;Action&nbsp;</td>
                <td class="f" noWrap width="7%">&nbsp;Action&nbsp;</td>
              </tr>
              <?php
        $bgrow = 0;
        ...
        while ($row = db_fetch_row($result)) {
            if (is_integer($i / 2)) {
              ...
              <tr class="a">
                ...
                ?></b><br><?php $result4 = db_query("select * from {$tableprefix}tbluser where username='" . $row["authorname"] . "'"); 
		$row4 = db_fetch_row($result4);
		echo($row4["level"]); ?><br><a href="userprofile.php?username=<?php print($row["authorname"]); ?>" title="Profile">Profile</a>
                  <div class="s">
                    <?=$image;
                ...
                ?></td>
                <td class="s"><a href="deletetopic.php?sfid=<?php echo($row["subforumid"]);

                ?>&fid1=<?php echo($row["forumid"]);

                ?>" title="delete">delete</a></td>
                <td class="s"><a href="edittopic.php?sfid=<?php echo($row["subforumid"]);
              ...
              <?php } else {
                ...
                ?>
              <tr class="b">
                <td><b><img alt border="0" hspace="2" src="pics/user2.gif" width="16" height="17"><?php echo($row["authorname"]);

                ?></b></a><br><?php $result4 = db_query("select * from {$tableprefix}tbluser where username='" . $row["authorname"] . "'"); 
		$row4 = db_fetch_row($result4);
		echo($row4["level"]); ?><br><a href="userprofile.php?username=<?php print($row["authorname"]); ?>" title="Profile">Profile</a>
                  <div class="s">
                    <?=$image;
        ...
        ?>
                <td class="f">&nbsp;<a href="addpost.php?fid=<?php echo($fid);
