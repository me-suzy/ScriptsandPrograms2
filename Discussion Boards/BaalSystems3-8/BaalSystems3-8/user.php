<?php
include("common.php");
// echo "Reg :".session_is_registered("whossession")."  Who :".$_SESSION['who']."<br>";
if (session_is_registered("whossession")) {
    if (($_SESSION['who']) == "user") {
		...
		$query2 = db_query("select * from {$tableprefix}tbluser where username='" . $username1 . "'");
		$row = db_fetch_row($query2);
		$date = $row["joindate"];
		$level = $row["level"];
		$days = (strtotime(date(Y-m-d)) - strtotime($date)) / (60 * 60 * 24);
		if ($level = 'New User'&&$days >= 30) {
			db_query("UPDATE {$tableprefix}tbluser set level='Experienced User' WHERE username='" . $username1 . "'");
			}
		if ($level = 'Experienced User'&&$days >= 90) {
			db_query("UPDATE {$tableprefix}tbluser set level='Expert User' WHERE username='" . $username1 . "'");
			}
		if ($level = 'Expert User'&&$days >= 180) {
			db_query("UPDATE {$tableprefix}tbluser set level='Professional' WHERE username='" . $username1 . "'");
			}
		if ($level = 'Professional'&&$days >= 360) {
			db_query("UPDATE {$tableprefix}tbluser set level='Master' WHERE username='" . $username1 . "'");
			}
        ?>
<html>
<head>
  ...
  <tbody>
    ...
    <?php
        ...
        if ($msgsystem == y) {
            ...
            ?>
    <tr>
	   <td><b>Welcome <?php echo($_SESSION['username']); ?>, to your user section</b>
       </td>
	</tr>
	<tr>
      <td><b>You have <?php echo $msgrow["row_count"] ?> unread msgs. <a href="usermsgs.php">Click here </a> to read your msgs.</b>
      <td>
    </tr>
    <?php } 
    ...
    <tr>
      <td colSpan="2"><table border="0" cellPadding="0" cellSpacing="0" width="100%">
        ...
        ?>.php" id="lnk" style="COLOR: #ffffff; TEXT-DECORATION: none" title="Click here to refresh the latest topics..."> Index</a></span></td>
                    ...
                    <tr class="z">
                      <td class="f" noWrap width="70%">&nbsp;<a style="color: #ffffff; font-size: 10px; text-decoration: none" href="user?sort=sj&order=<?=(($sort == "sj" && $order == "as") ? "de" : "as");
        ...
        ?>
&nbsp;</td>
                      <td class="f" noWrap width="10%">&nbsp;<a style="color: #ffffff; font-size: 10px; text-decoration: none" href="user?sort=au&order=<?=(($sort == "au" && $order == "as") ? "de" : "as");
        ...
        ?>
&nbsp;</td>
                      <td class="f" noWrap width="10%">&nbsp;<a style="color: #ffffff; font-size: 10px; text-decoration: none" href="user?sort=lp&order=<?=(($sort == "lp" && $order == "as") ? "de" : "as");
        ...
        ?>
&nbsp;</td>
                      <td class="f" noWrap width="5%">&nbsp;<a style="color: #ffffff; font-size: 10px; text-decoration: none" href="user?sort=tp&order=<?=(($sort == "tp" && $order == "as") ? "de" : "as");
        ...
        ?>
&nbsp;</td>
                      <td class="f" noWrap width="5%">&nbsp;<a style="color: #ffffff; font-size: 10px; text-decoration: none" href="user?sort=vs&order=<?=(($sort == "vs" && $order == "as") ? "de" : "as");
...
&nbsp;</td>
        ...
        foreach ($unposes as $key => $row) {
            if (is_integer($i / 2)) {
                ...
                ?>
                    <tr class="a">
                      <td><a class="v" onfocus="blur();" href="subtopic1.php?fid=<?php echo($row["forumid"]);
                ...
                ?></b></a>
                        <div class="s"> <a class="v" onfocus="blur();" href="subtopic1.php?fid=<?php echo($row["forumid"]);
