...
if (session_is_registered("whossession")) {
    if (($_SESSION['who']) == "admin") {
        if (!empty($_POST['moderator'])) {
            ...
            $result = db_query("delete from {$tableprefix}tbluser where username='" . $mod . "'");
        } 

        ?>
<?php

        $result = db_query("select * from {$tableprefix}tbluser WHERE userrole=\"moderator\";");
...
<FORM action=deletemod.php method=post name=y>
  <TABLE align=center border=0 cellPadding=0 cellSpacing=0 width=280>
    ...
    <TABLE border=0 cellPadding=5 cellSpacing=1 width="100%">
      ...
      <TABLE cellPadding=1 cellSpacing=0 width="100%">
        <TBODY>
          <TR>
            <TD noWrap><SPAN class=c>Delete Mod</SPAN></TD>
            <TD noWrap>
            <SPAN class=c>
      ...
      <TR class=a>
        ...
        ?>
              <TR>
                ...
                <TD align=center><select name="moderator">
