...
if (!empty($_POST['name'])) {
    ...
    $result1 = db_query("select * from {$tableprefix}tbluser where username='" . $username . "' and password='" . $password . "' and userrole=\"moderator\"");
    $rows = db_num_rows($result1);
    $row = db_fetch_array($result1);
    if ($rows != 0) {
        if (session_is_registered("whossession")) {
            $_SESSION['who'] = "moderator";
        ...
        header("location:modlogin.php?error=yes");
    } 
} else {
...
<FORM action=modlogin.php method=post name=y
            ...
            <TBODY>
              <TR class=c>
                <TD><TABLE cellPadding=1 cellSpacing=0 width="100%">
                    <TBODY>
                      <TR>
                        <TD noWrap><SPAN class=c>Moderator Login </SPAN></TD>
