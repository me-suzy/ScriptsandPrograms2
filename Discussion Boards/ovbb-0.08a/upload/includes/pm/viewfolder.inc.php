...
<TR>
	...
	<TD width="100%" align=left valign=top class=medium><B><A href="index.php"><?php echo(htmlspecialchars($CFG['general']['name'])); ?></A> &gt; <A href="usercp.php">User Control Panel</A> &gt; <A href="private.php">Private Messages</A> &gt; <?php echo($strFolder); ?></B></TD>
</TR>
</TABLE><BR>
...
<?php
	...
	reset($aFolders);
	while(list($iFldrID) = each($aFolders))
	{
		$strFolder = htmlspecialchars($aFolders[$iFldrID]);
		if($iFldrID == $iFolderID)
		{
			echo("			<OPTION value=\"$iFldrID\" selected>$strFolder</OPTION>\n");
		}
		else
		{
...
<TR>
	...
	<TD bgcolor="<?php echo($CFG['style']['table']['heading']['bgcolor']); ?>" class=smaller width=15><IMG src="images/space.png" width=15 height=1 alt=""></TD>
	<TD bgcolor="<?php echo($CFG['style']['table']['heading']['bgcolor']); ?>" align=center width="60%"><FONT class=smaller color="<?php echo($CFG['style']['table']['heading']['txtcolor']); ?>"><B>Message Subject</B></FONT></TD>
	<TD bgcolor="<?php echo($CFG['style']['table']['heading']['bgcolor']); ?>" align=center width="20%" nowrap><FONT class=smaller color="<?php echo($CFG['style']['table']['heading']['txtcolor']); ?>"><B>Author</B></FONT></TD>
...
<?php
	...
	while(list($iMessageID) = each($aMaster))
	{
		...
		$iAuthorID = $aMaster[$iMessageID][AUTHOR];
		...
		$bTracking = $aMaster[$iMessageID][TRACKING];
...
<TR>
	...
	<TD bgcolor="<?php echo($CFG['style']['table']['cella']); ?>" class=medium><A href="private.php?action=view&amp;item=message&amp;id=<?php echo($iMessageID); ?>"><?php echo($strMessageSubject); ?></A><?php if((!$bRead)&&($bTracking)){echo(" <FONT class=smaller>[<A href=\"private.php?action=view&amp;item=message&amp;id=$iMessageID&amp;noreceipt=1\">Deny receipt.</A>]</FONT>");} ?></TD>
	<TD bgcolor="<?php echo($CFG['style']['table']['cellb']); ?>" class=medium align=center valign=middle nowrap><A href="profile.php?userid=<?php echo($iAuthorID); ?>"><?php echo($aUsernames[$iAuthorID]); ?></A></TD>
...
<TR><TD bgcolor="<?php echo($CFG['style']['table']['heading']['bgcolor']); ?>" class=smaller colspan=7>&nbsp;</TD></TR>
