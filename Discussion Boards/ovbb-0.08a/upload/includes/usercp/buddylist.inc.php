...
<TR>
	...
	<TD width="100%" align=left valign=top class=medium><B><A href="index.php"><?php echo(htmlspecialchars($CFG['general']['name'])); ?></A> &gt; <A href="usercp.php">User Control Panel</A> &gt; Edit Buddy List</B></TD>
</TR>
</TABLE><BR>
...
<FORM style="margin: 0px;" name=theform action="usercp.php?section=buddylist" method=post>
<INPUT type=hidden name=posting value=1>

<TABLE cellpadding=4 cellspacing=1 border=0 bgcolor="<?php echo($CFG['style']['table']['bgcolor']); ?>" width=200 align=center>

<TR><TD bgcolor="<?php echo($CFG['style']['table']['section']['bgcolor']); ?>" colspan=2 align=center class=medium style="color: <?php echo($CFG['style']['table']['section']['txtcolor']); ?>;"><B>Edit Buddy List</B></TD></TR>
...
<?php
	...
	if(is_array($aUsernames))
	{
		foreach($aUsernames as $strUsername)
		{
			...
			echo("		<INPUT class=tinput type=text name=\"buddylist[]\" value=\"$strUsername\" size=30 maxlength=16><BR>\n");
		}
	}
?>
		<INPUT class=tinput type=text name="buddylist[]" size=30 maxlength=16><BR>
		<INPUT class=tinput type=text name="buddylist[]" size=30 maxlength=16>
	</TD>
</TR>
