...
//***************************************************************************//
	...
	$sqlResult = sqlquery("SELECT id, name, parent FROM board WHERE type=0 OR id=$iForumID");
	...
	// Header.
	$strPageTitle = " :: $strForumName :. New Thread";
...
<TR>
	...
	<TD width="100%" align=left valign=top class=medium><B><A href="index.php"><?php echo(htmlspecialchars($CFG['general']['name'])); ?></A> &gt; <A href="forumdisplay.php?forumid=<?php echo($iCategoryID); ?>"><?php echo($strCategoryName); ?></A> &gt; <A href="forumdisplay.php?forumid=<?php echo($iForumID); ?>"><?php echo($strForumName); ?></A></B></TD>
</TR>
</TABLE>
...
<FORM style="margin: 0px;" name=theform action="newthread.php?forumid=<?php echo($iForumID); ?>" enctype="multipart/form-data" method=post>
<TABLE cellpadding=4 cellspacing=1 border=0 bgcolor="<?php echo($CFG['style']['table']['bgcolor']); ?>" width="100%" align=center>

<TR>
	<TD bgcolor="<?php echo($CFG['style']['table']['heading']['bgcolor']); ?>" colspan=2 class=medium style="color: <?php echo($CFG['style']['table']['heading']['txtcolor']); ?>;"><B>Post New Thread</B></TD>
</TR>
...
<TR>
	<TD bgcolor="<?php echo($CFG['style']['table']['cellb']); ?>" class=medium nowrap><B>Thread Subject</B></TD>
	<TD bgcolor="<?php echo($CFG['style']['table']['cellb']); ?>"><INPUT class=tinput type=text name=subject size=40 maxlength=64 value="<?php echo(htmlspecialchars($strSubject)); ?>"></TD>
</TR>

<TR>
	<TD bgcolor="<?php echo($CFG['style']['table']['cellb']); ?>" class=medium nowrap><B>Thread Description</B></TD>
	<TD bgcolor="<?php echo($CFG['style']['table']['cellb']); ?>" class=smaller><INPUT class=tinput type=text name=description size=40 maxlength=128 value="<?php echo(htmlspecialchars($strDescription)); ?>"> (Optional)</TD>
</TR>
...
<TR>
	...
	<TABLE cellpadding=0 cellspacing=0 border=0>
		...
		<TR>
			<TD valign=top><INPUT type=checkbox name=parseemails<?php if($bParseEMails){echo(' checked');} ?>></TD>
		...
		<TR><TD colspan=2><IMG src="images/space.png" width=1 height=3 alt=""></TD></TR>
		<TR>
			<TD valign=top><INPUT type=checkbox name=dsmilies<?php if($bDisableSmilies){echo(' checked');} ?>></TD>
...
<TR>
	...
	<TD bgcolor="<?php echo($CFG['style']['table']['cella']); ?>">
		<TABLE cellpadding=0 cellspacing=0 border=0>
		<TR>
			<TD align=left valign=top><INPUT type=checkbox name=makepoll<?php if($bMakePoll){echo(' checked');} ?>></TD>
		...
		<TR>
			...
			<TD><INPUT class=tinput type=text name=numchoices size=5 value="<?php echo($iNumberChoices); ?>"></TD>
...
<CENTER><BR><INPUT class=tinput type=submit name=submit value="Submit Thread" accesskey="s"> <INPUT class=tinput type=submit name=submit value="Preview Post" accesskey="p"></CENTER>
</FORM><BR>
...
{
	global $CFG, $aPermissions, $iForumID;
	...
	$strDescription = mysql_real_escape_string($strDescription);

	// Icon
	if(($iThreadIcon < 0) || ($iThreadIcon > 14))
	{
	...
	{
		// No user logged in.
		$aError[] = 'You must be logged in to post new threads.';
	}
	...
	if($bMakePoll && $aPermissions['cmakepolls'])
	{
	...
	sqlquery("INSERT INTO post(author, datetime_posted, title, body, ipaddress, icon, dsmilies) VALUES({$_SESSION['userid']}, {$CFG['globaltime']}, '$strSubject', '$strMessage', $iAuthorIP, $iThreadIcon, $bDisableSmilies)");
	...
	sqlquery("INSERT INTO thread(title, description, parent, viewcount, lposter, icon, author, poll, open, visible, sticky) VALUES('$strSubject', '$strDescription', $iForumID, 0, {$_SESSION['userid']}, $iThreadIcon, {$_SESSION['userid']}, $strPatch, 0)");

	// Again, before we continue, we need the ID of the thread we created.
	$iThreadID = mysql_insert_id();

	// Now with the thread ID, we can update the post record we originally created.
	sqlquery("UPDATE post SET parent=$iThreadID WHERE id=$iPostID");

	// Only do these if we're not posting a poll.
	if((!$bMakePoll) || (!$aPermissions['cmakepolls']))
	{
		// Third, we need to update the record of the forum that contains the thread we just created.
		...
		// Fourth, we need to update the poster's profile.
	...
	if($bMakePoll && $aPermissions['cmakepolls'])
	{
		$strRedirect = "poll.php?action=newpoll&amp;threadid=$iThreadID&amp;numchoices=$iNumberChoices";
	}
	else
	{
		$strRedirect = "thread.php?threadid=$iThreadID";
	}
	Msg("<b>Thank you for posting. You should be redirected to your post momentarily.</b><br><br><font class=\"smaller\">Click <a href=\"$strRedirect\">here</a> if you do not want to wait any longer or if you are not redirected.</font>", html_entity_decode($strRedirect), 'center');
}
?>