 ...
 *	   $Id: admin_user_search.php,v 1.8 2004/11/17 22:45:16 rasadam Exp $
 *    
 *
 ****************************************************************************/
...
else
{
	...
	switch($mode)
	{
		case 'search_username':
			$username = ( isset($HTTP_GET_VARS['username']) ) ? rawurldecode($HTTP_GET_VARS['username']) : $HTTP_POST_VARS['username'];
			...
			break;
		case 'search_email':
			$email = ( isset($HTTP_GET_VARS['email']) ) ? rawurldecode($HTTP_GET_VARS['email']) : $HTTP_POST_VARS['email'];
			...
			break;
		case 'search_ip':
			$ip_address = ( isset($HTTP_POST_VARS['ip_address'] ) ) ? $HTTP_POST_VARS['ip_address'] : rawurldecode($HTTP_GET_VARS['ip_address']);
		...
		case 'search_joindate':
			$date_type = ( isset($HTTP_POST_VARS['date_type'] ) ) ? $HTTP_POST_VARS['date_type'] : rawurldecode($HTTP_GET_VARS['date_type']);
			$date_day = ( isset($HTTP_POST_VARS['date_day'] ) ) ? $HTTP_POST_VARS['date_day'] : rawurldecode($HTTP_GET_VARS['date_day']);
			$date_month = ( isset($HTTP_POST_VARS['date_month'] ) ) ? $HTTP_POST_VARS['date_month'] : rawurldecode($HTTP_GET_VARS['date_month']);
			$date_year = ( isset($HTTP_POST_VARS['date_year'] ) ) ? $HTTP_POST_VARS['date_year'] : rawurldecode($HTTP_GET_VARS['date_year']);
			...
			break;	
		case 'search_group':
			$group_id = ( isset($HTTP_POST_VARS['group_id'] ) ) ? $HTTP_POST_VARS['group_id'] : rawurldecode($HTTP_GET_VARS['group_id']);
			if(!$group_id)
			{
		...
		case 'search_postcount':
			$postcount_type = ( isset($HTTP_POST_VARS['postcount_type'] ) ) ? $HTTP_POST_VARS['postcount_type'] : rawurldecode($HTTP_GET_VARS['postcount_type']);
			$postcount_value = ( isset($HTTP_POST_VARS['postcount_value'] ) ) ? $HTTP_POST_VARS['postcount_value'] : rawurldecode($HTTP_GET_VARS['postcount_value']);
		...
		case 'search_userfield':
			$userfield_type = ( isset($HTTP_POST_VARS['userfield_type'] ) ) ? $HTTP_POST_VARS['userfield_type'] : rawurldecode($HTTP_GET_VARS['userfield_type']);
			$userfield_value = ( isset($HTTP_POST_VARS['userfield_value'] ) ) ? $HTTP_POST_VARS['userfield_value'] : rawurldecode($HTTP_GET_VARS['userfield_value']);
		...
		case 'search_language':
			$language_type = ( isset($HTTP_POST_VARS['language_type'] ) ) ? $HTTP_POST_VARS['language_type'] : rawurldecode($HTTP_GET_VARS['language_type']);
		...
		case 'search_timezone':
			$timezone_type = ( isset($HTTP_POST_VARS['timezone_type'] ) ) ? $HTTP_POST_VARS['timezone_type'] : rawurldecode($HTTP_GET_VARS['timezone_type']);
			...
			break;
		case 'search_style':
			$style_type = ( isset($HTTP_POST_VARS['style_type'] ) ) ? $HTTP_POST_VARS['style_type'] : rawurldecode($HTTP_GET_VARS['style_type']);
		...
		case 'search_moderators':
			$moderators_forum = ( isset($HTTP_POST_VARS['moderators_forum'] ) ) ? $HTTP_POST_VARS['moderators_forum'] : rawurldecode($HTTP_GET_VARS['moderators_forum']);
		...
		case 'search_misc':
		default:
			$misc = ( isset($HTTP_POST_VARS['misc'] ) ) ? $HTTP_POST_VARS['misc'] : rawurldecode($HTTP_GET_VARS['misc']);
			if(!$misc)
			{
	...
	$select_sql = "SELECT u.user_id, u.username, u.user_email, u.user_posts, u.user_regdate, u.user_level, u.user_active
	...
	switch($mode)
	{
		...
		case 'search_postcount':
			...
			switch($postcount_type)
			{
				case 'greater':
					...
					$text = sprintf($lang['Search_by_postcount_greater'], $postcount_value);
				...
				case 'lesser':
					...
					$text = sprintf($lang['Search_by_postcount_lesser'], $postcount_value);
				...
				case 'equals':
					...
					if(strstr($postcount_value, '-'))
					{
						...
						$text = sprintf($lang['Search_by_postcount_range'], $range_begin, $range_end);
					...
					else
					{
						...
						$text = sprintf($lang['Search_by_postcount_equals'], $postcount_value);
		...
		case 'search_style':
			...
			break;
		case 'search_moderators':
			$base_url .= "&search_moderators=true&style_type=".rawurlencode($moderators_forum);
