 ...
 *
 * Administration for the Syntax Highlighter MOD. For help with this mod,
 * please use one of these threads:
 *  http://phpbb.com/phpBB/viewtopic.php?t=217723 (CHECK)
 *  http://qbnz.com/phpBB
 ***************************************************************************/
...
function clear_cache ()
{
	...
	$dh = @opendir($phpbb_root_path . 'cache/syntax') or message_die(GENERAL_ERROR, 'Could not open the cache directory for reading');
	$file = readdir($dh);
	...
	while ( $file !== false )
	{
		if ( strlen($file) == 36 )
		{
			@unlink($phpbb_root_path . 'cache/syntax/' . $file) or message_die(GENERAL_ERROR, 'Could not delete file ' . $file . ' from the cache check that the directory cache/syntax is CHMODed to 777 and that all files in that directory are CHMODed to 644.');
		}
	...
	/*// Syntax highlighting status: 0 = disabled, 1 = disabled but parse [syntax] blocks as [code], 2 = enabled
	// Whenever you change this value, you should clear the cache - you can do this in the admin panel
	$config['syntax_status'] = 2;
	// Whether to enable the cache directory or not (absolutely recommended). The directory
	// used is $phpbb_root_path . 'cache/syntax/', make sure you CHMOD it to 777.
	$config['syntax_enable_cache'] = true;
	// How often to check the cache dir (in seconds) - best to leave this quite large
	$config['syntax_cache_check_time'] = 5000;
	// Syntax cache directory max size (bytes, 20 megs by default)
	$config['syntax_cache_dir_size'] = 20 * 1024 * 1024;
	// Syntax cache directory file expiry date (files older than this many seconds will be purged, 30 days by default)
	$config['syntax_cache_files_expire'] = 60 * 60 * 24 * 30;
	// Enable line numbers for [syntax] blocks (only works if mod is fully enabled)
	$config['syntax_enable_line_numbers'] = false;
	// Enable function to URL conversion for [syntax] blocks (only works if mod is fully enabled)
	$config['syntax_enable_urls'] = true;*/
...
$change_needed = false;*/
			if ( $change_needed )
			{
				eval('@chmod(\'' . $phpbb_root_path . 'includes/geshi/' . $language . '\', $mode) or message_die(GENERAL_ERROR, \'Could not CHMOD the ' . $language . ' language file\');');
...
else
{
	...
	$syntax_enabled_checked = ( $board_config['syntax_status'] == 2 ) ? ' checked="checked"' : '';
	$syntax_partial_checked = ( $board_config['syntax_status'] == 1 ) ? ' checked="checked"' : '';
	$syntax_disabled_checked = ( $board_config['syntax_status'] == 0 ) ? ' checked="checked"' : '';
