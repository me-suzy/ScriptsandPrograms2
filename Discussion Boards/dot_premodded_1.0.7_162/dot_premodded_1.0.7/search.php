...
else if ( $search_keywords != '' || $search_author != '' || $search_id )
{
	...
	//
		if ( $search_id == 'newposts' || $search_id == 'egosearch' || $search_id == 'unanswered' || $search_keywords != '' || $search_author != ''  || $search_id == 'usertopics')
	{
		if ( $search_id == 'newposts' || $search_id == 'egosearch' || $search_id == 'usertopics' ||( $search_author != '' && $search_keywords == '' )  )
		{
...
// Topics a user has started MOD, By Manipe (End)
	...
	if ( $search_results != '' )
	{
		...
		{
			$sql = "SELECT t.*, f.forum_id, f.forum_name, u.username, u.user_id, u2.username as user2, u2.user_id as id2, p.post_username, p2.post_username AS post_username2, p2.post_time, pt.post_text 
				FROM " . TOPICS_TABLE . " t, " . FORUMS_TABLE . " f, " . USERS_TABLE . " u, " . POSTS_TABLE . " p, " . POSTS_TABLE . " p2, " . USERS_TABLE . " u2, " . POSTS_TEXT_TABLE . " pt
				WHERE t.topic_id IN ($search_results) AND pt.post_id = t.topic_first_post_id
