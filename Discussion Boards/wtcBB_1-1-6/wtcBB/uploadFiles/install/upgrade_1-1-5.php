...
iHeader("wtcBB Upgrade to wtcBB 1.1.5");
...
// uh oh!
if($bboptions['version_text'] != "wtcBB 1.1.4" AND $_GET['step'] < 3) {
	iTitle("Error!");
	print('<p>Sorry, this is an upgrade from wtcBB 1.1.4 to wtcBB 1.1.5.</p>');
	iFooter();
	exit;
}
...
if($_GET['step'] == 1 OR empty($_GET)) {
	...
	print('<p>I have succssfully connected to the database. This process will upgrade your current version, wtcBB 1.1.4 to wtcBB 1.1.5. It is <strong>strongly</strong> recommended that before you proceed with this process, that you have a backup of your database, as if you encounter any problems throughout this upgrade process, you will need to reinstate the old database, and repeat this upgrade process. Click next to proceed.</p>');
	iForm(2,"upgrade_1-1-5.php");
}
	...
	foreach($mysql['upgrade_1-1-5']['update'] as $query) {
		query($query);
	}

	print('<p>Process complete.</p>');
	iForm(3,"upgrade_1-1-5.php");
}
	...
	iForm(4,"upgrade_1-1-5.php");
}

else if($_GET['step'] == 4) {
	iTitle("Upgrade Complete!");
	print('<p>You have successfully upgraded your wtcBB 1.1.4 message board to <strong>wtcBB 1.1.5</strong>.</p>');
