...
iHeader("wtcBB Upgrade to wtcBB 1.0.0");
...
// uh oh!
if($bboptions['version_text'] != "wtcBB 1.0.0 Release Candidate 5" AND $_GET['step'] < 3) {
	iTitle("Error!");
	print('<p>Sorry, this is an upgrade from wtcBB 1.0.0 Release Candidate 5 to wtcBB 1.0.0.</p>');
	iFooter();
	exit;
}
...
if($_GET['step'] == 1 OR empty($_GET)) {
	...
	print('<p>I have succssfully connected to the database. This process will upgrade your current version, wtcBB 1.0.0 Release Candidate 5 to wtcBB 1.0.0. It is <strong>strongly</strong> recommended that before you proceed with this process, that you have a backup of your database, as if you encounter any problems throughout this upgrade process, you will need to reinstate the old database, and repeat this upgrade process. Click next to proceed.</p>');
	iForm(2,"upgrade_1-0-0.php");
}

else if($_GET['step'] == 2) {
	iTitle("Step 2 - Updating and Changing Table data and information");

	print('<p>Updating and Changing Table data and information</p>');

	include("./install_mysql.php");
	query($mysql['upgrade_1-0-0']['update']['user_info']);
	query($mysql['upgrade_1-0-0']['update']['wtcBBoptions']);
	query($mysql['upgrade_1-0-0']['delete']['templates_default']);

	print('<p>Process complete.</p>');
	iForm(3,"upgrade_1-0-0.php");
}

else if($_GET['step'] == 3) {
	iTitle("Step 3 - Inserting Pre-written FAQ Data");

	print('<p>Inserting Pre-written FAQ Data</p>');

	include("./install_mysql.php");
	query($mysql['upgrade_1-0-0']['insert']['faq']);

	print('<p>Process complete.</p>');
	iForm(4,"upgrade_1-0-0.php");
}
	...
	iForm(5,"upgrade_1-0-0.php");
}

else if($_GET['step'] == 5) {
	iTitle("Upgrade Complete!");
	print('<p>You have successfully upgraded your wtcBB 1.0.0 Release Candidate 5 message board to <strong>wtcBB 1.0.0</strong>.</p>');
