...
// ########################################################### \\
// ########################################################### \\
// #### //INSTALL - UPGRADE to 1.0.0 Release Candidate 4\\ ### \\
// ########################################################### \\
// ########################################################### \\
// ########################################################### \\
...
iHeader("wtcBB Upgrade to wtcBB 1.0.0 Release Candidate 4");
...
// uh oh!
if($bboptions['version_text'] != "wtcBB 1.0.0 Release Candidate 3" AND $_GET['step'] < 3) {
	iTitle("Error!");
	print('<p>Sorry, this is an upgrade from wtcBB 1.0.0 Release Candidate 3 to wtcBB 1.0.0 Release Candidate 4.</p>');
	iFooter();
	exit;
}
...
if($_GET['step'] == 1 OR empty($_GET)) {
	...
	print('<p>I have succssfully connected to the database. This process will upgrade your current version, wtcBB 1.0.0 Release Candidate 3 to wtcBB 1.0.0 Release Candidate 4. It is <strong>strongly</strong> recommended that before you proceed with this process, that you have a backup of your database, as if you encounter any problems throughout this upgrade process, you will need to reinstate the old database, and repeat this upgrade process. Click next to proceed.</p>');
	iForm(2,"upgrade_1-0-0_RC_4.php");
}

else if($_GET['step'] == 2) {
	iTitle("Step 2 - Adding Search Indexes");

	print('<p>Adding Search Indexes</p>');

	include("./install_mysql.php");
	foreach($mysql['upgrade_1-0-0_RC_4']['alter'] as $table => $query) {
		query($query);
	}

	print('<p>Process complete.</p>');
	iForm(3,"upgrade_1-0-0_RC_4.php");
}

else if($_GET['step'] == 3) {
	iTitle("Step 3 - Updating and Changing Table data and information");

	print('<p>Updating and Changing Table data and information</p>');

	include("./install_mysql.php");
	query($mysql['upgrade_1-0-0_RC_4']['change']['forums']);
	foreach($mysql['upgrade_1-0-0_RC_4']['update'] as $table => $query) {
		query($query);
	}

	print('<p>Process complete.</p>');
	iForm(4,"upgrade_1-0-0_RC_4.php");
}

else if($_GET['step'] == 4) {
	iTitle("Step 4 - Update Style Information &amp; Templates");
	...
	iForm(5,"upgrade_1-0-0_RC_4.php");
}

else if($_GET['step'] == 5) {
	iTitle("Upgrade Complete!");
	print('<p>You have successfully upgraded your wtcBB 1.0.0 Release Candidate 3 message board to <strong>wtcBB 1.0.0 Release Candidate 4</strong>.</p>');
