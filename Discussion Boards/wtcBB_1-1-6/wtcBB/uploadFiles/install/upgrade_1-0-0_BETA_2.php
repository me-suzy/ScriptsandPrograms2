...
// ########################################################### \\
// ########################################################### \\
// ########## //INSTALL - UPGRADE to 1.0.0 BETA 2\\ ########## \\
// ########################################################### \\
// ########################################################### \\
// ########################################################### \\
...
iHeader("wtcBB Upgrade to wtcBB 1.0.0 BETA 2");
...
// uh oh!
if($bboptions['version_text'] != "wtcBB 1.0.0 BETA 1" AND $_GET['step'] < 4) {
	iTitle("Error!");
	print('<p>Sorry, this is an upgrade from wtcBB 1.0.0 BETA 1 to wtcBB 1.0.0 BETA 2.</p>');
	iFooter();
	exit;
}
...
if($_GET['step'] == 1 OR empty($_GET)) {
	...
	print('<p>I have succssfully connected to the database. This process will upgrade your current version, wtcBB 1.0.0 BETA 1 to wtcBB 1.0.0 BETA 2. It is <strong>strongly</strong> recommended that before you proceed with this process, that you have a backup of your database, as if you encounter any problems throughout this upgrade process, you will need to reinstate the old database, and repeat this upgrade process. Click next to proceed.</p>');
	iForm(2,"upgrade_1-0-0_BETA_2.php");
}
	...
	foreach($mysql['upgrade_1-0-0_BETA_2']['alter'] as $tableName => $value) {
		query($value);
	}

	print('<p>Tables altered. Click next to proceed.</p>');
	iForm(3,"upgrade_1-0-0_BETA_2.php");
}

else if($_GET['step'] == 3) {
	iTitle("Step 3 - Update information");

	print('<p>Updating Information...</p>');

	include("./install_mysql.php");

	foreach($mysql['upgrade_1-0-0_BETA_2']['update'] as $tableName => $value) {
		query($value);
	}

	print('<p>Information was updated successfully. Click next to proceed.</p>');
	iForm(4,"upgrade_1-0-0_BETA_2.php");
}
	...
	iForm(5,"upgrade_1-0-0_BETA_2.php");
}

else if($_GET['step'] == 5) {
	iTitle("Upgrade Complete!");
	print('<p>You have successfully upgraded your wtcBB 1.0.0 BETA 1 message board to wtcBB 1.0.0 BETA 2.</p>');
