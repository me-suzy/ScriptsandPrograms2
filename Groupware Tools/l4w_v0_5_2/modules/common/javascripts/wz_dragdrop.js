...
function my_DropFunc()
{
    ...
    for (i=0; i < dd.elements.length; i++) {
        if (dd.elements[i] == dd.obj) continue;    
        x = dd.elements[i].x;    
        y = dd.elements[i].y;
        w = dd.elements[i].w;
        h = dd.elements[i].h;
        //alert (x + ", " + y + ", " + w + ", " + h)
        if (dd.obj.x >= (x-offset_x) && dd.obj.x <= (x+w+offset_x) &&
            dd.obj.y >= (y-offset_y) && dd.obj.y <= (y+h+offset_y)) {
            //alert ("*"+dd.elements[i].name);
            targetname = dd.elements[i].name;
            hit = true;     
            break;   
        }        
    }
    // move to old coordinate    
    if (hit) {
        source = dd.obj.name.substr(4); 
        target = targetname.substr(4);
        link   = exec_path+"index.php?command=move&"+entry_type+"_"+source+"=on&move_to="+target+"&parent="+currentParent;    
