    ...
    function clear_filters () {
        link = "index.php?command=show_entries&my_group=all&my_owner=all&my_state=all";
        document.location.href=link;
    }    
   ...
   /* function delete_attachment (type, id) {
            ...
            case "add references":
                ...
                for (i = 0; i < inputs.length; i++) {
                    ...
                    if (att.nodeValue.slice (0,reference.length) == reference && elem.checked) {
		        	    ...
		        	    newEntry  = new Option ('Todo (ref. #'+to_ref_id+')','todo_'+to_ref_id,true, true);
        		        orig.added_references.options[position]     = newEntry;
                        orig.new_references.value += 'todo_'+to_ref_id+'|';
                    }   
                }    
                window.close();
                break;
    ...
    function clone_me (show_text) {
        document.formular.command.value="add_todo";
        alert(show_text);
    }
