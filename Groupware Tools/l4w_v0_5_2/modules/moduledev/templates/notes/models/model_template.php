<?php
   ...
   /**
    *
    * @version      $Id: model_template.php,v 1.6 2005/08/01 14:55:13 carsten Exp $
    * @author       ###author###
    * @copyright    ###copyright###
    * @package      ###package###
    */    
    class ###name###_model extends l4w_model {
        ...
        */
        function add_folder ($params) {
            ...
            if (mysql_error() != '') {
                ...
                mysql_query ("DELETE FROM memos WHERE memo_id='$inserted_id'");
      ...
      /**
        ...
        */
        function add_entry ($params) {
            ...
            // --- validate -----------------------------------------
            $validation = $this->###name###_validation();
            if ($validation != "success") return $validation;
            
            // --- inheritance by refering object? ------------------
            /*if ($params['ref_object_type'] != '' && $params['ref_object_id'] > 0) {
                $ref_meta_info = get_entries_for_primary_key(
                                       "metainfo", array ("object_type" => $params['ref_object_type'],
                                                          "object_id"   => $params['ref_object_id']));
            ...
            if (!$res = $this->ExecuteQuery ($add_query, 'mysql_error')) return "failure";
            $inserted_id = mysql_insert_id();
            $this->entry['memo_id']->set ($inserted_id);
      ...
      /**
        ...
        */
        function    update_entry (&$params) {
            ...
            // --- update entry ----------------------------------------
            $contact_query = "UPDATE memos SET 
            ...
            if (isset ($_REQUEST['attachment_content']) && ($_REQUEST['attachment_content'] != '')) {
                ...
                $done = $ref_tab->addAttachment ($this->entry, '###name###', $_REQUEST);
                if ($done != "success") 
                    return "failure";
            }
            ...
            if (isset ($_REQUEST['external_link_path']) && ($_REQUEST['external_link_path'] != '')) {
                ...
                $external_refs = new external_references ($this->entry['memo_id']->get(),'###name###',false); 
      ...
      /**
        ...
        */
        function delete_entry ($params, $use_inheritance = false, $parent_type = NULL) {
            ...
            // --- history, get_old values -----------------------------
            $old_###name###_values = get_entries_for_primary_key (
        ...
        */
        function import_entry ($params) {
            ...
            if ($cnt == 0) { // entry does not yet exist, add it
                ...
                $query    = "SELECT * FROM memos WHERE memo_id=".$sync_row['object_id'];
                if (!$###name###_res = $this->ExecuteQuery ($query, 'mysql_error')) return "failure";
                $###name###_row      = mysql_fetch_array($###name###_res);
                if ($###name###_row['headline'] != $params['headline'] ||
                    $###name###_row['content']  != $params['content']) return "changed";
                return "not changed";
            }        
        }
        ...
        function sync_entry ($params) {
            ...
            switch ($result) {
                ...
                case "changed" :
                    ...
                    $query = "SELECT last_change + 0 AS last_change, ".TABLE_PREFIX."metainfo.state, ".TABLE_PREFIX."metainfo.owner FROM memos
                             ...
                             ";
                    if (!$meta_res = $this->ExecuteQuery ($query, 'mysql_error')) return "failure";
                    ...
                    if ($update_entry) {   
                        ...
                        if ($result == "success") {
                            ...
                            if (!$sync_res = $this->ExecuteQuery ($query, 'mysql_error')) 
                                return "failure";
                        }    
      ...
      /**
        ...
        */
        function show_entries (&$params, $query = null) {
            ...
            if ($_SESSION['use_my_group'] > 0)
                $query .= " AND metainfo.grp=".$_SESSION['use_my_group'];       
            else {
        ...
        function show_locked (&$params) {
            ...
            $query = "
                   SELECT
                    ...
                    LEFT JOIN ".TABLE_PREFIX."gacl_aro_groups ON ".TABLE_PREFIX."metainfo.grp=".TABLE_PREFIX."gacl_aro_groups.id
                    LEFT JOIN ".TABLE_PREFIX."users ON ".TABLE_PREFIX."users.id=".TABLE_PREFIX."metainfo.owner
                    LEFT JOIN ".TABLE_PREFIX."useronline ON ".TABLE_PREFIX."useronline.object_id=".TABLE_PREFIX."memos.memo_id
                    WHERE ".TABLE_PREFIX."metainfo.object_type='".$this->entry_type."' AND
                          ".TABLE_PREFIX."useronline.object_type='".$this->entry_type."' AND
                    	  parent=".$this->entry['parent']->get();
        ...
        */
        function show_entry (&$params) {
            global $db_hdl, $easy, $PING_TIMER, $use_headline;
            ...
            $query ="
                SELECT * FROM memos
                LEFT JOIN metainfo ON ".TABLE_PREFIX."metainfo.object_id=".TABLE_PREFIX."memos.memo_id
      ...
      /**
        ...
        * be moved onto itself, and the taret must be a folder. On success, an event (changed ###name###) is fired.
        * If there are any problems, examine model->error_msg and model->info_msg. 
        * 
        * @access       public
        * @param        array holds request variables
            ...
            $query = "
                SELECT * 
                  ...
                  LEFT JOIN ".TABLE_PREFIX."metainfo
                    ON memos.memo_id=".TABLE_PREFIX."metainfo.object_id                
