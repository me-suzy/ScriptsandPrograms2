...
class CForm extends CLibrary{
	...
	*/
	function SimpleList($form , $items = "", $count = "", $extra = null , $search = false) {
		...
		//hey the search options, this is kinda buggy for this version, will be fixed in other
...
//								$items[$key][$k] = $items[$key][$k] > 0 ? @date($v["params"] , ($v["now"] == "true" ? time() : $items[$key][$k])) : "N/A";
		...
		//crap, append some extra elements to this form, in case, hey, i dont know how to handle if there will be 2 paging functions.
	...
	function DrawElement( $form , $_field , $values , $draw_referer = 0 ) {
		...
		switch ($field["type"]) {
			...
			case "textarea":
				...
				//uhm ... this is a crappy part because i have to integrate the html editor here so ...
				if ($field["html"] == "true") {
