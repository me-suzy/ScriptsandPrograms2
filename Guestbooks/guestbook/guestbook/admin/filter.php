...
if($HTTP_COOKIE_VARS['loged'] == 'yes') {
   if($HTTP_COOKIE_VARS['user_level'] == '1') {
      $sql = "SELECT * FROM ".$prefix."_filter ORDER BY 'word' DESC";
      $result = $db->query($sql);
      ...
      while($row = $db->fetch($result)) {
            ...
            $template->add_block_vars("filter", array(
                                       'ID' => $id,
                                       'WORD' => $word,
                                       'REPLACE' => $replace)
            );
