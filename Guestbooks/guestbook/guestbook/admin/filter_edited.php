...
if($HTTP_COOKIE_VARS['loged'] == 'yes') {
   if($HTTP_COOKIE_VARS['user_level'] == '1') {
      ...
      $sql = "UPDATE ".$prefix."_filter SET filter_word='$HTTP_POST_VARS[word]', filter_replace='$HTTP_POST_VARS[replace]' WHERE id='$HTTP_POST_VARS[id]'";
      $result = $db->query($sql);

      if(!$result) {
         include("page_header.php");
         $display = "Could not edit the word filter";
         $template->getFile(array(
                            'error' => 'admin/error.tpl')
         );
         $template->add_vars(array(
         ...
         include("page_header.php");
         $display = "The word has been edited.";
         $link = "filter.php";
         $template->getFile(array(
