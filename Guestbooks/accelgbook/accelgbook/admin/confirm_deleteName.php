...
<?php
$colname_rsDelete = "1";
if (isset($_POST['deletionName'])) {
  $colname_rsDelete = (get_magic_quotes_gpc()) ? $_POST['deletionName'] : addslashes($_POST['deletionName']);
}
mysql_select_db($database, $accelgook);
$query_rsDelete = sprintf("SELECT * FROM accelgbook WHERE `date` = '%s'", $colname_rsDelete);
...
<body>
<form action="deletedName.php?name=%22.$HTTP_POST_VARS%5B%27deletionName%27%5D.%22" method="POST" name="modify" id="modify">
  <table width="600" border="0" cellpadding="2" cellspacing="1" bgcolor="#FFFFFF">
    ...
    <tr>
      <td bgcolor="#F3F3F3" class="forTableBgLeft">            <input name="deletionName" type="hidden" id="deletionName" value="<?php echo $_POST['deletionName']; ?>">
      <input name="date" type="hidden" id="date" value="<?php echo $row_rsDelete['date']; ?>"></td><td bgcolor="#F8F8F8" class="forTableBgLeft"><table width="100%"  border="0" cellspacing="0" cellpadding="2">
        <tr>
