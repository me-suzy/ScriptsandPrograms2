...
// ----------------------------------------------------------------------
// Filename: entrystatus.php
// Original  Author(s): Khaled Al-Sham'aa khaled@al-shamaa.com
// Purpose:  Update interactive entry status
// ----------------------------------------------------------------------
...
if(is_numeric($_GET["status"])){ $newstatus = @$_GET["status"]; }
...
$strsql = "SELECT `page_id` FROM `interactive` WHERE `id`=".$tkey;
...
// update
$updateSQL = "UPDATE `interactive` SET status=$newstatus WHERE `id`=".$tkey;
