...
if (isset($_POST['submit'])){
	$fp = fopen("design/meta.html","w");
	flock($fp,2);
	while (list ($key, $val) = each ($_POST)) {
		if($key != "submit"){
			$val = htmlspecialchars($val);
			fputs($fp,"<META NAME=\"$key\" CONTENT=\"$val\">\n");
		}
	}
	flock($fp,3);
	fclose($fp);
...
<form action="meta.php" method="post">
  <table width="98%" border="0" align="center" cellpadding="5" cellspacing="2" dir="<?php echo DIRECTION; ?>">
    ...
    <?php
	$meta_tags = file ('design/meta.html');
	while (list ($line_num, $line) = each ($meta_tags)) {
		$temp = explode("\"", $line);
		if($temp[0] == "<META NAME="){
			$key = $temp[1];
			$val = $temp[3];
?>
    <tr>
      <td class="pageTitle"><?php echo $key; ?></td>
      <td class="bodyBlock"><input type="text" name="<?php echo $key; ?>" size="50" value="<?php echo htmlspecialchars ($val); ?>"></td>
    </tr>
    <?php
		}
	}
