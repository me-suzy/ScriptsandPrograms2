...
if (($_POST["mode"] == 'uploadfile') || ($_POST["mode"] == 'createdir')) {
	if ($GLOBALS["canadd"]) {
		if ($_POST["mode"] == 'uploadfile') {
			$success = UploadNewFile($GLOBALS["script_home"].$_POST["subdir"]);
		} else {
			$success = CreateNewDir($GLOBALS["script_home"].$_POST["subdir"],$_POST["dirname"]);
		}
	}
}
...
function frmScripts($success)
{
	...
	if ($success) {
		?>
		<body marginwidth="0" marginheight="0" leftmargin="0" topmargin="0" class="mainback" onload="opener.location='<?php echo buildlink('m_scripts.php'); ?>&subdir=<?php echo $_GET["subdir"]; ?>';">
		<center>
	...
	?>
	<form name="form0" method="post" enctype="multipart/form-data" action="uploadscript.php" onSubmit="return validatefile()">
	<input type="hidden" name="form_submitted" value="yes">
	<input type="hidden" name="subdir" value="<?php echo $_GET["subdir"]; ?>">
	<tr class="tablecontent">
		<?php FieldHeading("ScriptFilename","filename"); ?></td>
		<td valign=top>
	...
	?>
	<form name="form1" method="post" enctype="multipart/form-data" action="uploadscript.php" onSubmit="return validatedir()">
	<input type="hidden" name="form_submitted" value="yes">
	<input type="hidden" name="subdir" value="<?php echo $_GET["subdir"]; ?>">
	<tr class="tablecontent">
