...
function frmDownloads()
{
	...
	adminbuttons('',$GLOBALS["tAddNewDownload"],$GLOBALS["tEditDownload"],$GLOBALS["tDeleteDownload"]);
	...
	$lRecCount = lCountFiles($GLOBALS["downloads_home"],'download');
	...
	frmDisplayDir($colcount,$GLOBALS["downloads_home"]);
	safeModeWarning($colcount);


	frmDownloadsHdFt($colcount,$nCurrentPage,$nPages);
	?>
	<tr class="teaserheadercontent"><?php
		...
		if ($GLOBALS["ShowFilePermissions"] != 'N') {
			if ($GLOBALS["OS"] != "Windows") { 
				adminlistitem(42,$GLOBALS["tDownloadName"],'');
				adminlistitem(10,'Permissions','');
				adminlistitem(10,'Owner','');
				adminlistitem(10,'Group','');
			} else {
				adminlistitem(62,$GLOBALS["tDownloadName"],'');
				adminlistitem(10,'Permissions','');
			}
		} else {
			adminlistitem(72,$GLOBALS["tDownloadName"],'');
		}
		adminlistitem(12,'&nbsp;','');
	?>
	</tr>
	...
	if (isset($GLOBALS["files"])) {
		...
		while (list($i,$val) = each($GLOBALS["files"])) {
			if ($GLOBALS["files"][$i]["filetype"] == 'download') {
				if ($nDownloadNr >= $lStartRec && $nDownloadShowed < $GLOBALS["RECORDS_PER_PAGE"]) {
					?>
					<tr class="teasercontent">
						<td align="center" valign="middle" class="content">
							...
							if ($GLOBALS["candelete"] === False) { echo $GLOBALS["iBlank"];
								...
								?><a href="javascript:DelDownload('subdir=<?php echo $_GET["subdir"]; ?>&Download=<?php echo $GLOBALS["files"][$i]["filename"]; ?>&page=<?php echo $GLOBALS["page"]; ?>')" <?php echo BuildLinkMouseOver($GLOBALS["tDeleteDownload"]); ?>><?php echo $GLOBALS["iDelete"]; ?></a><?php
							}
							?>
						</td>
						...
						<td valign="top" class="content">
							<a href="javascript:ShowDownload('<?php echo $GLOBALS["rootdp"].$GLOBALS["downloads_home"].$_GET["subdir"].$GLOBALS["files"][$i]["filename"]; ?>')"><?php echo $GLOBALS["files"][$i]["filename"]; ?></a>
						</td>
						<?php
						if ($GLOBALS["ShowFilePermissions"] != 'N') {
							?>
					...
					if ($nDownloadNr >= $lStartRec && $nDownloadShowed < $GLOBALS["RECORDS_PER_PAGE"]) {
						...
						<tr class="teasercontent">
							<td align="center" valign="middle" class="content">
								<?php
								if ($GLOBALS["files"][$i]["filename"] != '..') {
									?><a href="<?php echo BuildLink('m_downloads.php'); ?>&subdir=<?php echo $newsubdir; ?>"><?php echo $GLOBALS["iOpenFolder"]; ?></a>&nbsp;<?php
									if ($GLOBALS["candelete"] === False) { echo $GLOBALS["iBlank"];
									} else {
										?><a href="javascript:DelDir('subdir=<?php echo $_GET["subdir"]; ?>&folder=<?php echo $GLOBALS["files"][$i]["filename"]; ?>&page=<?php echo $_GET["page"]; ?>')" <?php echo BuildLinkMouseOver($GLOBALS["tDeleteDownloadDir"]); ?>><?php echo $GLOBALS["iDelete"]; ?></a><?php
									}
								}
								?>
							</td>
							...
							<td valign="top" class="content">
								<a href="<?php echo BuildLink('m_downloads.php'); ?>&subdir=<?php echo $newsubdir; ?>"><?php echo $GLOBALS["files"][$i]["filename"]; ?></a>
							</td>
							<?php
							if ($GLOBALS["ShowFilePermissions"] != 'N') {
								?>
	...
	frmDownloadsHdFt($colcount,$nCurrentPage,$nPages);
	frmFreeSpace($colcount,$GLOBALS["downloads_home"]);
	?>
	</table>
	</body>
	</html>
...
function frmDownloadsHdFt($colspan,$nCurrentPage,$nPages)
{
	global $_GET;

	$pLink = BuildLink('m_downloads.php');
	$linkmod = '&subdir='.$_GET["subdir"];
	$hlink = '<a href="javascript:UploadDownload();" '.BuildLinkMouseOver($GLOBALS["tAddNew"]).'>';
			...
			<table height="100%" width="100%" cellspacing="0" cellpadding="0">
				<tr><?php
					if (($GLOBALS["canadd"] === True) && ($GLOBALS["file_uploads"])) {
						...
						echo displaybutton('addbutton','downloads',$GLOBALS["tAddNew"].'...',$hlink);
						?></td><?php
					}
					?>
					<td align="<?php echo $GLOBALS["right"]; ?>" valign="bottom"><?php
						...
						echo RenderPageList($nCPage,$nPages,'m_downloads.php',$linkmod);
	...
	<?php
	echo '</form>';
} // function frmDownloadsHdFt()


?>
<script language="Javascript" type="text/javascript">
	<!-- Begin
	function DelDownload(sParams) {
		if (window.confirm('<?php echo $GLOBALS["tConfirmDeletion"]; ?>')) {
			location.href='<?php echo BuildLink('m_downloads.php'); ?>&mode=delete&' + sParams;
		}
	}

	function DelDir(sParams) {
		if(window.confirm('<?php echo $GLOBALS["tConfirmDeletion"]; ?>')) {
			location.href='<?php echo BuildLink('m_downloads.php'); ?>&mode=deletedir&' + sParams;
		}
	}

	function ShowDownload(sDownloadName) {
		window.open(sDownloadName, "Download", "width=500,height=400,status=no,resizable=yes,scrollbars=yes");
	}

	function UploadDownload() {
		window.open("<?php echo BuildLink('uploaddownload.php'); ?>&subdir=<?php echo $_GET["subdir"]; ?>", "UploadDownload", "width=600,height=310,status=no,resizable=no,scrollbars=no");
	}
	//  End -->
</script>
<?php
...
function DeleteDownload()
{
	...
	$path = $GLOBALS["rootdp"].$GLOBALS["downloads_home"].$_GET["subdir"];
	$ffile = $_GET["Download"];
