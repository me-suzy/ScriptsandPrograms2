...
if ($_GET["Theme"] != '') {
	...
	$strQuery = "SELECT themecode FROM ".$GLOBALS["eztbThemes"]." WHERE themecode='".$_GET["Theme"]."' AND themeenabled='1'";
	$sresult = dbRetrieve($strQuery,true,0,0);
	$sRecCount = dbRowsReturned($sresult);
	dbFreeResult($sresult);

	if ($sRecCount > 0) { $EZ_SESSION_VARS["Theme"] = $_GET["Theme"];
	} else { $EZ_SESSION_VARS["Theme"] = ''; }
} else {
