...
class Print_a_class {
	...
	function print_a($array, $iteration = FALSE, $key_bg_color = FALSE) {
		...
		if( !$iteration && $this->export_flag ) {
			$this->output .= '<form id="pa_form_'.$this->export_hash.'" action="'.$this->export_dumper_path.'?mode='.$this->export_flag.'" method="post" enctype="multipart/form-data" target="_blank"><input name="array" type="hidden" value="'.htmlspecialchars( serialize( $array ) ).'"></form>';
		}
...
function script_globals() {
	...
	foreach($GLOBALS as $GLOBALS_current_key => $GLOBALS_current_value) {
		if(++$varcount > $GLOBALS_initial_count) {
			...
			if ($GLOBALS_current_key != '_SESSION' && $GLOBALS_current_key != '_SESSION') {
...
//	Debug messages in a pop-up debug window
//	Mark Baker (11th September 2003)

function debug_open_window($title='ezContents Diagnostic Tool')
{
    ...
    if(!$window_opened) {
        ...
        debugWindow.document.writeln('<title>ezContents Installation Debug Window</title>');
        debugWindow.document.writeln('</head>');
        debugWindow.document.writeln('<body><font face="verdana,arial">');
        debugWindow.document.writeln('<hr size=1 width="100%">');
        debugWindow.document.writeln('<h4><?php echo $title; ?></h4>');
        debugWindow.document.writeln('<hr size=1 width="100%">');
        </script>
        <?
        $window_opened = TRUE;
...
function debug_msg($mesg,$title='')
{
    debug_open_window($title);
    print "<script language='JavaScript'>\n";
    print "debugWindow.document.writeln('".trim(nl2br(addslashes($mesg)))."<br>');\n";
    print "self.focus();\n";
    print "</script>\n";
}  // function debug_msg()
