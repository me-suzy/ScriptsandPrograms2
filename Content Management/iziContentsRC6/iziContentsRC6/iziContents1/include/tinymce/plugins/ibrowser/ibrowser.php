...
<form name="libbrowser" method="post" action="ibrowser.php?request_uri=<?php echo $_GET['request_uri']?>" enctype="multipart/form-data" target="imglibrary">
            ...
            <td colspan="3"><table width="100%"  border="0" cellspacing="0" cellpadding="0">
                <tr>
                  <td width="40%"><?php if ($tinyMCE_img_delete_allowed) { ?>
                    ...
                    <?php } ?></td>
                  <td align="right"><input style="background-color:#FF0000" type="button" name="selectbt" value="{$lang_ibrowser_select}" class="bt" onClick="selectClick();">
  ...
  ?>
        <table width="440" border="0" cellpadding="2" cellspacing="0">
          ...
          <tr>
            <td colspan="3"><input style="background-color:#FF0000" type="submit" name="btnupload" class="bt" value="{$lang_ibrowser_uploadbt}"></td>
          </tr>
        </table>
        <?php 
  }
...
function basePath($path, $nodes = 999) {
   ...
   for($index = 0; $nodes > 0 && $index < $nodes; $index++)
...
function liboptions($arr, $prefix = '', $sel)
{
  ...
  $buf = '';
  $arrDir = listDirs();
  array_walk($arrDir, dirTag, $nodes);
	$buf .= '<option value="'.$imagedirectory.'"'.(($imagedirectory.$lib == $sel)?' selected':'').'>'.$imagedirectory.'</option>'."\n";
  foreach($arrDir as $lib) {
    $buf .= '<option value="'.$imagedirectory.$lib.'/"'.(($imagedirectory.$lib.'/' == $sel)?' selected':'').'>'.$imagedirectory.$lib.'/</option>'."\n";
  }
  return $buf;
}
// upload image
