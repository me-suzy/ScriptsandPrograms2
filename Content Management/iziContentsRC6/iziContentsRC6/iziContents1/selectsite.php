...
if ($_GET["Site"] != '') {
	...
	$strQuery = "SELECT sitecode FROM ".$GLOBALS["eztbSites"]." WHERE sitecode='".$_GET["Site"]."' AND siteenabled='1'";
	$sresult = dbRetrieve($strQuery,true,0,0);
	$sRecCount = dbRowsReturned($sresult);
	dbFreeResult($sresult);

	if ($sRecCount > 0) { $EZ_SESSION_VARS["Site"] = $_GET["Site"];
	} else { $EZ_SESSION_VARS["Site"] = ''; }
} else {
...
Header("Location: ".BuildLink('index.php'));
