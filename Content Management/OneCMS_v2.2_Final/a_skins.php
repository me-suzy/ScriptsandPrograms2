...
if ($delete == TRUE) {
echo "<link rel='stylesheet' type='text/css' href='ta3.css'>Chat message deleted. <a href='a_skins.php?view=chat'>Go back to the chat</a>";
}
} else {
...
if ($_GET['view'] == "chat3") {
	...
	if ($chatmessage == TRUE) {
		echo "<link rel='stylesheet' type='text/css' href='ta3.css'>Message posted. <a href='a_skins.php?view=chat'>View it</a>";
	}
}

if ($_GET['view'] == "chat") {
	
echo "<link rel='stylesheet' type='text/css' href='ta3.css'><table cellspacing=\"5\" cellpadding=\"3\" border=\"0\" align=\"center\"><tr><td>Refresh Chat <input type=checkbox onclick=\"window.location='a_skins.php?view=chat'; return true;\"></td><td><a href='a_skins.php?view=chat2'>Post Message</a><br><br></td></tr>";
...
$rowr = mysql_fetch_row($sqal);
	...
	echo "&nbsp;&nbsp;<a href='a_skins.php?view=chatdelete&id=".$row[id]."'><b>Delete</b></a>";
	}
...
if ($_GET['view'] == "cat") {
$tempm = "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href='a_index.php?cat={name}'>+ {name}</a><br>";

$sql = mysql_query("SELECT * FROM onecms_cat ORDER BY `name` ASC");
while($row = mysql_fetch_array($sql)) {

echo str_replace("{name}", $row[name], $tempm);
}
}
?>