...
function body_far($valoare = "0")
	...
	*/
	...
	if ($valoare == "content") // returneaza pagina ceruta
		...
		if (isset($_GET['m'])) // daca se cere pagina din bd
			{
			$m = $_GET['m'];			
			if (!empty($m)) // daca se cere o pagina...
				{
				$modul = "modules/".$m.".php";				
				if (file_exists($modul))
					{
					include($modul); // daca se face cu include_once modulele de genu 'online' sau 'login' nu se mai includ...
					}
				if (!file_exists($modul))
					{
			...
			if (empty($m)) // daca nu se cere nici o pagina
				{
				echo $mesaj[68]; // afiseaza mesajul de eroare 		
				}
			}
