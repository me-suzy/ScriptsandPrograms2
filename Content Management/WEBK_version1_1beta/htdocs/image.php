...
//---------------------------------------------------------------------------//
	...
	$mtime = filesize(PATH_ROOT.$path.$file);
	
	$filename = PATH_ROOT.$path.".meta/.image_".md5($file)."_".$mtime;
	
	if (file_exists($filename))
	{
		header("Content-type: image/jpeg");
		
		readfile(PATH_ROOT.$path.".meta/.image_".md5($file)."_".$mtime);
		die();
	}
	else
	{
		die("Invalid image location: ".$filename);
	}
?>