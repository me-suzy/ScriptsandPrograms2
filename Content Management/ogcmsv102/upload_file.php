...
function upload_form () {
         ...
         print "<form action=\"{$_SERVER['PHP_SELF']}\" method=\"post\" enctype=\"multipart/form-data\">
                      ...
                      <input type=\"Submit\" name = \"Submit\" value=\"{$lang[upload_file_button]}\">
              </form>";
         }

//call file handler
if ($_POST['Submit'] == "{$lang[upload_file_button]}") {
   $upload_class = new Upload_Files;
   $upload_class->temp_file_name = trim($_FILES['upload']['tmp_name']); 
   $upload_class->file_name = trim(strtolower($_FILES['upload']['name'])); 
   $upload_class->upload_dir = "files/";
   $upload_class->upload_log_dir = "files/upload_logs/";
   $upload_class->max_file_size = 5242880;
   $upload_class->banned_array = array(""); 
   //$upload_class->ext_array = array(".zip",".rar",".ace",".tar",".mp3",".log",".php");
