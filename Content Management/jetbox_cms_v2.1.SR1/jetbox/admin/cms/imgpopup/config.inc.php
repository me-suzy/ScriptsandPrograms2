...
if (isset($_SESSION["uid"])){
  $result = mysql_query("SELECT * FROM user WHERE uid='" . $_SESSION["uid"] ."'");
	...
	if ($fileuploaduserrestricted==true) {
		...
		define("IMAGE_DIR", IMAGE_DIR_RT.$login."/");
		define("IMAGE_URL", IMAGE_URL_RT.$login."/");
	}
	else{
		define("IMAGE_DIR", IMAGE_DIR_RT);
		define("IMAGE_URL", IMAGE_URL_RT);
	}
}
else{
	exit;
...
define("SUPPORT_CREATE", FALSE);
define("SUPPORT_DELETE", FALSE);
define("SUPPORT_METAFILE", FALSE);// supported by Internet Explorer ONLY!
define("SUPPORT_UPLOAD", FALSE);
