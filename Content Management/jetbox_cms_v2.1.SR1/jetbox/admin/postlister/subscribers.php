...
if ($epostadresseTilfoej) {
	...
	$kommando = mysql_query("select epostadresse from $liste where epostadresse = '$epostadresseTilfoej'");
	...
	$date = date("ymd");
	mysql_query("insert into $liste values ('$epostadresseTilfoej', '$uniktId', '1','$date')") or die(mysql_error());
...
if ($sletDenne) {
	...
	mysql_query("delete from $liste where epostadresse = '$sletDenne'");
...
echo "</select>\n";
echo "$s46\n";
echo "<input type=hidden name=soeg value=\"$s13\">";
...
if ($soeg) {
	...
	if ($soegevalg == 1) {
		if ($visHvad1 == "alle") {
			$kommando = mysql_query("select epostadresse, godkendt, date from $liste");
		}
		if ($visHvad1 == "godkendte") {
			$kommando = mysql_query("select epostadresse, godkendt, date from $liste where godkendt = '1'");
		}
		if ($visHvad1 == "ikke-godkendte") {
			$kommando = mysql_query("select epostadresse, godkendt, date from $liste where godkendt = '0'");
		}
	}

	if ($soegevalg == 2) {
		if ($visHvad2 == "alle") {
			$kommando = mysql_query("select epostadresse, godkendt, date from $liste where epostadresse like '$alfabetet%'");
		}
		if ($visHvad2 == "godkendte") {
			$kommando = mysql_query("select epostadresse, godkendt, date from $liste where epostadresse like '$alfabetet%' and godkendt = '1'");
		}
		if ($visHvad2 == "ikke-godkendte") {
			$kommando = mysql_query("select epostadresse, godkendt, date from $liste where epostadresse like '$alfabetet%' and godkendt = '0'");
		}
	}

	if ($soegevalg == 3) {
		if ($visHvad3 == "alle") {
			$kommando = mysql_query("select epostadresse, godkendt, date from $liste where epostadresse like '%$soegeord%'");
		}
		if ($visHvad3 == "godkendte") {
			$kommando = mysql_query("select epostadresse, godkendt, date from $liste where epostadresse like '%$soegeord%' and godkendt = '1'");
		}
		if ($visHvad3 == "ikke-godkendte") {
			$kommando = mysql_query("select epostadresse, godkendt, date from $liste where epostadresse like '%$soegeord%' and godkendt = '0'");
		}
	}
