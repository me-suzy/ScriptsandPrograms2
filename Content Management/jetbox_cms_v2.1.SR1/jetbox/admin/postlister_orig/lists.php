...
if ($listeOpret) {
	...
	mysql_prefix_query("create table $listeOpret (
		epostadresse char ( 50 ),
		id char ( 20 ),
		godkendt int ( 1 )
	)");

	mysql_prefix_query("insert into $mainTable ( liste, tilmeldingsbesked, afmeldingsbesked ) values (
		'$listeOpret',
		'$s32',
		'$s81'
	)");
...
if ($listeSletBekraeft) {
	...
	mysql_prefix_query("drop table $listeSletBekraeft");

	mysql_prefix_query("delete from $mainTable where liste = '$listeSletBekraeft'");
...
$kommando = mysql_prefix_query("select liste from $GLOBALS[mainTable]");
