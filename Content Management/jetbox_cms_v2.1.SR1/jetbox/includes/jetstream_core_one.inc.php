...
function general_listrecords($error='', $blurbtype='notify') {
	...
	// TC - 04/10/05 Modify method to allow paging for large tables
	$rest = new MyPagina; // instantiate class
	$rest->sql = $q; //"SELECT * FROM mp_base"; // the (basic) sql statement (use the SQL whatever you like)
	$nav_links = $rest->navigation("&nbsp;&nbsp;", "currentStyle"); // the navigation links (define a CSS class selector for the current link)
	$total_recs = $rest->get_total_rows(); // the total number of records
	//$nav_info = $rest->page_info("to"); // information about the number of records on page ("to" is the text between the number)
	$result = $rest->get_page_result(); // result set
	...
	echo "<tr><td background='images/stab-bg.gif' align=\"left\">".$nav_links." ".$total_recs." items total</td></tr>";
	echo "</table>";
	echo "<table border=\"0\" width=100% cellspacing=\"0\" cellpadding=\"5\" id=\"table_overview_list\">";
	echo $ruler;
	reset($listformat);
	...
	if ($wf){
		...
		echo "<td ".$sorter_display_class." title=\"Sort by status\"><a style=\"DISPLAY: block;\" href=\"". $jetstream_url . $thisfile. "?orderby=struct.status&logic=".$_REQUEST["logic"] . "\">Status".$sort_img."</a></td>";	
		$sorter_display_class="class=\"tab-g-sort\"";
		$sort_img="&nbsp;<img src=\"images/clearpixel.gif\" border=\"0\" />";
		...
		echo "<td ".$sorter_display_class." title=\"Sort by author\"><a style=\"DISPLAY: block;\" href=\"". $jetstream_url . $thisfile. "?orderby=struct.u_id&logic=".$_REQUEST["logic"] . "\">Author".$sort_img."</a></td>";	

	}
	//echo $sorted_by;
	while ($listitem = each($listformat)) {
		if ($sorted_by==$listitem[1][0]) {
			echo "<td class='tab-g-sort-sel' title=\"Sort by ".strtolower($listitem[1][2])."\"><a style=\"DISPLAY: block;\" href=\"". $jetstream_url . $thisfile. "?orderby=".$listitem[1][0]."&logic=".$_REQUEST["logic"] . 	"\">".$listitem[1][2]."&nbsp;<img src=\"images/caret-".strtolower($sorted_by_order).".gif\" border=\"0\" /></a></td>";
		}
		else{
			echo "<td class='tab-g-sort' title=\"Sort by ".strtolower($listitem[1][2])."\"><a style=\"DISPLAY: block;\" href=\"". $jetstream_url . $thisfile. "?orderby=".$listitem[1][0]."&logic=".$_REQUEST["logic"] . "\">".$listitem[1][2]."<img src=\"images/clearpixel.gif\" border=\"0\" /></a></td>";	
		}
	}
	if ($order_column<>'') {
...
function jetstream_ShowSections($sections, $pages, $navid){
	global $primarykey, $$primarykey, $jetstream_url, $nav_links;
	$tabs='';
...
// gets the total number of records 
	...
	global $q, $nav_links;
	echo "q= ".$q;
		$tmp_result = mysql_query($q);
		$nav_links = mysql_num_rows($tmp_result);
