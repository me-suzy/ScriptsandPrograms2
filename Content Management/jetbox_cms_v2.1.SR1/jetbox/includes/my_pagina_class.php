...
// TC - 04/10/05 Jetbox settings
//include("general_settings.inc.php");
...
//error_reporting(E_ALL); // only for testing
	...
	//define("NUM_ROWS", 20); // the number of records on each page (defined in general settings)
	define("STR_FWD", " &gt;&gt;"); // the string is used for a link (step forward)
	define("STR_BWD", "&lt;&lt; "); // the string is used for a link (step backward)
	//define("NUM_LINKS", 20); // the number of links inside the navigation (the default value) (defined in general settings)
	...
	function rebuild_qs($curr_var) {
		if(is_array($_GET)  && count($_GET)>0){
			foreach ($_GET as $key => $value) {
				if ($key<>$curr_var && $key<>'logic')  {
					$qs.="&".$key."=".$_GET[$key];
				}
			}
			$_REQUEST["logic"] == "asc" ? $qs.="&logic=desc" :$qs.="&logic=asc";
	...
	function navigation($separator = " | ", $css_current = "", $back_forward = false) {
		...
		if ($curr_pages <= $all_pages && $curr_pages >= 0) {
			...
			if($all_pages >= 1) {
				...
				$navi_string = ($curr_pages > 0) ? "<a href=\"".$_SERVER['PHP_SELF']."?".$var."=".$backward.$qs."\">".$this->str_backward."</a>&nbsp;" : $this->str_backward."&nbsp;";
				if (!$back_forward) {
					for($a = $start + 1; $a <= $end; $a++){
						$theNext = $a - 1; // because a array start with 0
						if ($theNext != $curr_pages) {
							$navi_string .= "<a href=\"".$_SERVER['PHP_SELF']."?".$var."=".$theNext.$qs."\">";
							$navi_string .= $a."</a>";
				...
				$navi_string .= ($curr_pages < $all_pages) ? "&nbsp;<a href=\"".$_SERVER['PHP_SELF']."?".$var."=".$forward.$qs."\">".$this->str_forward."</a>"." - " : "&nbsp;".$this->str_forward." - ";
			}
		}
		return $navi_string;
	}
