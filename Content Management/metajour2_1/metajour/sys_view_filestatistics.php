...
class sys_view_filestatistics extends basic_view {

	function loadLanguage() {
		basic_view::loadLanguage();
		$this->loadLangFile('sys_view_filestatistics');
	}
	...
	function stats($site, $date1, $date2, $groupby, $sumwhat, $wherecol, $whereval, $col1_sort, $col2_sort) {
		...
		if ($groupby == 'pageid') {
			$part1 = ", binfile.name as name ";
			$part2 = "binfile, ";
			$part3 = " binfile.objectid = stats.pageid AND ";
			if ($col1_sort <> "") {
				$orderby = " ORDER BY binfile.name $col1_sort";
			}
		}
		...
		$baseurl = $this->callgui('sys','','','filestatistics','','','','stat=stat&date1='.$date1.'&date2='.$date2.'&groupby='.$groupby.'&sumwhat='.$sumwhat.'&wherecol='.$wherecol.'&whereval='.$whereval.'&ulang='.$_REQUEST['ulang']);
		...
		$sql = "select $group as samler, count($sum) as antal $part1 from $part2 binfile_statistics stats where $where $part3 stats.site='$site' AND timestamp >= '$cdate1' AND timestamp <= '$cdate2' $iplimit $ulanglimit group by $group $orderby";
		...
		while ($row = $res->fetchrow()) {
			...
			if ($groupby == 'pageid') {
				$name = $db->getone("select name from binfile where objectid = ".$samler);
