...
$obj = owNew('template');
$objdata['name'] = "standard_gallery_full";
...
{if \$config.usefull}
	<a href=\"{\$me}&_ext={\$_ext}&_extcf={\$_extcf}&_ext_objectid={\$request._ext_objectid}&_cmd=full\" {if \$config.fullnewwindow}target=\"_blank\"{/if}>[Full size]</a>&nbsp;&nbsp;
	{elseif \$config.linktofull}
	<a href=\"getfile.php?objectid={\$request._ext_objectid}\" {if \$config.fullnewwindow}target=\"_blank\"{/if}>[Full size]</a>&nbsp;&nbsp;
{/if}
<a href=\"{\$me}&_ext={\$_ext}&_extcf={\$_extcf}&_ext_index={\$result.index}&_cmd=index\">[Back to index]</a>&nbsp;&nbsp;
<br><br>
...
{if \$config.commentpos == 1}
	...
	{if \$config.addcomment}
		{if \$post._ext_content != \"\"}
			{createobj type=\"note\" parentid=\$post._ext_objectid name=\$post._ext_content uname=\$post._ext_uname}
		{/if}
		...
		<th class=\"extheader\">&nbsp;Skriv besked&nbsp;</th>
		</tr>
		{if \$user.level == ACCESS_ANONYMOUS}
			<tr>
				<td class=\"extcolorcell\" width=\"100\" valign=\"top\"><b>Forfatter</b></td>
				<td class=\"extcolorcell\"><input type=\"text\" name=\"_ext_uname\" style=\"width: 95%\"></td>
			</tr>
		{/if}
		<tr>
			<td class=\"extcolorcell\" width=\"100\" valign=\"middle\"><b>Besked</b></td>
	...
	{if \$config.listcomment}
		...
		{if \$res|@sizeof != 0}
			<table class=\"exttablebg\" width=\"100%\" cellspacing=\"1\">
			<tr>
			<th class=\"extheader\">&nbsp;Forfatter&nbsp;</th>
			<th class=\"extheader\">&nbsp;Besked&nbsp;</th>
			</tr>
			{section loop=\$res name=\"i\" step=-1}
				<tr>
					...
					<td class=\"{if \$smarty.section.i.index is odd}extcolorcell{else}extlightcell{/if}\"><div style=\"float: left\"><b>{\$res[i].uname}</b></div><div style=\"float: right\">Sendt: {\$res[i].object.created|date_format:\"%Y-%m-%d %H:%M:%S\"}</div></td>
				</tr>
				<tr>
					<td class=\"{if \$smarty.section.i.index is odd}extcolorcell{else}extlightcell{/if}\" align=\"center\"></td>
					<td class=\"{if \$smarty.section.i.index is odd}extcolorcell{else}extlightcell{/if}\"><span class=\"extmsgbody\">{\$res[i].name}<br><br><br><br></span></td>
				</tr>
...
.extheader {
	PADDING-RIGHT: 4px; PADDING-LEFT: 4px; FONT-WEIGHT: bold; FONT-SIZE: 85%; BACKGROUND-IMAGE: url(img/design/standard/extbg.gif); COLOR: #ffffff; WHITE-SPACE: nowrap; HEIGHT: 24px; BACKGROUND-COLOR: #006699
}
.exttablebg {
