<?
$files_to_ignore = array('blank.php', 'index.php', 'upgrade_functions.php', 'upgrade_template.php');

require_once('../../init.php');

$GLOBALS['UPGRADE_FROM'] = '2.8.0';
$GLOBALS['UPGRADE_TO']   = '2.10.0';
...
function cmp_sort_upgrade_scripts($a, $b) {
	static $re = '/^([^0-9]*)([0-9.]+).*$/';
	if (preg_match($re, $a, $a_matches) && preg_match($re, $b, $b_matches)) {
		if ($a_matches[1] == $b_matches[1]) {
			return version_no_compare($a_matches[2], $b_matches[2]);

		}

	} 
	// otherwise just default to the string compare
	return strcmp($a, $b);
}

usort($upgrade_files, 'cmp_sort_upgrade_scripts');
