...
if($_GET['action'] == 'finished') {
	...
	if ($db->table_exists('xtra_web_extension_notitia_attribute_xtra')) {
		$todo_attributeids = $db->single_column('SELECT DISTINCT(attributeid) FROM xtra_web_extension_notitia_attribute_xtra ORDER BY attributeid');
		...
		echo 'Attribute Xtra table xtra_web_extension_notitia_attribute_xtra does not exist. This upgrade cannot be run. It also does not need to be run.';
...
function do_upgrade_attribute_values() {
	...
	if (isset($num) && isset($num_to_process)) {
		...
		if (!$started) {
			# First popup ever
			$num = 5;
			$num_to_process = count($attributeids);
			$now = time();
			...
			echo status_popup(1,$process_url,false,false,'#330099','Performing Attribute Upgrading - Please wait', "Starting to clean attribute xtras for $num_to_process pages");
			exit();
		}
		...
		if (!empty($attributeids)) {
			...
			foreach($attributeids as $attributeid) {
				$attribute_xtra_ids = $db->associative_array('SELECT xtraid, order_no FROM xtra_web_extension_notitia_attribute_xtra WHERE attributeid = '.$attributeid.' ORDER BY order_no');
				if (!empty($attribute_xtra_ids)) {
					$incremental_order_no = 0;
					$save_order_nos = false;
					foreach($attribute_xtra_ids as $xtra_id => $order_no) {
						if ($incremental_order_no != $order_no) {
							$attribute_xtra_ids[$xtra_id] = $incremental_order_no;
							$save_order_nos = true;
						}
						$incremental_order_no++;
					}

					if ($save_order_nos) {
						foreach($attribute_xtra_ids as $xtra_id => $order_no) {
							$db->update('UPDATE xtra_web_extension_notitia_attribute_xtra SET order_no = '.$order_no.' WHERE xtraid = '.$xtra_id);
						}
						$report .= 'Attribute ID '.$attributeid.' Xtras have been cleaned<br />';
					} else {
						$report .= 'Attribute ID '.$attributeid.' Xtras have been ok<br />';
					}
				} else {
					$report .= 'No cycling required<br />';
				}
		...
		echo status_popup($percent,$process_url,false,$finish,'#330099','Cleaning Attribute Xtras - Please wait',$status);
		exit();
	}
