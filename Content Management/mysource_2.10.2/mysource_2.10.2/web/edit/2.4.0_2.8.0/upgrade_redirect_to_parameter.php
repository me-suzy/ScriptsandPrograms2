<?  ##############################################
 ...
 #-- Copyright Squiz.net ---------------------#
##############################################
## $Source: /home/cvsroot/mysource/web/edit/2.4.0_2.8.0/upgrade_redirect_to_parameter.php,v $
## $Revision: 1.2.2.1 $
## $Author: brobertson $
...
if (!$SESSION->logged_in()) {
	$SESSION->login_screen("Upgrade Redirect", "You must be logged in.");
	exit();
} else if (!user_root()) {
	$SESSION->login_screen("Upgrade Redirect", "You must be a <b>root</b> to upgrade the system.",$SESSION->user->login);
	exit();
}

if (!is_file($SYSTEM_ROOT."/xtras/page/templates/redirect/redirect.inc")) {
...
?>
Upgrading MySource Redirect.<br><br>
<?
...
if(!$_REQUEST['step']) {
	...
	# Web DB Changes
	$sql = array();
	$query = "CREATE TABLE xtra_page_template_redirect_backup (
	...
	if(!$webdb->select($query)) {
		...
		echo "Error - There was a problem creating a backup redirect table. The rest of the script can't run until this problem is fixed.<p>";
		report_failure($_SERVER['SCRIPT_FILENAME']);
		exit;
	}

	$number = $webdb->single_element("SELECT count(*) from xtra_page_template_redirect");

	if($number) { # yes, we have some data we have to back up.
		$query = "INSERT INTO xtra_page_template_redirect_backup 
			(pageid, to_pageid, to_url, extra_url, new_window, window_options) SELECT sp.pageid, sp.to_pageid, sp.to_url, sp.extra_url, sp.new_window, sp.window_options FROM xtra_page_template_redirect sp";
	...
	echo 	"<p>Backup tables created and data copied OK. Starting database changes to Redirect table.";

	$sql[] = "ALTER TABLE xtra_page_template_redirect DROP COLUMN to_pageid";
	$sql[] = "ALTER TABLE xtra_page_template_redirect DROP COLUMN to_url";
	$sql[] = "ALTER TABLE xtra_page_template_redirect DROP COLUMN extra_url";
	$sql[] = "ALTER TABLE xtra_page_template_redirect DROP COLUMN new_window";
	$sql[] = "ALTER TABLE xtra_page_template_redirect DROP COLUMN window_options";
	$sql[] = "ALTER TABLE xtra_page_template_redirect ADD  COLUMN parameters LONGTEXT";
	foreach($sql as $run) {
		if (!$webdb->select($run)) report_failure($_SERVER['SCRIPT_FILENAME']);
	}

	?> 
	<p>The database changes for the redirect upgrade are done.
	<br>Do you have any serious <b>database errors</b> (e.g. Not including trying to drop a column that doesn't exist)?
	<p>
	<form action="upgrade_redirect_to_parameter.php" name='upgrade' method="POST"> 
		<input type=hidden name=step value="">
		<input type="button" name="no" value="No" onClick="document.upgrade.step.value=2; document.upgrade.submit()">
		<input type="button" name="yes" value="Yes" onClick="javascript:alert('sorry, you can\'t go on any further with this upgrade - please check the database backup table exists, the data has been copied correctly and columns for the redirct table have been changed (there should be only two - pageid and parameters.)'); return false;">
	</form>
	<?		
	...
	$forms = $webdb->associative_array("SELECT * from xtra_page_template_redirect_backup");
	$clean_convert = true;
	foreach($forms as $pageid => $data) {
		...
		if(!$webdb->update("UPDATE xtra_page_template_redirect set parameters='".addslashes(serialize($parameters))."' where pageid='$pageid'")) {
	...
	if($clean_convert) {
		echo "All redirect pages have been converted. Dropping backup table...<p>";
		$webdb->select("DROP TABLE xtra_page_template_redirect_backup");
