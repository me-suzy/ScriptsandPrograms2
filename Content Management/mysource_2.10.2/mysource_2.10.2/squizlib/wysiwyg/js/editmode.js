...
function setEditMode(htmlMode) {
	if (htmlMode == true) {
		//ensure we dont change to the same mode
		if (doc.txtView != false) {
		...
		if (doc.txtView != true) {
			...
			doc.txtView = true;
			doc.focus();
			doc.innerHTML = codeSweeper('TEXT');
		}
	}
}
...
function ccParser(html) {
	...
	html = html.replace(parameter,"=<span class=ccp_hrefvalue>$1</span>");
