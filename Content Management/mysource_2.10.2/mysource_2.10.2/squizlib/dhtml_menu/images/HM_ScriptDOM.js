...
Updated and added to by Squiz.Net for MySource - http://mysource.squiz.net
...
HM_a_Parameters = [
	...
	["FontColor",				"black"],
	["FontColorOver",			"white"],
	["semiPermHilite",			false,                  "boolean"], // added by mmcintyre 03/07/03
	["currHilite",			    "red"],							    // added by mmcintyre 03/07/03
	["BGColor",					"white"],
	["BGColorOver",				"black"],
	...
	["ScreenPadding",			10,						"number"],
	["EnforceScreenWidth",		0,						"number"],
	["EnforceScreenWidthAlign",	'left']
]
...
function HM_f_StartIt() {
	...
	HM_DOCUMENT_CANVAS = (HM_IE && document.compatMode && document.compatMode ==  "CSS1Compat") ? document.documentElement : document.body;
	HM_AreLoaded = true;
	if (HM_ClickKill) {
...
function HM_f_MakeTrees(){
	...
	for(var t=0; t<HM_a_TreesToBuild.length; t++) {
		...
		HM_CurrentTree = {
			...
			currHilite          : HM_f_EvalParameters(TreeParams[21], HM_currHilite),  // added by mmcintyre 03/07/03
			semiPermHilite      : HM_f_EvalParameters(TreeParams[22], false, "boolean") // added by mmcintyre 03/07/03
		}
...
function HM_f_SetItemProperties(itemidsuffix) {
	...
	// modified by mmcintyre 03/07/03
	this.hasRollover = ((!this.permHilite || this.tree.semiPermHilite) && HM_f_EvalParameters(this.array[2],true,"boolean"));
...
function HM_f_SetItemStyle() {
	with(this.style){
		...
		backgroundColor = (HM_CurrentTree.semiPermHilite && this.permHilite) ? HM_CurrentTree.currHilite : backgroundColor;
		...
		if(
			(
				this.menu.showImage && 
				(
					!this.menu.IsVariableWidth || 
					(this.menu.IsVariableWidth && this.tree.RightToLeft && !this.menu.isHorizontal)
				)
			) 
			|| 
			(
				this.menu.IsVariableWidth && this.imgLyr
			)
		) {
...
function HM_f_PopMenu(e){
	...
	HM_CurrentMenu.mouseX = mouse_x_position = (HM_IE) ? (e.clientX + HM_DOCUMENT_CANVAS.scrollLeft) : e.pageX;
	HM_CurrentMenu.mouseY = mouse_y_position = (HM_IE) ? (e.clientY + HM_DOCUMENT_CANVAS.scrollTop)  : e.pageY;
	...
	if (HM_KeepHilite) {
		if (this.menu.currentItem && this.menu.currentItem != this && this.menu.currentItem.hasRollover) {
			with(this.menu.currentItem.style){
				...
				if(this.tree.semiPermHilite && this.menu.currentItem.permHilite) { // mmcintyre
					backgroundColor = this.tree.currHilite
					color = this.tree.FontColorOver
				} else { // mmcintyre
...
function HM_f_ItemOut() {
	...
	if ( (!HM_KeepHilite || ((this.tree.TopIsPermanent && (this.tree.treeParent==this)) && !this.menu.hasChildVisible)) && this.hasRollover) {
		...
		if(this.tree.semiPermHilite && this.permHilite) { // mmcintyre
			this.style.backgroundColor = this.tree.currHilite
			this.style.color = this.tree.FontColorOver
		}
	}
}
...
function HM_f_ShowIt(on) {
	...
	if (HM_KeepHilite && this.currentItem && this.currentItem.hasRollover) {
		with(this.currentItem.style){
			if(this.tree.semiPermHilite && this.currentItem.permHilite) { // mmcintyre
				backgroundColor = this.tree.currHilite; // mcintyre	
				color = this.tree.FontColorOver;
			} else { // mmcintyre
				backgroundColor = this.tree.BGColor;
				color = this.tree.FontColor;
...
function HM_f_KeepInWindow() {
	var WindowWidth	     = (HM_IE) ? HM_DOCUMENT_CANVAS.clientWidth  : window.innerWidth;
	var WindowHeight     = (HM_IE) ? HM_DOCUMENT_CANVAS.clientHeight : window.innerHeight;
	
	var WindowLeftEdge   = (HM_IE) ? HM_DOCUMENT_CANVAS.scrollLeft   : window.pageXOffset;
	var WindowTopEdge    = (HM_IE) ? HM_DOCUMENT_CANVAS.scrollTop	 : window.pageYOffset;
	var WindowRightEdge  = WindowLeftEdge + WindowWidth
	var WindowBottomEdge = WindowTopEdge  + WindowHeight;

	// if we are trying to enforce the screen width then recalculate the above values
	if (HM_EnforceScreenWidth > 0) {
		switch(HM_EnforceScreenWidthAlign) {
			case 'right':
				WindowLeftEdge = WindowRightEdge - HM_EnforceScreenWidth;
			break;

			case 'centre':
				var centrePos = WindowLeftEdge + Math.round((WindowRightEdge - WindowLeftEdge) / 2);
				WindowLeftEdge  = centrePos - Math.round(HM_EnforceScreenWidth / 2);
				WindowRightEdge = centrePos + Math.round(HM_EnforceScreenWidth / 2);
			break;

			default : // aka left
				WindowRightEdge = HM_EnforceScreenWidth;

		}// end switch

	}// end if

	// now just allow for the screen padding
	WindowLeftEdge   -= HM_ScreenPadding;
	WindowTopEdge    -= HM_ScreenPadding;
	WindowRightEdge  -= HM_ScreenPadding;
	WindowBottomEdge -= HM_ScreenPadding;
