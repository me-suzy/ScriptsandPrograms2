...
function HM_f_KeepInWindow() {
	...
	// we're in a frame, so add four pixels - don't ask me why
	if (self != top) {
		WindowWidth  += 4;
		WindowHeight += 4;

	// else NS window measurements include the space that are reserved for the scrollbars
	// so take that number off the figure
	} else {
		WindowWidth  -= (HM_Mac) ? 15 : 16;
		WindowHeight -= (HM_Mac) ? 15 : 16;
	}
	...
	if (HM_EnforceScreenWidth > 0) {
		switch(HM_EnforceScreenWidthAlign) {
			...
			case 'centre':
				...
				WindowRightEdge = WindowLeftEdge + HM_EnforceScreenWidth;
			break;
