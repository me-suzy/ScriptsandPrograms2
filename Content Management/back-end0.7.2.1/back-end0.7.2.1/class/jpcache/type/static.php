<?php
    ...
    /* $Id: static.php,v 1.1 2004/01/02 00:55:03 mgifford Exp $ */
    ...
    {
        $cachedata["jpcache_expire"] = -1;
        // Construct filename
        $filename = $GLOBALS["JPCACHE_DIR"]."/".$GLOBALS["JPCACHE_FILEPREFIX"].$GLOBALS["jpcache_key"];
        
        // read file and unserialize the data
        $cachedata["jpcachedata_gzdata"]=jpcache_fileread($filename);
        if($cachedata["jpcachedata_gzdata"] != NULL) {
           $cachedata["jpcache_expire"] = filemtime($filename);
        }
    ...
    function jpcache_write($gzdata, $datasize, $datacrc) 
    {
        ...
        // And write the data
        if (jpcache_filewrite($filename, $gzdata))
        {
       ...
       global $varkey;
        ...
        switch($method) {
            ...
            default:
		     ...
		     if( !preg_match("/.".$GLOBALS['varkey']."$/i", $filename)) {
			// only delete files that end with varkey extension
                        continue;
		     }

                     $cachedata["jpcache_expire"] = filemtime($filename) + $GLOBALS['JPCACHE_TIME'] ;
