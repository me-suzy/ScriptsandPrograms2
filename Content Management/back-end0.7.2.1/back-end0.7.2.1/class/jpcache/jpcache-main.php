<?php
    ...
    /* $Id: jpcache-main.php,v 1.2 2005/03/21 04:28:01 mgifford Exp $ */
     ...
     * want the script-key to be hashed, change JPCACHE_HASHSCRIPTKEY.
     */
    function jpcache_key($key='', $name='',$varkey='')
    {
            if(empty($key)) {
                if(empty($name)) {
                   $name=jpcache_scriptkey();
                }
                if(empty($varkey)) {
                   $varkey=jpcache_varkey();
                }
                if($GLOBALS['JPCACHE_HASHSCRIPTKEY'] > 0) {
                    $key = md5($name.$varkey);
                } else {
                    $key = $name . md5($varkey);
                }
     ...
     *
     */
    function jpcache_check($key='', $name='',$varkey='')
    {
        ...
        // with no parameters.
        $GLOBALS['jpcache_key'] = jpcache_key($key, $name,$varkey);
    ...
    function jpcache_init()
    {
        ...
        if ($GLOBALS['JPCACHE_TIME'] == -1)
        {
		  ...
		  if (!isset($GLOBALS['JPCACHE_VERSION'])) $GLOBALS['JPCACHE_VERSION'] = '';
     ...
     */
    function jpcache_gc($method='cachetimeout', $argv='', $probability='')
    {
        if(empty($probability)) {
           $probability=$GLOBALS['JPCACHE_GC'];
        }
     ...
     */
    function jpcache_start($key='', $name='',$varkey='')
    {
        // Initialize cache
        jpcache_init();
        ...
        // Check cache
        if (jpcache_check($key, $name, $varkey))
        {
