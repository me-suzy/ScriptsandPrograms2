...
class phpOpenTracker_DB_mysql_merge extends phpOpenTracker_DB_mysql {
  ...
  function _createNewTable($type, $name) {
    parent::query(
      sprintf(
        ...
        'IF NOT EXISTS',
        $name,
        ($type == 'accesslog') ? 'DELAY_KEY_WRITE=1 PACK_KEYS=1' : 'DELAY_KEY_WRITE=1'
      )
    );
  }
  ...
  */
  function _rewriteSelectQuery($query) {
    ...
    $startDay   = $day   = date('d', $parsedQuery[$between + 1]);
    $startMonth = $month = date('m', $parsedQuery[$between + 1]);
    $startYear  = $year  = date('Y', $parsedQuery[$between + 1]);
    $endDay              = date('d', $parsedQuery[$between + 3]);
    $endMonth            = date('m', $parsedQuery[$between + 3]);
