...
$tempDir = TEMP_PKG_PATH.'pdffonts/';
mkdir_p( $tempDir );
  if (substr($name,-4)=='.afm'){
    $name=substr($name,0,strlen($name)-4);
  }
  $this->addMessage('openFont: '.$font.' - '.$name);
  if (file_exists( $tempDir.'php_'.$name.'.afm')){
    $this->addMessage('openFont: php file exists '.$dir.'php_'.$name.'.afm');
    $tmp = file($tempDir.'php_'.$name.'.afm');
  ...
  if (!isset($this->fonts[$font]) && file_exists($dir.$name.'.afm')){
    ...
    $fp = fopen($tempDir.'php_'.$name.'.afm','w');
...
//        $fontObj = $this->numObj;
        ...
        if ($fbtype=='pfb'){
          $l1 = (hexdec(bin2hex($data[5].$data[4].$data[3].$data[2])));
          $l2 = (hexdec(bin2hex($data[$l1 + 11].$data[$l1 + 10].$data[$l1 + 9].$data[$l1 + 8])));
          $l3 = (hexdec(bin2hex($data[$l1 + $l2 + 17].$data[$l1 + $l2 + 16].$data[$l1 + $l2 + 15].$data[$l1 + $l2 + 14])));
          $this->o_contents($this->numObj,'add',array('Length1'=>$l1+6,'Length2'=>$l2+6,'Length3'=>$l3+6));
        } else if ($fbtype=='ttf'){
          $l1 = strlen($data);
...
function setCurrentFont(){
  if (strlen($this->currentBaseFont)==0){
    ...
    $this->selectFont('./fonts/LuxiSerif.afm');
  }
...
function getFontDecender($size){
  ...
  if (!$this->numFonts){
    $this->selectFont('./fonts/LuxiSerif');
  }
...
function addText($x,$y,$size,$text,$angle=0,$wordSpaceAdjust=0){
  if (!$this->numFonts){$this->selectFont('./fonts/LuxiSerif');}
...
/* Cut this out for now so we can embed - spiderr
	  $this->fontFamilies['LuxiSerif.afm']=array(
         'b'=>'LuxiSerif-Bold.afm'
        ,'i'=>'LuxiSerif-Oblique.afm'
        ,'bi'=>'LuxiSerif-BoldOblique.afm'
        ,'ib'=>'LuxiSerif-BoldOblique.afm'
      );
