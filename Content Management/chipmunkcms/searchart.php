...
<?php
if(isset($_POST['submit']) || isset($_GET['phrase']))
{
  $phrase=$_POST['phrase'];
  $numentries=20;
   if(!isset($_GET['start']))
   ...
   $getarticles="SELECT * FROM b_articles AS a, b_users AS b LEFT JOIN b_posts c ON (c.articleidentifier=a.artID) WHERE MATCH(a.body) AGAINST ('$phrase') and b.userID=a.authorID and a.validates='1' order by thetime DESC limit $start, $numentries";
   ...
   $order="SELECT a.titles from b_articles AS a, b_users AS b LEFT JOIN b_posts c ON (c.articleidentifier=a.artID) WHERE MATCH(a.body) AGAINST ('$phrase') and b.userID=a.authorID and a.validates='1' order by thetime DESC";
   ...
   if($start>=$numentries)
   {
      print "<A href='searchart.php?start=$prev&word=$phrase'><<</a>&nbsp;";
   }
   while($order3=mysql_fetch_array($order2))  
   {
     if($f>=$start-3*$numentries&&$f<=$start+7*$numentries)
     {
       if($f%$numentries==0)
       {
          print "<A href='searchart.php?start=$d&word=$phrase'>$g</a> ";
       }
     }
     $d=$d+1;
     $g=1+$d/$numentries;
   ...
   {
     print "<A href='searchart.php?start=$next&word=$phrase'>>></a>&nbsp;";
   }
}
?>
