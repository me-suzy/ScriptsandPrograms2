...
{
  die("<table class='maintable'><tr class='headline'><td><center>Unlock Thread</center></td></tr><tr class='forumrow'><td><center>You do not have permission to unlock this thread</center></td></tr></table>");
}
else
{
  if(isset($_POST['submit']))
  {
    ...
    $lock="Update b_posts set locked='0' where ID='$threadid'";
    mysql_query($lock) or die("Could not lock thread");
    $lock3="Update b_posts set locked='0' where threadparent='$threadid'";
  ...
  {
    print "<form action='unlockthread.php' method='post'>";
    print "Are You sure you want to unlock this thread?<br>";
    print "<input type='hidden' name='threadid' value='$ID'>";
    print "<input type='submit' name='submit' value='Unlock Thread'></form>";  
  }
}