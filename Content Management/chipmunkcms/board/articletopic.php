...
if(isset($_POST['submitpost']))
 {
    if(!$_POST['title'] || !$_POST['post'] || !$_GET['forumID'])
    ...
    else
      {
       ...
       $posting="INSERT INTO b_posts (author, title, post,timepost, telapsed, postforum,lastpost,nosmilies,ipaddress,articleidentifier) values ('$name', '$title', '$post', '$day', '$timegone','$forumID','$user','$nosmiley','$s','$artt')";
  ...
  {
    $artID=$_GET['artID'];
    $getarticle="SELECT * from b_articles where artID='$artID'";
    $getarticle2=mysql_query($getarticle) or die("Could not get articles");
    $getarticle3=mysql_fetch_array($getarticle2);
    print "<table class='maintable'>";
    print "<tr class='headline'><td><center>New Topic</center></td></tr>";
    print "<tr class='forumrow'><td><center>";
    print "<table border='0'><tr class='forumrow'><td>";
    print "<form action='articletopic.php?forumID=$forumID' method='post' name='form'>"; 
    if(isset($_SESSION['user']))
    {
    ...
    print "<input type='hidden' name='artt' value='$artID'>";
    print "<b>Topic:</b><br>";
    print "<input type='text' name='title' length='15' value='$getarticle3[titles]'><br><br>";
    ...
    print "<textarea rows='6' name='post' cols='45'>[quote]$getarticle3[body][/quote]</textarea><br><br>";
