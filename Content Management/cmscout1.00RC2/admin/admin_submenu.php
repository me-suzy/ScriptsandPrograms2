...
if ($action == "delete") 
{
	$delete = $data->delete_query("submenu", "id = '$id'", "Menus", "Deleted item $rid from $id");
	$action = "";		
    if ($delete)
    {   
        echo "<script> alert('Item Deleted'); window.location = '$pagename&site=$sitename';</script>\n";
        exit;   
    }  
}
...
if ($submit == "Submit") 
{
	if ($action == "new") 
    {
		...
		do 
		{
			$temp = $data->select_query("submenu", "WHERE side=$side AND pos = '$pos'");
        ...
        $site = safesql($sitename, "text");
		$update = $data->insert_query("submenu", "'', $name, $url, $item, $site, $pos, $side", "Menus", "Added menu item $name");
        if ($update)
        {
            echo "<script> alert('Your menu item has been added'); window.location = '$pagename&site=$sitename';</script>\n";
            exit;     
        }            
	} elseif ($action == "edit") {
		$name = safesql($_POST['name'], "text");
		...
		$update = $data->update_query("submenu", "name = $name, url = $url, item = $item, side=$side", "id=$id", "Menus", "Added menu item $name");
		$action = "";
        if($update)
        {
            echo "<script> alert('Your menu item has been updated'); window.location = '$pagename&site=$sitename';</script>\n";
            exit;   
        }            
	}
}
...
if (($action =="") || ($action == "view")) 
{
    ...
    $sql = $data->select_query("submenu", "WHERE site='$sitename' AND side='side' ORDER BY pos ASC");
    ...
    while ($temp = $data->fetch_array($sql))
    {
        if ($temp['item'] != "url") 
        {
            ...
            {
                $itemsql = $data->select_query("subcontent", "WHERE name='{$temp['item']}'");
                if ($data->num_rows($itemsql) == 1)
                {
    ...
    $sql = $data->select_query("submenu", "WHERE site='$sitename' AND side='top' ORDER BY pos ASC");
	...
	$sql = $data->select_query("submenu", "WHERE site='$sitename' AND side='bottom' ORDER BY pos ASC");
...
{
	$sql = $data->select_query("functions", "WHERE type = 5");
	$numfunc = $data->num_rows($sql);
	$func = array();
	while ($func[] = $data->fetch_array($sql));
    
	$sql = $data->select_query("subcontent", "WHERE site = '$sitename'");
...
elseif($action == "moveup")
{
    ...
    $pos1 = $row['pos'];
    $temppos = $pos1 -1;
    $sql = $data->select_query("submenu", "WHERE side='{$row['side']}' AND pos='$temppos'");
    $row2 = $data->fetch_array($sql);
    
    $pos2= $row2['pos'];
    if ($pos2 == 0 || $pos1 == 0)
    {
        header("Location: $server"."?page=submenu&site=$sitename");
    }
