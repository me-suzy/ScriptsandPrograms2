...
class AtomAPI {
	...
	*/
	function AtomAPI($endpoint, $username, $password, $authtype) {
		...
		// Set the endpoint and authtype, then make sure they worked, or die
	...
	*/
	function exec() {
		...
		switch ($this->get_method()) {
			case 'POST' :
				// Authentication header and content-type (required for Blogger.com)
			...
			case 'PUT' :
				...
				// Authentication headers and content-type (for Blogger.com)
				$headers = $this->auth->get_header(true);
				$headers[] = 'Content-type: application/xml';
				curl_setopt($ch, CURLOPT_HTTPHEADER, $headers); 
