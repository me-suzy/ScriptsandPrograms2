...
class paging
{
	...
	function print_link()
	{
//$link = "https://sslrelay.com/secure.on-drive.co.uk/open/viewfolderpg.php";
$link = $PHP_SELF;
		//generate template
		...
		if ($this->p["count"]>$this->p["baris"]) {
			...
			// print prev
			if ($this->page>1)
			$print_link .= "<a href=\"".$link."?".$_SERVER["QUERY_STRING"]."&page=".($this->page-1)."\">".$this->prev."</a>\n";
			...
			if ($this->page<>1)
			{
				for ($i=$this->p["bawah"];$i<=$this->page-1;$i++)
					$print_link .="<a href=\"".$link."?".$_SERVER["QUERY_STRING"]."&page=$i\">".number($i,$this->number)."</a>\n";
			}
			// print active
			...
			$print_link .= "<a href=\"".$link."?".$_SERVER["QUERY_STRING"]."&page=".($this->page+1)."\">".$this->next."</a>\n";
