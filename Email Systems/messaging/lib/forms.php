...
class CForm extends CLibrary{
	...
	*/
	function SimpleList($form , $items = "", $count = "", $extra = null , $search = false , $returnArray  = false) {
		...
		//check to see where is the returnurl, i was so something when i used the returnurl variable with URL
		...
		if (is_array($form["fields"])) {
			...
			//ok i know this is something, but now i dont have other idees
		...
		//ok the search options, this is kinda buggy for this version, will be fixed in other
...
//								$items[$key][$k] = $items[$key][$k] > 0 ? @date($v["params"] , ($v["now"] == "true" ? time() : $items[$key][$k])) : "N/A";
							...
							case "multiple":
								$items[$key][$k] = $template->blocks["SimpleListMultiple"]->Replace(array("value" => $items[$key][$v["value"]] , "field" => $v["field"]));
							break;
		...
		//stuff, append some extra elements to this form, in case, ok, i dont know how to handle if there will be 2 pagging functions.
...
//		$field["value"] = stripslashes($field["value"]);
		...
		switch ($field["type"]) {
			...
			case "textarea":
				...
				//uhm ... this is a stuffpy part becouse i have to integrate the html editor here so ...
				if ($field["html"] == "true") {
			...
			case "image":
				...
				if ($file) {
					...
					if (is_file($_CONF["path"] . $_CONF["upload"] . "tmp/" . $values["values"][$field["name"] . "_temp"])) {
						...
						//this sux becouse if the add process is not completed then i get stuff in the temp folder.					
			...
			case "date":
				...
				if (is_array($field["fields"])) {
					...
					if ($date_vals > 1000) {
						...
						//stuff, adding the time values too
