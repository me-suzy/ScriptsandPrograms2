...
<tr><td colspan=2 align=center><input type=submit value=Change></td></tr>
        ...
        print "<form method=post action=$GAME_SELF?p=options&view=zone&step=gmt>";
        print "Change my time zone to GMT (hours different, EG: -4 or 10)<input type=text name=gmt value=$user[gmt]>, please. <input type=submit value=Change>";
        print "</form>";
        if ($step == gmt) {
                if (empty ($gmt)) { print "Can't do that."; }else{
                        mysql_query("update `users` set `gmt`=$gmt where `id`=$user[id]");
                        print "You changed your timezone to GMT <b>$gmt</b>.";
               }
   }
}
...
print"<li><a href=$GAME_SELF?p=options&view=zone>Change Timezone</a>";
print"</ul>";
}
