...
if($_GET[thing]!=register){
        if($login==yes){
                ...
                if($user[id]){
                if($remember){
                                setcookie("username",$username,time()+10000000, "/", "", 0);
                                setcookie("pass",$pass,time()+10000000, "/", "", 0);
                }else{
                ...
                $pass=$_COOKIE['pass'];
                $user = @mysql_fetch_array(mysql_query("select * from users where username='$username' and pass='$pass'"));


                print"Welcome $user[user] to $gametitle<br>
