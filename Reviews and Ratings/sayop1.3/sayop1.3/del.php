...
<html>
   <head>
      <title>Delete database entries</title>
...
if($nrows=='0') { echo "No objects found"; }
else {
...
echo" <a href='del.php?obj_name=$irow[1]&catid=$irow[0]'>$irow[1]</a> ($countedrows)<br /> ";
                                      }
     }
echo "
</p>
...
<div class='rselect'><a href='newobj.php'>Add new object</a> / <a href='delobj.php'>Delete objects</a> / <a href='index.php'>Add new entries</a> / <b>Delete entries</b> / <a href='ip.php'>IP control</a> / <a href='smile.php'>Smilies</a> / <a href='com/logout.php'>Logout</a></div> 
...
$catid = $_GET['catid'];
if($_GET['obj_name']=$obj_name){

$sql = mysql_query("SELECT * FROM ".$so_prefix."_main WHERE obj_name='$obj_name' ORDER BY id"); 
echo "<h3>$obj_name entries:</h3><hr />";
while ($irow = mysql_fetch_row($sql)) {
echo "

<div class='container'>
<div class='spacer'>
  <b><a href='#' onclick='javascript: sel_ids($irow[0])'>ID: $irow[0]</a></b>  
</div>
<div class='ufloat'>
 By $irow[3] &nbsp;&nbsp;&nbsp;&nbsp; $irow[4]<br />
  </div>
...
Please enter the ID(s) of the row(s) that you want to delete. If more than one they must be separated by a comma. Eg: 1,3,5
<br /><br />To delete all records, input <i>*</i>
    </div>
...
";
      ...
      if(!$nrows=='0') { echo "<br /><br /><br />Please select an object from the left panel to delete entries."; }
      else { echo "<br /><br /><br />There are no objects available to delete entries."; }
     }
