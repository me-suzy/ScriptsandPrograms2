...
if ($row['radminsuper'] == 1) {
	...
	function BannersAdmin() {
		global $prefix, $db, $bgcolor2, $banners, $admin_file, $ad_admin_menu_main;
		include ("header.php");
		GraphicAdmin();
		OpenTable();
	...
	function ad_plans_add($name, $description, $delivery, $type, $price, $buy_links, $status) {
		global $prefix, $db, $banners, $admin_file, $ad_admin_menu;
		if ($name != "" AND $description != "" AND $delivery != "" AND $type != "" AND $price != "" AND $buy_links != "" AND $status != "") {
	...
	function ad_plans_edit($pid) {
		...
		echo "<tr><td>&nbsp;</td><td><input type=\"hidden\" name=\"op\" value=\"ad_plans_save\"><input type=\"submit\" value=\""._SAVECHANGES."\"></td></tr></table></form><br><center><i>"._PLANSNOTE."</i></center>";
		CloseTable();
		include ("footer.php");
	}
	...
	function ad_plans_save($pid, $name, $description, $delivery, $type, $price, $buy_links, $status) {
		global $prefix, $db, $banners, $admin_file, $ad_admin_menu;
		if ($name != "" AND $description != "" AND $delivery != "" AND $type != "" AND $price != "" AND $buy_links != "" AND $status != "") {
			$db->sql_query("UPDATE ".$prefix."_banner_plans SET active='$status', name='$name', description='$description', delivery='$delivery', delivery_type='$type', buy_links='$buy_links' WHERE pid='$pid'");
			Header("Location: ".$admin_file.".php?op=ad_plans");
			die();
		} else {
			include ("header.php");
