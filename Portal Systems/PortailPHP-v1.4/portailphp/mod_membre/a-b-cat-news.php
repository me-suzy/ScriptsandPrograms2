...
echo "<img border='0' src='themes/" . $_SESSION["App_Theme"] . "/ico-puce01.gif' />&nbsp;<strong>$Mod_Membres_Rub_News_Cat</strong><br /><br />" ;

if ($action == "Cat-News-Add")
{
    $res_catnews2 = sql_query("INSERT INTO $BD_Tab_docs_cat (DOC_nom ) VALUES ('" . AuAddSlashes($categorie) . "')", $sql_id) or
                    die("$Err_Insert") ;
    echo "<strong>$Mod_Membres_Rub_News_Cat_OK</strong>" ;
}

$res_catnews1 = sql_query("SELECT * FROM `$BD_Tab_docs_cat` ORDER BY DOC_nom ASC", $sql_id) ;
?>
...
<?php
while($row = mysql_fetch_object($res_catnews1))
{
    echo "<img border='0' src='themes/" . $_SESSION["App_Theme"] . "/ico-cat01.gif' />&nbsp;" . $row->DOC_nom . "<br />" ;
}
?>

<form name='formcatnews' method='post' onSubmit='return valider_formulaire(this)'
      action='<?php echo $chemin ; ?>/index.php?<?php echo $sid ; ?>affiche=Admin&admin=Cat-News&action=Cat-News-Add'>
<div align='left'>
