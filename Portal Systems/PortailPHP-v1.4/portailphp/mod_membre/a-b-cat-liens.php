...
echo "<img border='0' src='themes/" . $_SESSION["App_Theme"] . "/ico-puce01.gif' />&nbsp;<strong>$Mod_Membres_Rub_Liens_Cat</strong><br /><br />" ;

if ($action == "Cat-Liens-Add")
{
    $res_catlien2 = sql_query("INSERT INTO $BD_Tab_liens_cat (LIC_nom ) VALUES ('" . AuAddSlashes($categorie) . "')", $sql_id) or
                    die("$Err_Insert") ;
    echo "<strong>$Mod_Membres_Rub_Liens_Cat_OK</strong><br /><br />" ;
}

$res_catlien1 = sql_query("SELECT * FROM `$BD_Tab_liens_cat` ORDER BY LIC_nom ASC", $sql_id) ;
?>
...
{
    echo "<img border='0' src='themes/" . $_SESSION["App_Theme"] . "/ico-cat01.gif' />&nbsp:" . $row->LIC_nom . "<br />" ;
}
?>

<form name='formcatlien' method='post' onSubmit='return valider_formulaire(this)'
      action='<?php echo $chemin ; ?>/index.php?<?php echo $sid ; ?>affiche=Admin&admin=Cat-Liens&action=Cat-Liens-Add'>
<div align='left'>
