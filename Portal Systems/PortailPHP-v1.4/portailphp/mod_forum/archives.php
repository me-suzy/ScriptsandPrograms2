...
// Récupère la recherce
if (isset($_POST["search"]))
{
    $search = AuAddSlashes($_POST["search"]) ;
}
...
// Récupère le nombre de réponse possible
$request = mysql_query("SELECT id FROM $BD_Tab_forum WHERE texte LIKE '%$search%'");
...
$req = mysql_query("SELECT * FROM $BD_Tab_forum WHERE texte LIKE '%$search%' ORDER BY date_verif DESC LIMIT $limit1, $limit");
                ...
                <tr>
                  <td width="100%" align="center">
                    ...
                    <table border="0" cellpadding="4" cellspacing="0" width="<?php echo $largeur ; ?>">
                      <tr class='tabTitre'>
                        <td class='tabTitre'>&nbsp;<?php echo $Mod_Forum_ResultatRecherche ; ?> '<strong><?php echo $search ; ?></strong>'</td>
                        <td class='tabTitre' >&nbsp;Auteur :</td>
                        ...
                        <?php
                            ...
                            while($i != $res)
                            {
                                ...
                                if ($nb_rep >= 1)
                                {
                                    ...
                                    $TexteReponse = $row["date"] . " &agrave; " . $row["heure"] ;
                                }
                                else
                                {
                                    $TexteReponse = "" ;
...
for ($i = 1; $i < $nbre_page; $i++)
{
    ...
    {
        echo "<a href='index.php?" . $sid . "affiche=Forum-archives&pos=" . ($i - 1) . "&search=" . urlencode($search) . "'>" . $i ."</a>, " ;
    }
}
