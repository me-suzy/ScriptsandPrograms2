...
//Here there are 244 country rule lines but the free database included in this distribution (available from http://ip-to-country.com) has only 179 countries and it`s by far not as accurate and updated as our commercial database, available at http://www.analysespider.com
...
// EDIT cr/exceptions_ips.dat		//Exceptions (the script do not execute) apply to all visitors with IP addresses included in exceptions_ips.dat so edit and place there all IP addresses or IP ranges you want to be excluded from the scan process.
...
//Blocks certain IP address's from your site,It will be redirected to $anp_default_redirect_link
//and if $anp_default_reject_link is blank they will be accepted to view your pages
     ...
     if (anp_block_ips($anp_ip,$anp_ip_number)==1) 
       {
        anp_save_status("T",3);
        $ip_blocked = 2;
        $anp_url=$anp_default_reject_link;
       }  
    }
   }
...
// 1 : Save Visitor`s blocked status
