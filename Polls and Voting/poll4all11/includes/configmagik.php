...
class ConfigMagik{
	...
	*/
	...
	/**
	 * Incrementa di 1 il valore della chiave.
	 *
	 */
	function increment( $key, $section){
         	if ( $this->PROCESS_SECTIONS and !$section) {
			$err = "ConfigMagik::set() - Passed no section when in section-mode, nothing was set.";
			array_push( $this->ERRORS, $err);
			return false;
  		...
  		// check if section was passed
		if ( $section===true) $this->PROCESS_SECTIONS = true;
		// increment by one the value of the key in given section (if enabled)
		if ( $this->PROCESS_SECTIONS) {
		        if (is_numeric($this->VARS[$section][$key])) $this->VARS[$section][$key]++;
		        else return false;
		}else{
		        if (is_numeric($this->VARS[$key])) $this->VARS[$key]++;
			else return false;
		}
		// synchronize memory with file when enabled
		if ( $this->SYNCHRONIZE) {
