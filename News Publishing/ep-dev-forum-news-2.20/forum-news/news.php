...
class EP_Dev_Forum_News
{
	...
	function EP_Dev_Forum_News()
	{
		...
		$this->CONFIG = new EP_Dev_Forum_News_Config($absolute_path_override);
	...
	function news($number, $ids, $forums, $headlines = false)
	{
		...
		for($i=0; $i<count($forums); $i++)
		{
			...
			if (!is_object($this->FORUM[$forums[$i]]))
				$this->ERROR->stop("forum_disabled");

			// fetch data from forum db
			$this->FORUM[$forums[$i]]->fetch_Posts($number, $ids[$i], $headlines) ;
	...
	function display_News($in_number=NULL, $in_ids=NULL, $in_forums=NULL, $template_id=0, $headlines = false)
	{
		...
		if(($recycled_data = $this->getRecycledData($number, $ids, $forums, $template_id, $headlines)) !== false && $this->CONFIG->NEWS['recycle_time'] != 0)
		{
			echo $recycled_data;
		}
	...
	function getRecycledData($number, $ids, $forums, $template_id, $headlines)
	{
		/*
		...
		*/
		...
		if ($recycled_data)
		{
			if ($recycled_data[0] > (time() - $this->CONFIG->NEWS['recycle_time'] * 60))
