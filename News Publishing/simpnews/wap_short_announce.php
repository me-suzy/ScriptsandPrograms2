...
{
	if($wap_an_maxentries>0)
		$limitentries=$wap_an_maxentries;
}
$numentries=0;
...
if($start==0)
{
	$totalentries=0;
	$wap_data.="<card>".$crlf;
	$wap_data.="<p>".wml_encode($wap_an_title)."</p>".$crlf;
	$wap_data.="<p>".wml_encode($wap_an_description)."</p>".$crlf;
	if($catname)
		$wap_data.="<p>".wml_encode("$l_category: ".$catname)."</p>".$crlf;
	if($wap_copyright)
		$wap_data.="<p>".wml_encode($wap_copyright)."</p>".$crlf;
	$changetime=0;
	$sql = "select max(date) as maxdate, count(*) as totalentries from ".$tableprefix."_announce where wap_nopublish=0 and (expiredate>=$acttime or expiredate=0) and (firstdate<=$acttime or firstdate=0) ";
	if($category>=0)
	...
	$wap_data.="<p>".$crlf."<a href=\"".$baseurl."&amp;start=1\">".wml_encode($l_goto_announcements)."</a>".$crlf."</p>".$crlf;
	if(bittst($wap_options,BIT_10))
	{
		$wap_data.="<p><br /></p>".$crlf;
		$wap_data.="<p><a href=\"".$backscript.".php?mode=announce&amp;$langvar=$act_lang&amp;layout=$layout&amp;start=0\">";
...
{
	$sql = "select * from ".$tableprefix."_announce where wap_nopublish=0 and (expiredate>=$acttime or expiredate=0) and (firstdate<=$acttime or firstdate=0) ";
	if($category>=0)
		$sql.="and category='$category' ";
	else
	{
