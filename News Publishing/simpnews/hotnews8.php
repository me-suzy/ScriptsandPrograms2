...
<TD ALIGN="CENTER" VALIGN="MIDDLE" colspan="2"><font face="<?php echo $headingfont?>" size="<?php echo $headingfontsize?>" color="<?php echo $headingfontcolor?>"><b><?php echo display_encoded($heading)?></b></font></td></tr>
<?php
}
			...
			if($hotnewsmaxchars!=0)
			{
				...
				if(($hotnewsmaxchars>0) && strlen($displaytext)>$hotnewsmaxchars)
				{
					...
					if($usehnlinkdest==1)
						...
						$linkdest=$url_simpnews."/announce.php?$langvar=$act_lang&layout=$layout&category=".$tmprow["category"]."&announcenr=".$tmprow["entrynr"];
					$displaytext.="<br><a href=\"$linkdest\"";
					if($hotnews6target)
						$displaytext.=" target=\"$hotnews6target\"";
					$displaytext.=">$l_wholestory</a>";
...
else
{
	do
	{
		...
		if($hotnewsdisplayposter && (strlen($entrydata["poster"])>0))
		{
			...
			if($hotnewsnohtmlformatting==0)
				echo get_start_tag($posterstyle);
			echo "$l_poster: ".display_encoded($entrydata["poster"]);
			if($hotnewsnohtmlformatting==0)
				echo get_end_tag($posterstyle);
			echo "</font></td></tr>";
		}
