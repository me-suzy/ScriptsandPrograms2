...
if($dodebug)
	echo "<!-- Layout after get_settings: ".$layout." -->";
...
do
{
	...
	if(($allowcomments==1) && ($entrydata["allowcomments"]==1) && ($hotnewscommentslink==1))
	{
		...
		if($numcomments>0)
		{
			...
			else
			{
				...
				$commentlink.="?$langvar=$act_lang&layout=$layout&category=$category&newsnr=".$entrydata["newsnr"]."&srcscript=$srcscript";
			}
	...
	if(strlen($entrydata["heading"])>0)
	{
		...
		$displayheading=undo_html_ampersand($entrydata["heading"]);
		if($hotnewsnohtmlformatting==1)
			$displayheading=strip_tags($displayheading);
		$displayheading=do_htmlentities($displayheading);
		$displayheading=undo_htmlspecialchars($displayheading);
