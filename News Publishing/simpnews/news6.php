...
echo "<table align=\"left\" width=\"100%\" cellpadding=\"0\" cellspacing=\"0\">";
...
while($catrow=mysql_fetch_array($catresult))
{
	...
	else
	{
		...
		echo "<td align=\"left\" width=\"98%\">";
		echo "<font face=\"$categoryfont\" size=\"$categoryfontsize\" color=\"$categoryfontcolor\">";
		echo "&nbsp;<a class=\"catlink\" href=\"news4.php?$langvar=$act_lang&amp;layout=$layout&amp;category=0&amp;srcurl=$backurl\" target=\"$news2target\">";
		echo get_start_tag($categorystyle);
		echo do_htmlentities(stripslashes($l_general));
		echo get_end_tag($categorystyle);
		echo "</a></font></td></tr></table></td></tr>";
	}
	$announceavail=false;
	if(bittst($announceoptions,BIT_2))
	{
			...
			$tmpsql = "select * from ".$tableprefix."_announce where (expiredate>=$acttime or expiredate=0)  and (firstdate<=$acttime or firstdate=0) and category=".$localcatnr." ";
			if(isset($link_date))
	...
	$sql2 = "select * from ".$tableprefix."_data where category=".$localcatnr." ";
	if($separatebylang==1)
	{
...
<a class="morelink" href="news4.php?<?php echo $langvar=$act_lang?>&amp;layout=<?php echo $layout?>&amp;category=<?php echo $localcatnr?>&amp;srcurl=<?php echo $backurl?>" target="<?php echo $news2target?>">
