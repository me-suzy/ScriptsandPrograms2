...
if(isset($mode))
{
	if($mode=="impfile")
	{
		...
		if($errors==0)
		{
			$imported=import_eventfile($orgfile, $eventlang, $eventcat, $strip_tags);
...
function import_eventfile($inputfile, $eventlang, $eventcat, $strip_tags)
{
	global $tableprefix, $db, $userdata, $comments_allowed;

	$eventfile=fopen($inputfile,"r");
	...
	$workline=0;
	while($fileLine=fgets($eventfile,$maxsize))
	{
		...
		{
			$workline++;
			list($tmpdate,$tmptime)=explode(" ",$fileLine);
			list($evday,$evmonth,$evyear)=explode(".",$tmpdate);
			if($tmptime)
				list($evhour,$evmin)=explode(":",$tmptime);
			else
			{
				$evhour=0;
				$evmin=0;
			}
			$evdate=mktime($evhour,$evmin,0,$evmonth,$evday,$evyear);
			$eventdate=date("Y-m-d H:i:s",$evdate);
		}
		else if($workline==1)
		{
			$workline++;
		...
		else if(eregi("^\{newentry\}",$fileLine))
		{
			$workline=0;
			$entrytxt=addslashes($entrytxt);
			$sql="insert into ".$tableprefix."_events (lang, date, text, heading, poster, category, dontemail, posterid) values ('$eventlang','$eventdate','$entrytxt', '$evheading','$poster', $eventcat, 1, ".$userdata["usernr"].")";
			if(!$result = mysql_query($sql, $db))
