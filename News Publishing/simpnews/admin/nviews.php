...
$page_title="$l_viewcounts ($l_news)";
$page="nviews";
...
if(!isset($dostorefilter) && ($admstorefilter==1))
{
	...
	if($admcookievals)
	{
		if(sn_array_key_exists($admcookievals,"nviews_filterlang"))
			$filterlang=$admcookievals["nviews_filterlang"];
		if(sn_array_key_exists($admcookievals,"nviews_filtercat"))
			$filtercat=$admcookievals["nviews_filtercat"];
		if(sn_array_key_exists($admcookievals,"nviews_sorting"))
			$sorting=$admcookievals["nviews_sorting"];
	}
}
if($admin_rights < 2)
{
...
if(isset($mode))
{
	if($mode=="resetviews")
	{
		$sql="update ".$tableprefix."_data set views=0 where newsnr=$input_newsnr";
	...
	if($mode=="resetall")
	{
		$sql="update ".$tableprefix."_data set views=0";
		if(!$result=mysql_query($sql,$db))
		{
			echo "<table align=\"center\" width=\"80%\" CELLPADDING=\"1\" CELLSPACING=\"0\" border=\"0\" valign=\"top\">";
			echo "<tr><TD BGCOLOR=\"#000000\">";
...
$sql="select * from ".$tableprefix."_data where views>$minviews ";
...
else
{
	...
	while($myrow=mysql_fetch_array($result))
	{
		...
		{
			if($myrow["linknewsnr"]==0)
				echo "<a class=\"listlink\" href=\"".do_url_session($baseurl."&mode=resetviews&input_newsnr=$act_id")."\"><img src=\"gfx/clear.gif\" border=\"0\" title=\"$l_resetviews\" alt=\"$l_resetviews\"></a>";
			else
				echo "&nbsp;";
		}
		else
