...
<td><?php echo language_select2("all","filterlang","../language/")?></td></tr>
...
<tr class="optionrow"><td>&nbsp;</td><td><input type="checkbox" name="preview" value="1"> <?php echo $l_previewlist2?></td></tr>
<tr class="actionrow"><td align="center" colspan="2"><input type="submit" class="snbutton" name="submit" value="<?php echo $l_send?>"></td></tr></form>
</table></td></tr></table>
<?php
include('./trailer.php');
exit;
...
<tr><TD BGCOLOR="#000000">
<table align="center" width="100%" border="0" cellspacing="1" cellpadding="1">
<tr class="inforow"><td align="center" colspan="5"><?php echo $l_snprevnote?></td></tr>
<form name="sendlist" method="post" action="<?php echo $act_script_url?>">
<?php
	if(is_konqueror())
		echo "<tr><td></td></tr>";
	if($sessid_url)
...
<input type="hidden" name="<?php echo $langvar?>" value="<?php echo $act_lang?>">
<input type="hidden" name="mode" value="send">
<input type="hidden" name="onlyselected" value="0">
...
<?php
	...
	if($filterlang=="all")
		...
		$displaylang=$filterlang;
	echo "&nbsp;&nbsp;&nbsp;<b>$l_language:</b> $displaylang";
	echo "</td></tr>";
	...
	if($filterlang=="all")
		...
		$langs=array($filterlang);
	$hasheading=false;
	while(list($null, $sublang) = each($langs))
	{
		$sql="select min(lastsent) as minlast from ".$tableprefix."_subscriptions where confirmed=1 and language='$sublang'";
		if($catnr>0)
			$sql.=" and category=$catnr";
		if(!$result = mysql_query($sql, $db))
		    die("Unable to connect to database.".mysql_error());
		if($myrow=mysql_fetch_array($result))
		{
			$sql2="select * from ".$tableprefix."_data where linknewsnr=0 and added>'".$myrow["minlast"]."' and lang='".$sublang."' and dontemail=0";
			if($catnr>0)
			...
			$sql2.=" order by category, date desc";
			...
			if($myrow2=mysql_fetch_array($result2))
			{
				...
				do{
					$found++;
					if($myrow2["category"]>0)
					{
						$catsql="select * from ".$tableprefix."_categories where catnr=".$myrow2["category"];
						if(!$catresult = mysql_query($catsql, $db))
							die("Unable to connect to database.".mysql_error());
	...
	if($evnewsletterinclude==1)
	{
		...
		while(list($null, $sublang) = each($langs))
		{
			...
			if($myrow=mysql_fetch_array($result))
			{
				$sql2="select * from ".$tableprefix."_events where linkeventnr=0 and added>'".$myrow["minlast"]."' and lang='".$sublang."' and dontemail=0";
				if($catnr>0)
					$sql2.=" and category=".$catnr;
				else
				{
				...
				if($myrow2=mysql_fetch_array($result2))
				{
					if(!$hasheading)
					{
						...
						echo "<td align=\"center\" width=\"1%\">&nbsp;</td>";
						echo "<td align=\"center\" width=\"5%\"><b>#</b></td>";
						echo "<td align=\"center\" width=\"40%\"><b>$l_event</b></td>";
						echo "<td align=\"center\" width=\"20%\"><b>$l_date</b></td>";
						echo "<td align=\"center\" width=\"20%\"><b>$l_category</b></td>";
						echo "</tr>";
						$hasheading=true;
	...
	echo "<input type=\"button\" onclick=\"sendselected();\" value=\"$l_sendselectednews\" name=\"mailsel\" class=\"snbutton\">&nbsp;";
	echo "<input type=\"button\" value=\"$l_sendallnews\" name=\"mailall\" class=\"snbutton\" onclick=\"sendall()\">&nbsp;";
...
if($catnr>0)
	$sql.=" and category=$catnr or category=0";
if(!$result = mysql_query($sql, $db))
    die("Unable to connect to database.".mysql_error());
$sentmails=0;
$workedon=0;
...
while($myrow=mysql_fetch_array($result))
{
	...
	if(($onlyselected==0) || isset($selnews))
		include("./includes/mail_news.inc");
	if($evnewsletterinclude==1)
	{
		if(($onlyselected==0) || isset($selev))
			include("./includes/mail_events.inc");
	}
	...
	if($myrow["category"]>0)
	{
		$sql3="update ".$tableprefix."_categories set nlsenddate='$actdate' where catnr=".$myrow["category"];
...
<tr><TD BGCOLOR="#000000">
<table align="center" width="100%" border="0" cellspacing="1" cellpadding="1">
<tr class="displayrow"><td align="center"><?php echo "$l_emailssent ($sentmails/$workedon)"?></td></tr>
<?php
if($showsendprogress==1)
{
