...
if(!isset($dostorefilter) && ($admstorefilter==1))
{
	...
	if($admcookievals)
	{
		if(sn_array_key_exists($admcookievals,"smil_sorting"))
			$sorting=$admcookievals["smil_sorting"];
	}
}
?>
...
?>
<tr class="inputrow"><td align="right" width="30%"><?php echo $l_code?>:</td><td><input class="sninput" type="text" name="code" size="20" maxlength="20"></td></tr>
<tr class="inputrow"><td align="right" width="30%"><?php echo $l_url?>:</td><td><input class="sninput" type="text" name="emoticon_url" size="40" maxlength="100">
<?php
if($upload_avail)
	echo "<input class=\"snbutton\" type=\"button\" value=\"$l_emoticonupload\" onClick=\"openWindow('".do_url_session("emoticon_upload.php?$langvar=$act_lang")."')\">";
else
	echo "<input class=\"snbutton\" type=\"button\" value=\"$l_choose\" onClick=\"openWindow('".do_url_session("emoticon_upload.php?$langvar=$act_lang")."')\">";
?>
</td></tr>
<tr class="inputrow"><td align="right" width="30%"><?php echo $l_emotion?>:</td><td><input class="sninput" type="text" name="emotion" size="40" maxlength="80"></td></tr>
...
<?php
		...
		if($errors==0)
		{
			...
			$sql = "INSERT INTO ".$tableprefix."_emoticons (code, emoticon_url, emotion) ";
			$sql .="VALUES ('$code','$emoticon_url', '$emotion')";
			if(!$result = mysql_query($sql, $db))
			    die("<tr class=\"errorrow\"><td>Unable to add emoticon to database. ".mysql_error());
...
<tr class="inputrow"><td align="right" width="30%"><?php echo $l_code?>:</td><td><input class="sninput" type="text" name="code" size="20" maxlength="20" value="<?php echo do_htmlentities(stripslashes($myrow["code"]))?>"></td></tr>
<tr class="inputrow"><td align="right" width="30%"><?php echo $l_url?>:</td><td><input class="sninput" type="text" name="emoticon_url" size="40" maxlength="100" value="<?php echo do_htmlentities(stripslashes($myrow["emoticon_url"]))?>">
<?php
if($upload_avail)
	echo "<input class=\"snbutton\" type=\"button\" value=\"$l_emoticonupload\" onClick=\"openWindow('".do_url_session("emoticon_upload.php?$langvar=$act_lang")."')\">";
else
...
<tr class="inputrow"><td align="right" width="30%"><?php echo $l_emotion?>:</td><td><input class="sninput" type="text" name="emotion" size="40" maxlength="80" value="<?php echo do_htmlentities(stripslashes($myrow["emotion"]))?>"></td></tr>
...
<?php
		...
		if($errors==0)
		{
			...
			$emoticon_url=addslashes($emoticon_url);
			$emotion=addslashes($emotion);
			$sql = "UPDATE ".$tableprefix."_emoticons SET code='$code', emoticon_url='$emoticon_url', emotion='$emotion' ";
...
<a href="<?php echo do_url_session("$act_script_url?mode=new&$langvar=$act_lang")?>"><?php echo $l_addemoticon?>
</td></tr>
<?php
}
...
{
	case 11:
		$sql.="order by emoticon_url asc";
		break;
	case 12:
		$sql.="order by emoticon_url desc";
		break;
	case 21:
	...
	case 22:
		...
		break;
	case 31:
		$sql.="order by emotion asc";
		break;
	case 32:
		$sql.="order by emotion desc";
		break;
}
if(!$result = mysql_query($sql, $db))
...
else
{
	...
	echo "<td align=\"center\" width=\"40%\"><b>";
	echo "<a href=\"".do_url_session($sorturl)."\" class=\"sorturl\">";
	echo "$l_emotion</a>";
