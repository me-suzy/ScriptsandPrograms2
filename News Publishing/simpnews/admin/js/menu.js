...
function menuConstructor (id, content)
{
	...
	for (j = 0; j <= items.length - 2; j = j + 2)
	{
		...
		layerBody += '<td class=menurow height=' + itemHeight + ' width=' + this.width + ' bgcolor=' + backColor + '><a class=subMenu href='+ items [j + 1] +' ' + controlBlock + '>' + items [j] + '</a></td>';
		if (j < items.length - 2)
	...
	if (!isNC)
		layerHeader = '<div class=menurow id=Menu' + this.ID +
		...
		layerHeader = '<layer class=menurow id=Menu' + this.ID +
					   ' onMouseOver="enterMenu (' + this.ID + ');" onMouseOut = "exitMenu (' + this.ID + ');"' +
