...
<?php
if(($admin_rights<3) && bittst($secsettings,BIT_18) && !bittst($secsettings,BIT_1))
	$numavailcats=0;
else
	$numavailcats=1;
if(($admin_rights>2) || !bittst($secsettings,BIT_18))
...
else if(!bittst($secsettings,BIT_18))
{
	echo "<option value=\"0\"";
	if($newscat==0)
		echo "selected";
...
echo "<p></p>";
$sql = "select dat.* from ".$tableprefix."_data dat, ".$tableprefix."_search search where dat.newsnr = search.newsnr ";
if($newslang!="all")
{
	$sql.="and dat.lang='$newslang' ";
}
if($newscat>=0)
{
	$sql.="and dat.category=$newscat ";
}
else if(($admin_rights<3) && bittst($secsettings,BIT_18))
{
	$possiblecats=array();
	...
	do{
		...
		if($allowactions)
		{
			...
			if($myrow["linknewsnr"]==0)
			{
				...
				if($numattachs>0)
				{
					$dellink=do_url_session("news.php?$langvar=$act_lang&newslang=".$myrow["lang"]."&mode=delattach&input_newsnr=$act_id&catnr=".$myrow["category"]);
					if($admdelconfirm==2)
					...
					echo "<img src=\"gfx/delattach.gif\" border=\"0\" align=\"absmiddle\" alt=\"$l_delattach ($numattachs)\" title=\"$l_delattach ($numattachs)\"></a>";
				}
				else
					echo "&nbsp;";
			}
			...
			echo "<td class=\"adminactions\" align=\"center\" width=\"2%\" valign=\"top\">";
			if($myrow["linknewsnr"]==0)
				echo "<a href=\"".do_url_session("news.php?$langvar=$act_lang&newslang=".$myrow["lang"]."&mode=catlink&input_newsnr=$act_id&catnr=".$myrow["category"])."\"><img src=\"gfx/link.gif\" border=\"0\" title=\"$l_catlink\" alt=\"$l_catlink\"></a>";
			else
				echo "&nbsp;";
			echo "</td>";
			echo "<td class=\"adminactions\" align=\"center\" width=\"2%\" valign=\"top\">";
			if($myrow["linknewsnr"]==0)
				echo "<a href=\"".do_url_session("news.php?$langvar=$act_lang&newslang=".$myrow["lang"]."&mode=catmove&input_newsnr=$act_id&catnr=".$myrow["category"])."\"><img src=\"gfx/move.gif\" border=\"0\" title=\"$l_catmove\" alt=\"$l_catmove\"></a>";
			else
				echo "&nbsp;";
			echo "</td>";
			echo "<td class=\"adminactions\" align=\"center\" width=\"2%\" valign=\"top\">";
			if($myrow["linknewsnr"]==0)
				echo "<a href=\"".do_url_session("news.php?$langvar=$act_lang&newslang=".$myrow["lang"]."&mode=edit&input_newsnr=$act_id&catnr=".$myrow["category"])."\"><img src=\"gfx/edit.gif\" border=\"0\" title=\"$l_edit\" alt=\"$l_edit\"></a>";
			else
				echo "<a href=\"".do_url_session("news.php?$langvar=$act_lang&newslang=".$entrydata["lang"]."&mode=edit&input_newsnr=".$myrow["linknewsnr"]."&catnr=".$entrydata["category"])."\"><img src=\"gfx/edit.gif\" border=\"0\" title=\"$l_editoriginalentry\" alt=\"$l_editoriginalentry\"></a>";
			echo "</td>";
			echo "<td class=\"adminactions\" align=\"center\" width=\"2%\" valign=\"top\">";
			$dellink=do_url_session("news.php?$langvar=$act_lang&newslang=".$myrow["lang"]."&mode=del&input_newsnr=$act_id&catnr=".$myrow["category"]);
			if($admdelconfirm==2)
