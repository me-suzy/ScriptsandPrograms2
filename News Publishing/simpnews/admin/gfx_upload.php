...
if($user_loggedin==0)
	die($l_loginfirst);
if(($mode!=3) && ($userdata["rights"]<3))
	die("$l_functionnotallowed");
else if($userdata["rights"]<2)
	die("$l_functionnotallowed");
if(($userdata["rights"]==2) && bittst($userdata["addoptions"],BIT_4))
	$upload_avail=false;
...
if($myrow = mysql_fetch_array($result))
{
	...
	die("No settings for SimpNews defined");
if($mode!=3)
{
	$gfx_dir=$path_gfx;
	$pic_url=$url_gfx;
...
{
	$gfx_dir=$path_inline_gfx;
	$pic_url=$url_inline_gfx;
}
$gfx_basedir=$gfx_dir;
$gfx_baseurl=$pic_url;
if($subdir)
