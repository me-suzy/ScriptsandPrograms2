...
<?php
if(($admin_rights<3) && bittst($secsettings,BIT_19) && !bittst($secsettings,BIT_2))
	$numavailcats=0;
else
	$numavailcats=1;
if(($admin_rights>2) || !bittst($secsettings,BIT_19))
...
<td><?php echo language_select2($newslang,"newslang","../language/")?></td></tr>
...
else if(!bittst($secsettings,BIT_19))
{
	echo "<option value=\"0\"";
	if($newscat==0)
		echo "selected";
...
echo "<p></p>";
$sql = "select ev.* from ".$tableprefix."_events ev, ".$tableprefix."_evsearch evsearch where ev.eventnr = evsearch.eventnr ";
if($newslang!="all")
{
	$sql.="and ev.lang='$newslang' ";
}
if($newscat>=0)
{
	$sql.="and ev.category=$newscat ";
}
else if(($admin_rights<3) && bittst($secsettings,BIT_19))
{
	$possiblecats=array();
...
if(isset($timeframe))
{
	...
	$sql.= "and ev.date >= '$s_startdate' and ev.date <= '$s_enddate' ";
}
$musts=array();
$cans=array();
$nots=array();
...
if(count($musts)>0)
{
	...
	for($i=0;$i<count($musts);$i++)
	{
		if($first==1)
			...
			$sql .=" and ";
		$sql.="evsearch.text like '%".$musts[$i]."%'";
	}
	$sql .=")";
}
$first=1;
if(count($nots)>0)
{
	...
	for($i=0;$i<count($nots);$i++)
	{
		if($first==1)
			...
			$sql.=" and ";
		$sql.="evsearch.text not like '%".$nots[$i]."%'";
	}
	$sql .=")";
}
$first=1;
...
<?php
	do{
		...
		if($myrow["linkeventnr"]!=0)
			echo "<img src=\"gfx/link_small.gif\" border=\"0\" align=\"top\" title=\"$l_islink: ".$myrow["linkeventnr"]."\" alt=\"$l_islink: ".$myrow["linknewsnr"]."\"> ";
		else
		{
		...
		if($allowactions)
		{
			...
			if($myrow["linkeventnr"]==0)
			{
				...
				if($numattachs>0)
				{
					$dellink=do_url_session("events.php?$langvar=$act_lang&eventlang=".$myrow["lang"]."&mode=delattach&input_eventnr=$act_id&catnr=".$myrow["category"]);
					if($admdelconfirm==2)
					...
					echo "<img src=\"gfx/delattach.gif\" border=\"0\" align=\"absmiddle\" alt=\"$l_delattach ($numattachs)\" title=\"$l_delattach ($numattachas)\"></a>";
				}
				else
					echo "&nbsp;";
			}
			...
			echo "<td class=\"adminactions\" align=\"center\" width=\"2%\" valign=\"top\">";
			if($myrow["linkeventnr"]==0)
				echo "<a href=\"".do_url_session("events.php?$langvar=$act_lang&eventlang=".$myrow["lang"]."&mode=catlink&input_eventnr=$act_id&catnr=".$myrow["category"])."\"><img src=\"gfx/link.gif\" border=\"0\" title=\"$l_catlink\" alt=\"$l_catlink\"></a>";
			else
				echo "&nbsp;";
			echo "</td>";
			echo "<td class=\"adminactions\" align=\"center\" width=\"2%\" valign=\"top\">";
			if($myrow["linkeventnr"]==0)
				echo "<a href=\"".do_url_session("events.php?$langvar=$act_lang&eventlang=".$myrow["lang"]."&mode=catmove&input_eventnr=$act_id&catnr=".$myrow["category"])."\"><img src=\"gfx/move.gif\" border=\"0\" title=\"$l_catmove\" alt=\"$l_catmove\"></a>";
			else
				echo "&nbsp;";
			echo "</td>";
			echo "<td class=\"adminactions\" align=\"center\" width=\"2%\" valign=\"top\">";
			if($myrow["linkeventnr"]==0)
				echo "<a href=\"".do_url_session("events.php?$langvar=$act_lang&eventlang=".$myrow["lang"]."&mode=edit&input_eventnr=$act_id&catnr=".$myrow["category"])."\"><img src=\"gfx/edit.gif\" border=\"0\" title=\"$l_edit\" alt=\"$l_edit\"></a>";
			else
				echo "<a href=\"".do_url_session("events.php?$langvar=$act_lang&eventlang=".$entrydata["lang"]."&mode=edit&input_eventnr=".$myrow["linkeventnr"]."&catnr=".$entrydata["category"])."\"><img src=\"gfx/edit.gif\" border=\"0\" title=\"$l_editoriginalentry\" alt=\"$l_editoriginalentry\"></a>";
			echo "</td>";
			echo "<td class=\"adminactions\" align=\"center\" width=\"2%\" valign=\"top\">";
			$dellink=do_url_session("events.php?$langvar=$act_lang&eventlang=".$myrow["lang"]."&mode=del&input_eventnr=$act_id&catnr=".$myrow["category"]);
			if($admdelconfirm==2)
