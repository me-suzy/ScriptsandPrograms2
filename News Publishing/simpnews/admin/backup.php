...
if(isset($dobackup))
{
	...
	if($onlydata==1)
	{
		$dump_buffer.= "# SIMPNEWSDATABACKUP".$crlf;
		$dump_buffer.= "# SIMPNEWSBACKUPVERSION ".$backup_version.$crlf;
	}
	$dump_buffer.= "# SimpNews DatabaseBackup".$crlf;
	...
	$dump_buffer.= "# Version of SimpNews Backup generated by ".$version.$crlf;
	if($onlydata==1)
	{
		$dump_buffer.= "# Only table data dumped".$crlf;
		$dump_buffer.= "# --------------------------------------------------------".$crlf;
	...
	while($i < $num_tables)
	{
		...
		if($backuptable)
		{
			...
			if(($onlydata==0) || (!in_array($table,$exclude_tables)))
			{
				if($onlydata==1)
				{
					$dump_buffer.= "# deleting old data for table '".$table."'".$crlf;
					$dump_buffer.= "DELETE from ".$table.";#%%".$crlf.$crlf;
				}
				get_table_content($dbname, $table, 0, 0, 'my_handler', $onlydata);
...
<tr class="actionrow"><td align="center" colspan="2"><input type="submit" class="snbutton" name="submit" value="<?php echo $l_ok?>"></td></tr></form>
</table></td></tr></table>
<?php
include('./trailer.php');
...
function get_table_content($db, $table, $limit_from = 0, $limit_to = 0, $handler, $onlydata)
{
    ...
    get_table_content_fast($db, $table, $add_query, $handler, $onlydata);

}

function get_table_content_fast($db, $table, $add_query = '', $handler, $onlydata)
{
...
function my_handler($sql_insert, $onlydata)
{
	...
	if(empty($asfile))
		$tmp_buffer.= do_htmlspecialchars("$sql_insert;");
	else
