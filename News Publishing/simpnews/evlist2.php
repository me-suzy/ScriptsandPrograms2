...
if($mode=="print")
{
	...
	$news5monthbgcolor="#ffffff";
	$news5monthfontcolor="#000000";
	$news5yearbgcolor="#ffffff";
	$news5yearfontcolor="#000000";
	$nostyles=true;
	if($printheader==0)
	{
		$usecustomheader=0;
...
$displaytime=mktime(23,59,59,$month,$day,$year);
...
echo "</font></td></tr>";
if(($mode!="print") && (($news5noglobalprint==0) || ($asclist==1) || ($enableevpropose==1) || ($enableevsearch==1) || ($exporttype>0)))
{
?>
...
{
	echo "<a href=\"$baseurl&mode=print\" target=\"printwindow\">";
	if($printpic)
	...
	if(mysql_num_rows($tmpresult)>0)
	{
		...
		while($tmprow=mysql_fetch_array($tmpresult))
		{
			...
			if($ev2onlyheadings==1)
			{
				...
				echo "<td align=\"left\" bgcolor=\"$newsheadingbgcolor\">";
				echo "<font face=\"$newsheadingfont\" size=\"$ev2_newsheadingfontsize\" color=\"$newsheadingfontcolor\">";
				$linkdest="announce.php?$langvar=$act_lang&amp;layout=$layout&amp;announcenr=".$tmprow["entrynr"]."&amp;category=".$tmprow["category"]."&amp;backurl=$backurl";
			...
			else
			{
				...
				if(strlen($tmprow["heading"])>0)
				{
					...
					echo do_htmlentities($tmprow["heading"]);
				...
				if($news5displayposter && (strlen($tmprow["poster"])>0) && (($attachpos==0) || ($mode=="print")))
				{
					$posterlinked=false;
...
if($myrow=mysql_fetch_array($result))
{
	do
	{
		...
		if($currentmonth!=$month)
		{
			...
			echo "</font></td></tr>";
			if(bittst($announceoptions,BIT_1) && bittst($announceoptions,BIT_6))
				display_announcements2($year,$month,$category, $act_lang, $layout, ($mode=="print")?true:false, $maxannounce);
		}
		echo "<tr>";
		...
		if($ev2onlyheadings==1)
		{
			if(strlen($entrydata["heading"])>0)
				$displaytext=do_htmlentities($entrydata["heading"]);
			else
			{
			...
			echo "<td align=\"left\" bgcolor=\"$newsheadingbgcolor\">";
			echo "<font face=\"$newsheadingfont\" size=\"$ev2_newsheadingfontsize\" color=\"$newsheadingfontcolor\">";
			$linkdest="events.php?$langvar=$act_lang&amp;layout=$layout&amp;link_date=".$link_date."&amp;category=".$entrydata["category"]."&amp;backurl=$backurl&amp;highlight=".$entrydata["eventnr"]."#".$entrydata["eventnr"];
			echo "<a title=\"$l_showentry\" class=\"newslink\" href=\"$linkdest\" target=\"$news5linktarget\">";
			echo $displaytext;
			echo "</a></font></td></tr>";
		}
		...
		$displaytext = undo_htmlspecialchars($displaytext);
		echo $displaytext."</font></td></tr>";
		if($news5displayposter && (strlen($entrydata["poster"])>0) && (($attachpos==0) || ($mode=="print")))
		{
			$posterlinked=false;
			echo "<tr bgcolor=\"$posterbgcolor\"><td align=\"left\" colspan=\"3\">";
			if($linkposter==1)
