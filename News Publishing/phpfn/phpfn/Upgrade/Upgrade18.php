...
<HTML>
	<HEAD>
		<TITLE><?= $SiteDescription ?> Upgrade to Version 1.8</TITLE>
		...
		<BODY>
			...
			if ($Step == '')
			{
				// First Stage
				?>
				Welcome to the Version 1.8 Upgrade Script for <B><?= $SiteDescription ?></B>.<BR>
				<BR>
				This script will update your database to make it compliant with Version 1.8 of <?= $SiteDescription ?>.
				<BR>
				The installer will upgrade database <B>"<?= $db ?>"</B>. Remember to upgrade every database you have.
				<BR><BR>
				The MySql User in your Config.php file must have the following permissions: ALTER.<BR>
				Once the database has been installed you can safely revoke ALTER.
				<BR>
				<BR>
			...
			elseif ($Step == '1')
			{
				...
				$sql_query = mysql_query("ALTER TABLE news_posts ADD Archived TINYINT DEFAULT '0' NOT NULL");
				if ($sql_query)
				{
					echo ('<b> Done!</b>');
				}
