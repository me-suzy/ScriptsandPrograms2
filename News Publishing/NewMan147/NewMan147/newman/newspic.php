...
if($psw == TRUE)
 ...
 else if ($action == "multidel") MultiDelete($db_news_pics,"id","news_del");
 else ShowMain();
include ("footer.php");
...
function ShowMain()
{
 ...
 <div align="right" style="padding-right:5px;">
  <form action="<?=$GLOBALS['PHP_SELF'];?>" method="post" style="padding: 0;">
   ...
   <select name="filter" class="news">
    <option value="">/</option>
     <?
      $rest = mysql_query("SELECT * FROM ".$GLOBALS['db_news_pics_groups']." ORDER BY name");
 ...
 <table width="630" cellspacing="2" cellpadding="1" class="MojText">
  <tr bgcolor="#<?=_COLOR02;?>">
   ...
   <td><font color="#<?=_COLOR05;?>"><?= _ID;?></font></td>
  ...
  <?
   ...
   while ($ar = mysql_fetch_array($res))
   {
    ...
    {
     $res2 = mysql_query("SELECT name from ".$GLOBALS['db_news_pics_groups']." WHERE id='".$ar[gid]."'");
...
function Edit()
{
 ...
 if ($_POST['confirm'] == "picture") 
 {
  ...
  mysql_query("UPDATE ".$GLOBALS['db_news_pics']." SET picture='".$HTTP_POST_FILES['filename']['name']."' WHERE id='".$_POST['id']."'") or die("<b>LINE 262:</b>".mysql_error());
  ShowMain();
 } 
