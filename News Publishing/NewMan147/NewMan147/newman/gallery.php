...
if($psw == TRUE)
 ...
 else if ($action == "multidel") MultiDelete($db_gallery_groups,"id","gallery_del");
 else ShowMain();
include ("footer.php");
...
function ShowMain()
{
 ...
 <table width="630" cellspacing="2" cellpadding="0" class="MojText">
  <tr>
   <td width="35"><a href="<?=$GLOBALS['PHP_SELF'];?>?action=add"><img src="gfx/partners_big.jpg" width="32" height="32" border="0" alt="<?=_ADDGALLERY;?>"/></a></td>
   <td width="100"><a href="<?=$GLOBALS['PHP_SELF'];?>?action=add">&nbsp;<?=_ADDGALLERY;?></a></td>
   <td><a href="<?=$GLOBALS['PHP_SELF'];?>?action=upload"><?=_UPLOADPICTURE;?></a></td>
   <td align="center">
   <?
    $res = mysql_query("SELECT * FROM ".$GLOBALS['db_gallery_groups']) or die("<b>LINE 31:</b>".mysql_error());
    $num = mysql_num_rows($res);
    $myopt = ShowPages($num,$GLOBALS['page'],$GLOBALS['hits'],$GLOBALS['show']);
   ?>
   </td>
   <td align="right"><?=_SUBMITEDGALLERIES.": ".$num;?></td>
  </tr>
 </table>

 <form action="<?=$GLOBALS['PHP_SELF'];?>" name="myform" method="post" style="top:-100px;">
 <table width="630" cellspacing="2" cellpadding="1" class="MojText">
  <tr bgcolor="#<?=_COLOR02;?>">
   ...
   <td width="30"><font color="#<?=_COLOR05;?>">#</font></td>
   <td width="20"><font color="#<?=_COLOR05;?>"><?= _CHECK;?></font></td>
  </tr>  
  <?
   $res = mysql_query("SELECT * from ".$GLOBALS['db_gallery_groups']." ORDER BY name LIMIT $myopt[0],$myopt[1]");
   while ($ar = mysql_fetch_array($res))
   {
    if($ar[root] == 0) $root = "/";
    else
    {
     $res2 = mysql_query("SELECT name from ".$GLOBALS['db_gallery_groups']." WHERE id='".$ar[root]."'");
     $ar2 = mysql_fetch_array($res2);
     $root = $ar2[name];
    }
    $res3 = mysql_query("SELECT COUNT(*) as num FROM ".$GLOBALS['db_gallery']);
    $ar3 = mysql_fetch_array($res3);
    //if(empty($ar3[num])) $ar3[num] = 0;
    ?>
    <tr>
     <td width="44">
     ...
     <td align="right" valign="top"><?=$ar3[num];?></td>
...
function AddGallery()
{
 ...
 <table width="630" cellspacing="2" cellpadding="0" class="MojText">
  <tr>
   <td width="20"><a href="<?=$GLOBALS['PHP_SELF'];?>"><img src="gfx/category_big.jpg" width="32" height="32" border="0" alt="<?=_ADDGALLERY;?>"/></a></td>
   <td width="100"><a href="<?=$GLOBALS['PHP_SELF'];?>"><?=_MAINMENU;?></a></td>
   <td><a href="<?=$GLOBALS['PHP_SELF'];?>?action=upload"><?=_UPLOADPICTURE;?></a></td>
   <td align="center"><font size="4" face="Arial"> <b><?=_ADDGALLERY;?></b></font></td>
   <td align="right">
    <?
    $res = mysql_query("SELECT * FROM ".$GLOBALS['db_gallery_groups']) or die("<b>Line 246:</b>".mysql_error());
    echo _SUBMITEDGALLERIES.": ".mysql_num_rows($res);
    ?>
   </td>
  </tr>
 </table>
 ...
 <table width="630" cellspacing="2" cellpadding="0" class="MojText">
  <tr>
   <td>
     ...
     <?=_DESCRIPTION;?><br/>
     <textarea name="message" class="news" rows="" cols="" style="width:98%;height:150px;"><?=$GLOBALS['description'];?></textarea><br />
     <?=_CATEGORY;?><br/>
 ...
 if ($_POST['confirm'] == "true") 
 {
  ...
  $res = mysql_query("INSERT INTO ".$GLOBALS['db_gallery_groups']." VALUES (0,'".$GLOBALS['login']."','".$_POST['name']."','".$_POST['description']."',NOW(),'".$_POST['root']."','".$HTTP_POST_FILES['filename']['name']."')") or die("<b>LINE 200:</b>".mysql_error()); 
  ShowMain();
 } 
}
...
function EditGallery()
{
 ...
 {
  $res = mysql_query("SELECT * FROM ".$GLOBALS['db_gallery_groups']." WHERE id='".$_GET['id']."'");
   ...
   <td>
	...
	while($ar2 = mysql_fetch_array($res2))
	{
	  if($ar[id]<>$ar2[id])
	  {
	   ...
	   if($ar2[id] == $ar[root]) echo ' selected="selected"';
 ...
 if ($_POST['confirm'] == "true") 
 {
  ...
  $res = mysql_query("UPDATE ".$GLOBALS['db_gallery_groups']." SET name='".$_POST['name']."', description='".$_POST['description']."', root='".$_POST['root']."' WHERE id='".$_POST['id']."'") or die("<b>LINE 262:</b>".mysql_error());
  ShowMain();
 } 
 ...
 if ($_POST['confirm'] == "picture") 
 {
  ...
  mysql_query("UPDATE ".$GLOBALS['db_gallery_groups']." SET picture='".$HTTP_POST_FILES['filename']['name']."' WHERE id='".$_POST['id']."'") or die("<b>LINE 262:</b>".mysql_error());
  ShowMain();
 } 
...
function DeleteGallery()
{
 ...
 if(CheckPriv("gallery_edit") == 1) 
   mysql_query("DELETE FROM ".$GLOBALS['db_gallery_groups']." WHERE id='".$_GET['id']."'"); 
 else
  echo "<script type=\"text/javascript\">alert('"._NOTENOUGHPRIV."');</script>";
 ShowMain();
}
