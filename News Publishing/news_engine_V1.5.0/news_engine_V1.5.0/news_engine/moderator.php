...
|
|	> $Id: moderator.php 2 2005-10-08 09:40:29Z alex $
|
+--------------------------------------------------------------------------
*/
...
if($_POST['mod']=='confirmedit' && $_POST['comid']) {
    if($auth->user['caneditcomments']) {
        ...
        if(!$_POST['delete_comment']) {
            $db_sql->sql_query("UPDATE $newscomment_table SET com_headline='".addslashes($_POST['headline'])."', post_comment='".$comment_message."', user_comname='".addslashes($_POST['poster'])."', posticon='".addslashes($_POST['posticon'])."' WHERE comid='".intval($_POST['comid'])."'");
            $message .= $lang['rec_error36'];
        }
    }
    
    if($auth->user['candeletecomments'] && $_POST['delete_comment']) {
        $db_sql->sql_query("DELETE FROM $newscomment_table WHERE comid='".intval($_POST['comid'])."'");
        $message .= $lang['rec_error37'];
    }
    
   rideSite($sess->url('newscomments.php?postid='.$_POST['postid']), $message);
   exit();    
}
