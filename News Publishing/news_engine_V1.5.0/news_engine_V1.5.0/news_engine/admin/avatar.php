...
|
|	> $Id: avatar.php 2 2005-10-08 09:40:29Z alex $
|
+--------------------------------------------------------------------------
*/
...
if(isset ($action) && $action=="add") {
    if(!@is_writeable($filesdir)) {
    	...
    	if($my_upload->uploadFile("upl_av")) {
    		$message = $a_lang['uploads_ok1'];
    		$new_name = $my_upload->getDestName();
			$avatid = InsertNewAvatar($new_name);
    	} else {
...
if($step == "rename") {
    ...
    buildFormFooter($a_lang['afunc_200'], "", $colspan = 2, $a_lang['afunc_11']);
}
...
#### Avatar löschen ###
    ...
    buildHeaderRow($a_lang['afunc_12'],"delart.gif");
    buildFormHeader("avatar.php", "post", "del");
    buildHiddenField("avatarid",$avatarid);
    buildTableHeader("$a_lang[afunc_259]: <u>$del[avatardata]</u>");
