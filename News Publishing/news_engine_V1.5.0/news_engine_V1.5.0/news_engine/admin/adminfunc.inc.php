...
|
|	> $Id: adminfunc.inc.php 2 2005-10-08 09:40:29Z alex $
|
+--------------------------------------------------------------------------
*/
...
$source_file_array = array("adminutil.php","avatar.php","bbhelp.php","comment.php","frame.php","global.php","groups.php","head.php","index.php","league.php","main.php","news.php","member.php","navi.php","upload.php","settings.php","templates.php","newscat.php","newsletter.php");
...
function ConfirmComment() {
    global $config, $newscomment_table, $preart_table, $bbcode,$a_lang,$db_sql,$sess;
    $no = 1;
    $result = $db_sql->sql_query("SELECT $newscomment_table.* FROM $newscomment_table WHERE com_status='2'");
	...
	if($db_sql->num_rows($result) >= 1) { 
        while ($com = $db_sql->fetch_array($result)) {
            ...
            if(strlen($com['post_comment'])>50) $com['post_comment'] = substr($com['post_comment'],0,35)."...";
            ...
            buildDarkColumn("<b>$com[newsid]</b>");
            buildDarkColumn(ConfNews($com['newsid']));
            ...
            buildDarkColumn("<a href=\"".$sess->adminUrl("comment.php?step=conf&comid=".$com['comid'])."\"><img src=\"images/okcom.gif\" width=\"16\" height=\"16\" border=\"0\" alt=\"$a_lang[afunc_7]\"></a>&nbsp;
                            <a href=\"".$sess->adminUrl("comment.php?step=del&comid=".$com['comid'])."\"><img src=\"images/delcom.gif\" width=\"16\" height=\"16\" border=\"0\" alt=\"$a_lang[afunc_8]\"></a>&nbsp;
		...
		buildDarkColumn($a_lang['afunc_315'],1,1,6);
        buildTableFooter("",6);	
	}
}

function ConfirmNews2() {
    global $config, $news_table,$newscat_table, $bbcode,$a_lang,$db_sql,$sess;
    $no = 1;
	$result = $db_sql->sql_query("SELECT * FROM $news_table WHERE published='0'");    
    if($db_sql->num_rows($result) >= 1) buildFormHeader("news.php", "post", "conf_news_multi");
    echo "<table bgcolor=\"#000000\" width=\"100%\" cellspacing=\"0\" cellpadding=\"0\" border=\"0\">\n<tr>\n<td>\n";
    echo "<table cellpadding=\"3\" cellspacing=\"1\" border=\"0\" width=\"100%\">\n";
    echo "<tr>\n<td class=\"menu_desc\">&nbsp;</td>";
    echo "<td class=\"menu_desc\" width=\"5%\"><b>$a_lang[afunc_1]:</b></td>";
	...
	if($db_sql->num_rows($result) >= 1) { 
		while ($newnews = $db_sql->fetch_array($result)) {
			$newnews = stripslashes_array($newnews);
			$news_date = $newnews['newsdate'];
			$date = getdate($news_date);
			
			$poster = CheckUserID($newnews['userid']);
			$newsposter = "<a class=\"menu\" href=\"mailto:$poster[useremail]\">$poster[username]</a>";
			
			$newstext = $bbcode->rebuildText($newnews['newstext']);
			$newstext = trim($newstext);
			if(strlen($newnews['hometext'])>80) $newnews['hometext'] = substr($newnews['hometext'],0,80)."...";
			$hometext = $bbcode->rebuildText($newnews['hometext']);
			$hometext = trim($hometext);	        
	         
	        buildDarkColumn("<input type=\"checkbox\" name=\"newsid[$newnews[newsid]]\">",1);             
            buildDarkColumn("<b>$newnews[newsid]</b>");
            buildDarkColumn($a_lang['afunc_14'].": <b>".stripslashes(trim($newnews['headline']))."</b><br>".NewNews($newnews['newsid']));
            buildDarkColumn($hometext);
            buildDarkColumn("<b>".trim($newsposter)."</b> / <b>$date[mday].$date[mon].$date[year]</b>");
            buildDarkColumn("<a href=\"".$sess->adminUrl("news.php?step=conf&newsid=".$newnews['newsid'])."\"><img src=\"images/okcom.gif\" width=\"16\" height=\"16\" border=\"0\" alt=\"$a_lang[afunc_11]\"></a>&nbsp;
                            <a href=\"".$sess->adminUrl("news.php?step=del&newsid=".$newnews['newsid'])."\"><img src=\"images/delcom.gif\" width=\"16\" height=\"16\" border=\"0\" alt=\"$a_lang[afunc_12]\"></a>&nbsp;
                            <a href=\"".$sess->adminUrl("news.php?step=edit&newsid=".$newnews['newsid'])."\"><img src=\"images/edit.gif\" width=\"16\" height=\"16\" border=\"0\" alt=\" $a_lang[afunc_13]\"></a>",0,1);
            $no++;
        }	
    	echo "<tr class=\"table_footer\">\n<td colspan=\"6\" align=\"left\">\n";
        echo "<img src= \"images/arrow.gif\" border=\"0\" align=\"absmiddle\">";
        echo "<input type=\"submit\" name=\"public\" value=\"   ".$a_lang['afunc_11']."   \" class=\"button\">\n";
        echo "<input type=\"submit\" name=\"delete\" value=\"   ".$a_lang['afunc_12']."   \" class=\"button\">\n";
    	echo "&nbsp;\n</td>\n</tr>\n</table>\n";
    	echo "</td>\n</tr>\n</table>\n";
    	echo "</form><br />\n";        
	} else {
		buildDarkColumn($a_lang['afunc_316'],1,1,6);
        buildTableFooter("",6);	
	}
    
}

function ConfNews($newsid) {
	global $newscat_table, $news_table,$config,$a_lang,$db_sql;
	$result = $db_sql->sql_query("SELECT catid,headline FROM $news_table WHERE newsid='$newsid'");
	list($catid,$headline) = mysql_fetch_row($result);
	
	$result2 = $db_sql->sql_query("SELECT titel FROM $newscat_table WHERE catid='$catid'");
	list($titel) = mysql_fetch_row($result2);	
	return $a_lang['afunc_15'].": <b>".stripslashes($headline)."</b><br><span class=\"smalltext\">$a_lang[afunc_16]: ".stripslashes($titel)."</font>";
}

function NewNews($newsid) {
	global $newscat_table, $news_table,$config,$a_lang,$db_sql;
	$result = $db_sql->sql_query("SELECT catid,headline FROM $news_table WHERE newsid='$newsid'");
	list($catid,$headline) = mysql_fetch_row($result);
	...
	return "<span class=\"smalltext\">$a_lang[afunc_16]: ".stripslashes($titel)."</font>";
}

function GetCatInfo($catid) {
	global $newscat_table,$db_sql;
	$sql = $db_sql->query_array("SELECT * FROM $newscat_table WHERE catid='$catid'");
	return stripslashes_array($sql);
} 
		 
function GetNewsPic($pic_n) {
    global $a_lang;
    $sel_option = "<select class=\"input\" name=\"pic_n\">\n";
    if ($pic_n == 0) {
        $select = "selected";
    } elseif ($pic_n == 1) {
        $select1 = "selected";
    ...
    $sel_option .= "<option value=\"0\" $select>$a_lang[afunc_126]</option>\n";
    $sel_option .= "<option value=\"1\" $select1>$a_lang[afunc_127]</option>";
    $sel_option .= "<option value=\"2\" $select2>$a_lang[afunc_128]</option>\n";		 
    $sel_option .= "</select>";
    return $sel_option;
}

function getPreviewTemplate($tpl,$endung="html") {
    global $config, $templatecache, $_ENGINE;
    $tpl_folder = $_ENGINE['template_folder'];
    
    if(!isset($templatecache[$tpl])) {
        $templatecache[$tpl] = implode("",file("../".$tpl_folder."/".$tpl.".".$endung));
    }
    
    $template = str_replace("\"","\\\"",$templatecache[$tpl]);
    
    if($config['addtplname'] == 1) {
        $template = "\n<!-- START TEMPLATE: $tpl.".$endung." -->\n$template\n<!-- END TEMPLATE: $tpl.".$endung." -->\n";
    }
    
    return $template;
}	
	
function displayPreview($tpl,$ispopup=0) {
    global $config,$user,$own_header,$own_footer,$db_sql,$_SESSION,$u_id,$u_pw,$title,$sess,$auth,$lang,$_ENGINE;
    global $set_table,$user_table,$news_table,$newscat_table,$newslinks_table,$newscomment_table,$avat_table,$group_table;
    
    $tpl = str_replace("{newsscripturl}","$config[newsscripturl]",$tpl);
    $tpl = str_replace("{mainurl}","$config[mainurl]",$tpl);
    $tpl = str_replace("{smilieurl}","$config[smilieurl]",$tpl);
    $tpl = str_replace("{grafurl}","$config[grafurl]",$tpl);
    $tpl = str_replace("{graflanguageurl}","$_ENGINE[languageurl]",$tpl);    
    $tpl = str_replace("{avaturl}","$config[avaturl]",$tpl);
    $tpl = str_replace("{fileurl}","$config[fileurl]",$tpl);
    $tpl = str_replace("{thumburl}","$config[thumburl]",$tpl);
    $tpl = str_replace("{scriptname}","$config[scriptname]",$tpl);
    $tpl = str_replace("{textcol}","$config[textcol]",$tpl);
    $tpl = str_replace("{textcol2}","$config[textcol2]",$tpl);
    $tpl = str_replace("{hovercol}","$config[hovercol]",$tpl);
    $tpl = str_replace("{bordercol}","$config[bordercol]",$tpl);
    $tpl = str_replace("{maincol}","$config[maincol]",$tpl);
    $tpl = str_replace("{primcol}","$config[primcol]",$tpl);
    $tpl = str_replace("{postcol1}","$config[postcol1]",$tpl);
    $tpl = str_replace("{postcol2}","$config[postcol2]",$tpl);
    $tpl = str_replace("{fontf}","$config[fontf]",$tpl);
    $tpl = str_replace("{mainwidth}","$config[mainwidth]",$tpl);
    $tpl = str_replace("{sess_name}",$sess->sess_name,$tpl);
    $tpl = str_replace("{sess_id}",$sess->sess_id,$tpl);
    $tpl = str_replace("{user}","".stripslashes($user['username'])."",$tpl);
    $tpl = str_replace("{APP_VERSION}",APP_VERSION,$tpl);
    $tpl = str_replace("{doctype}",$lang['doctype'],$tpl);
    $tpl = str_replace("{charset}",$lang['charset'],$tpl);         
    echo $tpl;
}	
	 
function memberList($username="",$start=0) {
    global $config,$group_table,$avat_table,$user_table,$newscomment_table, $a_lang,$db_sql,$sess,$_ENGINE,$start;
    $count = 0;
    ...
    if($username == "") {    
        ...
        $page_link = "<br><span class=\"smalltext\">&nbsp;$a_lang[afunc_291]: $pagecount</span>";  
    } else {
	...
	/*echo "<table bgcolor=\"#000000\" width=\"100%\" cellspacing=\"0\" cellpadding=\"0\" border=\"0\">\n<tr>\n<td>\n";
	echo "<table cellpadding=\"3\" cellspacing=\"1\" border=\"0\" width=\"100%\">\n";   */
	buildTableDescription(array($a_lang['afunc_168'],$a_lang['afunc_24'],$a_lang['afunc_170'],$a_lang['afunc_171'],$a_lang['afunc_172'],$a_lang['afunc_173']),1);	
   /* echo "<tr class=\"menu_desc\">";
    buildListColumn("<b>$a_lang[afunc_168]</b>");
    buildListColumn("<b>$a_lang[afunc_24]</b>");
    ...
    echo "</tr>";*/
            ...
            while($user = $db_sql->fetch_array($result2)) {
				...
				$kom_i = 0;
				$zahler = $db_sql->sql_query("SELECT comid FROM $newscomment_table where userid='$user[userid]' and com_status='1'");
    ...
    buildExternalItems($a_lang['afunc_318'],"member.php?step=add","add.gif");
}
...
function memberForm($userid="") {
    global $config,$group_table,$avat_table,$user_table,$guestcomment_table, $a_lang,$lang,$db_sql,$_ENGINE;
    ...
    //buildInputRow($a_lang['afunc_186']."<br><a class=\"menu\" href=\"JavaScript:Avatar()\">$a_lang[afunc_201]</a>", "avatarid", $user['avatarid'], "35");
    ...
    buildFormFooter($a_lang['afunc_190'], $a_lang['afunc_79']);		 
}
...
function AddMemberData($username,$userpassword,$useremail,$userhp,$groupid,$avatarid,$show_email_global,$blocked,$regdate,$lastvisit) {
    ...
    $db_sql->sql_query("INSERT INTO $user_table (username,userpassword,useremail,regdate,lastvisit,userhp,groupid,avatarid,show_email_global,blocked,activation,canuploadfile)
                VALUES ('".addslashes($username)."','$userpassword','".addslashes($useremail)."',".$regdatestamp.",".$lastvisitstamp.",'".reBuildURL(addslashes($userhp))."','$groupid','$avatarid','$show_email_global','$blocked','1','$canuploadfile')");
...
function GetCateg() {
	...
	while ($cat = $db_sql->fetch_array($result)) {
		$option .= "<option value=\"$cat[catid]\">".stripslashes($cat['titel'])."</option>\n\t\t\t\t";
	}
    return $option;
}
		
function GetActCat($catid) {
	global $newscat_table,$db_sql;
	$result = $db_sql->sql_query("SELECT catid, titel FROM $newscat_table");
	while ($cat = $db_sql->fetch_array($result)) {
		if ($cat['catid'] == $catid) {
		...
		$option_list .= "<option value=\"$cat[catid]\" $sel>".stripslashes($cat['titel'])."</option>\n\t\t\t\t";
	}
    return $option_list;
}	
		
function GetCommentNor($news) { 
	global $config,$newscomment_table;
	return mysql_num_rows(mysql_query("SELECT comid from $newscomment_table where newsid='$news' and com_status='1'"));
}
...
<html>
<head>
<title>NewsEngine - AdminCenter</title>
...
function Uploadimage() {
    ...
    neu.location.href = '<?php echo str_replace("&amp;","&",$sess->adminUrl("newscat.php?step=thumb")); ?>';
    }
}

function UploadNewsimage() {
    neu = window.open('', 'Neues', 'width=600,height=500,resizable=0,scrollbars=1');
    if (neu != null) {
    if (neu.opener == null) {
        neu.opener = self;
        }
    neu.location.href = '<?php echo str_replace("&amp;","&",$sess->adminUrl("news.php?step=catgraf")); ?>';
    }
}
...
function Helpfile() {
    ...
    neu.location.href = '<?php echo str_replace("&amp;","&",$sess->adminUrl("news.php?step=bbhelp")); ?>';
    }
}
//-->
</script>	
...
?>
<img style="margin-left: -20px" src="images/gif.gif" width="50" height="18" border="0">
	<?php		
		}
	}    
...
function buildRedirectJS($targetpage, $timeout = 2) {
	...
	echo '<p align="center" class="smallfont"><a href="' . $targetpage . '" onclick="javascript:clearTimeout(timerID);">Bitte hier klicken, falls keine automatische Weiterleitung erfolgt</a></p>';
	...
	if ($timeout == 0) {
		...
		function perform_refresh() {
			window.status=\"Bitte hier klicken, falls keine automatische Weiterleitung erfolgt\"+myvar; myvar = myvar + \" .\";
...
function buildUploadInput($title, $name, $value="", $size="40",$html=0,$upload_js="") {
	...
	if ($html) $value=htmlspecialchars($value);
	echo "<tr class=\"".switchBgColor()."\">\n<td valign=\"top\"><p>".$title."</p></td>\n";
	echo "<td nowrap><p><input type=\"text\" size=\"".$size."\" name=\"".$name."\" value=\"".$value."\">&nbsp;<a href=\"Javascript:".$upload_js."\"><img src=\"images/upload.gif\" width=\"16\" height=\"16\" border=\"0\" align=\"absmiddle\">$a_lang[afunc_319]</a></p></td>\n</tr>\n";
}
...
function buildExternalItems($name,$link,$img) {
    global $config,$sess;
	echo "<table width=\"100%\" cellspacing=\"0\" cellpadding=\"0\" border=\"0\">\n<tr>\n<td>";
	...
	if(is_array($name)) {
		...
		for($i=0;$i<count($name);$i++) {
			...
			echo "<a href=\"".$sess->adminUrl($link[$i])."\" class=\"menu\"><img class=\"img_button\" align=\"absmiddle\" src=\"images/".$img[$i]."\" alt=\"".$name[$i]."\" width\"16\" height=\"16\" border=\"0\">".$name[$i]."</a>\n";
		    echo "</td>\n";
			if($i<$totalitems) echo "<td style=\"padding-left: 2px;\">|</td>";
		}
	} else {
		echo "<td class=\"MenuOut\" onmouseover=\"this.attributes['class'].value='MenuOver';\" onmouseout=\"this.attributes['class'].value='MenuOut';\">\n";
		echo "<a href=\"".$sess->adminUrl($link)."\" class=\"menu\"><img class=\"img_button\" align=\"absmiddle\" src=\"images/".$img."\" alt=\"".$name."\" width\"16\" height=\"16\" border=\"0\">".$name."</a>\n";
	    echo "</td>\n";	
	}
	echo "<td><img hspace=\"1\" src=\"images/gif.gif\" width=\"4\" border=\"0\"></td>\n</tr>\n</table>";
	echo "</td>\n</tr>\n<tr>";
...
function buildHint($head,$text,$pic=1) {
    global $config;
    $icon = array(
        1 => "info_set",
        2 => "security"
    );
    echo "<div class=\"infomessage\"><img class=\"infomessage\" src=\"".$config['engine_mainurl']."/admin/images/".$icon[$pic].".gif\" align=\"absmiddle\"><span class=\"info\"><b>".$head."</b></span><br>\n";
    echo $text."<br></div>\n";
}
