...
* @access public
* @author Alex Höntschel <info@alexscriptengine.de>
* @version $Id: class.upload.php 2 2005-10-08 09:40:29Z alex $
...
class upload {
	 ...
	 * @return 
	 */
	function setChangeFilename($change_filename=false) { // darf der Filename ge&auml;ndert werden
    ...
    function saveFile() {   // speichern und file hochladen, evtl. umbenennen
        ...
        if(empty($this->upload_file) || $this->HTTP_POST_FILES[$this->fieldname]['tmp_name'] == "none") { // File ist nicht dabei
            $this->setErrorCode($this->lang['uploads_copy']);
            $this->uploadWrong = true;
        }
        
		if (!$this->checkMaxFileSize()) {
			$this->setErrorCode($this->lang['uploads_size']." ".$this->maxFileSize." Bytes");
			$this->uploadWrong = true;
		}       
        
		if (!$this->checkAllowedExtensions()) {
			$this->setErrorCode($this->lang['uploads_extens']);
			$this->uploadWrong = true;
		}  
        ...
        if(!$this->uploadWrong) {
    		...
    		if(@move_uploaded_file($this->upload_file,$this->filesdir."/".$this->new_destname)) { // Datei kopieren
                ...
                $this->setErrorCode($this->lang['uploads_success']);	
