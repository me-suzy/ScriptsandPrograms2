...
|
|	> $Id: groups.php 2 2005-10-08 09:40:29Z alex $
|
+--------------------------------------------------------------------------
*/
...
if(isset ($action) && $action=='add') {
	$db_sql->sql_query("INSERT INTO $group_table (title,canaccessadmincent,canaccessofflineengine,canuseenginesearch,canmodifyownprofile,canseemembers,canpostcomments,caneditcomments,candeletecomments,canpostnews)
				VALUES ('".addslashes($title)."','".intval($canaccessadmincenter)."','".intval($canaccessofflineengine)."','".intval($canuseenginesearch)."','".intval($canmodifyownprofile)."','".intval($canseemembers)."','".intval($canpostcomments)."','".intval($caneditcomments)."','".intval($candeletecomments)."','".intval($canpostnews)."')");
...
if(isset ($action) && $action=='edit') {
	...
	$db_sql->sql_query("UPDATE $group_table SET
				...
				canpostnews='".intval($canpostnews)."'
				$where_def");
...
if(!isset ($step) && $change == '') {
  echo " <p><b>Es wurde keine Auswahl getroffen. Bitte wähle links aus der Navigation die gewünschte Option aus.</b></p>";
    ...
    if($step == 'edit') {
        ...
        buildRadioRow($a_lang['groups_21'], "canpostnews", $usergroup['canpostnews']);
        buildFormFooter($a_lang['afunc_57'],"",2,$a_lang['afunc_75']);    
    }        
