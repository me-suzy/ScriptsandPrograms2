...
|
|	> $Id: member.php 2 2005-10-08 09:40:29Z alex $
|
+--------------------------------------------------------------------------
*/
...
if(isset ($action) && $action=='del') {
	...
	$db_sql->sql_query("UPDATE $newscomment_table SET userid='0', user_comname='".$com_poster['username']."' WHERE userid='".intval($memberid)."'");
...
if(!isset ($step) && $action == '') {
    ...
    if($step == 'del') {
        ...
        buildTableHeader("$a_lang[afunc_259]: <u>$del[username]</u>");
  ...
  if($step == 'activation') {
    ...
    if($step == 'avatar') {
        ...
        buildHeaderRow($a_lang['member_avatars']."<br><span class=\"smalltext\">&nbsp;$a_lang[afunc_291] ($pages): $pagecount</span>","avat.gif");		            
