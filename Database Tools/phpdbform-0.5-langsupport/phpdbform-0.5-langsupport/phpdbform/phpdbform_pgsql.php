...
class phpdbform_db {
	...
	function get_fields( $table )
	{
		...
		$ret = array();
		$id = pg_exec ( $this->conn, "select * from $table limit 30" );
		$nb_fields = pg_num_fields ( $id);
		for ($i = 0 ; $i < $nb_fields ; $i++ )
		{
			$field = pg_fieldname($id, $i);
			$type = pg_fieldtype($id, $i);
			$length = pg_fieldsize($id, $i);
			// print "Field: $field - PGsql: $type - Type: $type - Length:$length<br>\n";
			$ret[$field]["type"]=$type;
