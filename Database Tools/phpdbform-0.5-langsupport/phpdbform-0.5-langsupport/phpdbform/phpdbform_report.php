...
require_once("phpdbform/phpdbform_rep_field_dummy.php");
...
class phpdbform_report {
    ...
    function phpdbform_report( $db, $stmt, $title )
    {
        $this->db = $db;
        $this->stmt = $stmt;
        $this->title = $title;
    ...
    function process()
    {
        ...
        for( $i = 0; $i < $this->numfields; $i++ ) {
			...
			if( $type == "string" or $type == "varchar" or $type =="text" or $type == "interval" )
				$this->fields[$curr] = new phpdbform_rep_field_string( $curr, $curr );
			else if( $type == "int" or $type == "int2" or $type == "int4" )
				$this->fields[$curr] = new phpdbform_rep_field_int( $curr, $curr );
			...
			$size = $this->db->field_len($this->ret, $i);
			if( $size > 30 ) $size = 30;
            // debug here:
	...
	function add_dummy( $field, $title, $value="" )
	{
		$this->fields[$field] = new phpdbform_rep_field_dummy( $field, $title, $value );
		++$this->numfields;
	}
    ...
    function draw_header()
    {
        ...
        if( $this->draw_ruler ) {
            print "<tr><th colspan=\"{$this->numfields}\" background=\"phpdbform/ruler.png\" height=50>&nbsp;</th></tr>";
        }
        reset( $this->fields );
        while( $fld = each( $this->fields ) ) {
            if( $fld[1]->print ) {
    ...
    function load_row( &$row, $line )
    {
        if( $row = $this->db->fetch_array( $this->ret ) ) {
            ...
            while( $fld = each( $this->fields ) ) {
				...
				if( $this->fields[$fld[1]->field]->type == "dummy" ) continue;
                print "\t\t";
