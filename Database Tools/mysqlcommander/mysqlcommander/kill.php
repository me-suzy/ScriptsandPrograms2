...
$title = $funcs->text("L&ouml;sche Backupdateien", "Delete Backupfiles");
...
if ($version) {
		...
		if (isset($HTTP_POST_VARS["files"])) {
			...
			for ($i=0; $i<count($files); $i++) {
				if (is_dir($pfad.$files[$i])) {
					chmod($pfad.$files[$i], 0777);
					$blobfiles = $funcs->getBlobFilesForTable($dbs, "");
					for ($j=0; $j<count($blobfiles); $j++) {
						//echo $pfad.$files[$i]."/".$blobfiles[$j];
						@chmod($pfad.$files[$i]."/".$blobfiles[$j], 0777);
						if (!@unlink($pfad.$files[$i]."/".$blobfiles[$j])){
							$content->html_text($funcs->text("Kann Datei '$blobfiles[$j]' nicht l&ouml;schen", "Unable to kill '$blobfiles[$j]'"));
						} else {
							$content->html_text($funcs->text("Datei '$blobfiles[$j]' gel&ouml;scht", "'$blobfiles[$j]' killed"));
						}
					}
					if (!rmdir($pfad.$files[$i])){
						$content->html_text($funcs->text("Kann Verzeichnis '$files[$i]' nicht l&ouml;schen", "Unable to kill directory '$files[$i]'"));
					} else {
						$content->html_text($funcs->text("Verzeichnis '$files[$i]' gel&ouml;scht", "Directory '$files[$i]' killed"));
					}
				} else {
					@chmod($pfad.$files[$i], 0777);
					if (!@unlink($pfad.$files[$i])){
						$content->html_text($funcs->text("Kann Datei '$files[$i]' nicht l&ouml;schen", "Unable to kill '$files[$i]'"));
					} else {
						$content->html_text($funcs->text("Datei '$files[$i]' gel&ouml;scht", "'$files[$i]' killed"));
					}
				}
			}
		} else {
	...
	<?php 
		...
		for ($i=0; $i<count($db_names); $i++) {
			$formtext.= "<option value=\"kill.php?dbs=$db_names[$i]\"";
	...
	<?php 	if ($dbs) {
			...
			if (count($fl_names)>0) {
				...
				$size = count($fl_names);
				if ($size>20) $size = 20;
				$formtext = "<form method=\"post\" action=\"kill.php\" name=\"doing\">\n";
			...
			$content->html_black();
			$content->config->bgcolor="#ebebeb";
			$content->html_headtext($funcs->text("W&auml;hle Dateien f&uuml;r den L&ouml;schvorgang", "Choose files to be killed"), "txtblaufett");
			$content->config->bgcolor="White";
			if (count($fl_names)>0) $content->html_link("javascript:choose_all();", $funcs->text("Alle Dateien anw&auml;hlen", "Select all files"));
			$content->html_text($formtext);
			if (count($fl_names)>0) {
				$content->html_link("javascript:send();", $funcs->text("L&ouml;sche Dateien", "Kill files"));
	?>
