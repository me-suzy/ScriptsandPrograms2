<?php
$table='proteins'; // mysql table name
...
///////////////////////////////////////////////////////////////////////////////
// build upper table
echo ('<table width="750" summary="top" cellpadding="5" cellspacing="1" style="background-color:#dcdcdc; border:0;  background-image:url(\'../images/proteins_small.gif\'); background-position: top right;background-repeat: no-repeat;"><tr><td>');
include ("../top_part.php");
if ($num_sat !==0) // IF START ################################################
{
// column headings
echo ('<colgroup><col width="40" valign="top" align="left" /><col valign="top" align="left" /><col valign="top" align="left" /><col valign="top" align="left" /><col valign="top" align="left" /><col valign="top" width="170" align="left" /></colgroup><tr style="background-color:'.$heading_color.';" valign="top"><td></td><td >Name/description</td><td>Nature</td><td>Source</td><td >Location</td><td></td></tr>');  
...
if ($row["reactivity"] != '')
{echo ('<br />Against&nbsp;'.str_replace('~',' ',$row["reactivity"]));}

if ($row["isotype"] != '')
{echo ('<br />'.$row["isotype"].'&nbsp;isotype');}

if ($row["conjugate"] != '')
{echo ('<br />'.$row["conjugate"].'&nbsp;conjugate');}
  //---------------------------------------------------
echo ('</td><td>'
.$row["source"]
  //---------------------------------------------------
.'</td><td>'
.$row["box"].'<br />'.$row["fridge"]);
  //---------------------------------------------------
...
echo ('Updated '.$row["modified_on"].'<br />');
   ...
   // start - edit, detail, update links
    // detail link
echo ('<a href="../interface_creator/index_short.php?table_name=proteins&amp;function=details&amp;where_field=id&amp;where_value='.$row["id"].'" onclick="return popitup(\'../interface_creator/index_short.php?table_name=proteins&amp;function=details&amp;where_field=id&amp;where_value='.$row["id"].'\')">Details</a>'
);
    // edit and delete link
if (!($all_affect_items == "no" and $client == "not_allowed")){echo (' | <a href="../interface_creator/index_short.php?table_name=proteins&amp;function=edit&amp;where_field=id&amp;where_value='.$row["id"].'" onclick="return popitup(\'../interface_creator/index_short.php?table_name=proteins&amp;function=edit&amp;where_field=id&amp;where_value='.$row["id"].'\')"> Edit</a> | <a href="../interface_creator/index_short.php?table_name=proteins&amp;function=delete&amp;where_field=id&amp;where_value='.$row["id"].'" onclick="return confipop(\'../interface_creator/index_short.php?table_name=proteins&amp;function=delete&amp;where_field=id&amp;where_value='.$row["id"].'\')"> Delete</a>');}
