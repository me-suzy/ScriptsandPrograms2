...
HAUTPAGEWEB('Discoman - Record update');
LAYERS3();
...
if ($nom != "") {
	...
	include("link.inc.php");
    	$query = "
			...
			ORDER BY
            	disco_disques.date ASC, disco_formats.type ASC, disco_pays.abrege ASC, disco_titres.titre ASC";
       ...
       <tr>
      	<th colspan=2>$txt_resultat ".stripslashes($row2["nom"])."</th>
       </tr>";

        if($numrows == 0) {
    		echo "
            <tr>
        		<td  bgcolor=\"#FFFFFF\"><b>$txt_result_0</b></td>
            </tr>
        </table>\n";
 	 		}
 		else {
       	echo "
        	<tr>
       			<td  bgcolor=\"#FFFFFF\">$txt_choisir_disque :</td>
       		</tr>
       	</table>";
LAYERINTERNE();
        ...
        echo "<th width=\"5%\">".ucfirst("id")."</th>\n";
   		...
   		while ($row = mysql_fetch_assoc($result)) {
        		// alternate color
       		if($i%2 == 0) echo "<tr class=\"TRalter\" onMouseOver='this.style.background=\"#66CCFF\"' onMouseOut='this.style.background=\"#F0F0F0\"' onClick='location=\"title_update.php?id_disque=".$row['id_disque']."&curlevel=$curlevel\"'>\n";
       		else echo "<tr class=\"main\" onMouseOver='this.style.background=\"#66CCFF\"' onMouseOut='this.style.background=\"white\"' onClick='location=\"title_update.php?id_disque=".$row['id_disque']."&curlevel=$curlevel\"'>\n";
    ...
    mysql_close($link);
		...
		echo "
            ...
            <FORM METHOD=\"GET\" ACTION=\"$PHP_SELF\">
    			<tr>
    				<td colspan='3'>".$txt_choisir_artiste."</td>
     			</tr>
    	<tr>
       		<td>$txt_artiste :</td>
        	<td>";
				...
				affichartiste('');
    echo "      <input name=\"curlevel\" type=\"hidden\" value=\"$curlevel\"></td>
