...
// CHECK IF USER LOGGED IN :: APPLY SELECT ALL DELETE
if($_SESSION['loggedin'] == 'yes')
	{
		$tpl->assign("select_delete", "<button type=\"Submit\" name=\"Submit\" value=\"delete\" class=\"formbutton\" title=\"Delete Selected\">Delete</button>");
	} else {
		$tpl->assign("select_delete", "");
	}

// Get total number of rows in result
$select_num = "SELECT * FROM pmc_comic WHERE title = '". mysql_real_escape_string($urldata[1]) ."' AND volume = '". mysql_real_escape_string($urldata[2]) ."' AND type = '". $urldata[3] ."' ORDER BY title, issue, issueltr";
$get_num = mysql_db_query($sql['data'], $select_num) or die("Select Search Failed!");
$numrows = mysql_num_rows($get_num); 
      
// MYSQL QUERY :: GET ISSUES AFTER A SERIES
//$select_issues = mysql_query("SELECT * FROM pmc_comic WHERE title = '". mysql_real_escape_string($urldata[1]) ."' AND volume = '". mysql_real_escape_string($urldata[2]) ."' AND type = '". $urldata[3] ."' ORDER BY title, issue, issueltr") or die("Select issues after series failed!");
$select = "SELECT * FROM pmc_comic WHERE title = '". mysql_real_escape_string($urldata[1]) ."' AND volume = '". mysql_real_escape_string($urldata[2]) ."' AND type = '". $urldata[3] ."' ORDER BY title, issue, issueltr";
		   		...
		   		if ($Result_Set < $numrows && $Result_Set > 0)
		      		{
		      			...
		      			$tpl->assign("prev_page", "&laquo; <a href=\"serieslist.php/$urldata[1]/$urldata[2]/$urldata[3]?Result_Set=$Res1\">$lang_page_prev</a>");
		      			...
		      			for ($b=0,$c=1; $b < $Pages; $b++,$c++)
		          			{
		          				// NEED FUNCTION LIKE 
		          				$Res1=$Per_Page * $b;
		          				$get_pages[] = "<b><a href=\"serieslist.php/$urldata[1]/$urldata[2]/$urldata[3]?Result_Set=$Res1\">$c</a></b>";
		   		...
		   		if ($Result_Set >= 0 && $Result_Set < $numrows)
		 			{
		      			...
		      			if ($Res1 < $numrows)
		        			{
		         				$tpl->assign("next_page", "<a href=\"serieslist.php/$urldata[1]/$urldata[2]/$urldata[3]?Result_Set=$Res1\">$lang_page_next</a> &raquo;");
...
while ($get_issues = mysql_fetch_array($data))
	{
		// GET VALUES FROM DB
      	...
      	// OPEN TEMPLATE BLOCK :: COMIC_LIST
