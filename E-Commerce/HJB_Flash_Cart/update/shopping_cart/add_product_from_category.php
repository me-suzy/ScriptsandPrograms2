...
****************************************************************************************************************************
	*/
	...
	$querycheck="SELECT * FROM cartitems_to_cartcategories WHERE cartItemsID='".$_GET['productID']."' AND cartCategoriesID='".$_GET['catID']."'";
	$resultcheck=mysql_query($querycheck);
	$rowcheck=mysql_fetch_array($resultcheck);
	if (!$rowcheck['id']){
		$query="INSERT INTO cartitems_to_cartcategories (cartItemsID, cartCategoriesID) VALUES ('".$_GET['productID']."', '".$_GET['catID']."')";
		...
		header ("location: view_category.php?catID=".$_GET['catID']);
	?>