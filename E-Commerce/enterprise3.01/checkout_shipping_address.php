...
// a shipping address is not needed
  if ($order->content_type == 'virtual') {
    ...
    $shipping = false;
    if (!escs_session_is_registered('sendto')) escs_session_register('sendto');
    $sendto = false;
...
// process the selected shipping destination
      ...
      if ($check_address['total'] == '1') {
        if ($reset_shipping == true) escs_session_unregister('shipping');
        escs_redirect(escs_href_link(FILENAME_CHECKOUT_SHIPPING, '', 'SSL'));
      } else {
...
// if no shipping destination address was selected, use their own address as default
  ...
  $breadcrumb->add(NAVBAR_TITLE_2, escs_href_link(FILENAME_CHECKOUT_SHIPPING_ADDRESS, '', 'SSL'));
