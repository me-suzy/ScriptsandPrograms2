     <table border="0" width="100%" cellspacing="0" cellpadding="<?php echo CELLPADDING_SUB; ?>">
      ...
      <tr>
        <td><table width="100%" border="0" cellspacing="0" cellpadding="2">
          ...
          <tr>
            <td class="smallText" align="center"><?php echo TEXT_AFFILIATE_INDIVIDUAL_BANNER_INFO . escs_draw_form('individual_banner', escs_href_link(FILENAME_AFFILIATE_BANNERS) ) . "\n" . escs_draw_input_field('individual_banner_id', '', 'size="5"') . "&nbsp;&nbsp;" . escs_image_submit('button_affiliate_build_a_link.gif', IMAGE_BUTTON_BUILD_A_LINK); ?></form></td>
          </tr>
<?php
  if (escs_not_null($HTTP_POST_VARS['individual_banner_id']) || escs_not_null($HTTP_GET_VARS['individual_banner_id'])) {

    if (escs_not_null($HTTP_POST_VARS['individual_banner_id'])) $individual_banner_id = $HTTP_POST_VARS['individual_banner_id'];
    if ($HTTP_GET_VARS['individual_banner_id']) $individual_banner_id = $HTTP_GET_VARS['individual_banner_id'];
    $affiliate_pbanners_values = escs_db_query("select p.products_image, pd.products_name from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd where p.products_id = '" . $individual_banner_id . "' and pd.products_id = '" . $individual_banner_id . "' and p.products_status = '1' and pd.language_id = '" . $languages_id . "'");
    if ($affiliate_pbanners = escs_db_fetch_array($affiliate_pbanners_values)) {
...
?>
          ...
          <tr>
            <td align="center"><?php echo escs_draw_textarea_field('affiliate_banner', 'soft', '60', '6', $link); ?></td>
          </tr>
<?php
  }
?>
      ...
      <tr>
        <td><?php echo escs_draw_separator('pixel_trans.gif', '100%', '20'); ?></td>
      </tr>
<?php
  if (escs_db_num_rows($affiliate_banners_values)) {

    while ($affiliate_banners = escs_db_fetch_array($affiliate_banners_values)) {
      $affiliate_products_query = escs_db_query("select products_name from " . TABLE_PRODUCTS_DESCRIPTION . " where products_id = '" . $affiliate_banners['affiliate_products_id'] . "' and language_id = '" . $languages_id . "'");
      $affiliate_products = escs_db_fetch_array($affiliate_products_query);
...
?>
      <tr>
        <td><table width="100%" border="0" cellspacing="0" cellpadding="2">
          ...
          <tr>
            <td class="smallText" align="center"><?php echo escs_draw_textarea_field('affiliate_banner', 'soft', '60', '6', $link); ?></td>
          </tr>
        </table></td>
      </tr>
      <tr>
