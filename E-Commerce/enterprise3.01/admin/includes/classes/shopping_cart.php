      ...
      if ($this->contents) {
        ...
        while (list($products_id, ) = each($this->contents)) {
          ...
          if (!escs_db_num_rows($product_query)) {
            escs_db_query("insert into " . TABLE_CUSTOMERS_BASKET . " (customers_id, products_id, customers_basket_quantity, customers_basket_date_added) values ('" . (int)$customer_id . "', '" . escs_db_input($products_id) . "', '" . escs_db_input($qty) . "', '" . date('Ymd') . "')");
            ...
            escs_db_query("update " . TABLE_CUSTOMERS_BASKET . " set customers_basket_quantity = '" . escs_db_input($qty) . "' where customers_id = '" . (int)$customer_id . "' and products_id = '" . escs_db_input($products_id) . "'");
          }
        }
      }
...
// insert into database
        if ($customer_id) escs_db_query("insert into " . TABLE_CUSTOMERS_BASKET . " (customers_id, products_id, customers_basket_quantity, customers_basket_date_added) values ('" . (int)$customer_id . "', '" . escs_db_input($products_id) . "', '" . escs_db_input($qty) . "', '" . date('Ymd') . "')");
...
// insert into database
            if ($customer_id) escs_db_query("insert into " . TABLE_CUSTOMERS_BASKET_ATTRIBUTES . " (customers_id, products_id, products_options_id, products_options_value_id) values ('" . (int)$customer_id . "', '" . escs_db_input($products_id) . "', '" . (int)$option . "', '" . (int)$value . "')");
          }
        }
...
// update database
      if ($customer_id) escs_db_query("update " . TABLE_CUSTOMERS_BASKET . " set customers_basket_quantity = '" . escs_db_input($quantity) . "' where customers_id = '" . (int)$customer_id . "' and products_id = '" . escs_db_input($products_id) . "'");
      ...
      if (is_array($attributes)) {
        ...
        while (list($option, $value) = each($attributes)) {
          $this->contents[$products_id]['attributes'][$option] = $value;
// update database
          if ($customer_id) escs_db_query("update " . TABLE_CUSTOMERS_BASKET_ATTRIBUTES . " set products_options_value_id = '" . (int)$value . "' where customers_id = '" . (int)$customer_id . "' and products_id = '" . escs_db_input($products_id) . "' and products_options_id = '" . (int)$option . "'");
        }
      }
    }
...
// products price
        $product_query = escs_db_query("select products_id, products_price, products_tax_class_id, products_weight from " . TABLE_PRODUCTS . " where products_id='" . (int)escs_get_prid($products_id) . "'");
...
// attributes price
    ...
    function get_products() {
      ...
      reset($this->contents);
      while (list($products_id, ) = each($this->contents)) {
        $products_query = escs_db_query("select p.products_id, pd.products_name, p.products_model, p.products_price, p.products_weight, p.products_tax_class_id from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd where p.products_id='" . (int)escs_get_prid($products_id) . "' and pd.products_id = p.products_id and pd.language_id = '" . (int)$languages_id . "'");
