<?php
  ...
  //---------------------------------------------------------------------------//
  function escs_get_category_heading_title($category_id, $language_id) {
    $category_query = escs_db_query("select categories_heading_title from " . TABLE_CATEGORIES_DESCRIPTION . " where categories_id = '" . $category_id . "' and language_id = '" . $language_id . "'");
    $category = escs_db_fetch_array($category_query);
    return $category['categories_heading_title'];
  }

  function escs_get_category_description($category_id, $language_id) {
    $category_query = escs_db_query("select categories_description from " . TABLE_CATEGORIES_DESCRIPTION . " where categories_id = '" . $category_id . "' and language_id = '" . $language_id . "'");
    $category = escs_db_fetch_array($category_query);
