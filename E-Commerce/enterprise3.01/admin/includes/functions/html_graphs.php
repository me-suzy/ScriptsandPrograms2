...
// draws a double vertical bar graph for the banner views vs clicks statistics
  function escs_banner_graph_infoBox($banner_id, $days) {
    $names = array();
    $values = array();
    $dvalues = array();

    $banner_stats_query = escs_db_query("select dayofmonth(banners_history_date) as name, banners_shown as value, banners_clicked as dvalue from " . TABLE_BANNERS_HISTORY . " where banners_id = '" . $banner_id . "' and to_days(now()) - to_days(banners_history_date) < " . $days . " order by banners_history_date");
    while ($banner_stats = escs_db_fetch_array($banner_stats_query)) {
...
////
// draws a double vertical bar graph for the banner views vs clicks statistics
  function escs_banner_graph_yearly($banner_id) {
    global $banner, $HTTP_GET_VARS;

    $banner_stats_query = escs_db_query("select year(banners_history_date) as year, sum(banners_shown) as value, sum(banners_clicked) as dvalue from " . TABLE_BANNERS_HISTORY . " where banners_id = '" . $banner_id . "' group by year(banners_history_date)");
    while ($banner_stats = escs_db_fetch_array($banner_stats_query)) {
...
////
// draws a double vertical bar graph for the banner views vs clicks statistics
  function escs_banner_graph_monthly($banner_id) {
    global $banner, $HTTP_GET_VARS;
    ...
    $banner_stats_query = escs_db_query("select month(banners_history_date) as banner_month, sum(banners_shown) as value, sum(banners_clicked) as dvalue from " . TABLE_BANNERS_HISTORY . " where banners_id = '" . $banner_id . "' and year(banners_history_date) = '" . $year . "' group by month(banners_history_date)");
    while ($banner_stats = escs_db_fetch_array($banner_stats_query)) {
...
////
// draws a double vertical bar graph for the banner views vs clicks statistics
  function escs_banner_graph_daily($banner_id) {
    global $banner, $HTTP_GET_VARS;
    ...
    $banner_stats_query = escs_db_query("select dayofmonth(banners_history_date) as banner_day, banners_shown as value, banners_clicked as dvalue from " . TABLE_BANNERS_HISTORY . " where banners_id = '" . $banner_id . "' and month(banners_history_date) = '" . $month . "' and year(banners_history_date) = '" . $year . "'");
    while ($banner_stats = escs_db_fetch_array($banner_stats_query)) {
