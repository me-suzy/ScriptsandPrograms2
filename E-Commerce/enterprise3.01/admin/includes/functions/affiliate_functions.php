...
function affiliate_delete ($affiliate_id) {
  $affiliate_query = escs_db_query("SELECT affiliate_rgt, affiliate_lft, affiliate_root  FROM " . TABLE_AFFILIATE . " WHERE affiliate_id = '" . $affiliate_id . "' ");
  if ($affiliate = escs_db_fetch_array($affiliate_query)) {
    if ($affiliate['affiliate_root'] == $affiliate_id) {
      // a root entry is deleted -> his childs get root
      $affiliate_child_query = escs_db_query("
      ...
      while ($affiliate_child = escs_db_fetch_array($affiliate_child_query)) {
        escs_db_query ("UPDATE  " . TABLE_AFFILIATE . " SET affiliate_root = " . $affiliate_child['affiliate_id'] . " WHERE affiliate_root =  " . $affiliate['affiliate_root'] . "  AND affiliate_lft >= " . $affiliate_child['affiliate_lft']  . " AND affiliate_rgt <= " . $affiliate_child['affiliate_rgt']  . " ");
        $substract =  $affiliate_child['affiliate_lft'] -1;
        escs_db_query ("UPDATE  " . TABLE_AFFILIATE . " SET affiliate_lft = affiliate_lft - " . $substract . " WHERE  affiliate_root = " . $affiliate_child['affiliate_id']);
        escs_db_query ("UPDATE  " . TABLE_AFFILIATE . " SET affiliate_rgt = affiliate_rgt - " . $substract . " WHERE  affiliate_root = " . $affiliate_child['affiliate_id']) ;
      }
      escs_db_query("DELETE FROM " . TABLE_AFFILIATE . "  WHERE affiliate_id = " . $affiliate_id);
      escs_db_query("UNLOCK TABLES");
    } else {
      escs_db_query("LOCK TABLES " . TABLE_AFFILIATE . " WRITE");
      escs_db_query("DELETE FROM " . TABLE_AFFILIATE . "  WHERE affiliate_id = " . $affiliate_id . " AND affiliate_root = " . $affiliate['affiliate_root'] . " ");

      escs_db_query("UPDATE " . TABLE_AFFILIATE . "
...
// TABLES: tax_rates, zones_to_geo_zones
  function escs_get_affiliate_tax_rate($class_id, $country_id, $zone_id) {
