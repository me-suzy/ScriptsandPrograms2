...
*/
  ...
  $OldNewStatusValues = (escs_field_exists(TABLE_ORDERS_STATUS_HISTORY, "old_value") && escs_field_exists(TABLE_ORDERS_STATUS_HISTORY, "new_value"));
  $CommentsWithStatus = escs_field_exists(TABLE_ORDERS_STATUS_HISTORY, "comments");
  $SeparateBillingFields = escs_field_exists(TABLE_ORDERS, "billing_name");
  ...
  if (escs_not_null($action)) {
	...
	case 'update_order':
		...
		$UpdateOrders = "update " . TABLE_ORDERS . " set
			customers_name = '" . escs_db_input(stripslashes($update_customer_name)) . "',
			customers_company = '" . escs_db_input(stripslashes($update_customer_company)) . "',
			customers_street_address = '" . escs_db_input(stripslashes($update_customer_street_address)) . "',
			customers_suburb = '" . escs_db_input(stripslashes($update_customer_suburb)) . "',
			customers_city = '" . escs_db_input(stripslashes($update_customer_city)) . "',
			customers_state = '" . escs_db_input(stripslashes($update_customer_state)) . "',
			customers_postcode = '" . escs_db_input($update_customer_postcode) . "',
			customers_country = '" . escs_db_input(stripslashes($update_customer_country)) . "',
			customers_telephone = '" . escs_db_input($update_customer_telephone) . "',
			customers_email_address = '" . escs_db_input($update_customer_email_address) . "',";

		if($SeparateBillingFields)
		{
		$UpdateOrders .= "billing_name = '" . escs_db_input(stripslashes($update_billing_name)) . "',
			billing_company = '" . escs_db_input(stripslashes($update_billing_company)) . "',
			billing_street_address = '" . escs_db_input(stripslashes($update_billing_street_address)) . "',
			billing_suburb = '" . escs_db_input(stripslashes($update_billing_suburb)) . "',
			billing_city = '" . escs_db_input(stripslashes($update_billing_city)) . "',
			billing_state = '" . escs_db_input(stripslashes($update_billing_state)) . "',
			billing_postcode = '" . escs_db_input($update_billing_postcode) . "',
			billing_country = '" . escs_db_input(stripslashes($update_billing_country)) . "',";
		}

		$UpdateOrders .= "delivery_name = '" . escs_db_input(stripslashes($update_delivery_name)) . "',
			delivery_company = '" . escs_db_input(stripslashes($update_delivery_company)) . "',
			delivery_street_address = '" . escs_db_input(stripslashes($update_delivery_street_address)) . "',
			delivery_suburb = '" . escs_db_input(stripslashes($update_delivery_suburb)) . "',
			delivery_city = '" . escs_db_input(stripslashes($update_delivery_city)) . "',
			delivery_state = '" . escs_db_input(stripslashes($update_delivery_state)) . "',
			delivery_postcode = '" . escs_db_input($update_delivery_postcode) . "',
			delivery_country = '" . escs_db_input(stripslashes($update_delivery_country)) . "',
			payment_method = '" . escs_db_input($update_info_payment_method) . "',
			cc_type = '" . escs_db_input($update_info_cc_type) . "',
			cc_owner = '" . escs_db_input($update_info_cc_owner) . "',";
		...
		$UpdateOrders .= "cc_expires = '$update_info_cc_expires',
			orders_status = '" . escs_db_input($status) . "'";

		if(!$CommentsWithStatus)
		{
			$UpdateOrders .= ", comments = '" . escs_db_input($comments) . "'";
		}

		$UpdateOrders .= " where orders_id = '" . escs_db_input($oID) . "';";

		escs_db_query($UpdateOrders);
		$order_updated = true;
		...
		if ($order->info['orders_status'] != $status)
		{
			...
			if($CommentsWithStatus)
			{
			escs_db_query("insert into " . TABLE_ORDERS_STATUS_HISTORY . "
				(orders_id, orders_status_id, date_added, customer_notified, comments)
				values ('" . escs_db_input($oID) . "', '" . escs_db_input($status) . "', now(), " . escs_db_input($customer_notified) . ", '" . escs_db_input($comments)  . "')");
			}
			else
			{
				...
				escs_db_query("insert into " . TABLE_ORDERS_STATUS_HISTORY . "
					...
					values ('" . escs_db_input($oID) . "', '" . escs_db_input($status) . "', '" . $order->info['orders_status'] . "', now(), " . escs_db_input($customer_notified) . ")");
				}
				else
				{
				escs_db_query("insert into " . TABLE_ORDERS_STATUS_HISTORY . "
					(orders_id, orders_status_id, date_added, customer_notified)
					values ('" . escs_db_input($oID) . "', '" . escs_db_input($status) . "', now(), " . escs_db_input($customer_notified) . ")");
				}
			}
		}
		...
		escs_redirect(escs_href_link("edit_orders.php", escs_get_all_get_params(array('action')) . 'action=edit'));
	...
	case 'add_product':
		if($step == 5)
		{
			...
			$result = escs_db_query($InfoQuery);
			$row = escs_db_fetch_array($result);
			extract($row, EXTR_PREFIX_ALL, "p");

			// Following functions are defined at the bottom of this file
			$CountryID = escs_get_country_id($order->delivery["country"]);
			$ZoneID = escs_get_zone_id($CountryID, $order->delivery["state"]);

			$ProductsTax = escs_get_tax_rate($p_products_tax_class_id, $CountryID, $ZoneID);
			...
			escs_db_query($Query);
			$new_product_id = escs_db_insert_id();
...
<!-- Begin Addresses Block -->
      <tr><?php echo escs_draw_form('edit_order', "edit_orders.php", escs_get_all_get_params(array('action','paycc')) . 'action=update_order'); ?>
	<td><table width="100%" border="0" cellspacing="0" cellpadding="2">
	  ...
	  <tr>
		...
		Name: <input name='update_customer_name' size='37' value='<?php echo escs_html_quotes($order->customer['name']); ?>'><br>
		Company: <input name='update_customer_company' size='37' value='<?php echo escs_html_quotes($order->customer['company']); ?>'><br>
		Address: <input name='update_customer_street_address' size='37' value='<?php echo escs_html_quotes($order->customer['street_address']); ?>'><br>
		Suburb: <input name='update_customer_surburb' size='37' value='<?php echo escs_html_quotes($order->customer['suburb']); ?>'><br>
		City: <input name='update_customer_city' size='15' value='<?php echo escs_html_quotes($order->customer['city']); ?>'> <br>
		State: <input name='update_customer_state' size='10' value='<?php echo escs_html_quotes($order->customer['state']); ?>'> <br>
		Post Code: <input name='update_customer_postcode' size='5' value='<?php echo $order->customer['postcode']; ?>'><br>
		Country: <input name='update_customer_country' size='37' value='<?php echo escs_html_quotes($order->customer['country']); ?>'>
		</td>
		</tr>
		</table>
	    </td>
...
<? if($SeparateBillingFields) { ?>
	  <tr>
		...
		<tr>
		<td colspan='2' class="main">
		Name: <input name='update_billing_name' size='37' value='<?php echo escs_html_quotes($order->billing['name']); ?>'><br>
		Company: <input name='update_billing_company' size='37' value='<?php echo escs_html_quotes($order->billing['company']); ?>'><br>
		Address: <input name='update_billing_street_address' size='37' value='<?php echo escs_html_quotes($order->billing['street_address']); ?>'><br>
		Suburb: <input name='update_billing_surburb' size='37' value='<?php echo escs_html_quotes($order->billing['suburb']); ?>'><br>
		City: <input name='update_billing_city' size='15' value='<?php echo escs_html_quotes($order->billing['city']); ?>'> <br>
		State: <input name='update_billing_state' size='10' value='<?php echo escs_html_quotes($order->billing['state']); ?>'> <br>
		Post Code: <input name='update_billing_postcode' size='5' value='<?php echo $order->billing['postcode']; ?>'><br>
		Country: <input name='update_billing_country' size='37' value='<?php echo escs_html_quotes($order->billing['country']); ?>'>
		</td>
		</tr>
		</table>
	    </td>
...
<? } ?>
		...
		<tr>
		<td class="main">
		Name: <input name='update_delivery_name' size='37' value='<?php echo escs_html_quotes($order->delivery['name']); ?>'><br>
		Company: <input name='update_delivery_company' size='37' value='<?php echo escs_html_quotes($order->delivery['company']); ?>'><br>
		Address: <input name='update_delivery_street_address' size='37' value='<?php echo escs_html_quotes($order->delivery['street_address']); ?>'><br>
		Suburb: <input name='update_delivery_surburb' size='37' value='<?php echo escs_html_quotes($order->delivery['suburb']); ?>'><br>
		City: <input name='update_delivery_city' size='15' value='<?php echo escs_html_quotes($order->delivery['city']); ?>'> <br>
		State: <input name='update_delivery_state' size='10' value='<?php echo escs_html_quotes($order->delivery['state']); ?>'> <br>
		Post Code: <input name='update_delivery_postcode' size='5' value='<?php echo $order->delivery['postcode']; ?>'><br>
		Country: <input name='update_delivery_country' size='37' value='<?php echo escs_html_quotes($order->delivery['country']); ?>'>
		</td>
		</tr>
		</table>
	    </td>
...
<!-- Begin Products Listing Block -->
	...
	<?
		...
		$orders_products_query = escs_db_query("select * from " . TABLE_ORDERS_PRODUCTS . " where orders_id = '" . (int)$oID . "'");
		while ($orders_products = escs_db_fetch_array($orders_products_query)) {
		...
		$attributes_query = escs_db_query($attributes_query_string);

		if (escs_db_num_rows($attributes_query)) {
	...
	for ($i=0; $i<sizeof($order->products); $i++) {
		...
		echo '	    </td>' . "\n" .
		     ...
		     '	    <td class="' . $RowStyle . '" align="center" valign="top">' . "<input name='update_products[$orders_products_id][tax]' size='3' value='" . escs_display_tax_value($order->products[$i]['tax']) . "'>" . '%</td>' . "\n" .
...
<?php
      	...
      	// Override order.php Class's Field Limitations
		$totals_query = escs_db_query("select * from " . TABLE_ORDERS_TOTAL . " where orders_id = '" . (int)$oID . "' order by sort_order");
		$order->totals = array();
		while ($totals = escs_db_fetch_array($totals_query)) { $order->totals[] = array('title' => $totals['title'], 'text' => $totals['text'], 'class' => $totals['class'], 'value' => $totals['value'], 'orders_total_id' => $totals['orders_total_id']); }
...
<?php
    $orders_history_query = escs_db_query("select * from " . TABLE_ORDERS_STATUS_HISTORY . " where orders_id = '" . escs_db_input($oID) . "' order by date_added");
    if (escs_db_num_rows($orders_history_query)) {
      while ($orders_history = escs_db_fetch_array($orders_history_query)) {
        ...
        if($CommentsWithStatus) {
        echo '            <td class="smallText">' . nl2br(escs_db_output($orders_history['comments'])) . '&nbsp;</td>' . "\n";
        }
...
?>
      ...
      <tr>
        ...
        <?
        if($CommentsWithStatus) {
        	echo escs_draw_textarea_field('comments', 'soft', '60', '5');
	}
	else
	{
		echo escs_draw_textarea_field('comments', 'soft', '60', '5', $order->info['comments']);
	}
	?>
        </td>
      </tr>
      ...
      <tr>
	<td align='center' valign="top"><?php echo escs_image_submit('button_update.gif', IMAGE_UPDATE); ?></td>
      </tr>
      </form>
<?php
  }
...
<?
	...
	// ############################################################################
		...
		if(($step > 2) && ($add_product_products_id > 0))
		{
			...
			$result = escs_db_query("SELECT * FROM " . TABLE_PRODUCTS_ATTRIBUTES . " pa LEFT JOIN " . TABLE_PRODUCTS_OPTIONS . " po ON po.products_options_id=pa.options_id LEFT JOIN " . TABLE_PRODUCTS_OPTIONS_VALUES . " pov ON pov.products_options_values_id=pa.options_values_id WHERE products_id='$add_product_products_id'");
...
<?
  ...
  //
  ////////////////////////////////////////////////////////////////////////////////////////////////
  function escs_get_country_id($country_name) {

    $country_id_query = escs_db_query("select * from " . TABLE_COUNTRIES . " where countries_name = '" . $country_name . "'");

    if (!escs_db_num_rows($country_id_query)) {
      return 0;
    }
    else {
      $country_id_row = escs_db_fetch_array($country_id_query);
  ...
  ////////////////////////////////////////////////////////////////////////////////////////////////
  //
  // Function    : escs_get_country_iso_code_2
  //
  ...
  //
  ////////////////////////////////////////////////////////////////////////////////////////////////
  function escs_get_country_iso_code_2($country_id) {

    $country_iso_query = escs_db_query("select * from " . TABLE_COUNTRIES . " where countries_id = '" . $country_id . "'");

    if (!escs_db_num_rows($country_iso_query)) {
      return 0;
    }
    else {
      $country_iso_row = escs_db_fetch_array($country_iso_query);
  ...
  //
  ////////////////////////////////////////////////////////////////////////////////////////////////
  function escs_get_zone_id($country_id, $zone_name) {

    $zone_id_query = escs_db_query("select * from " . TABLE_ZONES . " where zone_country_id = '" . $country_id . "' and zone_name = '" . $zone_name . "'");

    if (!escs_db_num_rows($zone_id_query)) {
      return 0;
    }
    else {
      $zone_id_row = escs_db_fetch_array($zone_id_query);
  ...
  //
  ////////////////////////////////////////////////////////////////////////////////////////////////
  function escs_field_exists($table,$field) {

    $describe_query = escs_db_query("describe $table");
    while($d_row = escs_db_fetch_array($describe_query))
    {
  ...
  //
  ////////////////////////////////////////////////////////////////////////////////////////////////
  function escs_html_quotes($string) {
    return str_replace("'", "&#39;", $string);
  }

  ////////////////////////////////////////////////////////////////////////////////////////////////
  //
  // Function    : escs_html_unquote
  //
  // Arguments   : string	any string
  //
  ...
  //
  ////////////////////////////////////////////////////////////////////////////////////////////////
  function escs_html_unquote($string) {
    return str_replace("&#39;", "'", $string);
  }
