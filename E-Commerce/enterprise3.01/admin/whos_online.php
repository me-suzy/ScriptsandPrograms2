...
<?php
  $whos_online_query = escs_db_query("select customer_id, full_name, ip_address, time_entry, time_last_click, last_page_url, session_id from " . TABLE_WHOS_ONLINE);
  while ($whos_online = escs_db_fetch_array($whos_online_query)) {
    ...
    if ($whos_online['session_id'] == $info) {
      ...
      echo '              <tr class="dataTableRow" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\'' . escs_href_link(FILENAME_WHOS_ONLINE, escs_get_all_get_params(array('info', 'action')) . 'info=' . $whos_online['session_id'], 'NONSSL') . '\'">' . "\n";
    }
?>
                ...
                <td class="dataTableContent"><?php if (eregi('^(.*)' . escs_session_name() . '=[a-f,0-9]+[&]*(.*)', $whos_online['last_page_url'], $array)) { echo $array[1] . $array[2]; } else { echo $whos_online['last_page_url']; } ?>&nbsp;</td>
              </tr>
<?php
  }
?>
              <tr>
                <td class="smallText" colspan="7"><?php echo sprintf(TEXT_NUMBER_OF_CUSTOMERS, escs_db_num_rows($whos_online_query)); ?></td>
              </tr>
            </table></td>
<?php
  $heading = array();
  ...
  if (isset($info)) {
    ...
    if (STORE_SESSIONS == 'mysql') {
      $session_data = escs_db_query("select value from " . TABLE_SESSIONS . " WHERE sesskey = '" . $info . "'");
      $session_data = escs_db_fetch_array($session_data);
      $session_data = trim($session_data['value']);
    } else {
      if ( (file_exists(escs_session_save_path() . '/sess_' . $info)) && (filesize(escs_session_save_path() . '/sess_' . $info) > 0) ) {
        $session_data = file(escs_session_save_path() . '/sess_' . $info);
    ...
    if ($length = strlen($session_data)) {
      ...
      if (is_object($cart)) {
        ...
        if (sizeof($products) > 0) {
          $contents[] = array('text' => escs_draw_separator('pixel_black.gif', '100%', '1'));
