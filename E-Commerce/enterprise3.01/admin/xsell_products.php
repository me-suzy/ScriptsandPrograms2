...
<!-- body_text //-->
	...
	{
	  $result_1 = escs_db_query($query_1);
 ...
 $query = "select b.products_name, a.xsell_id from " . TABLE_PRODUCTS_XSELL . " a, products_description b WHERE b.products_id = a.xsell_id and a.products_id ='".$PRODUCTS_id[$i]."' ORDER BY sort_order";
					...
					echo '<td class="dataTableContent"  valign="top">&nbsp;<a href="' . escs_href_link(FILENAME_XSELL_PRODUCTS, 'add_related_product_ID=' . $PRODUCTS_id[$i], 'NONSSL') . '">Add</a>&nbsp;</td><td class="dataTableContent" valign="top">&nbsp;<a href="' . escs_href_link(FILENAME_XSELL_PRODUCTS, 'add_related_product_ID=' . $PRODUCTS_id[$i], 'NONSSL') . '">Remove</a>&nbsp;</td>';

					if (count($Related_items)>1)
					{
					  echo '<td class="dataTableContent" valign="top">&nbsp;<a href="' . escs_href_link(FILENAME_XSELL_PRODUCTS, 'sort=1&add_related_product_ID=' . $PRODUCTS_id[$i], 'NONSSL') . '">Sort</a>&nbsp;</td>';
					} else {
	...
	if ($_POST && !$sort)
	{
	  ...
	  echo '<a href="' . escs_href_link(FILENAME_XSELL_PRODUCTS, '', 'NONSSL') . '">Click Here to add a new cross sale</a><br>' . "\n";
	  if ($_POST[xsell_id])
		echo '<a href="' . escs_href_link(FILENAME_XSELL_PRODUCTS, 'sort=1&add_related_product_ID=' . $_POST[add_related_product_ID], 'NONSSL') . '">Click here to sort (top to bottom) the added cross sale</a>' . "\n";
	}

        if ($add_related_product_ID && ! $_POST && !$sort)
	{	?>
 	  <table border="0" cellpadding="2" cellspacing="1" bgcolor="#999999">
              <form action="<?php escs_href_link(FILENAME_XSELL_PRODUCTS, '', 'NONSSL'); ?>" method="post">
                <tr class="dataTableHeadingRow">
<?php
			 ...
			 $num_of_products = sizeof($PRODUCTS_id);
				$query = "select * from " . TABLE_PRODUCTS_XSELL . " WHERE products_id = '".$add_related_product_ID."'";
					...
					<? echo "<td  class=\"dataTableContent\" align=center>".$PRODUCTS_id[$i]."</td>\n"
						...
						."<td class=\"dataTableContent\">".$currencies->display_price($PRODUCTS_price[$i], escs_get_tax_rate($product_info_values['products_tax_class_id']))."</td></tr>\n";
					}?>
					<tr>
                  <td colspan="4">
		...
		foreach ($_POST as $key_a => $value_a)
	  {
		...
		if ($value_a != 'Update')
			if (!escs_db_query($query))
				...
				if ($run_once==0)
				{
					echo '<b>The Database was updated <a href="' . escs_href_link(FILENAME_XSELL_PRODUCTS, '', 'NONSSL') . '">Click here to back to the main page</a></b><br>' . "\n";
					$run_once++;
				}

	}// end of foreach.
	?>
	<form method="post" action="<?php escs_href_link(FILENAME_XSELL_PRODUCTS, 'sort=1&add_related_product_ID=' . $add_related_product_ID, 'NONSSL'); ?>">
        ...
        $query = "select a.products_id, b.products_name, b.products_description, " .
                 ...
                 "b.products_url, a.products_price from products a, products_description b where b.products_id = a.products_id and a.products_id = ".$xsell_id_PR[$i]."";
					...
					<tr class="dataTableContentRow" bgcolor='#DFE4F4'>
					  ...
					  <td class="dataTableContent"><?php echo $currencies->display_price($PRODUCTS_price[0], escs_get_tax_rate($product_info_values['products_tax_class_id'])); ?></td>
