...
*/
  ...
  if (($HTTP_GET_VARS['action'] == 'send_email_to_user') && ($HTTP_POST_VARS['customers_email_address']) && (!$HTTP_POST_VARS['back_x'])) {
    ...
    $coupon_query = escs_db_query("select coupon_code from " . TABLE_COUPONS . " where coupon_id = '" . $HTTP_GET_VARS['cid'] . "'");
    $coupon_result = escs_db_fetch_array($coupon_query);
    $coupon_name_query = escs_db_query("select coupon_name from " . TABLE_COUPONS_DESCRIPTION . " where coupon_id = '" . $HTTP_GET_VARS['cid'] . "' and language_id = '" . $languages_id . "'");
    $coupon_name = escs_db_fetch_array($coupon_name_query);
...
// MaxiDVD Added Line For WYSIWYG HTML Area: EOF (Send HTML Email when WYSIWYG Enabled)
    ...
    escs_redirect(escs_href_link(FILENAME_COUPON_ADMIN, 'mail_sent_to=' . urlencode($mail_sent_to)));
  }
  ...
  switch ($HTTP_GET_VARS['action']) {
    case 'confirmdelete':
      $delete_query=escs_db_query("update " . TABLE_COUPONS . " set coupon_active = 'N' where coupon_id='".$HTTP_GET_VARS['cid']."'");
      break;
    case 'update':
      ...
      $query1 = escs_db_query("select coupon_code from " . TABLE_COUPONS . " where coupon_code = '" . escs_db_prepare_input($coupon_code) . "'");
      if (escs_db_num_rows($query1) && $HTTP_POST_VARS['coupon_code'] && $HTTP_GET_VARS['oldaction'] != 'voucheredit')  {
        $update_errors = 1;
    ...
    case 'update_confirm':
      if ( ($HTTP_POST_VARS['back_x']) || ($HTTP_POST_VARS['back_y']) ) {
        ...
        $sql_data_array = array('coupon_code' => escs_db_prepare_input($HTTP_POST_VARS['coupon_code']),
                                'coupon_amount' => escs_db_prepare_input($HTTP_POST_VARS['coupon_amount']),
                                'coupon_type' => escs_db_prepare_input($coupon_type),
                                'uses_per_coupon' => escs_db_prepare_input($HTTP_POST_VARS['coupon_uses_coupon']),
                                'uses_per_user' => escs_db_prepare_input($HTTP_POST_VARS['coupon_uses_user']),
                                'coupon_minimum_order' => escs_db_prepare_input($HTTP_POST_VARS['coupon_min_order']),
                                'restrict_to_products' => escs_db_prepare_input($HTTP_POST_VARS['coupon_products']),
                                'restrict_to_categories' => escs_db_prepare_input($HTTP_POST_VARS['coupon_categories']),
        ...
        for ($i = 0, $n = sizeof($languages); $i < $n; $i++) {
          $language_id = $languages[$i]['id'];
          $sql_data_marray[$i] = array('coupon_name' => escs_db_prepare_input($HTTP_POST_VARS['coupon_name'][$language_id]),
                                 'coupon_description' => escs_db_prepare_input($HTTP_POST_VARS['coupon_desc'][$language_id])
                                 );
        }
//        $query = escs_db_query("select coupon_code from " . TABLE_COUPONS . " where coupon_code = '" . escs_db_prepare_input($HTTP_POST_VARS['coupon_code']) . "'");
//        if (!escs_db_num_rows($query)) {
        if ($HTTP_GET_VARS['oldaction']=='voucheredit') {
          escs_db_perform(TABLE_COUPONS, $sql_data_array, 'update', "coupon_id='" . $HTTP_GET_VARS['cid']."'");
          for ($i = 0, $n = sizeof($languages); $i < $n; $i++) {
          $language_id = $languages[$i]['id'];
            $update = escs_db_query("update " . TABLE_COUPONS_DESCRIPTION . " set coupon_name = '" . escs_db_prepare_input($HTTP_POST_VARS['coupon_name'][$language_id]) . "', coupon_description = '" . escs_db_prepare_input($HTTP_POST_VARS['coupon_desc'][$language_id]) . "' where coupon_id = '" . $HTTP_GET_VARS['cid'] . "' and language_id = '" . $language_id . "'");
//            escs_db_perform(TABLE_COUPONS_DESCRIPTION, $sql_data_marray[$i], 'update', "coupon_id='" . $HTTP_GET_VARS['cid']."'");
          }
        } else {
          $query = escs_db_perform(TABLE_COUPONS, $sql_data_array);
          $insert_id = escs_db_insert_id($query);

          for ($i = 0, $n = sizeof($languages); $i < $n; $i++) {
            $language_id = $languages[$i]['id'];
            $sql_data_marray[$i]['coupon_id'] = $insert_id;
            $sql_data_marray[$i]['language_id'] = $language_id;
            escs_db_perform(TABLE_COUPONS_DESCRIPTION, $sql_data_marray[$i]);
          }
//        }
      }
    }
...
<?php
    ...
    $cc_query = escs_db_query($cc_query_raw);
    while ($cc_list = escs_db_fetch_array($cc_query)) {
      $rows++;
      if (strlen($rows) < 2) {
        $rows = '0' . $rows;
      }
      ...
      if ( (is_object($cInfo)) && ($cc_list['unique_id'] == $cInfo->unique_id) ) {
        echo '          <tr class="dataTableRowSelected" onmouseover="this.style.cursor=\'hand\'" onclick="document.location.href=\'' . escs_href_link('coupon_admin.php', escs_get_all_get_params(array('cid', 'action', 'uid')) . 'cid=' . $cInfo->coupon_id . '&action=voucherreport&uid=' . $cinfo->unique_id) . '\'">' . "\n";
      } else {
        echo '          <tr class="dataTableRow" onmouseover="this.className=\'dataTableRowOver\';this.style.cursor=\'hand\'" onmouseout="this.className=\'dataTableRow\'" onclick="document.location.href=\'' . escs_href_link('coupon_admin.php', escs_get_all_get_params(array('cid', 'action', 'uid')) . 'cid=' . $cc_list['coupon_id'] . '&action=voucherreport&uid=' . $cc_list['unique_id']) . '\'">' . "\n";
      }
$customer_query = escs_db_query("select customers_firstname, customers_lastname from " . TABLE_CUSTOMERS . " where customers_id = '" . $cc_list['customer_id'] . "'");
...
?>
                ...
                <td class="dataTableContent" align="center"><?php echo escs_date_short($cc_list['redeem_date']); ?></td>
                <td class="dataTableContent" align="right"><?php if ( (is_object($cInfo)) && ($cc_list['unique_id'] == $cInfo->unique_id) ) { echo escs_image(DIR_WS_IMAGES . 'icon_arrow_right.gif'); } else { echo '<a href="' . escs_href_link(FILENAME_COUPON_ADMIN, 'page=' . $HTTP_GET_VARS['page'] . '&cid=' . $cc_list['coupon_id']) . '">' . escs_image(DIR_WS_IMAGES . 'icon_info.gif', IMAGE_ICON_INFO) . '</a>'; } ?>&nbsp;</td>
              </tr>
<?php
    }
?>
...
<?php
    $heading = array();
    $contents = array();
      $coupon_description_query = escs_db_query("select coupon_name from " . TABLE_COUPONS_DESCRIPTION . " where coupon_id = '" . $HTTP_GET_VARS['cid'] . "' and language_id = '" . $languages_id . "'");
      $coupon_desc = escs_db_fetch_array($coupon_description_query);
      $count_customers = escs_db_query("select * from " . TABLE_COUPON_REDEEM_TRACK . " where coupon_id = '" . $HTTP_GET_VARS['cid'] . "' and customer_id = '" . $cInfo->customer_id . "'");

      $heading[] = array('text' => '<b>[' . $HTTP_GET_VARS['cid'] . ']' . COUPON_NAME . ' ' . $coupon_desc['coupon_name'] . '</b>');
      $contents[] = array('text' => '<b>' . TEXT_REDEMPTIONS . '</b>');
      $contents[] = array('text' => TEXT_REDEMPTIONS_TOTAL . '=' . escs_db_num_rows($cc_query));
      $contents[] = array('text' => TEXT_REDEMPTIONS_CUSTOMER . '=' . escs_db_num_rows($count_customers));
      $contents[] = array('text' => '');
?>
...
?>
      ...
      <tr>
          <tr><?php echo escs_draw_form('mail', FILENAME_COUPON_ADMIN, 'action=send_email_to_user&cid=' . $HTTP_GET_VARS['cid']); ?>
...
?>
      ...
      <tr>
          ...
          <tr><?php echo escs_draw_form('mail', FILENAME_COUPON_ADMIN, 'action=preview_email&cid='. $HTTP_GET_VARS['cid']); ?>
...
/*
              <tr>
                ...
                <td><?php echo escs_draw_checkbox_field('customers_restrict', $customers_restrict);?></td>
              </tr>
              <tr>
                <td colspan="2"><?php echo escs_draw_separator('pixel_trans.gif', '1', '10'); ?></td>
              </tr>
...
?>
      ...
      <tr>
      <td>
<?php echo escs_draw_form('coupon', 'coupon_admin.php', 'action=update_confirm&oldaction=' . $HTTP_GET_VARS['oldaction'] . '&cid=' . $HTTP_GET_VARS['cid']); ?>
...
<?php
        ...
        for ($i = 0, $n = sizeof($languages); $i < $n; $i++) {
          $language_id = $languages[$i]['id'];
          echo escs_draw_hidden_field('coupon_name[' . $languages[$i]['id'] . ']', $HTTP_POST_VARS['coupon_name'][$language_id]);
          echo escs_draw_hidden_field('coupon_desc[' . $languages[$i]['id'] . ']', $HTTP_POST_VARS['coupon_desc'][$language_id]);
       }
    echo escs_draw_hidden_field('coupon_amount', $HTTP_POST_VARS['coupon_amount']);
    echo escs_draw_hidden_field('coupon_min_order', $HTTP_POST_VARS['coupon_min_order']);
    echo escs_draw_hidden_field('coupon_free_ship', $HTTP_POST_VARS['coupon_free_ship']);
    echo escs_draw_hidden_field('coupon_code', $c_code);
    echo escs_draw_hidden_field('coupon_uses_coupon', $HTTP_POST_VARS['coupon_uses_coupon']);
    echo escs_draw_hidden_field('coupon_uses_user', $HTTP_POST_VARS['coupon_uses_user']);
    echo escs_draw_hidden_field('coupon_products', $HTTP_POST_VARS['coupon_products']);
    echo escs_draw_hidden_field('coupon_categories', $HTTP_POST_VARS['coupon_categories']);
    echo escs_draw_hidden_field('coupon_startdate', date('Y-m-d', mktime(0, 0, 0, $HTTP_POST_VARS['coupon_startdate_month'],$HTTP_POST_VARS['coupon_startdate_day'] ,$HTTP_POST_VARS['coupon_startdate_year'] )));
    echo escs_draw_hidden_field('coupon_finishdate', date('Y-m-d', mktime(0, 0, 0, $HTTP_POST_VARS['coupon_finishdate_month'],$HTTP_POST_VARS['coupon_finishdate_day'] ,$HTTP_POST_VARS['coupon_finishdate_year'] )));
?>
     <tr>
        <td align="left"><?php echo escs_image_submit('button_confirm.gif',COUPON_BUTTON_CONFIRM); ?></td>
        <td align="left"><?php echo escs_image_submit('button_back.gif',COUPON_BUTTON_BACK, 'name=back'); ?></td>
      </td>
      </tr>
...
<?php
  ...
  case 'voucheredit':
    ...
    for ($i = 0, $n = sizeof($languages); $i < $n; $i++) {
      $language_id = $languages[$i]['id'];
      $coupon_query = escs_db_query("select coupon_name,coupon_description from " . TABLE_COUPONS_DESCRIPTION . " where coupon_id = '" .  $HTTP_GET_VARS['cid'] . "' and language_id = '" . $language_id . "'");
    ...
    $coupon_query=escs_db_query("select coupon_code, coupon_amount, coupon_type, coupon_minimum_order, coupon_start_date, coupon_expire_date, uses_per_coupon, uses_per_user, restrict_to_products, restrict_to_categories from " . TABLE_COUPONS . " where coupon_id = '" . $HTTP_GET_VARS['cid'] . "'");
    $coupon=escs_db_fetch_array($coupon_query);
...
?>
      ...
      <td>
<?php
    echo escs_draw_form('coupon', 'coupon_admin.php', 'action=update&oldaction='.$HTTP_GET_VARS['action'] . '&cid=' . $HTTP_GET_VARS['cid']);
?>
      <table border="0" width="100%" cellspacing="0" cellpadding="6">
<?php
        $languages = escs_get_languages();
...
?>
      <tr>
        ...
        <td align="left"><?php echo escs_draw_input_field('coupon_name[' . $languages[$i]['id'] . ']', $coupon_name[$language_id]) . '&nbsp;' . escs_image(DIR_WS_CATALOG_LANGUAGES . $languages[$i]['directory'] . '/images/' . $languages[$i]['image'], $languages[$i]['name']); ?></td>
...
?>
      ...
      <tr>
        ...
        <td align="left" valign="top"><?php echo escs_draw_textarea_field('coupon_desc[' . $languages[$i]['id'] . ']','physical','24','3', $coupon_desc[$language_id]) . '&nbsp;' . escs_image(DIR_WS_CATALOG_LANGUAGES . $languages[$i]['directory'] . '/images/' . $languages[$i]['image'], $languages[$i]['name']); ?></td>
...
?>
      <tr>
        ...
        <td align="left"><?php echo escs_draw_input_field('coupon_amount', $coupon_amount); ?></td>
        <td align="left" class="main"><?php echo COUPON_AMOUNT_HELP; ?></td>
      </tr>
      <tr>
        <td align="left" class="main"><?php echo COUPON_MIN_ORDER; ?></td>
        <td align="left"><?php echo escs_draw_input_field('coupon_min_order', $coupon_min_order); ?></td>
        <td align="left" class="main"><?php echo COUPON_MIN_ORDER_HELP; ?></td>
      </tr>
      <tr>
        <td align="left" class="main"><?php echo COUPON_FREE_SHIP; ?></td>
        <td align="left"><?php echo escs_draw_checkbox_field('coupon_free_ship', $coupon_free_ship); ?></td>
        <td align="left" class="main"><?php echo COUPON_FREE_SHIP_HELP; ?></td>
      </tr>
      <tr>
        <td align="left" class="main"><?php echo COUPON_CODE; ?></td>
        <td align="left"><?php echo escs_draw_input_field('coupon_code', $coupon_code); ?></td>
        <td align="left" class="main"><?php echo COUPON_CODE_HELP; ?></td>
      </tr>
      <tr>
        <td align="left" class="main"><?php echo COUPON_USES_COUPON; ?></td>
        <td align="left"><?php echo escs_draw_input_field('coupon_uses_coupon', $coupon_uses_coupon); ?></td>
        <td align="left" class="main"><?php echo COUPON_USES_COUPON_HELP; ?></td>
      </tr>
      <tr>
        <td align="left" class="main"><?php echo COUPON_USES_USER; ?></td>
        <td align="left"><?php echo escs_draw_input_field('coupon_uses_user', $coupon_uses_user); ?></td>
        <td align="left" class="main"><?php echo COUPON_USES_USER_HELP; ?></td>
      </tr>
       <tr>
        <td align="left" class="main"><?php echo COUPON_PRODUCTS; ?></td>
        <td align="left"><?php echo escs_draw_input_field('coupon_products', $coupon_products); ?> <A HREF="validproducts.php" TARGET="_blank" ONCLICK="window.open('validproducts.php', 'Valid_Products', 'scrollbars=yes,resizable=yes,menubar=yes,width=600,height=600'); return false">View</A></td>
        <td align="left" class="main"><?php echo COUPON_PRODUCTS_HELP; ?></td>
      </tr>
      <tr>
        <td align="left" class="main"><?php echo COUPON_CATEGORIES; ?></td>
        <td align="left"><?php echo escs_draw_input_field('coupon_categories', $coupon_categories); ?> <A HREF="validcategories.php" TARGET="_blank" ONCLICK="window.open('validcategories.php', 'Valid_Categories', 'scrollbars=yes,resizable=yes,menubar=yes,width=600,height=600'); return false">View</A></td>
...
?>
        ...
        <td align="left"><?php echo escs_draw_date_selector('coupon_startdate', mktime(0,0,0, $coupon_startdate[1], $coupon_startdate[2], $coupon_startdate[0], 0)); ?></td>
        <td align="left" class="main"><?php echo COUPON_STARTDATE_HELP; ?></td>
      </tr>
      <tr>
        <td align="left" class="main"><?php echo COUPON_FINISHDATE; ?></td>
        <td align="left"><?php echo escs_draw_date_selector('coupon_finishdate', mktime(0,0,0, $coupon_finishdate[1], $coupon_finishdate[2], $coupon_finishdate[0], 0)); ?></td>
        <td align="left" class="main"><?php echo COUPON_FINISHDATE_HELP; ?></td>
      </tr>
      <tr>
        <td align="left"><?php echo escs_image_submit('button_preview.gif',COUPON_BUTTON_PREVIEW); ?></td>
        <td align="left"><?php echo '&nbsp;&nbsp;<a href="' . escs_href_link('coupon_admin.php', ''); ?>"><?php echo escs_image_button('button_cancel.gif', IMAGE_CANCEL); ?></a>
      </td>
      </tr>
      </td></table></form>
      </tr>
    ...
    <td width="100%" valign="top"><table border="0" width="100%" cellspacing="0" cellpadding="2">
      <tr>
        <td width="100%"><table border="0" width="100%" cellspacing="0" cellpadding="0">
          <tr>
            <td class="pageHeading"><?php echo HEADING_TITLE; ?></td>
            <td class="main"><?php echo escs_draw_form('status', FILENAME_COUPON_ADMIN, '', 'get'); ?>
<?php
    ...
    echo HEADING_TITLE_STATUS . ' ' . escs_draw_pull_down_menu('status', $status_array, $status, 'onChange="this.form.submit();"');
?>
              </form>
           </td>
          </tr>
...
<?php
    ...
    if ($status != '*') {
      $cc_query_raw = "select coupon_id, coupon_code, coupon_amount, coupon_type, coupon_start_date,coupon_expire_date,uses_per_user,uses_per_coupon,restrict_to_products, restrict_to_categories, date_created,date_modified from " . TABLE_COUPONS ." where coupon_active='" . escs_db_input($status) . "' and coupon_type != 'G'";
    } else {
    ...
    while ($cc_list = escs_db_fetch_array($cc_query)) {
      ...
      if ( (is_object($cInfo)) && ($cc_list['coupon_id'] == $cInfo->coupon_id) ) {
        echo '          <tr class="dataTableRowSelected" onmouseover="this.style.cursor=\'hand\'" onclick="document.location.href=\'' . escs_href_link('coupon_admin.php', escs_get_all_get_params(array('cid', 'action')) . 'cid=' . $cInfo->coupon_id . '&action=edit') . '\'">' . "\n";
      } else {
        echo '          <tr class="dataTableRow" onmouseover="this.className=\'dataTableRowOver\';this.style.cursor=\'hand\'" onmouseout="this.className=\'dataTableRow\'" onclick="document.location.href=\'' . escs_href_link('coupon_admin.php', escs_get_all_get_params(array('cid', 'action')) . 'cid=' . $cc_list['coupon_id']) . '\'">' . "\n";
      }
      $coupon_description_query = escs_db_query("select coupon_name from " . TABLE_COUPONS_DESCRIPTION . " where coupon_id = '" . $cc_list['coupon_id'] . "' and language_id = '" . $languages_id . "'");
      $coupon_desc = escs_db_fetch_array($coupon_description_query);
?>
...
?>
            &nbsp;</td>
                ...
                <td class="dataTableContent" align="right"><?php if ( (is_object($cInfo)) && ($cc_list['coupon_id'] == $cInfo->coupon_id) ) { echo escs_image(DIR_WS_IMAGES . 'icon_arrow_right.gif'); } else { echo '<a href="' . escs_href_link(FILENAME_COUPON_ADMIN, 'page=' . $HTTP_GET_VARS['page'] . '&cid=' . $cc_list['coupon_id']) . '">' . escs_image(DIR_WS_IMAGES . 'icon_info.gif', IMAGE_ICON_INFO) . '</a>'; } ?>&nbsp;</td>
              </tr>
<?php
    }
?>
          <tr>
            <td colspan="5"><table border="0" width="100%" cellspacing="0" cellpadding="2">
              ...
              <tr>
                <td align="right" colspan="2" class="smallText"><?php echo '<a href="' . escs_href_link('coupon_admin.php', 'page=' . $HTTP_GET_VARS['page'] . '&cID=' . $cInfo->coupon_id . '&action=new') . '">' . escs_image_button('button_insert.gif', IMAGE_INSERT) . '</a>'; ?></td>
              </tr>
            </table></td>
          </tr>
        </table></td>
...
<?php
    ...
    case 'neww':
      ...
      $contents[] = array('text' => TEXT_NEW_INTRO);
      $contents[] = array('text' => '<br>' . COUPON_NAME . '<br>' . escs_draw_input_field('name'));
      $contents[] = array('text' => '<br>' . COUPON_AMOUNT . '<br>' . escs_draw_input_field('voucher_amount'));
      $contents[] = array('text' => '<br>' . COUPON_CODE . '<br>' . escs_draw_input_field('voucher_code'));
      $contents[] = array('text' => '<br>' . COUPON_USES_COUPON . '<br>' . escs_draw_input_field('voucher_number_of'));
      break;
    default:
      ...
      if ($HTTP_GET_VARS['action'] == 'voucherdelete') {
        $contents[] = array('text'=> TEXT_CONFIRM_DELETE . '</br></br>' .
                '<a href="'.escs_href_link('coupon_admin.php','action=confirmdelete&cid='.$HTTP_GET_VARS['cid'],'NONSSL').'">'.escs_image_button('button_confirm.gif','Confirm Delete Voucher').'</a>' .
                '<a href="'.escs_href_link('coupon_admin.php','cid='.$cInfo->coupon_id,'NONSSL').'">'.escs_image_button('button_cancel.gif','Cancel').'</a>'
                );
      } else {
        $prod_details = NONE;
        ...
        $coupon_name_query = escs_db_query("select coupon_name from " . TABLE_COUPONS_DESCRIPTION . " where coupon_id = '" . $cInfo->coupon_id . "' and language_id = '" . $languages_id . "'");
        $coupon_name = escs_db_fetch_array($coupon_name_query);
        $contents[] = array('text'=>COUPON_NAME . '&nbsp;::&nbsp; ' . $coupon_name['coupon_name'] . '<br>' .
                     COUPON_AMOUNT . '&nbsp;::&nbsp; ' . $amount . '<br>' .
                     COUPON_STARTDATE . '&nbsp;::&nbsp; ' . escs_date_short($cInfo->coupon_start_date) . '<br>' .
                     COUPON_FINISHDATE . '&nbsp;::&nbsp; ' . escs_date_short($cInfo->coupon_expire_date) . '<br>' .
                     ...
                     DATE_CREATED . '&nbsp;::&nbsp; ' . escs_date_short($cInfo->date_created) . '<br>' .
                     DATE_MODIFIED . '&nbsp;::&nbsp; ' . escs_date_short($cInfo->date_modified) . '<br><br>' .
                     '<center><a href="'.escs_href_link('coupon_admin.php','action=email&cid='.$cInfo->coupon_id,'NONSSL').'">'.escs_image_button('button_email.gif','Email Voucher').'</a>' .
                     '<a href="'.escs_href_link('coupon_admin.php','action=voucheredit&cid='.$cInfo->coupon_id,'NONSSL').'">'.escs_image_button('button_edit.gif','Edit Voucher').'</a>' .
                     '<a href="'.escs_href_link('coupon_admin.php','action=voucherdelete&cid='.$cInfo->coupon_id,'NONSSL').'">'.escs_image_button('button_delete.gif','Delete Voucher').'</a>' .
                     '<br><a href="'.escs_href_link('coupon_admin.php','action=voucherreport&cid='.$cInfo->coupon_id,'NONSSL').'">'.escs_image_button('button_report.gif','Voucher Report').'</a></center>'
                     );
        }
        break;
      }
