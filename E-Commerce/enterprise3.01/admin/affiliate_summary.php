...
*/
  ...
  if (AFFILIATE_DELETE_CLICKTHROUGHS != 'false' && is_numeric(AFFILIATE_DELETE_CLICKTHROUGHS)) {
    ...
    escs_db_query("delete from " . TABLE_AFFILIATE_CLICKTHROUGHS . " where affiliate_clientdate < '". $time . "'");
  }
  ...
  if (AFFILIATE_DELETE_AFFILIATE_BANNER_HISTORY != 'false' && is_numeric(AFFILIATE_DELETE_AFFILIATE_BANNER_HISTORY)) {
    ...
    $time = date("Y-m-d", $time);
    escs_db_query("delete from " . TABLE_AFFILIATE_BANNERS_HISTORY . " where affiliate_banners_history_date < '". $time . "'");
  }
  ...
  if ($affiliate_transactions > 0) {
	$affiliate_average = escs_round($affiliate_amount / $affiliate_transactions, 2);
  } else {
  ...
  $affiliates_raw_query = escs_db_query($affiliates_raw);
  $affiliates_raw = escs_db_fetch_array($affiliates_raw_query);
            ...
            <td><table width="100%" border="0" cellpadding="4" cellspacing="2" class="dataTableContent">
              <center>
                ...
                <tr>
                  ...
                  <td width="15%" class="dataTableContent"><?php echo escs_round(AFFILIATE_PERCENT, 2) . ' %'; ?></td>
