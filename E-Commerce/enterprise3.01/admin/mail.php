...
*/
  ...
  if ( ($action == 'send_email_to_user') && isset($HTTP_POST_VARS['customers_email_address']) && !isset($HTTP_POST_VARS['back_x']) ) {
    ...
    $mimemessage = new email(array('X-Mailer: Enterprise Shopping Cart'));
...
// MaxiDVD Added Line For WYSIWYG HTML Area: EOF (Send HTML Email when WYSIWYG Enabled)
    ...
    escs_redirect(escs_href_link(FILENAME_MAIL, 'mail_sent_to=' . urlencode($mail_sent_to)));
  }
...
?>
          <tr><?php echo escs_draw_form('mail', FILENAME_MAIL, 'action=send_email_to_user'); ?>
...
?>
          <tr><?php echo escs_draw_form('mail', FILENAME_MAIL, 'action=preview'); ?>
...
?>
              <tr>
                ...
                <td><?php echo escs_draw_pull_down_menu('customers_email_address', $customers, (isset($HTTP_GET_VARS['customer']) ? $HTTP_GET_VARS['customer'] : ''));?></td>
              </tr>
              <tr>
                <td colspan="2"><?php echo escs_draw_separator('pixel_trans.gif', '1', '10'); ?></td>
              </tr>
