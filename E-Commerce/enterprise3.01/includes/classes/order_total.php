...
// class constructor
    function order_total() {
      ...
      if (defined('MODULE_ORDER_TOTAL_INSTALLED') && escs_not_null(MODULE_ORDER_TOTAL_INSTALLED)) {
        ...
        while (list(, $value) = each($this->modules)) {
          ...
          if ($GLOBALS[$class]->enabled) {
            ...
            for ($i=0, $n=sizeof($GLOBALS[$class]->output); $i<$n; $i++) {
              if (escs_not_null($GLOBALS[$class]->output[$i]['title']) && escs_not_null($GLOBALS[$class]->output[$i]['text'])) {
...
//
    function credit_selection() {
      ...
      if (MODULE_ORDER_TOTAL_INSTALLED) {
       ...
       $header_string .= '       <tr><td width="10">' .  escs_draw_separator('pixel_trans.gif', '10', '1') .'</td>' . "\n";
        $header_string .= '           <td colspan="2"><table border="0" width="100%" cellspacing="0" cellpadding="2">' . "\n";
        $close_string   = '                           </table></td>';
        $close_string  .= '<td width="10">' .  escs_draw_separator('pixel_trans.gif', '10', '1') . '</td>';
        $close_string  .= '</tr></table></td></tr></table></td>';
        $close_string  .= '<tr><td width="100%">' .  escs_draw_separator('pixel_trans.gif', '100%', '10') . '</td></tr>';
        reset($this->modules);
        $output_string = '';
        while (list(, $value) = each($this->modules)) {
          $class = substr($value, 0, strrpos($value, '.'));
          if ($GLOBALS[$class]->enabled && $GLOBALS[$class]->credit_class) {
            ...
            if ( ($use_credit_string !='' ) || ($selection_string != '') ) {
              $output_string .=  '<tr colspan="4"><td colspan="4" width="100%">' .  escs_draw_separator('pixel_trans.gif', '100%', '10') . '</td></tr>';
              $output_string = ' <tr class="moduleRow" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" >' . "\n" .
                               '   <td width="10">' .  escs_draw_separator('pixel_trans.gif', '10', '1') .'</td>' .
                               '     <td class="main"><b>' . $GLOBALS[$class]->header . '</b></td>' . $use_credit_string;
              $output_string .= '<td width="10">' . escs_draw_separator('pixel_trans.gif', '10', '1') . '</td>';
              $output_string .= '  </tr>' . "\n";
...
//            if (!escs_session_is_registered($post_var)) escs_session_register($post_var);
...
//            if (escs_session_is_registered($post_var)) escs_session_unregister($post_var);
          }
        }
      }
    }
