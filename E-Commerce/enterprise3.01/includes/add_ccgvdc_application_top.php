...
// $salt needs some thought.
  ...
  function create_coupon_code($salt="secret", $length = SECURITY_CODE_LENGTH) {
    ...
    while ($good_result == 0) {
      ...
      $query = escs_db_query("select coupon_code from " . TABLE_COUPONS . " where coupon_code = '" . $id1 . "'");
      if (escs_db_num_rows($query) == 0) $good_result = 1;
    }
    return $id1;
  }
////
// Update the Customers GV account
  function escs_gv_account_update($customer_id, $gv_id) {
    $customer_gv_query = escs_db_query("select amount from " . TABLE_COUPON_GV_CUSTOMER . " where customer_id = '" . $customer_id . "'");
    $coupon_gv_query = escs_db_query("select coupon_amount from " . TABLE_COUPONS . " where coupon_id = '" . $gv_id . "'");
    $coupon_gv = escs_db_fetch_array($coupon_gv_query);
    if (escs_db_num_rows($customer_gv_query) > 0) {
      $customer_gv = escs_db_fetch_array($customer_gv_query);
      $new_gv_amount = $customer_gv['amount'] + $coupon_gv['coupon_amount'];
      $gv_query = escs_db_query("update " . TABLE_COUPON_GV_CUSTOMER . " set amount = '" . $new_gv_amount . "'");
    } else {
      $gv_query = escs_db_query("insert into " . TABLE_COUPON_GV_CUSTOMER . " (customer_id, amount) values ('" . $customer_id . "', '" . $coupon_gv['coupon_amount'] . "')");
    }
  }
////
// Get tax rate from tax description
  function escs_get_tax_rate_from_desc($tax_desc) {
    $tax_query = escs_db_query("select tax_rate from " . TABLE_TAX_RATES . " where tax_description = '" . $tax_desc . "'");
    $tax = escs_db_fetch_array($tax_query);
    return $tax['tax_rate'];
  }
?>