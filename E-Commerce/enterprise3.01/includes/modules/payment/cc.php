...
// class methods
    function update_status() {
      ...
      if ( ($this->enabled == true) && ((int)MODULE_PAYMENT_CC_ZONE > 0) ) {
        $check_flag = false;
        $check_query = escs_db_query("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_PAYMENT_CC_ZONE . "' and zone_country_id = '" . $order->billing['country']['id'] . "' order by zone_id");
    ...
    function selection() {
      ...
      $selection = array('id' => $this->code,
                         ...
                         'fields' => array(array('title' => MODULE_PAYMENT_CC_TEXT_CREDIT_CARD_OWNER,
                                                 'field' => escs_draw_input_field('cc_owner', $order->billing['firstname'] . ' ' . $order->billing['lastname'])),
                                           array('title' => MODULE_PAYMENT_CC_TEXT_CREDIT_CARD_NUMBER,
                                                 'field' => escs_draw_input_field('cc_number')),
                                           array('title' => MODULE_PAYMENT_CC_TEXT_CREDIT_CARD_EXPIRES,
                                                 'field' => escs_draw_pull_down_menu('cc_expires_month', $expires_month) . '&nbsp;' . escs_draw_pull_down_menu('cc_expires_year', $expires_year))));
    ...
    function process_button() {
      ...
      $process_button_string = escs_draw_hidden_field('cc_owner', $HTTP_POST_VARS['cc_owner']) .
                               escs_draw_hidden_field('cc_expires', $HTTP_POST_VARS['cc_expires_month'] . $HTTP_POST_VARS['cc_expires_year']) .
                               escs_draw_hidden_field('cc_type', $this->cc_card_type) .
                               escs_draw_hidden_field('cc_number', $this->cc_card_number);
    ...
    function before_process() {
      ...
      if ( (defined('MODULE_PAYMENT_CC_EMAIL')) && (escs_validate_email(MODULE_PAYMENT_CC_EMAIL)) ) {
    ...
    function after_process() {
      ...
      if ( (defined('MODULE_PAYMENT_CC_EMAIL')) && (escs_validate_email(MODULE_PAYMENT_CC_EMAIL)) ) {
        ...
        escs_mail('', MODULE_PAYMENT_CC_EMAIL, 'Extra Order Info: #' . $insert_id, $message, STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS);
      }
    }
    ...
    function check() {
      if (!isset($this->_check)) {
        $check_query = escs_db_query("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_PAYMENT_CC_STATUS'");
    ...
    function install() {
      ...
      escs_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Sort order of display.', 'MODULE_PAYMENT_CC_SORT_ORDER', '0', 'Sort order of display. Lowest is displayed first.', '6', '0' , now())");
