...
*/
  ...
  function use_credit_amount() {
    ...
    function credit_selection() {
      ...
      $selection_string = '';
        $selection_string .= '<tr>' . "\n";
        $selection_string .= '  <td width="10">' .  escs_draw_separator('pixel_trans.gif', '10', '1') .'</td>';
        ...
        $selection_string .= TEXT_ENTER_COUPON_CODE . escs_draw_input_field('gv_redeem_code') . '</td>';
...
// get some info from the coupon table
      $coupon_query=escs_db_query("select coupon_id, coupon_amount, coupon_type, coupon_minimum_order,
      ...
      if ($coupon_result['coupon_type'] != 'G') {
        ...
        if (escs_db_num_rows($coupon_query)==0) {
          escs_redirect(escs_href_link(FILENAME_CHECKOUT_PAYMENT, 'error_message=' . urlencode(ERROR_NO_INVALID_REDEEM_COUPON), 'SSL'));
        }

        $date_query=escs_db_query("select coupon_start_date from " . TABLE_COUPONS . "
                                where coupon_start_date <= now() and
                                coupon_code='".$HTTP_POST_VARS['gv_redeem_code']."'");

        if (escs_db_num_rows($date_query)==0) {
          escs_redirect(escs_href_link(FILENAME_CHECKOUT_PAYMENT, 'error_message=' . urlencode(ERROR_INVALID_STARTDATE_COUPON), 'SSL'));
        }

        $date_query=escs_db_query("select coupon_expire_date from " . TABLE_COUPONS . "
                                where coupon_expire_date >= now() and
                                coupon_code='".$HTTP_POST_VARS['gv_redeem_code']."'");

        if (escs_db_num_rows($date_query)==0) {
          escs_redirect(escs_href_link(FILENAME_CHECKOUT_PAYMENT, 'error_message=' . urlencode(ERROR_INVALID_FINISDATE_COUPON), 'SSL'));
        }

        $coupon_count = escs_db_query("select coupon_id from " . TABLE_COUPON_REDEEM_TRACK . "
                                          where coupon_id = '" . $coupon_result['coupon_id']."'");
        $coupon_count_customer = escs_db_query("select coupon_id from " . TABLE_COUPON_REDEEM_TRACK . "
                                                   where coupon_id = '" . $coupon_result['coupon_id']."' and
                                                   customer_id = '" . $customer_id . "'");

        if (escs_db_num_rows($coupon_count)>=$coupon_result['uses_per_coupon'] && $coupon_result['uses_per_coupon'] > 0) {
          escs_redirect(escs_href_link(FILENAME_CHECKOUT_PAYMENT, 'error_message=' . urlencode(ERROR_INVALID_USES_COUPON . $coupon_result['uses_per_coupon'] . TIMES ), 'SSL'));
        }

        if (escs_db_num_rows($coupon_count_customer)>=$coupon_result['uses_per_user'] && $coupon_result['uses_per_user'] > 0) {
          escs_redirect(escs_href_link(FILENAME_CHECKOUT_PAYMENT, 'error_message=' . urlencode(ERROR_INVALID_USES_USER_COUPON . $coupon_result['uses_per_user'] . TIMES ), 'SSL'));
        }
...
//        if (!escs_session_is_registered('cc_id')) escs_session_register('cc_id');
        $_SESSION['cc_id'] = $coupon_result['coupon_id'];
      }
      if ($HTTP_POST_VARS['submit_redeem_coupon_x'] && !$HTTP_POST_VARS['gv_redeem_code']) escs_redirect(escs_href_link(FILENAME_CHECKOUT_PAYMENT, 'error_message=' . urlencode(ERROR_NO_REDEEM_CODE), 'SSL'));
    }
  }
  ...
  function calculate_credit($amount) {
    ...
    $od_amount = 0;
    if ($cc_id) {
      $coupon_query = escs_db_query("select coupon_code from " . TABLE_COUPONS . " where coupon_id = '" . $cc_id . "'");
      if (escs_db_num_rows($coupon_query) !=0 ) {
        $coupon_result = escs_db_fetch_array($coupon_query);
        $this->coupon_code = $coupon_result['coupon_code'];
        $coupon_get = escs_db_query("select coupon_amount, coupon_minimum_order, restrict_to_products, restrict_to_categories, coupon_type from " . TABLE_COUPONS ." where coupon_code = '". $coupon_result['coupon_code'] . "'");
            ...
            for ($i=0; $i<sizeof($order->products); $i++) {
              if ($get_result['restrict_to_products']) {
                ...
                for ($ii = 0; $ii < count($pr_ids); $ii++) {
                  if ($pr_ids[$ii] == escs_get_prid($order->products[$i]['id'])) {
                ...
                for ($i=0; $i<sizeof($order->products); $i++) {
                  $my_path = escs_get_product_path(escs_get_prid($order->products[$i]['id']));
    ...
    if (escs_db_num_rows($coupon_query) !=0 ) {
      ...
      if ($get_result['restrict_to_products'] || $get_result['restrict_to_categories']) {
        ...
        for ($i=0; $i<sizeof($products); $i++) {
          $t_prid = escs_get_prid($products[$i]['id']);
          $cc_query = escs_db_query("select products_tax_class_id from " . TABLE_PRODUCTS . " where products_id = '" . $t_prid . "'");
          $cc_result = escs_db_fetch_array($cc_query);
          $valid_product = false;
          if ($get_result['restrict_to_products']) {
            $pr_ids = split("[,]", $get_result['restrict_to_products']);
          ...
          if ($get_result['restrict_to_categories']) {
            ...
            for ($c = 0; $c < sizeof($cat_ids); $c++) {
              $cat_query = escs_db_query("select products_id from products_to_categories where products_id = '" . $products_id . "' and categories_id = '" . $cat_ids[$i] . "'");
              if (escs_db_num_rows($cat_query) !=0 ) $valid_product = true;
            }
          }
          if ($valid_product) {
        ...
        if ($get_result['coupon_type'] == 'S') $ratio = 1;
          if ($method=='Credit Note') {
            $tax_rate = escs_get_tax_rate($this->tax_class, $order->delivery['country']['id'], $order->delivery['zone_id']);
            $tax_desc = escs_get_tax_description($this->tax_class, $order->delivery['country']['id'], $order->delivery['zone_id']);
            if ($get_result['coupon_type'] == 'P') {
            ...
            for ($p=0; $p<sizeof($valid_array); $p++) {
              $tax_rate = escs_get_tax_rate($valid_array[$p]['products_tax_class'], $order->delivery['country']['id'], $order->delivery['zone_id']);
              $tax_desc = escs_get_tax_description($valid_array[$p]['products_tax_class'], $order->delivery['country']['id'], $order->delivery['zone_id']);
              if ($tax_rate > 0) {
        ...
        if ($get_result['coupon_type'] =='F') {
          ...
          if ($method=='Credit Note') {
            ...
            while (list($key, $value) = each($order->info['tax_groups'])) {
              $tax_rate = escs_get_tax_rate_from_desc($key);
...
function update_credit_account($i) {
 ...
 function apply_credit() {
   ...
   $cc_id = $_SESSION['cc_id'];
   if ($this->deduction !=0) {
     escs_db_query("insert into " . TABLE_COUPON_REDEEM_TRACK . " (coupon_id, redeem_date, redeem_ip, customer_id, order_id) values ('" . $cc_id . "', now(), '" . $REMOTE_ADDR . "', '" . $customer_id . "', '" . $insert_id . "')");
   }
...
// Check if gift voucher is in cart and adjust total
    ...
    for ($i=0; $i<sizeof($products); $i++) {
      $t_prid = escs_get_prid($products[$i]['id']);
      $gv_query = escs_db_query("select products_price, products_tax_class_id, products_model from " . TABLE_PRODUCTS . " where products_id = '" . $t_prid . "'");
      $gv_result = escs_db_fetch_array($gv_query);
      if (ereg('^GIFT', addslashes($gv_result['products_model']))) {
        $qty = $cart->get_quantity($t_prid);
        $products_tax = escs_get_tax_rate($gv_result['products_tax_class_id']);
        if ($this->include_tax =='false') {
           $gv_amount = $gv_result['products_price'] * $qty;
        } else {
          $gv_amount = ($gv_result['products_price'] + escs_calculate_tax($gv_result['products_price'],$products_tax)) * $qty;
        }
       ...
       for ($i = 0; $i < count($cat_ids); $i++) {
         if (is_array($this->contents)) {
           ...
           while (list($products_id, ) = each($this->contents)) {
            $cat_query = escs_db_query("select products_id from products_to_categories where products_id = '" . $products_id . "' and categories_id = '" . $cat_ids[$i] . "'");
             if (escs_db_num_rows($cat_query) !=0 ) {
               $in_cat = true;
     ...
     if ($get_result['restrict_to_products']) {
       ...
       for ($i = 0; $i < sizeof($pr_ids); $i++) {
         for ($ii = 1; $ii<=sizeof($products_array); $ii++) {
           if (escs_get_prid($products_array[$ii-1]['id']) == $pr_ids[$i]) {
             $in_cart=true;
...
function get_product_price($product_id) {
    global $cart, $order;
    $products_id = escs_get_prid($product_id);
 // products price
    $qty = $cart->contents[$product_id]['qty'];
    $product_query = escs_db_query("select products_id, products_price, products_tax_class_id, products_weight from " . TABLE_PRODUCTS . " where products_id='" . $product_id . "'");
    if ($product = escs_db_fetch_array($product_query)) {
      $prid = $product['products_id'];
      $products_tax = escs_get_tax_rate($product['products_tax_class_id']);
      $products_price = $product['products_price'];
      $specials_query = escs_db_query("select specials_new_products_price from " . TABLE_SPECIALS . " where products_id = '" . $prid . "' and status = '1'");
      if (escs_db_num_rows ($specials_query)) {
        $specials = escs_db_fetch_array($specials_query);
        $products_price = $specials['specials_new_products_price'];
      }
      if ($this->include_tax == 'true') {
        $total_price += ($products_price + escs_calculate_tax($products_price, $products_tax)) * $qty;
      } else {
...
// attributes price
      if (isset($cart->contents[$product_id]['attributes'])) {
        ...
        while (list($option, $value) = each($cart->contents[$product_id]['attributes'])) {
          $attribute_price_query = escs_db_query("select options_values_price, price_prefix from " . TABLE_PRODUCTS_ATTRIBUTES . " where products_id = '" . $prid . "' and options_id = '" . $option . "' and options_values_id = '" . $value . "'");
          $attribute_price = escs_db_fetch_array($attribute_price_query);
          if ($attribute_price['price_prefix'] == '+') {
            if ($this->include_tax == 'true') {
              $total_price += $qty * ($attribute_price['options_values_price'] + escs_calculate_tax($attribute_price['options_values_price'], $products_tax));
            } else {
    ...
    function check() {
      if (!isset($this->check)) {
        $check_query = escs_db_query("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_ORDER_TOTAL_COUPON_STATUS'");
        $this->check = escs_db_num_rows($check_query);
      }
    ...
    function install() {
      escs_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Display Total', 'MODULE_ORDER_TOTAL_COUPON_STATUS', 'true', 'Do you want to display the Discount Coupon value?', '6', '1','escs_cfg_select_option(array(\'true\', \'false\'), ', now())");
      escs_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Sort Order', 'MODULE_ORDER_TOTAL_COUPON_SORT_ORDER', '9', 'Sort order of display.', '6', '2', now())");
      escs_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function ,date_added) values ('Include Shipping', 'MODULE_ORDER_TOTAL_COUPON_INC_SHIPPING', 'true', 'Include Shipping in calculation', '6', '5', 'escs_cfg_select_option(array(\'true\', \'false\'), ', now())");
      escs_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function ,date_added) values ('Include Tax', 'MODULE_ORDER_TOTAL_COUPON_INC_TAX', 'true', 'Include Tax in calculation.', '6', '6','escs_cfg_select_option(array(\'true\', \'false\'), ', now())");
      escs_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function ,date_added) values ('Re-calculate Tax', 'MODULE_ORDER_TOTAL_COUPON_CALC_TAX', 'None', 'Re-Calculate Tax', '6', '7','escs_cfg_select_option(array(\'None\', \'Standard\', \'Credit Note\'), ', now())");
      escs_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, use_function, set_function, date_added) values ('Tax Class', 'MODULE_ORDER_TOTAL_COUPON_TAX_CLASS', '0', 'Use the following tax class when treating Discount Coupon as Credit Note.', '6', '0', 'escs_get_tax_class_title', 'escs_cfg_pull_down_tax_classes(', now())");
    }
