...
<?php
    $row = 0;
    while ($expected = escs_db_fetch_array($expected_query)) {
      $row++;
      ...
      echo '                <td class="smallText">&nbsp;<a href="' . escs_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $expected['products_id']) . '">' . $expected['products_name'] . '</a>&nbsp;</td>' . "\n" .
           '                <td align="right" class="smallText">&nbsp;' . escs_date_short($expected['date_expected']) . '&nbsp;</td>' . "\n" .
           '              </tr>' . "\n";
    }
?>
              <tr>
