...
// Define Tertiary Section Output
  ...
  $mt_categories_query = escs_db_query("select cd.categories_name from " . TABLE_CATEGORIES . " c, " . TABLE_CATEGORIES_DESCRIPTION . " cd where c.parent_id = '0' and c.categories_id = cd.categories_id and cd.language_id='" . (int)$languages_id ."'");
  while ($mt_categories = escs_db_fetch_array($mt_categories_query))  {
...
// END UPDATE
  ...
  case CONTENT_INDEX_NESTED:
    $mt_category_query = escs_db_query("select categories_name from " . TABLE_CATEGORIES_DESCRIPTION . " where categories_id = '" . (int)$current_category_id . "' and language_id = '" . (int)$languages_id . "'");
    $mt_category = escs_db_fetch_array($mt_category_query);
  ...
  case CONTENT_INDEX_PRODUCTS:
    if (isset($HTTP_GET_VARS['manufacturers_id'])) {
	  $mt_manufacturer_query = escs_db_query("select manufacturers_name from " . TABLE_MANUFACTURERS . " where manufacturers_id = '" . (int)$HTTP_GET_VARS['manufacturers_id'] . "'");
      $mt_manufacturer = escs_db_fetch_array($mt_manufacturer_query);
  ...
  case CONTENT_PRODUCT_INFO:
    $mt_product_info_query = escs_db_query("select p.products_id, pd.products_name, pd.products_description, p.products_model, p.products_price, p.products_tax_class_id from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd where p.products_status = '1' and p.products_id = '" . (int)$HTTP_GET_VARS['products_id'] . "' and pd.products_id = p.products_id and pd.language_id = '" . (int)$languages_id . "'");
    $mt_product_info = escs_db_fetch_array($mt_product_info_query);

    if ($mt_new_price = escs_get_products_special_price($mt_product_info['products_id'])) {
      $mt_products_price = $currencies->display_price($mt_product_info['products_price'], escs_get_tax_rate($mt_product_info['products_tax_class_id'])) . $currencies->display_price($mt_new_price, escs_get_tax_rate($mt_product_info['products_tax_class_id']));
    } else {
      $mt_products_price = $currencies->display_price($mt_product_info['products_price'], escs_get_tax_rate($mt_product_info['products_tax_class_id']));
    }

    if (escs_not_null($mt_product_info['products_model'])) {
  ...
  case CONTENT_PRODUCT_REVIEWS:
    $mt_review_query = escs_db_query("select p.products_id, pd.products_name, p.products_model, p.products_price, p.products_tax_class_id from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd where p.products_status = '1' and p.products_id = '" . (int)$HTTP_GET_VARS['products_id'] . "' and pd.products_id = p.products_id and pd.language_id = '" . (int)$languages_id . "'");
    $mt_review = escs_db_fetch_array($mt_review_query);

    if ($mt_new_price = escs_get_products_special_price($mt_review['products_id'])) {
      $mt_products_price = $currencies->display_price($mt_review['products_price'], escs_get_tax_rate($mt_review['products_tax_class_id'])) . $currencies->display_price($mt_new_price, escs_get_tax_rate($mt_review['products_tax_class_id']));
    } else {
      $mt_products_price = $currencies->display_price($mt_review['products_price'], escs_get_tax_rate($mt_review['products_tax_class_id']));
    }

    if (escs_not_null($mt_review['products_model'])) {
