...
// include the language translations
  ...
  if (!$HTTP_SESSION_VARS['affiliate_ref']) {
    escs_session_register('affiliate_ref');
    escs_session_register('affiliate_clickthroughs_id');
    if (($HTTP_GET_VARS['ref'] || $HTTP_POST_VARS['ref'])) {
      ...
      escs_db_perform(TABLE_AFFILIATE_CLICKTHROUGHS, $sql_data_array);
      $affiliate_clickthroughs_id = escs_db_insert_id();
   ...
   // Banner has been clicked, update stats:
      if ($affiliate_banner_id && $affiliate_ref) {
        ...
        $banner_stats_query = escs_db_query($sql);

     // Banner has been shown today
        if (escs_db_fetch_array($banner_stats_query)) {
        escs_db_query("update " . TABLE_AFFILIATE_BANNERS_HISTORY . " set affiliate_banners_clicks = affiliate_banners_clicks + 1 where affiliate_banners_id = '" . $affiliate_banner_id . "' and affiliate_banners_affiliate_id = '" . $affiliate_ref. "' and affiliate_banners_history_date = '" . $today . "'");
   // Initial entry if banner has not been shown
        ...
        escs_db_perform(TABLE_AFFILIATE_BANNERS_HISTORY, $sql_data_array);
      }
    }
