...
// fetch the net total of an order
  ...
  $affiliate_total = escs_round($affiliate_total, 2);

// Check for individual commission
  $affiliate_percentage = 0;
  if (AFFILATE_INDIVIDUAL_PERCENTAGE == 'true') {
    $affiliate_commission_query = escs_db_query ("select affiliate_commission_percent from " . TABLE_AFFILIATE . " where affiliate_id = '" . $affiliate_ref . "'");
    $affiliate_commission = escs_db_fetch_array($affiliate_commission_query);
  ...
  $affiliate_payment = escs_round(($affiliate_total * $affiliate_percent / 100), 2);
  ...
  if ($HTTP_SESSION_VARS['affiliate_ref']) {
    ...
    escs_db_perform(TABLE_AFFILIATE_SALES, $sql_data_array);

    if (AFFILATE_USE_TIER == 'true') {
      $affiliate_tiers_query = escs_db_query ("SELECT aa2.affiliate_id, (aa2.affiliate_rgt - aa2.affiliate_lft) as height
      ...
      $i=0;
      while ($affiliate_tiers_array = escs_db_fetch_array($affiliate_tiers_query)) {
