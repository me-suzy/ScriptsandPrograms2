...
if($act=="insert") {
  ...
  $fcu->query("select count(*) as cnt from OPRODTABLE where oprodsku='$sku'");
  $fcu->next_record();
  if ( $fcu->f('cnt') == 0 ) {
   $fco->query("insert into OPRODTABLE (oprodsku,ostart,oend,ozid) ".
  ...
  $fco->query("update OPRODTABLE ".
  "set oprodsku='$sku',ostart=$sdate,oend=$ndate where oprodsku='$sku'"); 

} elseif($act=="delete"){

  $fco->query("delete from OPRODTABLE where oprodsku='$sku'"); 
