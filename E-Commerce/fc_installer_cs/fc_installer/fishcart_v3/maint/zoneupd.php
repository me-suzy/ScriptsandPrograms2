...
$res=$fcz->query("update ZONETABLE set zonedescr='$zonedescr', ".
  ...
  $fcm->query("select numzone from MASTERTABLE");
  $fcm->next_record();
  $numz=(int)$fcm->f("numzone");
  $fcm->free_result();
  $numz+=1;
  $fcm->query("update MASTERTABLE set numzone=$numz");
  ...
  if( $databaseeng=='odbc' && $dialect=='solid' ){
   $res=$fcz->query("call zone_INSID ".
	"('$zonedescr','$zonecurrsym',$zoneact,$zflag1,0)"); 
  }else{
   $res=$fcz->query("insert into ZONETABLE ".
  ...
  $res=$fcz->query("delete from ZONETABLE where zoneid=$zoneid");

}
if(!$res){
	$fcz->rollback();
	echo "<b>Failure updating ZONETABLE: $res</b><br>\n";
}else{
	$fcz->commit();
