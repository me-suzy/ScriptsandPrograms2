...
<?php exit;}
 ...
 if ( $databaseeng == 'odbc' && $dialect == 'solid' ){
  $max_seq->query("select seq from AUXLINKTABLE where rownum < 2 order by seq desc");
 }elseif ( $databaseeng == 'pgsql' ){
  $max_seq->query("select seq from AUXLINKTABLE order by seq desc limit 1,0");
 }elseif ( $databaseeng == 'mssql' ){
  $max_seq->query("select top 1 seq from AUXLINKTABLE order by seq desc");
 }elseif ( $databaseeng == 'oracle' ){
  $max_seq->query("select seq from AUXLINKTABLE where rownum < 2 order by seq desc");
 }elseif ( $databaseeng == 'mysql' ){
  $max_seq->query("select seq from AUXLINKTABLE order by seq desc limit 0,1");
 }
 $max_seq->next_record();	
 $seq=$max_seq->f("seq")+1;
 $max_seq->free_result();

 $res = $fcc->query("insert into AUXLINKTABLE (".
 "seq,loc,title,url) values ($seq,$loc,'$title','$url')");
	
} elseif($act=="mod") {

  $res = $fcc->query("update AUXLINKTABLE ".
  "set loc=$loc,title='$title', url='$url' where rid=$link"); 

} elseif($act=="del"){

  $res = $fcc->query("delete from AUXLINKTABLE ".
  	"where rid=$link"); 
} elseif($act=="seq"){
  ...
  while ( $i < $scount ) {
   ...
   $res = $fcc->query(
	"update AUXLINKTABLE set seq=$seq where rid='$rid'");
   $i++;
  }
}
if(!$res){
	$fcc->rollback();
	echo "<b>failure updating AUXLINKTABLE: $res</b><br />\n";
}else{
	$fcc->commit();
