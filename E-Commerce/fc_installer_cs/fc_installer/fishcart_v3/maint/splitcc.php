...
if($act=="update"){
 ...
 while($i<$tsi){
  if($fetched[$i]){
   ...
   if( $databaseeng=='odbc' && $dialect=='solid' ){
    $fccc->query("update DBDNAME_ccnum set fetched='1' where tstamp=$tst");
   }else{
 ...
 if( $databaseeng=='odbc' && $dialect=='solid' ){
  $fccc->query(
   "delete from DBDNAME_ccnum where tstamp<$delstamp and fetched='1'");
 }else{
  $fccc->query(
...
if($showall){
 if( $databaseeng=='odbc' && $dialect=='solid' ){
  $fccc->query("select count(*) as cnt from DBDNAME_ccnum");
 }else{
 ...
 if( $databaseeng=='odbc' && $dialect=='solid' ){
  $fccc->query(
		"select count(*) as cnt from DBDNAME_ccnum where fetched='0'");
 }else{
  $fccc->query(
...
if($showall){
 if( $databaseeng=='odbc' && $dialect=='solid' ){
  $selstr="select userid,tstamp,fetched,orderid,cc6 from DBDNAME_ccnum ".
   "order by tstamp";
 }else{
...
if( $databaseeng=='odbc' && $dialect=='solid' ){
 $fccc->query("select count(*) as cnt from DBDNAME_ccnum where fetched='1'");
}else{
