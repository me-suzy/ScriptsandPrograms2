...
if( $numkeys ){
 if ( $databaseeng == 'odbc' && $dialect == 'solid' ){
  $numkeys++;
  $fckey->query("select distinct * from KEYTABLE where rownum < $numkeys ".
  				"order by keycnt,keyres desc");
 }elseif ( $databaseeng == 'pgsql' ){
  $numkeys++;
  $fckey->query("select distinct * from KEYTABLE order by keycnt,keyres desc limit $numkeys,1");
 }elseif ( $databaseeng == 'mssql' ){
  $fckey->query("select distinct top $numkeys * from KEYTABLE order by keycnt,keyres desc");
 }elseif ( $databaseeng == 'oracle' ){
  $fckey->query("select distinct * from KEYTABLE where rownum < $numkeys ".
  				"order by keycnt,keyres desc");
 }elseif ( $databaseeng == 'mysql' ){
  $fckey->query("select distinct * from KEYTABLE order by keycnt,keyres desc limit 0,$numkeys");
 }
}else{
 $fckey->query("select distinct * from KEYTABLE order by keycnt,keyres desc");
}
