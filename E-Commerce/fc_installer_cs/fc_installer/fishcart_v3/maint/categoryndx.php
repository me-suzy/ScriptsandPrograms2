...
$fcc->query("select count(*) as cnt from CATTABLE ".
...
$get_cats->query("select * from CATTABLE where catzid=$zoneid and catlid=$langid order by catpath");
	...
	$patharray = explode(":",$get_cats->f("catpath"));
		...
		{
			if ($val != ""){
				$get_scats->query("select catval,catdescr from CATTABLE where catzid=$zoneid and catlid=$langid and catval=$val order by catval asc");
