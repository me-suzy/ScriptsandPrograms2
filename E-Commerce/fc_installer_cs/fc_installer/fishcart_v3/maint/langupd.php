...
if($act=="update"){
   ...
   $fcta->query("select langfppromo from LANGTABLE where langid=$lngid and ".
   "langzid=$zoneid");
   if($fcta->next_record()){
  ...
  if( $databaseeng=="oracle" ){
	$res = $fcl->query("update LANGTABLE set ".
  ...
  $fcm->query('select numlang from MASTERTABLE');
  $fcm->next_record();
  $numl=(int)$fcm->f('numlang');
  $fcm->free_result();
  $numl+=1;
  $fcm->query("update MASTERTABLE set numlang=$numl");

  // oracle clob
  if( $databaseeng=="oracle" ){
   $res = $fcl->query("insert into LANGTABLE (langzid,langdescr,langtmpl,".
  ...
  $newlid=(int)$fcl->insert_id('langid','LANGTABLE',
  ...
  $fct->free_result();
  if( $zonedeflid == 0 ){
   $fct->query("update ZONETABLE set zonedeflid=$newlid where zoneid=$zoneid"); 
   $deflangiso = $langiso;
   $zonedeflid = $newlid;
  }else{
   $fct->query(
     "select langiso from LANGTABLE ".
  ...
  $fct->query(
    "select count(*) as cnt from COUNTRYTABLE ".
  ...
  if( $cnt == 0 ){
	...
	while( $fcl->next_record() ){
	 ...
	 if( $deflangiso ){
	  ...
	  $fcm->query("select ctryseq,ctryactive from COUNTRYTABLE ".
  ...
  $fct->query("select * from SHIPTABLE ".
  ...
  while( $fct->next_record() ){
    ...
    $new_shipid=(int)$fcm->insert_id('shipid','SHIPTABLE',
  ...
  $res=$fcl->query( "delete from LANGTABLE where langid=$lngid");
  $fcl->query(
    "delete from COUNTRYTABLE where ctryzid=$zoneid and ctrylid=$lngid");

}
if(!$res){
	$fcl->rollback();
	echo "<b>failure updating LANGTABLE: $res</b><br>\n";
}else{
	$fcl->commit();
...
//update cattable first set catact to 0   (bvo)
     $fctb->query("update CATTABLE set catact=0 where catval=$langfppromo");
     $fctb->commit();

//set the oldfppromo cat back to active if it existed
if($oldfppromo){
     $fctc->query("update CATTABLE set catact=1 where catval=$oldfppromo");
     $fctc->commit();
}
}
