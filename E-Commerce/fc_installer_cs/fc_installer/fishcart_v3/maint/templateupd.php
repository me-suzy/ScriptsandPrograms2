...
if ($act=="AT") {
 ...
 if($tfile){
  $fp=fopen("DOCROOT/DIRECTORY/files/$tfile","r");
  $ttxt=fread($fp,filesize("DOCROOT/DIRECTORY/files/$tfile"));
 ...
 if( $databaseeng=='odbc' && $dialect=='solid' ){
  $result=$fct->query(
  "call template_INSID ($zoneid,$langid,$tstamp,'$tname','$tdesc','$ttxt')");
 }else{
 ...
 if( $databaseeng=='odbc' && $dialect=='solid' ){
  $result=$fcn->query("call template_INSID ($zoneid,$langid,$tstamp,".
