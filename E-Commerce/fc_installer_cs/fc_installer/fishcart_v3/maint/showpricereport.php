...
$fcp = new FC_SQL;
$fcp->query("select zonedescr from ZONETABLE where zoneid='$zoneid' and zonedeflid='$langid'");
if($fcp->next_record()){
...
while($sku->next_record() ){
       ...
       $query1= "select prodinvqty,prodsetup,prodsku,prodsalebeg,prodsaleend,prodsaleprice,prodprice,prodrtlprice,prodstart,prodstop ".
        "from PRODTABLE,PRODLANG where prodsku='$loopsku' and (prodzid=$zoneid | prodlzid=$zoneid)";
...
$tmp="select prodsku from PRODTABLE where prodzid=$zoneid order by $order";
if($descend =='1'){
   $tmp .= " DESC";
   }
...
while($sku->next_record() ){
       ...
       $query1= "select prodinvqty,prodsku,prodsalebeg,prodsaleend,prodsaleprice,prodprice,prodrtlprice,prodstart,prodstop,prodsetup ".
        "from PRODTABLE where prodsku='$loopsku' and prodzid=$zoneid";
