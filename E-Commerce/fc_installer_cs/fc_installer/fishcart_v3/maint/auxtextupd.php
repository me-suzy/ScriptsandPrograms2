...
<?php exit;}
 ...
 if ( $databaseeng == 'odbc' && $dialect == 'solid' ){
  $max_seq->query("select seq from AUXTXTTABLE where rownum < 2 order by seq desc");
 }elseif ( $databaseeng == 'pgsql' ){
  $max_seq->query("select seq from AUXTXTTABLE order by seq desc limit 1,0");
 }elseif ( $databaseeng == 'mssql' ){
  $max_seq->query("select top 1 seq from AUXTXTTABLE order by seq desc");
 }elseif ( $databaseeng == 'oracle' ){
  $max_seq->query("select seq from AUXTXTTABLE where rownum < 2 order by seq desc");
 }elseif ( $databaseeng == 'mysql' ){
  $max_seq->query("select seq from AUXTXTTABLE order by seq desc limit 0,1");
 }
 $max_seq->next_record();	
 $seq=$max_seq->f("seq")+1;
 $max_seq->free_result();

 $res = $fcc->query("insert into AUXTXTTABLE (".
 "seq,loc,title,text) values ($seq,$loc,'$title','$text')");
	
} elseif($act=="mod") {

  $res = $fcc->query("update AUXTXTTABLE ".
  "set loc=$loc,title='$title', text='$text' where rid='$rid'"); 

} elseif($act=="del"){

  $res = $fcc->query("delete from AUXTXTTABLE ".
  	"where rid=$rid"); 
} elseif($act=="seq"){
  ...
  $rid=(int)getparam('link'.$i);
   $res = $fcc->query(
	"update AUXTXTTABLE set seq=$seq where rid='$rid'");
   $i++;
  }
}
if(!$res){
	$fcc->rollback();
	echo "<b>failure updating AUXTXTTABLE: $res</b><br />\n";
}else{
	$fcc->commit();
