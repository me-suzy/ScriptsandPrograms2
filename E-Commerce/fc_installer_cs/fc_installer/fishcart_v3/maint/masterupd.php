...
$fcm = new FC_SQL;
$fcm->Auto_commit = 0;
$fcm->query("select count(*) as cnt from MASTERTABLE");
$fcm->next_record();
if( $fcm->f("cnt") > 0 ){
 $fcm->free_result();
 $res=$fcm->query(
  "update MASTERTABLE set zoneid=$zoneid");
}else{
 $res=$fcm->query(
  "insert into MASTERTABLE (zoneid) values ($zoneid)");
}
if(!$res){
 $fcm->rollback();
 echo "<b>Failure updating MASTERTABLE: $res</b><br>\n";
}else{
 $fcm->commit();
