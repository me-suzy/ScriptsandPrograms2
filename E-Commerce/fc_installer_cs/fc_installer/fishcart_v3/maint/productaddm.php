...
<?php exit;}
$fcw->query("select webdesctmpl from WEBTABLE ".
...
<?php 
$fcz->query("select count(*) as zcnt from ZONETABLE"); 
$fcz->next_record();
$zcnt=(int)$fcz->f('zcnt');
$fcz->free_result();
if( $zcnt > 1 ){
...
$fcz->query("select zoneid,zonedescr from ZONETABLE order by zoneid"); 
...
<?php
 $fcl->query("select count(*) as cnt from LANGTABLE where langzid=$zid");
 $fcl->next_record();
 $lt=(int)$fcl->f('cnt');
 $fcl->free_result();
 $fcl->query("select langdescr,langid from LANGTABLE ".
...
<?php */ ?>
	...
	or absolute with respect to the top of the Web site (<b>/DIRECTORY/...</b>).</i>
	</p><p>
...
<?php
$ccount=0;
$fcc->query("select catval,catdescr from CATTABLE ".
	"where catzid=$zid"); 
