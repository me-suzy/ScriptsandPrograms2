...
<?php 
$fcs->query("select count(*) as cnt from SUBZONETABLE where subzid=$zoneid"); 
$fcs->next_record();
$st=(int)$fcs->f('cnt')+1;
$fcs->free_result();
?>
...
<?php 
$fcs->query(
	"select subzsid,subzdescr,subzvendid from SUBZONETABLE ".
	"where subzid=$zoneid order by subzsid"); 
while ( $fcs->next_record() ) {
    $vendor=$fcs->f('subzvendid');
    $fcz->query("select vendid,vendname from VENDORTABLE where vendid=$vendor");
    $fcz->next_record();
...
$fcs->query("select subzsid,subzdescr,subzvendid from SUBZONETABLE order by subzsid");
while ($fcs->next_record()) {
    $vendor=$fcs->f('subzvendid');
    $fcz->query("select vendid,vendname from VENDORTABLE where vendid=$vendor");
    $fcz->next_record();
