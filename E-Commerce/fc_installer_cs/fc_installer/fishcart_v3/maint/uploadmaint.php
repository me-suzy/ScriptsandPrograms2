...
<?php 
if(file_exists("DOCROOT/DIRECTORY/images")){
 $dp=opendir("DOCROOT/DIRECTORY/images");
 $fp=readdir();
 $i=0;
 while($fp){
	$fp=readdir();
	if (filetype("DOCROOT/DIRECTORY/images/$fp")=="file" &&
...
<?php 
if(file_exists("DOCROOT/DIRECTORY/ra")){
 unset($fnarray);
 $fnarray=array();
 $dp=opendir("DOCROOT/DIRECTORY/ra");
 $fp=readdir();
 $i=0;
 while($fp){
	$fp=readdir();
	if (filetype("DOCROOT/DIRECTORY/ra/$fp")=="file" &&
...
<?php 
if(file_exists("DOCROOT/DIRECTORY/files")){
 unset($fnarray);
 $fnarray=array();
 $dp=opendir("DOCROOT/DIRECTORY/files");
 $fp=readdir();
 $i=0;
 while($fp){
	$fp=readdir();
	if (filetype("DOCROOT/DIRECTORY/files/$fp")=="file") {
		$fnarray["i$i"]=$fp;
	}
	$i++;
 }
