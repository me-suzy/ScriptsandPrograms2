...
if ($act == 'insert') {
    $fcu->query("select count(*) as cnt from NPRODTABLE where nprodsku='$sku'");
    $fcu->next_record();
    if ( $fcu->f('cnt') == 0 ) {
        $fcn->query("insert into NPRODTABLE (nprodsku,nstart,nend,nzid) ".
            "values ('$sku',$sdate,$ndate,$zoneid)"); 
    }
    $fcu->free_result();

} elseif ($act == 'update') {
    $fcn->query("update NPRODTABLE ".
        "set nprodsku='$sku',nstart=$sdate,nend=$ndate where nprodsku='$oldsku'"); 

} elseif ($act == 'delete') {
  $fcn->query("delete from NPRODTABLE where nprodsku='$sku'"); 
}
$fcn->commit();
?>
