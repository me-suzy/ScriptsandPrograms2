...
<?php 
$fcs->query("select * from SUBZONETABLE ".
...
$get_subz = new FC_SQL;
$get_subz->query("select subzsid,subzdescr from SUBZONETABLE ".
...
<?php
$fcz -> query ("select vendname from VENDORTABLE where vendid=$subzvendid");
$fcz ->next_record();
echo '<option value="'.$subzvendid.'" selected="selected">'.$fcz->f('vendname').'</option>';
$fcv->query("select vendid,vendname from VENDORTABLE where vendzid=$zoneid and vendid!=$subzvendid"); 
...
<?php $fcsh->query(
	"select shipid,shipdescr from SHIPTABLE ".
...
while ( $fcsh->next_record() ) {
    $fcj->query("select shipdef from SUBZSHIPTABLE ".
...
$fcsh->query(
	...
	"where shipzid=$zoneid and shiplid=$langid"); 
while ( $fcsh->next_record() ) {
    $fcj->query("select count(*) as scnt from SUBZSHIPTABLE ".
		"where shipid=".$fcsh->f('shipid').
		" and shipszid=$subzsid and shiplid=$langid"); 
    $fcj->next_record();
    if ($fcj->f('scnt') > 0) {
