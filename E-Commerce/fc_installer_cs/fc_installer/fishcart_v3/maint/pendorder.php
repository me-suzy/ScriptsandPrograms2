...
$oh->query("select count(*) as cnt from ORDERHEAD");
$oh->next_record();
$cnt=$oh->f("cnt");
echo "Count:$cnt\n\n";
$oh->free_result();

$oh->query("select orderid,tstamp from ORDERHEAD");
$tprod=0;
while( $oh->next_record() ){
  ...
  $ol->query("select * from ORDERLINE where orderid='$oi'");
  $j=0;
  while( $ol->next_record() ){
    ...
    $op->query("select prodprice from PRODTABLE where prodsku='$sk'");
    if( $op->next_record() ){
