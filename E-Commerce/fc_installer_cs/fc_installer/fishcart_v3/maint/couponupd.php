...
if($act=="update"){
	...
	$res = $coupon->query("update COUPONTABLE set ".
  ...
  $res = $coupon->query("insert into COUPONTABLE ".
  ...
  $res=$coupon->query("delete from COUPONTABLE where cpnid='$cpnid'");

} // end update & insert queries 

if(!$res){
	$coupon->rollback();
	echo "<b>failure updating COUPONTABLE: $res</b><br />\n";
}else{
	$coupon->commit();
