...
$fcl->query("select langtmpl,langshow,langstmpl,langterms from LANGTABLE ".
	"where langid=$lid");
...
<?php
$fcesp->query("select pwoid,pwdescr from PASSWORDTABLE where pwuid='$pwuid'");
$fcesp->next_record();
...
$fcesd->query(
	...
	"esddlfile from ESDTABLE where esdoid='$pwoid'");
while( $fcesd->next_record() ){
	...
	$fcesp->query("select sku from ORDERLINE where orderid='$esdoid'");
	$fcesp->next_record();
	$sku=$fcesp->f('sku');
	$fcesp->free_result();
