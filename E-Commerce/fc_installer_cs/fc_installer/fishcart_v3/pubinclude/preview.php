...
$pre7->Auto_free=1;
$pre7->query(
 "select langgeo,langshow,langordr from LANGTABLE where langid=$lid");
// echo "select langgeo,langshow,langordr from LANGTABLE where langid=$lid";
$pre7->next_record();
...
$pre8->query("select * from ORDERLINE where orderid='$cartid'"); 
if($pre8->next_record()){
?>
...
do{ // already have the first record
 ...
 if(!$pre10->next_record()){
  $pre11->query("delete from ORDERLINE where orderid='$cartid'");
  continue;
 }
 ...
 $pre10->query(
	"select prodprice,prodsetup,prodweight,prodflag1 from PRODTABLE ".
