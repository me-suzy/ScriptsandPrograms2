...
$fcwnav->query(
 "select webviewlogo,webviewlogoh,webviewlogow,webnewlogo,".
         "webnewlogoh,webnewlogow  from WEBTABLE where ".
...
$fcnewprod->query("select count(*) as cnt from NPRODTABLE where nzid=0 or nzid=$zid");
...
$fccloseout->query("select count(*) as cnt from OPRODTABLE where ozid=$zid");
...
$fcaux->query("select title, url from AUXLINKTABLE where loc=1 order by seq");
...
// under='0' tells mysql to return the top level cats only
   ...
   "CATTABLE where catact=1 and catlid=$lid and ".
