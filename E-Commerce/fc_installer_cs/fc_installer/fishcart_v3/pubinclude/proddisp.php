...
do{ // already have the first record
 ...
 if(!$fcp->next_record()){
  $fcdel->query("delete from ORDERLINE where orderid='$cartid'");
  continue;
 }
 ...
 $fcp->query("select prodprice,prodsetup,prodsaleprice,prodsalebeg,".
	 ...
	 "prodweight,prodflag1 from PRODTABLE ".
...
<?php
  $fcmt = new FC_SQL;
  $fcmt->query("select SHIPTABLE.shipid,shipdef,shipdescr ".
 	"from SHIPTABLE,SUBZSHIPTABLE ".
	"where shipzid=$zid ".
	"and shipszid=$subz ".
