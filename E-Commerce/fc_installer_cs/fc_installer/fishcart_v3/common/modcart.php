...
$fch->query(
 "update ORDERHEAD set contrib=$c,shipid=$shipid,scity='$city',sstate='$state',".
...
while( $i < $itot ){
	...
	if( $tqty == 0 ){
	 $fch->query(
	  "delete from ORDERLINE where orderid='$cartid' and compsku='$tcsku'"); 
	 $fch->query(
	  "delete from ORDERLINEOPT where orderid='$cartid' and compsku='$tcsku'");
	}else{
		// see if there is sufficient inventory or if over max
		$fcp->query("select prodinvqty,produseinvq,prodordmax from PRODTABLE ".
			"where prodsku='$tsku'");
		$fcp->next_record();
		...
		if( $useq > 0 && $tqty > $inv ){
			$fch->query("update ORDERLINE set qty=$inv, invover=1 ".
				"where orderid='$cartid' and compsku='$tcsku'"); 
		}else{
			$fch->query("update ORDERLINE set qty=$tqty, invover=0 ".
				"where orderid='$cartid' and compsku='$tcsku'"); 
		}
	}
	$i++;
