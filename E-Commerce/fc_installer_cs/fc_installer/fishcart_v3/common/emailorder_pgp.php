...
if('KEYID'!=''){
 ...
 // $pname="/usr/bin/gpg --lock-never --batch -r KEYID -ea | mail -s [Order] ".$fcv->f("vendoemail");

 $vemail=EscapeShellCmd($fcv->f("vendoemail"));
 $pname='pgp -feat +batchmode KEYID | mail -s Order '.$vemail;
}else{
...
if( $zflag1 & $flag_zonesplitcc ){
 ...
 if ( $databaseeng == 'odbc' && $dialect == 'solid' ){
  ...
  $fccc->query(
  "insert into DBDNAME_ccnum (userid,tstamp,fetched,orderid,cc6) ".
  "values (DBDUID,$now,'0','$cartid','$cc_lastsix')");
 }else{
