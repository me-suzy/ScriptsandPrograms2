...
// fputs($cf,"ITEM         QTY      PRICE     PRODUCT\n");
fputs ($cf,fc_text('itemcapfix').fc_text('qtycapfix').fc_text('pricecapfix').fc_text('productcapfix')."\n");
...
$fco->query("select subz from ORDERHEAD where orderid='$cartid'");
$fco->next_record();
$subz=$fco->f("subz");
$fco->free_result();

if( !empty($subz) ){
 $fco->query("select subzdescr from SUBZONETABLE where subzsid=$subz");
 $fco->next_record();
...
$tqty=0;
$fco->query("select sku,compsku,qty,olid from ORDERLINE where orderid='$cartid'"); 
while($fco->next_record()){
 ...
 $fcp->query("select prodsdescr from PRODLANG ".
  "where prodlzid=$zid and prodlid=$lid and prodlsku='$sku'");
 $fcp->next_record();
 // strip any HTML tags
 $dsc=ereg_replace("<[^>]+>"," ",$fcp->f("prodsdescr"));
 $dsc=substr($dsc,0,35);
 $fcp->free_result();
 ...
 if( $sku != $csku ){
  fputs($cf,fc_text('basepricefix').sprintf("%s%.2f\n",$csym,$prodbaseprice));
 }
 while( get_prodopts($csku) ){
   fputs($cf,fc_text('optionfix').$poptname."\n");
   if($poptqty){
    fputs($cf,
...
fputs($cf,"\n");
if( $prodsettot ){
 fputs($cf,fc_text('setuptotalfix').sprintf("%8.2f\n",$prodsettot));
}
if( $zflag1 & $flag_zonecoupon ){
 if( $cpndisc > 0 ){
  fputs($cf,fc_text('discountfix').sprintf("%8.2f\n",$cpndisc));
 }
}
if($stotal){
fputs($cf,fc_text('subtotalfix').sprintf("%8.2f\n",$stotal);
fputs($cf,fc_text('shippingfix').sprintf("%8.2f   %s\n",$shamt,$shipdescr));
} // if stotal
...
if($staxn){
 if( $staxcmtn ){
  fputs($cf,sprintf("   %9s:     %8.2f\n",$staxcmtn,$staxn));
 }else{
  fputs($cf,fc_text('salestaxfix').sprintf("%8.2f\n",$staxn));
 }
}
if($staxs){
 if( $staxcmts ){
...
if($contamt>0){
fputs($cf,fc_text('contributefix').sprintf("%8.2f\n",$contamt));
}
...
fputs($cf,fc_text('ordertotalfix').sprintf("%8.2f\n",$ttotal));
}
...
if($mtotal){
fputs($cf,fc_text('psubtotalfix').sprintf("%8.2f\n",$mtotal);
}
...
if($pstaxn){
 if( $staxcmtn ){
  ...
  fputs($cf,fc_text('psalestaxfix').sprintf("%8.2f\n",$pstaxn));
 }
}
if($pstaxs){
 if( $staxcmts ){
...
if( empty($contrib_only) && empty($payment_only) && $ptotal ){
 fputs($cf,fc_text('ptotalfix').sprintf("%8.2f\n",$ptotal));
}
?>
