...
if( !isset($fc_USERID_pw) && !isset($PHP_AUTH_USER) ){
...
if(isset($PHP_AUTH_USER)) {
	...
	ereg("(.*)_(.*)",base64_decode($fc_USERID_pw),$cr);
	$pwuid=$cr[1];
	$pwpw=$cr[2];
}
...
$fcpw = new FC_SQL;
$fcpw->query("select * from PASSWORDTABLE ".
...
*/
 ...
 if( $pwexp && $pwexp < time() ){
  ...
  $fcpw->query(
  "update PASSWORDTABLE set $mnth=$accesses where pwuid='$pwuid'");
 }
}?>
