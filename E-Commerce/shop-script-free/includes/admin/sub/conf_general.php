 ...
 ****************************************************************************/
	...
	if (!strcmp($sub, "general"))
	{
		...
		if (isset($_POST["save_general"])) //save system settings
		{
			...
			fputs($f,"<?php\n\tdefine('CONF_SHOP_NAME', '".str_replace("'","\'",stripslashes($_POST["shop_name"]))."');\n");
			fputs($f,"\tdefine('CONF_SHOP_URL', '".str_replace("'","\'",stripslashes($_POST["shop_url"]))."');\n");
			fputs($f,"\tdefine('CONF_GENERAL_EMAIL', '".str_replace("'","\'",stripslashes($_POST["general_email"]))."');\n");
			fputs($f,"\tdefine('CONF_ORDERS_EMAIL', '".str_replace("'","\'",stripslashes($_POST["orders_email"]))."');\n");
			fputs($f,"\tdefine('CONF_PAYPAL_EMAIL', '".str_replace("'","\'",stripslashes($_POST["paypal_email"]))."');\n");
