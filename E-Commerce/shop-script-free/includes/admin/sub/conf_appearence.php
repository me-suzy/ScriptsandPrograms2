 ...
 ****************************************************************************/
	...
	if (!strcmp($sub, "appearence"))
	{
		...
		if (isset($_POST["save_appearence"])) //save system settings
		{
			...
			fputs($f,"\tdefine('CONF_COLUMNS_PER_PAGE', '".str_replace("'","\'",stripslashes($_POST["colscount"]))."');\n");
			fputs($f,"\tdefine('CONF_SHOW_ADD2CART', '".$_POST["add2cart"]."');\n");
			fputs($f,"\tdefine('CONF_SHOW_BEST_CHOICE', '".$_POST["bestchoice"]."');\n");
			fputs($f,"\tdefine('CONF_DARK_COLOR', '".str_replace("'","\'",stripslashes($_POST["darkcolor"]))."');\n");
			fputs($f,"\tdefine('CONF_MIDDLE_COLOR', '".str_replace("'","\'",stripslashes($_POST["middlecolor"]))."');\n");
			fputs($f,"\tdefine('CONF_LIGHT_COLOR', '".str_replace("'","\'",stripslashes($_POST["lightcolor"]))."');\n?>");
			fclose($f);
