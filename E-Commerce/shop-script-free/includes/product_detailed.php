 ...
 ****************************************************************************/
	...
	if (isset($_GET["vote"]) && isset($_GET["productID"])) //vote for a product
	{
		if (!isset($_SESSION["vote_completed"][ $_GET["productID"] ]) && isset($_GET["mark"]) && $_GET["mark"])
		{
			$q = db_query("UPDATE ".PRODUCTS_TABLE." SET customers_rating=(customers_rating*customer_votes+'".$_GET["mark"]."')/(customer_votes+1), customer_votes=customer_votes+1 WHERE productID='".$_GET["productID"]."'") or die (db_error());
		}
