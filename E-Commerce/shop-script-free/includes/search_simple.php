 ...
 ****************************************************************************/
	...
	if (isset($_GET["searchstring"])) //make a simple search
	{
		...
		if ($_GET["searchstring"])
		{
			...
			if (isset($_GET["inside"]) && isset($_GET["oldproducts"]))
				$s_search = stripslashes(base64_decode($_GET["oldproducts"]))." AND";
			else
				$s_search = "SELECT count(*) FROM ".PRODUCTS_TABLE." WHERE ";
			...
			$smarty->assign("old_search_query",base64_encode($s_search));
			...
			//navigation
			$search_navigator = "";
			showNavigator($g_search_count, $offset, CONF_PRODUCTS_PER_PAGE, "index.php?searchstring=".$_GET["searchstring"]."&oldproducts=".base64_encode($s_search)."&",$search_navigator);
