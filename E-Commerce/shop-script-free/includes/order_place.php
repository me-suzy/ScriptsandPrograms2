 ...
 ****************************************************************************/
	...
	else if (isset($_POST["complete_order"])) //place order
	{
		...
		if (isset($_SESSION["gids"]) && $c)
		{
			  ...
			  if ($_SESSION["gids"][$i])
			  {
				...
				if ($r = db_fetch_row($q))
				{
					...
					$tmp = array(
						...
						$r[0],
						$_SESSION["counts"][$i],
						show_price($_SESSION["counts"][$i]*$r[1]),
						$r[2]
					);
			...
			$smarty_mail->assign("order_shipping_address", $_POST["country"].", ".$_POST["zip"].", ".$_POST["state"].",  ".$_POST["city"].", ".$_POST["address"]);

			$_SESSION["order_id"] = $oid;
			$_SESSION["order_amount"] = $k;

			//send message to customer
			mail($_POST["email"], EMAIL_CUSTOMER_ORDER_NOTIFICATION_SUBJECT, $smarty_mail->fetch("order_notification.tpl.html"), "From: \"".CONF_SHOP_NAME."\"<".CONF_ORDERS_EMAIL.">\n".stripslashes(EMAIL_MESSAGE_PARAMETERS)."\nReturn-path: <".CONF_ORDERS_EMAIL.">");
			...
			mail(CONF_ORDERS_EMAIL, EMAIL_ADMIN_ORDER_NOTIFICATION_SUBJECT, $od.$adm, "From: \"".CONF_SHOP_NAME."\"<".CONF_ORDERS_EMAIL.">\n".stripslashes(EMAIL_MESSAGE_PARAMETERS)."\nReturn-path: <".CONF_ORDERS_EMAIL.">");
