...
require_once( $tsep_config["absPath"]."/include/mmexfunctions.php" ); // mm functions which were placed in every file
require_once( $tsep_config["absPath"]."/include/configfunctions.php" );
require_once( $tsep_config["absPath"]."/include/datefunctions.php" ); // to read and write las index edit date
require_once( $tsep_config["absPath"]."/include/rststatistics.php" ); // show statistics
require_once( $tsep_config["absPath"]."/include/rstnavigation.php" ); // show statistics
require_once( $tsep_config["absPath"]."/include/indexer.class.php" ); // for timestamp
require_once( $tsep_config["absPath"]."/include/contentimages.class.php" );
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "frmIndexEditNew"))
{
  ...
  $db_tablename = $db_table_prefix."search";
  ...
  mysql_select_db($database_tsepdbconnection, $tsepdbconnection);
  $Result1 = mysql_query($insertSQL, $tsepdbconnection) or die(mysql_error());
}
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "frmIndexEdit"))
{
  ...
  mysql_select_db($database_tsepdbconnection, $tsepdbconnection);
  $Result1 = mysql_query($updateSQL, $tsepdbconnection) or die(mysql_error());
}
...
if ((isset($_POST['id'])) && ($_POST['id'] != "") && (isset($_POST['delete'])))
{
  ...
  mysql_select_db($database_tsepdbconnection, $tsepdbconnection);
  $Result1 = mysql_query($deleteSQL, $tsepdbconnection) or die(mysql_error());
...
$startRow_completeindex = $pageNum_completeindex * $tsep_config['config_maxRows_completeindex'];
...
$query_limit_completeindex = sprintf("%s LIMIT %d, %d", $query_completeindex, $startRow_completeindex, $tsep_config['config_maxRows_completeindex']);
$completeindex = mysql_query($query_limit_completeindex, $tsepdbconnection) or die(mysql_error());
...
$totalPages_completeindex = ceil($totalRows_completeindex/$tsep_config['config_maxRows_completeindex'])-1;
?>
...
<div class="tsepProject">
        <?php
        require( $tsep_config["absPath"]."/include/indexer_search_table.php" ); //use code-recycling ?>
  <div class="indexeditHeadline"><?php echo $tsep_lng['index_edit_head']; ?></div>
  <?php require( $tsep_config["absPath"]."/include/indexstatus.php" );  // output indexing status ?>
        <?php
  ...
  <div class="indexeditOldBlock">
        ...
        if ($totalRows_completeindex>0)
        {
                ...
                {
                        //statistics start
                        RSTStatistics ($startRow_completeindex, $tsep_config['config_maxRows_completeindex'],   $totalRows_completeindex );
                        //statistics end
                        //navigation start
                        list (
                ...
                do
                {
                  ...
                  ?>
                        <div class="indexeditBlock"<?php require( $tsep_config["absPath"]."/include/colorswitch.php" );?>>
                          <form action="<?php echo $_SERVER["PHP_SELF"]; ?>" name="frmIndexEdit" id="frmIndexEdit" method="POST">
                                   ...
                                   <label for="page_contentimg">
									...
									if ( $myContentImgs->useContentImages() == false ) {
										...
										echo $lContentPITitle . ":";
	                                   	echo "</label>\n";
										echo "<div class='formfieldvalueSmall'>\n";
										echo "<img src='" . $myContentImgs->getContentImageURL() . "' " . $myContentImgs->getContentImageGeometry() . " alt='$lContentPITitle' title='$lContentPITitle'>\n";
										if ( $myContentImgs->getContentImageType() == "default") {
                                  ...
                                  <div class="indexeditEntryLine">
                                        <?php echo $tsep_lng['indexer_last_indexed']." ".( $row_completeindex['last_indexed'] > 0 ? date( $tsep_config['config_Date_Style'], $row_completeindex['last_indexed'] ) : $tsep_lng['indexer_not_indexed'] ); ?><br />
                                        <?php echo $tsep_lng['editindex_last_edited']." ".( $row_completeindex['last_edited'] > 0 ? date( $tsep_config['config_Date_Style'], $row_completeindex['last_edited'] ) : $tsep_lng['editindex_not_edited'] ); ?>
                                  </div>                                  
...
<?php
require( $tsep_config["absPath"]."/include/copyright.php" ); // Olaf Noehring: Use code-recyling whereever possible
?>
</div>
</body>
</html>
