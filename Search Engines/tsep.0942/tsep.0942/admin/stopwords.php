...
require_once( $tsep_config["absPath"]."/include/putsortpicture.php" );
/*
***************
...
$sql .= "  FROM ${db_table_prefix}search";
...
{
  $db_tablename = $db_table_prefix."stopwords";
...
$startRow_stopwords = $pageNum_stopwords * $tsep_config['config_maxRows_Stopwords'];
*/
//$tsep_config['config_maxRows_Stopwords'] = 10;
$pageNum_stopwords = 0;
if (isset($_GET['pageNum_stopwords'])) {
  $pageNum_stopwords = $_GET['pageNum_stopwords'];
}
...
$query_limit_stopwords = sprintf("%s LIMIT %d, %d", $query_stopwords, $startRow_stopwords, $tsep_config['config_maxRows_Stopwords']);
$stopwords = mysql_query($query_limit_stopwords, $tsepdbconnection) or die(mysql_error());
...
$totalPages_stopwords = ceil($totalRows_stopwords/$tsep_config['config_maxRows_Stopwords'])-1;
...
{
            ?>
            <div class="stopwordsOldBlock"<?php require( $tsep_config["absPath"]."/include/colorswitch.php" );?>>
...
<?php
//statistics start
RSTStatistics ($startRow_stopwords, $tsep_config['config_maxRows_Stopwords'], $totalRows_stopwords );
//statistics end
//navigation start
list (
        $queryString_stopwords,
...
$z=0;
	do { ?>
	<div class="stopwordsBlock"<?php require($tsep_config["absPath"]."/include/colorswitch.php"); ?>>
...
{
   global $tsepdbconnection,$db_table_prefix;
   $arrInsert = preg_split("/ *, */", $pStopwords, -1, PREG_SPLIT_NO_EMPTY);
   while ( list($key,$data) = each($arrInsert) ) {
      $sql = "SELECT count(*) as ct FROM ${db_table_prefix}stopwords WHERE stopword=" . GetSQLValueStringEx($data, "text");
      $result = mysql_query($sql, $tsepdbconnection) or die(mysql_error());
      $row = mysql_fetch_assoc($result);
      if ( $row["ct"] != 0 )
         continue;
      $sql = sprintf("INSERT INTO ${db_table_prefix}stopwords (stopword)" .
                     " VALUES (%s)",
                     GetSQLValueStringEx($data, "text"));
      $Result1 = mysql_query($sql, $tsepdbconnection) or die(mysql_error());
   }
} // StopwordsInsert()
...
function extractnewmax($counter, $len)
{
	...
	for($x=0; $x<$len; $x++)
	{
		...
		foreach($counter as $key=>$value)
		{
			...
			if($value > $max)
			{
				$sql = "SELECT count(*) as ct FROM ${db_table_prefix}stopwords WHERE stopword=" . GetSQLValueStringEx($key, "text");
...
function createArray($counter)
{
    ...
    $x=0;
	$sql = "SELECT * FROM ${db_table_prefix}stopwords ";
	$all_stopwords = mysql_query($sql, $tsepdbconnection) or die(mysql_error());
