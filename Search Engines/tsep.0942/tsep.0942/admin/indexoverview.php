...
require_once( $tsep_config["absPath"]."/include/putsortpicture.php" ); // show sortorder pictures

$pageNum_completeindex = 0;
if (isset($_GET['pageNum_completeindex'])) {
  $pageNum_completeindex = $_GET['pageNum_completeindex'];
}
$startRow_completeindex = $pageNum_completeindex * $tsep_config['config_maxRows_indexoverview'];
...
// === START ==== write / get search order and field from database ============================
	...
	{	// write new values
		$db_tablename = $db_table_prefix."internal";	
		 $query = "UPDATE $db_tablename SET stringvalue='".$_GET['order']."' WHERE description='tsepindexovervieworder'";
		 $result = mysql_query($query, $tsepdbconnection) or die(mysql_error());
	...
	$Recordset1 = mysql_query($query_Recordset1, $tsepdbconnection) or die(mysql_error());
...
$query_limit_completeindex = sprintf("%s LIMIT %d, %d", $query_completeindex, $startRow_completeindex, $tsep_config['config_maxRows_indexoverview']);
...
$totalPages_completeindex = ceil($totalRows_completeindex/$tsep_config['config_maxRows_indexoverview'])-1;
?>
...
<div class="tsepProject">
  ...
  <div class="indexeditOldBlock">
    <?php
        //statistics start
        RSTStatistics ($startRow_completeindex, $tsep_config['config_maxRows_indexoverview'],   $totalRows_completeindex );
        //statistics end
        //navigation start
        list (
    ...
    <?php do { ?>
    <div class="indexoverviewBlock"<?php require( $tsep_config["absPath"]."/include/colorswitch.php" );?>>
