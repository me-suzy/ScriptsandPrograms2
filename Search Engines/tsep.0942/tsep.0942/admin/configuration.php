...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "tsepconfig"))
{
        ...
        require( $tsep_config["absPath"]."/include/config.php" );
        include( $tsep_config["absPath"]."/language/en_US/language.php" );
        include( $tsep_config["absPath"]."/language/".$tsep_config['config_Language']."/language.php" );
}
...
$howmanyresultsperpage = mysql_query($query_howmanyresultsperpage, $tsepdbconnection) or die(mysql_error());
...
?>
  ...
  <div class="maxResultsBlock">
        ...
        <?php do { ?>
                <div class="configurationBlock"<?php require( $tsep_config["absPath"]."/include/colorswitch.php" );?>>
...
function MaxResultsInsert($maxresults)
{
   ...
   while ( list($key,$data) = each($arrInsert) ) {
      $sql = "SELECT count(*) as ct FROM ${db_table_prefix}internal WHERE stringtag='possible_results' AND numericvalue=" . GetSQLValueStringEx($data, "long");
      $result = mysql_query($sql, $tsepdbconnection) or die(mysql_error());
      $row = mysql_fetch_assoc($result);
      if ( $row["ct"] != 0 )
         continue;

      $sql = sprintf("INSERT INTO ${db_table_prefix}internal (description, numericvalue, valuetype, fieldtype, stringtag)" .
