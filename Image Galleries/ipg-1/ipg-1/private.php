...
if($_SESSION['user_id']){
	$theCatId = $_SESSION['authorized_portfolio_id'];
    $catName = catId2CatName($_SESSION['authorized_portfolio_id']);
}

if($_SESSION['admin']){
	$theCatId = $_GET['cat_id'];
...
$sql = "SELECT i.image, i.public_view FROM " . PDB_PREFIX . "images i,
		" . PDB_PREFIX . "images_to_categories i2c  WHERE i.id = i2c.image_id AND i2c.cat_id = " . $theCatId . "
...
$sql = "SELECT * FROM " . PDB_PREFIX . "user_text ut WHERE content_cat = " . $theCatId;
...
<?php print '<div class="portfolio_table">' . display_portfolio($theCatId, $icl, $show_restricted, MAX_PORTFOLIO_THUMBNAIL_COLUMNS,THUMBNAIL_INFO_POS) . '</div>'; ?>
<?php
