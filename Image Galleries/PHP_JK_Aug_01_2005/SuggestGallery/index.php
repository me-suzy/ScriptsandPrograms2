<?php
	...
	Function Main()
	{
		...
		{
			Echo "<br>";
			G_STRUCTURE_HeaderBar("SuggestANewGallery.gif", "Suggest a new gallery", "", "Galleries");

			$sEmail = DOMAIN_Conf("IMAGEGALLERY_SUGGESTGALLERY_TO_EMAIL");
			// Send the email*********
			$sFullLetter = $CONF_SuggestGal;
			$sFullLetter = str_replace("1:", Request("sGalleryName"), $sFullLetter);
			$sFullLetter = str_replace("2:", Request("sDescription"), $sFullLetter);
			$sFullLetter = str_replace("3:", $_SERVER["SERVER_NAME"], $sFullLetter);

			$sEmailResponse = DOMAIN_Send_EMail($sFullLetter, DOMAIN_Conf("IMAGEGALLERY_SUGGESTGALLERY_FROM_NAME"), DOMAIN_Conf("IMAGEGALLERY_SUGGESTGALLERY_FROM_EMAIL"), "Administrator", $sEmail, $_SERVER["SERVER_NAME"] . " Image Gallery Suggestion", FALSE);
	...
	//************************************************************************************
	//*																					*
	//*	This allows the users to suggest a new gallery									*
	//*																					*
	//************************************************************************************
	Function WriteForm()
	{
		...
		<?php G_STRUCTURE_HeaderBar("SuggestANewGallery.gif", "Suggest a new gallery", "", "Galleries");?>
			...
			<table width=<?=$iTableWidth?> cellpadding = 10 cellspacing = 0 border = 0 class='TablePage_Boxed'>
				<tr>
					<td valign=top>
						Please enter the gallery name to add:
						<br>
						<input type = "text" name = "sGalleryName" size=<?=$iFormWidth?> maxlength=255>
						<br>
						Please describe the gallery and why you think it should be added:
						<br>
						<TEXTAREA COLS=<?=$iFormColumns?> ROWS=10 WRAP="soft" NAME="sDescription"></TEXTAREA>
					</td>
				</tr>
			</table>
			<input type='image' src="<?=G_STRUCTURE_DI("SubmitSuggestion.gif", $GLOBALS["SCHEMEBASED"])?>" style="BORDER: none; vertical-align: sub;">
		</form>
		<?php 
	}
	//************************************************************************************
