<?php
	...
	Function Main()
	{
		...
		If ( $iGalleryUnq != "" )
		{
			If ( G_ADMINISTRATION_HasGallerySubscription($iLoginAccountUnq, $iGalleryUnq) ) {
				$sMessage = "You are already subscribed to this gallery.";
			}Else{
				DB_Insert ("INSERT INTO IG_Subscriptions VALUES (-1, " . $iGalleryUnq . ", " . $iLoginAccountUnq . ", 'N')");
				$sMessage = "You are now subscribed to this gallery and will receive an email when new images are added to this gallery.<br><br>To unsubscribe, return to the gallery page and click the \"Unsubscribe Gallery\" button.";
			}
		}Else{
			$sMessage = "Unable to subscribe to the gallery -- missing GalleryUnq.";
		}
		WriteForm($sMessage);
	}
	//************************************************************************************
	...
	Function WriteForm($sMessage)
	{
		...
		Echo "<BR>";
		G_STRUCTURE_HeaderBar("SubscribeGalHead.gif", "", "", "Galleries");
		?>
