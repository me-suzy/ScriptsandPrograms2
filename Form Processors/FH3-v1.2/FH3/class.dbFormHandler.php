...
class dbFormHandler extends FormHandler
{
      ...
      $mergeArray = null)
    {
        ...
        // create new selectfield
        $fld =& new dbListField( $this, $name, $this->_db, $table, $fields, $extraSQL, $mergeArray );
    ...
    function value( $field, $errorOnUnknownField = true )
    {
        ...
        if( isset( $this->_db ) && is_object( $this->_db ) && !empty( $this->_table ))
        {
            ...
            if( in_array( $field, $fields ) )
            {
                ...
                else
                {
                    ...
                    $value = parent::value( $field, false );
        ...
        return parent::value( $field, $errorOnUnknownField );
    }
    ...
    function dbConnect($host = null, $username = '', $password = '')
    {
        ...
        if( is_object( $this->_db ) )
        {
            ...
            if( !$this->_db->connect( $host, $username, $password ) )
            {
                // connection failed..
            	trigger_error(
            	  'Error, database connecting failed: '.$this->_db->getError(),
            	  E_USER_WARNING
            	);
	...
	function flush( $return = false )
	{
        ...
        else
        {
            ...
            if( !is_null($this->_db) && !empty( $this->_table) )
            {
            ...
            if(sizeof( $unique ) > 0)
            {
                ...
                foreach( $unique as $index => $fields)
                {
                    ...
                    foreach( $fields as $field )
                    {
                        ...
                        else if( array_key_exists($field, $this->_add) )
                        {
                            // get hte value
                        ...
                        // added by the user. Get an empty value or NULL when possible
                        else
                        {
                            // field does not exists in the form, take a null value if
                            // possible, or else a empty string value
                            $value = in_array( $field, $notnull ) ? "''" : 'NULL';
                        }

                        $extra[] = $field . ($value == 'NULL' ? ' IS NULL ' : ' = '.$value );
                    }
    ...
    function _saveDbData( $data )
    {
        ...
        foreach( $data as $field => $value )
        {
            ...
            if( !in_array($field, $this->_dontSave) )
            {
     ...
     * @author Teye Heimans
     */
    function _getWhereClause( $operator = '=', $extra = array() )
    {
        ...
        // add the extra where data to the clause data
