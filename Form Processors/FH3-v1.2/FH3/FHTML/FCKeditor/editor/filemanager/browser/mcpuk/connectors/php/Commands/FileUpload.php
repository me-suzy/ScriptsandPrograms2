...
class FileUpload {
	...
	function FileUpload($fckphp_config,$type,$cwd) {
		...
		$this->actual_cwd=str_replace("//","/",($this->fckphp_config['UserFilesPath']."/".$this->raw_cwd));
				...
				if ($_FILES['NewFile']['size']<($typeconfig['MaxSize']*1024)) {
					...
					if ($lastdot!==false) {
						...
						if (in_array(strtolower($ext),$typeconfig['AllowedExtensions'])) {
							...
							$failSizeCheck=false;
							/*
							// This code checks the size of all the resource type dirs (like /File, /Image)
							// The FormHandler version does not use this.
								...
								if (file_exists($this->real_cwd."/$filename.$ext")) {
									...
									if (is_uploaded_file($_FILES['NewFile']['tmp_name'])) {
										if (move_uploaded_file($_FILES['NewFile']['tmp_name'],($this->real_cwd."/$filename.$ext"))) {
											...
											$disp="202,'Failed to upload file, internal error...: ".$this->real_cwd."/$filename.$ext'";
										}
									} else {
