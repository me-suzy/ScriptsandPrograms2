...
// This is by default not used but can be handy for the users
...
class FormHandler
{
      ...
      $extra     = null)
    {
        ...
        $fld =& new DateField( $this, $name, $display );

        if(!empty($validator))  $fld->setValidator( $validator );
        if(!is_null($required)) $fld->setRequired( $required );
        if(!empty($interval))   $fld->setInterval( $interval );
        if(!empty($extra))      $fld->setExtra( $extra );
      ...
      $extra     = null)
    {
        ...
        // create a new datefield
        $fld =& new jsDateField( $this, $name, $display );
    ...
    function setMaxLength( $field, $maxlength, $displaymessage = true )
    {
        ...
        $this->_fields[$field][1]->setExtraAfter(
          "<br /><script language='javascript' type='text/javascript'>\n".
          "displaylimit('".$this->_name."','".$field."','".$maxlength."',0".$displaymessage.",'".$this->_text(36) ."');\n".
          "</script>\n"
        );
    }
    ...
    function setHelpText( $field, $helpText, $helpTitle = null )
    {
    	...
    	$helpTitle = is_null($helpTitle) ? "%title% - Help" : htmlentities( $helpTitle );
	 ...
	 *
	 * @param boolean $enabled: The new state of the Forms View Mode
	 * @return void
	 */
	function enableViewMode($enable = true)
	{
	    $this->_viewMode = (bool) $enable;
	}
	  ...
	  $extra       = '')
	{
	    ...
	    if( !is_integer($cellspacing)) $cellspacing = FH_DEFAULT_TABLE_CELLSPACING;
	    if( !is_integer($cellpadding)) $cellpadding = FH_DEFAULT_TABLE_CELLPADDING;
	    if( !is_integer($border))      $border      = FH_DEFAULT_TABLE_BORDER;
     ...
     * @param boolean $errorOnUnknownField: Trigger error when the field could not be found ?
     * @return string
     * @access public
     * @author Teye Heimans
     */
    function getValue( $field, $errorOnUnknownField = true )
    {
    	return $this->value( $field, $errorOnUnknownField );
    }
     ...
     * @author Teye Heimans
     */
    function value( $field, $errorOnUnknownField = true )
    {
    ...
    function linkSelectFields( $filename, $fields )
    {
        ...
        for( $i = 1; $i < (func_num_args()-1); $i++)
        {
            ...
            $param = $this->edit ? '&value=' . $values[count($values)-1] : '';
            $param .= $extra;

            $js.=
            'function attach'.$fld1.'( argArray ) {'."\n".
			'    attachelement("'.$fld1.'", "change", load'.$fld2.');'."\n".
            '    load'.$fld2.'( argArray );'."\n".
            '}'."\n\n".
			'function load'.$fld2.'( args ) {'."\n".
        ...
        if( !empty( $this->_dynamicLoader ) )
        {
            $this->_dynamicLoader .= " new Array(  ";
            foreach( $values as $value )
            {
                if( is_array( $value ) )
                {
                    ...
                    {
                        $this->_dynamicLoader .= "'".addslashes($item)."', ";
                    }
                    $this->_dynamicLoader = substr( $this->_dynamicLoader, 0, -2) . "), ";
                }
                else
                {
                    $this->_dynamicLoader .= "'".addslashes($value)."', ";
                }
            }
     ...
     *
     * @param array $array: the new options for the select field
     * @return null
     * @access public
     * @author Teye Heimans
     */
    function setDynamicOptions( $array, $useArrayKeyAsValue = true )
    {
    ...
    function getJavascriptCode()
    {
    	...
    	if( !$return )
    	{
    		...
    		$result = '';
    		if( isset($this->_js['file']) && is_array($this->_js['file']) )
    		{
	    		foreach( $this->_js['file'] as $line )
	    		{
	    			$result .= '<script language="javascript" type="text/javascript" src="'.$line.'"></script>'."\n";
	    		}
    		}
    		// generate the other js script
    		if( isset($this->_js['code']) && is_array($this->_js['code']) )
    		{
    			$result .= '<script language="javascript" type="text/javascript">'."\n";
    			foreach( $this->_js['code'] as $code )
    			{
    				$result .= $code;
    			}
    			$result .= "</script>\n";
    ...
    function flush( $return = false )
    {
        ...
        else
        {
            ...
            {
                // is the oncorrect function a method ?
        ...
        foreach( $vars as $name => $value )
        {
        	...
        	if( !in_array($name, array('_cache', 'edit', 'insert', '_posted' ) ) )
        	{
        		unset( $this->{$name} );
        	}
        }
     ...
     * @author Teye Heimans
     */
    function _setJS( $js, $isFile = false)
    {
        $this->_js[$isFile?'file':'code'][] = $js;
    }
    ...
    function _getForm( $iDisplayPage = null )
    {
            ...
            switch( $field[0] )
            {
                ...
                default:
                  ...
                  else
                  {
                	  ...
                	  $fld = '';
                	  if( $this->_viewMode && method_exists($obj, 'getViewValue') )
                	  {
                	      $fld = $obj->getViewValue();
                	  }
                	  // not view mode...
                	  else
                	  {
                	   ...
                	   */
                	  $help = '';
                	  if( array_key_exists( $name, $this->_help) && (!isset($this->_viewMode) || !$this->_viewMode) )
                	  {
                	      $help = sprintf(
                	        FH_HELP_MASK,
                	        $this->_helpIcon,
        ...
        (FH_EXPOSE ?
          ...
          "This form is generated by </font></small><a href='http://www.formhandler.net' target='_blank'><small>".
          "<font color='B5B5B5' face='tahoma' style='font-family:Tahoma;font-size:10px;color:B5B5B5;font-weight:normal;'><b>FormHandler</b></font></small></a>" :''
        ).
        "</form>\n".
        "<!--\n".
        "  This form is automaticly being generated by FormHandler v3.\n".
        ...
        // load the first dynamic selectfield if this is used
        if( isset( $this->_dynamicLoader ) )
        {
        	$js .= $this->_dynamicLoader;
        }
        ...
        // add the javascript needed for this form
        if( !empty($js) )
        {
        	$result .=
