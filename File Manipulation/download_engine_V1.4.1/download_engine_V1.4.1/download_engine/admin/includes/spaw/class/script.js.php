  ...
  function SPAW_hyperlink_click(editor, sender)
  {
    window.frames[editor+'_rEdit'].focus();     
	  var x = 0;
	  var links=showModalDialog("<?php echo $spaw_dir ?>" + "dialogs/insert_link.php", '', "dialogWidth:430px; dialogHeight: 135px; resizable: no; help: no; status: no; scroll: no; ");
	if (links != 'none')
    {
	  var tControl = window.frames[editor+'_rEdit'].document.selection.createRange();
      var saved = tControl.text;
      tControl.pasteHTML(links+saved);
	}
  }  

  function SPAW_hyperlink_click2(editor, sender)
  {
    window.frames[editor+'_rEdit'].focus();     
