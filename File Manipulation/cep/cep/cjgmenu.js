...
function menuhtml(sm,id) { var i,s,o,starget,shref,sfolder,sfile,splus,sjoin,stomenu,sonclick;
...
if(o[VHREF].substr(0,11).toLowerCase()=='javascript:') { starget=''; shref=''; sonclick=o[VHREF].substr(11); }
if(shownodelines)  splus='<IMG src="'+menuimg(IPLUS,o[VPOS])+'" '+imgattrs+'>'; else splus='';
sjoin='<IMG src="'+menuimg(IJOIN,o[VPOS])+'" '+imgattrs+'>';
if(o[VMENU]!='') {
s+="<TR><TD><A class=pm onclick='menuanchor(this,\""+sm+"\","+i+",true);'>"+splus+"</A></TD><TD nowrap>"+sfolder+"<A class=lb id=t"+id+"_"+i+shref+starget+" onclick='menusel(this);menuanchor(this,\""+sm+"\","+i+",false);"+sonclick+"'>"+o[VLABEL]+"</A></TD></TR>\n";
} else {
if(showfileicon) sfile='<IMG align=absmiddle src="'+menuimg(IFILE,o[VICON])+'" '+imgattrs+'>'; else sfile='';
s+="<TR><TD>"+sjoin+"</TD><TD nowrap>"+sfile+"<A class=lb id=t"+id+"_"+i+shref+starget+" onclick='menusel(this);"+sonclick+"'>"+o[VLABEL]+"</A></TD></TR>\n";  
} }
s+="</TABLE>\n"; return(s); }

function menuanchor(este,sm,opt,op) { if(mo_lock) return;
var m=eval(sm); 
if(m[opt][VOPEN]) { if(op) menuclose(este,sm,opt); } else menuopen(este,sm,opt); }
...
if(m[opt][VMENU][0]=='%%') {
mo_lock=1; mo_este=este; mo_sm=sm; mo_opt=opt; mo_m=m;
if(showfoldericon) {
var x1=este.parentElement.parentElement.children.tags('TD')[1];
x1.children.tags('IMG')[0].src=menuimg(IFOLDER,IWAIT); }
dyn.src='cjgdyn.php?dy='+escape(m[opt][VMENU][1]); 
return;	}
...
m0.backgroundColor=document.bgColor; // force property change 
}
...
if(o[VMENU]!="") { if(o[VMENU][0]!='%%') { o[VNAME]=""; o[VOPEN]=0; o[VHEIGHT]=0;
...
setclass(selback,'lb');
setclass(menurec,'lb');
selback=menurec=t.id; setclass(t.id,'sel'); }
...
case IFOLDER: switch(s) {
	      ...
	      case IOPEN: img=imgfolderopen; break;
	      case IWAIT: img=imgfolderwait; break; } break;
case IPLUS: switch(s) {	      
...
function menuexpandb(r,i) { return(menuclick(r,i,false,true,true)); }

function menuexpandc(r,i) { return(menuclick(r,i,1,false,true)); }

function menuexpand(r,i) { return(menuclick(r,i,false,false,false)); }

function menucollapse(r,i) { return(menuclick(r,i,true,false,false)); }

function menuclick(r,n,oc,na,ho) { var m,qt,mh,o,i;
...
if(o[VMENU]!='') { if(!o[VOPEN] || !oc) qt[i].children.tags('TD')[na?1:0].children.tags('A')[0].click(); }
else { if(ho) qt[i].children.tags('TD')[1].children.tags('A')[0].click(); }
return(o); }	
...
if(heavy) { meto[nmeto]=o; setTimeout("menuexpandall(meto["+nmeto+"],1)",100); nmeto++; }
else { menuexpandall(o); } } } }
...
function menuhere() { document.write(menuhtml()); document.close(); }

var menurec='';
function keygo(arriba) { var aes,sb,i,nsb;
aes=document.body.innerHTML.match(/id=t[0-9]*_[0-9]*/g);
sb=(menurec=='')?selback:menurec;
for(i=0;i<aes.length;i++) if(aes[i]=='id='+sb) break; if(i==aes.length) return;
if(arriba) { i--; if(i<0) i=aes.length-1; } else { i++; if(i==aes.length) i=0; } nsb=aes[i].substr(3);
if(sb!=selback) { setclass(sb,'lb'); }
if(nsb!=selback) setclass(nsb,'rec'); menurec=nsb;
delete aes;
var h,y0,y1,m0,m1;
h=eval(nsb).offsetHeight;
y0=h*i; y1=y0+h;
m0=document.body.scrollTop;
m1=document.body.scrollTop+document.body.clientHeight;
if(y1>m1) window.scrollBy(0,y1-m1); else if(y0<m0) window.scrollBy(0,y0-m0); }

function setclass(id,clase) {
if(document.all[id]) document.all[id].className=clase; }

function keyenter() { var sb;
sb=(menurec=='')?selback:menurec;
eval(sb).onclick(); }

function keyplusminus() { var sb,a,tr;
sb=(menurec=='')?selback:menurec;
tr=eval(sb).parentElement.parentElement;
tr.children.tags('TD')[0].children.tags('A')[0].click(); }
