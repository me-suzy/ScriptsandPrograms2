<?php 

// skin_login.php
// Skin file auto generated by RW::Download v4.0 Beta 3 [14/05/04]
// It is highly recommended you do NOT edit this file
// Doing so could break the skinning engine and the script
// This skin can be edited by using the Skin Controls in the Admin CP

class skin_stats
{
function topdownloads($data)
{
global $rwdInfo;
//--START--//
$SHTML = <<<RWS
- <a href='index.php?dlid={$data['did']}'>{$data['name']}</a> ({$data['downloads']})<br>
RWS;
//--END--//
return $SHTML;
}

function topRatedDownloads($data)
{
global $rwdInfo;
//--START--//
$SHTML = <<<RWS
- <a href='index.php?dlid={$data['did']}'>{$data['name']}</a> ({$data['userrating']})<br>
RWS;
//--END--//
return $SHTML;
}

function latestDownloads($data)
{
global $rwdInfo;
//--START--//
$SHTML = <<<RWS
- <a href='index.php?dlid={$data['did']}'>{$data['name']}</a><br>
RWS;
//--END--//
return $SHTML;
}

}

?>
