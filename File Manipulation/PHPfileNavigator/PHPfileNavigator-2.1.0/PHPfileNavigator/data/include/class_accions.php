...
class PFN_Accions extends PFN_Niveles {
	...
	*/
	function eliminar ($cal) {
		...
		if ($this->e_dir($cal)) {
			...
			return $this->estado['eliminar_dir'];
		} else {
			...
			return $this->estado['eliminar_arq'];
		}
	}
	...
	*/
	function enlace ($orixe, $destino) {
		...
		if ($this->e_dir($orixe)) {
			...
			return $this->estado['enlazar_arq'];
		}
	}
	...
	*/
	function editar ($cal, $texto) {
		...
		$texto = (get_magic_quotes_gpc() == 1)?stripslashes($texto):$texto;
	...
	*/
	...
	/**
	* function log_ancho_banda (integer $tamano)
	*
	...
	* return boolean
	*/
	function log_ancho_banda ($tamano) {
