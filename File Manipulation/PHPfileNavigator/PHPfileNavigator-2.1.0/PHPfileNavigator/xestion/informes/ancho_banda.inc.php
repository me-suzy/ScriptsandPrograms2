...
for (; $usuarios->mais(); $usuarios->seguinte()) {
	...
	if (is_file($paths['info'].'usuario'.$usuarios->get('id').'/descargas.'.$ab_ano.$ab_mes.'.php')) {
		$actual = include ($paths['info'].'usuario'.$usuarios->get('id').'/descargas.'.$ab_ano.$ab_mes.'.php');
