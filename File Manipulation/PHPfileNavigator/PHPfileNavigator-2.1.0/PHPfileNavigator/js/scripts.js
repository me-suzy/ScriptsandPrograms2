...
function marca_ancho_corpo (directo) {
	...
	max_ancho = ancho_fiestra();

	if (directo) {
		cambiado = (obx_corpo.style.width == max_ancho);

		obx_corpo.style.width = cambiado?(isIE?'760px':'740px'):max_ancho;
		obx_pe.style.width = obx_corpo.style.width;
	} else {
	  ancho_corpo = PFN_readCookie('PFN_anchoCorpo');

		if (ancho_corpo == 'max') {
			obx_corpo.style.width = max_ancho;
			obx_pe.style.width = max_ancho;
		} else {
			obx_corpo.style.width = (isIE?'760px':'740px');
			obx_pe.style.width = obx_corpo.style.width;
		}
	}
...
function garda_ancho_corpo () {
	var obx_corpo = MM_findObj('corpo');
	var max_ancho = ancho_fiestra();

	estado = (obx_corpo.style.width == max_ancho)?'max':'';
  PFN_createCookie('PFN_anchoCorpo', estado, 365);
}
