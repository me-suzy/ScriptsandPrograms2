<?php
	class HeldTicket extends Ticket
	{
		var $query;
		
		function HeldTicket($id = false, $regUser = false)
		{
			$this->id = $id;
			if ($id) {
		...
		{
			if ($this->regUser) {
				$this->query = "select * from " . DB_PREFIX . "data where ticketVisi = 0 and ID = $this->id and regUser = $this->regUser";
			}
			else {
				$this->query = "select * from " . DB_PREFIX . "data where ticketVisi = 0 and ID = $this->id";
			}
		}
