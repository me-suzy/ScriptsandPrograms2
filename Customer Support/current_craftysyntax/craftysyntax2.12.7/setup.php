...
if(\$installed==true){
 ...
 ?>";
  ...
  $fcontents = ereg_replace("INPUT-PASSWORD",str_replace("\$","\\\$",$UNTRUSTED['mypassword_setup']),$fcontents);
  ...
  $insert_query = "INSERT INTO livehelp_config (version, site_title, use_flush, membernum, offset, show_typing,webpath,s_webpath,speaklanguage,maxexe,refreshrate,chatmode,adminsession,maxreferers,maxvisits,maxmonths,maxoldhits,maxrecords,admin_refresh) VALUES ('$version', 'Live Help!', 'YES', 0, 0, 'Y','".filter_sql($homepage)."','".filter_sql($s_homepage)."','".filter_sql($UNTRUSTED['speaklanguage'])."',300,1,'xmlhttp-flush-refresh','N',50,75,12,1,75000,'auto')";
  $insert_query2 = "INSERT INTO livehelp_users (username,password,isonline,isoperator,isadmin,isnamed,email,show_arrival,user_alert,auto_invite,greeting,photo) VALUES ('".filter_sql($UNTRUSTED['username'])."','".filter_sql($UNTRUSTED['password1'])."','N','Y','Y','Y','".filter_sql($UNTRUSTED['email'])."','N','N','N','How may I help You?','')";   
  $insert_query3 = "INSERT INTO livehelp_departments (nameof, onlineimage, offlineimage, requirename, messageemail, leaveamessage, opening, offline, creditline, layerinvite, imagemap,whilewait,timeout,topframeheight,topbackground,colorscheme,busymess) VALUES ('default', 'online.gif', 'leavemessage.gif', 'Y', '".filter_sql($UNTRUSTED['email'])."', 'YES', '<blockquote>".filter_sql($UNTRUSTED['opening'])."</blockquote>', '<blockquote>Sorry no operators are currently online to provide Live support at this time.</blockquote>', 'L','dhtmlimage.gif','<MAP NAME=myimagemap><AREA HREF=javascript:openLiveHelp() SHAPE=RECT COORDS=0,0,400,197><AREA HREF=javascript:openLiveHelp() SHAPE=RECT COORDS=0,157,213,257><AREA HREF=javascript:closeDHTML() SHAPE=RECT COORDS=237,157,400,257></MAP>','Please be patient while an operator is contacted... ','500','35','linedback.gif','yellow','<blockquote>Sorry all operators are currently helping other clients and are unable to provide Live support at this time.<br>Would you like to continue to wait for an operator or leave a message?<br><table width=450><tr><td width=40%><a href=livehelp.php?page=livehelp.php&department=[department]&tab=1 target=_top><font size=+1>Continue to wait</font></a></td><td width=20% align=center><b>or</b></td><td width=40%><a href=leavemessage.php?department=[department]><font size=+1>Leave A Message</a></td></tr></table><blockquote>')";
   ...
   if($installationtype == "upgrade25"){
    ...
    if($installationtype == "upgrade26"){
     ...
     if( ($installationtype == "upgrade271") || 
         ...
         $sql = "ALTER TABLE `livehelp_config` ADD `scratch_space` TEXT NOT NULL ";
         mysql_query($sql,$conn);
...
<h2 align=\"center\">Welcome to Crafty Syntax Live Help</h2>
<p>
All the administrative functions are located to the left of this text. 
</p>
<p>
You can use this section to keep notes for yourself and other admins, etc. </p>
<p>
To change the text that is located in this box just click on the small edit button on the top right corner of this box.</p>
...
if( ($installationtype == "upgrade282") ){ 
     ...
     $sql = "ALTER TABLE `livehelp_departments` ADD `imagemap` TEXT NOT NULL";
     mysql_query($sql,$conn);
...
if( ($installationtype == "upgrade284") ){                                 
     $sql = "ALTER TABLE `livehelp_departments` ADD `whilewait` TEXT NOT NULL";
     mysql_query($sql,$conn);
     ...
     $sql = "ALTER TABLE `livehelp_users` ADD `sessiondata` TEXT NOT NULL ";
     mysql_query($sql,$conn);
    ...
    $sql = "UPDATE `livehelp_departments` SET `timeout`=600";
    mysql_query($sql,$conn);
      
    $sql = "ALTER TABLE `livehelp_transcripts` ADD `sessionid` VARCHAR( 40 ) NOT NULL , ADD `sessiondata` TEXT NOT NULL , ADD `department` INT( 10 ) NOT NULL ";
    mysql_query($sql,$conn);      
...
if( ($installationtype == "upgrade290") ){ 
    ...
    $sql = "ALTER TABLE `livehelp_departments` ADD `leavetxt` TEXT NOT NULL";
    mysql_query($sql,$conn);               
...
if( ($installationtype == "upgrade291") || ($installationtype == "upgrade292") ){ 
    ...
    $sql = "ALTER TABLE `livehelp_users` ADD `greeting` TEXT NOT NULL";
    mysql_query($sql,$conn); 
...
if($installationtype == "upgrade296"){
   ...
   mysql_query($sql,$conn);
   $sql = "ALTER TABLE `livehelp_config` ADD `maxexe` int(5) DEFAULT '300'";
   mysql_query($sql,$conn);
...
if($installationtype == "upgrade297"){
   ...
   mysql_query($sql,$conn);   
   $sql = "UPDATE livehelp_departments set topframeheight='35',topbackground='linedback.gif',colorscheme='yellow'";
   mysql_query($sql,$conn);   
...
if( ($installationtype == "upgrade298") || ($installationtype == "upgrade299") ){
   ...
   $sql = "ALTER TABLE `livehelp_config` ADD `adminsession` CHAR( 1 ) DEFAULT 'N' NOT NULL";
   mysql_query($sql,$conn);  
...
CREATE TABLE `livehelp_operator_history` (
  ...
  INDEX ( `dateof` )   
   ...
   $sql = "ALTER TABLE `livehelp_departments` ADD `busymess` TEXT NOT NULL";
   mysql_query($sql,$conn);

   $sql = "ALTER TABLE `livehelp_config` ADD `ignoreips` TEXT NOT NULL";
   mysql_query($sql,$conn);   
...
if($installationtype == "upgrade2117"){
    ...
    mysql_query($sql,$conn);    
	  ...
	  $sql = "ALTER TABLE `livehelp_departments` ADD `everythingelse` text NOT NULL";
    mysql_query($sql,$conn);   
    ...
    mysql_query($sql,$conn);   
 	  ...
 	  $sql = "ALTER TABLE `livehelp_config` ADD `everythingelse` text NOT NULL";
    mysql_query($sql,$conn);   
...
if( ($installationtype == "upgrade2125" ) || ($installationtype == "upgrade2126" ) ){           	 
	 $sql = "UPDATE livehelp_config set version='$version'";
   mysql_query($sql,$conn);
 }  
...
$mydatabase->query("INSERT INTO livehelp_modules_dep VALUES (2, 1, 2, 2, 'Y')");
		...
		$sql = "INSERT INTO livehelp_autoinvite VALUES (1, 'Y', 0, '1', '', 0, '', 'layer')";
$mydatabase->query($sql);
...
"CREATE TABLE `livehelp_config` (
  `version` varchar(10) NOT NULL default '2.12.7',
  ...
  `adminsession` char(1) NOT NULL default 'N',
  `ignoreips` text NOT NULL,
...
CREATE TABLE `livehelp_departments` (
  ...
  `imagemap` text NOT NULL,
  `whilewait` text NOT NULL,
  `timeout` int(5) NOT NULL default '0',
  `leavetxt` text NOT NULL,
...
problems on this page?!? <a href=http://www.craftysyntax.com/bb/ TARGET=_blank>CLICK HERE TO GO TO SUPPORT PAGE</a>
<hr>
<?php
if ($errors != ""){
