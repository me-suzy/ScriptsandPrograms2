...
if($randomNumber == 7){
 ...
 if($dbtype!="txt-db-api"){
  $sqlquery = "OPTIMIZE TABLE `livehelp_referers_daily` , `livehelp_referers_monthly` , `livehelp_users` , `livehelp_visit_track` , `livehelp_visits_daily` , `livehelp_visits_monthly` ";	
  $mydatabase->query($sqlquery); 
 }
...
if($randomNumber == 3){     
    ...
    $oldtime = date("YmdHis",$prev);
    $query = "SELECT user_id,lastaction FROM livehelp_users WHERE isoperator='Y' AND authenticated='Y' AND lastaction<'$oldtime'";
    ...
    while($row = $data2->fetchRow(DB_FETCHMODE_ASSOC)){
       ...
       $lastaction = $row['lastaction'];
        ...
        $query = "INSERT INTO livehelp_operator_history (opid,action,dateof,sessionid,totaltime) VALUES ($opid,'Logout','$lastaction','".$identity['SESSIONID']."',$seconds)";
        $mydatabase->query($query);
...
// chatters who closed their chat window timeout after a minute of inactivity..:
 $aminute = mktime ( date("H"), date("i")-1, date("s"), date("m"), date("d"), date("Y") );
