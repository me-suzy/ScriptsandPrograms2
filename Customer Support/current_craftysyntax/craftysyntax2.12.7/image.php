...
// default command:
if(empty($UNTRUSTED['cmd'])){ $UNTRUSTED['cmd'] = "getstate"; }

// if this is a credit image ghost the session:
if($UNTRUSTED['cmd'] == "getcredit")
  $ghost_session = true;
...
//----------------------------------------------------------------
if($UNTRUSTED['cmd'] == "getlayerinvite"){
...
//----------------------------------------------------------------
if($UNTRUSTED['cmd'] == "changestat"){
   
   $rightnow = date("YmdHis");
   $sqlquery = "UPDATE livehelp_users set status='".filter_sql($UNTRUSTED['what'])."' WHERE (status='' OR status='DHTML' OR status='invited' OR status='request')  AND sessionid='".$identity['SESSIONID']."'";
   $mydatabase->query($sqlquery);
...
//----------------------------------------------------------------
if($UNTRUSTED['cmd'] == "startedtyping"){       
