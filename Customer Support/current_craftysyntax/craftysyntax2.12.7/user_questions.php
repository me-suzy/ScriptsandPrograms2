...
if($data->numrows() == 0){
 ?>
    <SCRIPT>
    window.parent.location.replace("livehelp.php?tab=1&doubleframe=yes&page=offline.php&department=<?php echo $UNTRUSTED['department']; ?>");       
    </SCRIPT>
 <?php
...
if($askquestions=="N"){
 ?>
    <SCRIPT>
    window.location.replace("user_connect.php?try=0&tab=1&doubleframe=yes&page=offline.php&department=<?php echo $UNTRUSTED['department']; ?>");       
    </SCRIPT>
 <?php
  $mydatabase->close_connect();
  exit;
     ...
     if( ($row['fieldtype'] == "username") || ($row['fieldtype'] == "textfield") || ($row['fieldtype'] == "email") ){	     	   
      ...
      if($row['fieldtype'] == "dropmenu"){
          if($indent){
            ...
            if( !(empty($UNTRUSTED[$fieldname])) ) { ?>
               <option value="<?php echo $UNTRUSTED[$fieldname]; ?>" selected><?php echo $UNTRUSTED[$fieldname]; ?></option>
            <?php } 
         	  print "<option value=\"\"> </option>\n";
 	          for($i=0;$i<count($options);$i++){
         	   print "<option>$options[$i]</option>\n";
