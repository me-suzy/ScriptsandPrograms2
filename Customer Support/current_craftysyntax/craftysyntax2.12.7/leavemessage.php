...
//         Copyright (C) 2003-2004 Eric Gerdes   (http://www.craftysyntax.com )
// --------------------------------------------------------------------------
...
$qQry = "SELECT user_id,camefrom FROM livehelp_users WHERE sessionid='${identity['SESSIONID']}'";
...
if($UNTRUSTED['action']=="send" ) {
  ...
  if( $errors=="" ) {
    ...
    $comments = $heading . $comments;		
		...
		//    Fix: added (via CSLH) to From: header
		//  - 1.9 MIME_HEADER_CTYPE_ONLY 'Content-Type' found without required MIME headers
		//    Fix: added "Content-type: text/plain; charset=${lang['charset']}" header
    //  - 1.7 HTML_MIME_NO_HTML_TAG HTML-only message, but there is no HTML tag
		//    Fix: No problem, identified as plain text eMail now
    // Other error messages were caused by it assuming HTML eMail
    
    // Replaced:
    //    mail("$messageemail","CSLH Contact Submission","$comments","From: $useremail");
    // With:
    // mail("$messageemail","CSLH Contact Submission","$comments",
		//		  "From: $useremail (via CSLH)"
		//		   .$sys_eol."Content-type: text/plain; charset=${lang['charset']}"
		//		); 
  ...
  if($emailfun!="N"){
   if($sys_eol=="\r\n"){
     if (!(send_mail("Contact", $useremail, $departmentname, $messageemail, $subjectline, $comments, "text/plain", $lang['charset'], true))) {
         send_mail("Contact", $useremail, $departmentname, $messageemail, $subjectline, $comments, "text/plain", $lang['charset'], false);
     }				
   } else {
     if (!(send_mail("Contact", $useremail, $departmentname, $messageemail, $subjectline, $comments, "text/plain", $lang['charset'], false))) {
         send_mail("Contact", $useremail, $departmentname, $messageemail, $subjectline, $comments, "text/plain", $lang['charset'], true);
     }	   	
   }
  }
  ...
  while($row = $result->fetchRow(DB_FETCHMODE_ORDERED)){
     ...
     if( ($fieldtype == "username") || ($fieldtype == "subject_textfield") || ($fieldtype == "textfield") || ($fieldtype == "email") ){
        ...
        <tr><td class="tdFldHdr"><?php print $headertext . $star; ?></td>
            <td><input type=text size=30 name="<?php echo $fieldname; ?>" value="<?php echo empty($UNTRUSTED[$fieldname])? '': $UNTRUSTED[$fieldname]; ?>"></td></tr>
<?php }
      // text areas:
      if($fieldtype == "textarea"){ ?>
        <tr><td colspan=2><table class="tblGroup" width="100%" border=0 cellspacing=0 cellpadding=0>
              ...
              <tr><td><textarea cols=30 rows=5 name="<?php echo $fieldname; ?>" style="width: 90%;"><?php echo (empty($UNTRUSTED[$fieldname])? '': $UNTRUSTED[$fieldname]); ?></textarea></td></tr>
            </table></td></tr>
<?php }
      // Drop Menu
...
<?php if( !(empty($UNTRUSTED[$fieldname])) ) { ?>
                  <option value="<?php echo $UNTRUSTED[$fieldname] ?>" selected><?php echo $UNTRUSTED[$fieldnsme] ?></option>
<?php } else { ?>
                  <option value=""> </option>
<?php }
