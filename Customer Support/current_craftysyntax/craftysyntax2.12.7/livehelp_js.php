...
// --------------------------------------------------------------------------  
// BIG NOTE:
//     At the time of the release of this version of Crafty Syntax, Version 3.1.0 
//     which is a more modular, extendable , skinable version of Crafty Syntax
...
{	 
   ...
   cscontrol = new Image;
	 var u = WEBPATH + 'image.php?' + 
					'cmd=userstat' + 
					'&page=' + escape(<?php echo $parentdot; ?>document.location) + 
					'&randu=' + randu +
					'&pageid=' + csID +
					'&department=' + <?php echo intval($UNTRUSTED['department']); ?> +
					'&cslhVISITOR=' + '<?php echo $identity['SESSIONID']; ?>' +
					'&title=' + escape(<?php echo $parentdot; ?>document.title) + 
					'&referer=' + escape(<?php echo $parentdot; ?>document.referrer) + 					
	 ...
	 var w = 0;
      ...
      // layer invite:
	    if ((w == 25) && !(openDHTMLalready)) {
  		   loadKey_<?php echo intval($UNTRUSTED['department']); ?>(1);
	    } 	        	        
...
function openLiveHelp(department)
{
  ...
  <?php if ($identity['COOKIE_SET'] == "Y") { ?>	
    window.open(WEBPATH + 'livehelp.php?department=' + department + '&cslhVISITOR=<?php echo $identity['SESSIONID']; ?><?php echo $querystringadd; ?>', 'chat54050872', 'width=585,height=390,menubar=no,scrollbars=0,resizable=1');
  <?php } else { ?>
    window.open(WEBPATH + 'livehelp.php?department=' + department + '<?php echo $querystringadd; ?>', 'chat54050872', 'width=585,height=390,menubar=no,scrollbars=0,resizable=1');
  <?php } ?>
}
...
{  
  ...
  // open the window.. 
  window.open(WEBPATH + 'livehelp.php?cmd=chatinsession&department=<?php echo intval($UNTRUSTED['department']); ?>&cslhVISITOR=<?php echo $identity['SESSIONID']; ?><?php echo $querystringadd; ?>', 'chat54050872', 'width=585,height=390,menubar=no,scrollbars=0,resizable=1');
}

<?php 
 if(!(isset($UNTRUSTED['cmd']))) { $UNTRUSTED['cmd'] = ""; }
...
if ( ($UNTRUSTED['cmd'] != "hidden") && (!( $noonehome && ($leaveamessage != "YES")))){
  ...
  var urltohelpimage = '<?php print $WEBPATH . "image.php?cmd=getstate&department=" . $UNTRUSTED['department'] . "&nowis=" . date("YmdHis") . "&cslhVISITOR=" . $identity['SESSIONID']; ?>' + 
					'&page=' + escape(<?php echo $parentdot; ?>document.location) + 
					'&title=' + escape(<?php echo $parentdot; ?>document.title) + 
					'&referer=' + escape(<?php echo $parentdot; ?>document.referrer) + 
					'<?php echo $querystringadd; ?>';

  var urltocreditimage = '<?php print $WEBPATH . "image.php?cmd=getcredit&department=" . $UNTRUSTED['department'] . "&nowis=" . date("YmdHis") . "&cslhVISITOR=" . $identity['SESSIONID']; ?>' + 
					'&page=' + escape(<?php echo $parentdot; ?>document.location) + 
					'&title=' + escape(<?php echo $parentdot; ?>document.title) + 
					'&referer=' + escape(<?php echo $parentdot; ?>document.referrer) +
					'<?php echo $querystringadd; ?>';
...
if (ismac > -1) {
	randu=Math.round(Math.random()*9999);
  document.write('<tr>');
  document.write('<td align="center" valign="top"><img id="imageformac" name="imageformac" src="' + WEBPATH + 'images/blank.gif" border="0"><img id="imageformac" name="imageformac_one" src="' + WEBPATH + 'images/blank.gif" border="0"><img id="imageformac" name="imageformac_ten" src="' + WEBPATH + 'images/blank.gif" border="0"><img id="imageformac" name="imageformac_hun" src="' + WEBPATH + 'images/blank.gif" border="0"></td>');
  document.write('</tr>');
}
