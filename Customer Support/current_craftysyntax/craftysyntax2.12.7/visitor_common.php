...
if(empty($UNTRUSTED['serversession'])){ $serversession=0; } else { $serversession = intval($UNTRUSTED['serversession']); }
...
if( !($cookiesession) && !($allow_ip_host_sessions) && (!($serversession)) ){
   $allow_ip_host_sessions=1;
   $serversession=0;
   $cookiesession=1;
}
