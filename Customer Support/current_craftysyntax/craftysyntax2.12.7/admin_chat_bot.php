...
while($channel_a = $mychannels->fetchRow(DB_FETCHMODE_ASSOC)){
 ...
 if($channel_a['lastaction'] < $oldtime){
   $query = "DELETE FROM livehelp_operator_channels WHERE userid=" . intval($channel_a['userid']);	
   $mydatabase->query($query);
 } else {
 	 $alltabs[$j] = $channel_a;
 	 $j++;
...
?>
    ...
    <?php if($CSLH_Config['showdirectory']=="Y"){ ?>    
      &nbsp;<a href=javascript:directory()><img src=images/directory.gif width=25 height=25 border=0></a>
    <?php } ?>  
    </tr>
<tr>
<?php 
...
function directory(){
  window.open('http://directory.craftysyntax.com/', 'directorywindow', 'width=620,height=420,menubar=no,scrollbars=1,resizable=1');	
}
</SCRIPT>
...
?>      	
<table width=100% cellpadding=0 cellspacing=0 border=0 bgcolor=EFEFEF><tr><td><b><?php echo $lang['message_to']; ?> <font color=<?php echo $myusercolor; ?> size=+1><?php echo $myuser; ?></font>:</b></td><td align=right><a href=javascript:showsmile()><img src=chat_smiles/icon_smile.gif border=0>smilies</a> :: <a href=javascript:editsmile()><?php echo $lang['edit']; ?></a></tr></table>
...
<?php 
  ...
  while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
   if(note_access($row['visiblity'],$row['department'],$row['user'])){ 
     $j++;
     $in= $j + 1;
     print "if(num == $in){ document.chatter.comment.value='[PUSH]$row[message][/PUSH]';  }\n";	
  }
 } ?>
}
</SCRIPT>
