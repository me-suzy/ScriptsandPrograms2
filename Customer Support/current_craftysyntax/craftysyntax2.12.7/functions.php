...
function get_identitystring($ipaddress,$sessionname="SESSIONID"){
   ...
   $identitystring = "$hostip_array[0].$hostip_array[1].$hostip_array[2]";
...
function identity($PHPSESSID="",$sessionname="PHPSESSID",$allow_ip_host_sessions=false,$serversession=false,$cookiesession=true,$ghost_session=false){
   global $CSLH_Config,$username,$mydatabase,$REMOTE_ADDR,$HTTP_USER_AGENT,$HTTP_REFERER;
   ...
   if(empty($HTTP_REFERER))
    ...
    // Disable auto-start of a session
    @ini_set('session.auto_start', 0);

    // using cookies or not:
    @ini_set('session.use_cookies', $cookiesession ? 1 : 0);
     ...
     if($ghost_session==false){
      ...
      if( ($identity['IP_ADDR'] == $currentusername[0]) && (!(empty($UNTRUSTED['username'])))){
       ...
       if(!(empty($newusername))){
        $sqlquery = "UPDATE livehelp_users SET isnamed='Y',username='".filter_sql($newusername)."' WHERE sessionid='".$identity['SESSIONID']."'";
        $mydatabase->query($sqlquery);   
       }
      } 
...
$messages = $mydatabase->query($sqlqueryrun);
	...
	$index=0;	
            ...
            $tableend = "</td></tr></table>";
	          ...
	          if($typeof=="writediv"){
                     ...
                     $chatmessage = "nullstring";
   	                ...
   	                $chatmessage = "<SCRIPT LANGUAGE=\"JavaScript\">\n";
   	                if(!(ereg("nullstring",$safemessage))){ 
               ...
               if(ereg("\[PUSH\]",$message)){
                 if(!(is_operator($myid))){  // this should work too: (ereg("cslhVISITOR",$identity['IDENTITY'])){
                      ...
                      $newmessage = preg_replace('#((http|https|ftp|news|file)://)([^\s,]*)#', '<a href="\\1\\3" target=_blank>\\1\\3</a>', $newmessage);
                       ...
                       if(!($diliminated))
                        $message = "<SCRIPT> $javascript </SCRIPT> $newmessage";
                       else
                        $message =  $newmessage;
                    ...
                    if( ereg("cslhVISITOR",$identity['IDENTITY']) && ( ereg("\[transfer\]",$message) )){ 
                      ...
                      if(!($diliminated))
                         $message = "<SCRIPT> $javascript </SCRIPT>";
	                    $message .= "..transfered.."; 
     ...
     if($diliminated){   
       ...
       if($typeof!="writediv")
        $resultchat .= "<SCRIPT> whatissaid[$register] = 'nullstring'\n update_typing();\n </SCRIPT>";
     }  
   $index++;	
...
$sesssiondata .= "<b>New Session :</b>" . $user_info['new_session'] . "<br>";
  ...
  for($l=0;$l<count($datapairs);$l++){
        ...
        $question_row = $questiondata->fetchRow(DB_FETCHMODE_ORDERED);    	          
    	  $sesssiondata .= "<b> ". $question_row[0] . ":</b> <br><font color=000000>" . urldecode($dataset[1]) . "</font><br>";
...
/**
  ...
  */ 
function send_mail($myname, $myemail, $contactname, $contactemail, $subject, $message, $contenttype, $charset,$useCRLF=false) {
  if ($useCRLF)
     $newline = "\r\n";
  else
     $newline = "\n";
   ...
   if(ereg("cslhVISITOR",$identity['IDENTITY'])){      
      ...
      $sqlquery = "SELECT isonline 
                ...
                WHERE livehelp_users.user_id=livehelp_operator_departments.user_id
                 ...
                 AND livehelp_users.auto_invite='Y' AND livehelp_users.isonline='Y' LIMIT 1";
      ...
      if($tmpdata->numrows() != 0){
        ...
        //fwrite( $fa, date("YmdHis") . " $refpage,$camefrom) && ereg($page,$pageurl) \n", 1000);
        //fclose($fa);
               // if match:
...
function archivefootsteps($sessionid){
    ...
    // move all old visit data into visit tracking database.. 
    $sqlquery = "SELECT DISTINCT location FROM livehelp_visit_track WHERE sessionid='".filter_sql($sessionid)."'";
...
function stopchat($sessionid){
      ...
      if($trans_array['transcript'] != ""){      
        ...
        $query = "INSERT INTO livehelp_operator_history (opid,action,dateof,transcriptid,totaltime) VALUES ($opid,'Stopchat','".date("YmdHis")."','".$transcriptid."',$seconds)";
