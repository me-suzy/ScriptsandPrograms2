...
if(!(empty($UNTRUSTED['tab']))){
    ...
    if(empty($UNTRUSTED['page']))
     $UNTRUSTED['page'] = $row['path'];
    $UNTRUSTED['page'] .= "?department=".$UNTRUSTED['department']."&" . $row['query_string']; 			
}
...
if(empty($UNTRUSTED['tab'])){
   ...
   if( $data->numrows() == 0){
     $doubleframe = "yes";
     $UNTRUSTED['page'] = "offline.php";
     $UNTRUSTED['tab'] = 1;
   } else {
    $row = $data->fetchRow(DB_FETCHMODE_ASSOC);
    if(empty($UNTRUSTED['page']))
      $UNTRUSTED['page'] = $row['path'];
    $UNTRUSTED['page'] .= "?department=".$UNTRUSTED['department']."&referer=" . $person['camefrom'] . "&" . $row['query_string'];
...
<frame src="<?php echo $UNTRUSTED['page']; ?>?department=<?php echo $UNTRUSTED['department']; ?><?php echo $querystringadd; ?>" name="1b" scrolling="AUTO" border="0" marginheight="0" marginwidth="0" NORESIZE=NORESIZE>
</frameset>
<?php
} else {
?><SCRIPT>
