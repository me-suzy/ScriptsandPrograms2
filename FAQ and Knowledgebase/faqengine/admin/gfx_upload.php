...
{
	$alloweddirs=array();
	$sql="select dir.* from ".$tableprefix."_dir_access dir, ".$tableprefix."_prog_dirs pd where dir.entrynr=pd.dirnr and pd.prognr=$prognr";
	if(!$result = faqe_db_query($sql, $db))
...
?>
	var newvalue="[img]<?php echo $faqe_prot?>://<?php echo $myserver.$pic_url?>/" + filestr + "[/img]";
<?php
	if(is_msie() && is_win() && (get_browser_version()>=4))
	{
		echo "	if (strSelection == \"\")\n";
		echo "		eval('mywin.document.inputform.$usedfield.focus();');\n";
		echo "	mywin.document.selection.createRange().text = newvalue;\n";
	}
 	else
 	{
 		echo "	mywin.document.inputform.$usedfield.value += newvalue;\n";
 		echo "	mywin.document.inputform.$usedfield.focus();\n";
 	}
?>
	window.close()
}
...
<tr class="headingrow"><td align="CENTER"><b><?php echo $l_inlinegfx?></b></td>
...
if($upload_avail)
{
	...
	echo "<input type=\"hidden\" name=\"prognr\" value=\"$prognr\">";
...
if($userdata["rights"]>2)
{
	...
	echo "<input class=\"faqefile\" name=\"NEWDIR\" type=\"text\" size=\"20\">&nbsp;";
...
if($subdir)
{
	...
	for($i=0;$i<count($subdirparts);$i++)
	{
		...
		echo "<a class=\"listlink\" href=\"".do_url_session("$act_script_url?$langvar=$act_lang&usedfield=$usedfield&subdir=$newsubdir&prognr=$prognr")."\">";
		echo $subdirparts[$i];
		echo "</a>/";
	}
}
...
if($subdir)
{
		if(substr_count($subdir,"/")>1)
		{
			...
			$newsubdir="";
			echo "<td class=\"listlink\" align=\"right\" colspan=\"2\"><a class=\"listlink\" href=\"".do_url_session("$act_script_url?$langvar=$act_lang&usedfield=$usedfield&subdir=$newsubdir&prognr=$prognr")."\"><img src=\"gfx/opendir\" border=\"0\" title=\"$l_changedir\" alt=\"$l_changedir\"></a></td>";
			echo "</tr>";
		}
        	echo "<tr class=\"listrow3\">";
		...
		if($parentend=strrpos($subdir,"/"))
		{
			...
			$newsubdir="";
		echo "<td align=\"right\" colspan=\"2\"><a class=\"listlink\" href=\"".do_url_session("$act_script_url?$langvar=$act_lang&usedfield=$usedfield&subdir=$newsubdir&prognr=$prognr")."\"><img src=\"gfx/opendir.gif\" border=\"0\" title=\"$l_changedir\" alt=\"$l_changedir\"></a></td>";
		echo "</tr>";
}
	...
	if ((strlen($entry)>2) && (filetype($entry)=="dir"))
	{
		...
		if($userdata["rights"]<3)
		{
			...
			if(is_in_array(trim($testdir),$alloweddirs))
				echo "<a class=\"listlink\" href=\"".do_url_session("$act_script_url?$langvar=$act_lang&usedfield=$usedfield&subdir=$newsubdir&prognr=$prognr")."\"><img src=\"gfx/opendir.gif\" border=\"0\" title=\"$l_changedir\" alt=\"$l_changedir\"></a>";
			else
				echo "&nbsp;";
		}
		else
			echo "<a class=\"listlink\" href=\"".do_url_session("$act_script_url?$langvar=$act_lang&usedfield=$usedfield&subdir=$newsubdir&prognr=$prognr")."\"><img src=\"gfx/opendir\" border=\"0\" title=\"$l_changedir\" alt=\"$l_changedir\"></a>";
		echo "</td>";
		echo "</tr>";
	}
}
...
while ($entry=$cdir->read())
{
	if ((strlen($entry)>2) && (filetype($entry)=="file"))
	{
		...
		echo "<td align=\"center\"><img src=\"$pic_url/$entry\" border=\"0\"></a></td>";
		echo "<td>$entry</a></td>";
		echo "<td align=\"right\">".filesize($entry)." bytes</a></td>";
		echo "<td align=\"right\"><a class=\"listlink\" href=\"javascript:choosepic('$entry')\"><img src=\"gfx/transfer.gif\" border=\"0\" title=\"$l_choose\" alt=\"$l_choose\"></a></td>";
		echo "<td align=\"right\"><a class=\"listlink\" href=\"".do_url_session("$act_script_url?ACTION=DEL&DELFILE=$entry&subdir=$subdir&$langvar=$act_lang&usedfield=$usedfield&prognr=$prognr")."\"><img src=\"gfx/delete.gif\" border=\"0\" alt=\"$l_delete\" title=\"$l_delete\"></a>";
		echo "</tr>";
	}
}
echo "</table>";
