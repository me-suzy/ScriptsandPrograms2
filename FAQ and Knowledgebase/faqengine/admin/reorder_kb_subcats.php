...
<td align="left"><select name="input_catnr">
<?php
$sql = "select prog.programmname, prog.language, cat.catname, cat.catnr from ".$tableprefix."_programm prog, ".$tableprefix."_kb_cat cat where prog.prognr=cat.programm  order by cat.catnr, prog.prognr";
...
if (!$myrow = faqe_db_fetch_array($result))
{
	...
	die("no categories defined");
}
do{
	echo "<option value=\"".$myrow["catnr"]."\"";
	if(isset($input_catnr) && ($input_catnr==$myrow["catnr"]))
		echo " selected";
	echo ">".display_encoded($myrow["catname"])." (".display_encoded($myrow["programmname"])." [".$myrow["language"]."])";
...
if(isset($mode))
{
	...
	if($mode=="move")
	{
		...
		if($direction=="up")
		{
			$sql="select displaypos from ".$tableprefix."_kb_subcat where catnr=$input_subcatnr";
			if(!$result = faqe_db_query($sql, $db))
			    die("<tr class=\"errorrow\"><td align=\"center\" colspan=\"2\">Could not connect to the database.".faqe_db_error());
			if(!$myrow=faqe_db_fetch_array($result))
			    die("<tr class=\"errorrow\"><td align=\"center\" colspan=\"2\">Calling error.");
			$newpos=$myrow["displaypos"]-1;
			$sql="update ".$tableprefix."_kb_subcat set displaypos=displaypos+1 where displaypos=$newpos and category=$input_catnr";
			if(!$result = faqe_db_query($sql, $db))
			    die("<tr class=\"errorrow\"><td align=\"center\" colspan=\"2\">Could not connect to the database.".faqe_db_error());
			$sql="update ".$tableprefix."_kb_subcat set displaypos=$newpos where catnr=$input_subcatnr";
			if(!$result = faqe_db_query($sql, $db))
			    die("<tr class=\"errorrow\"><td align=\"center\" colspan=\"2\">Could not connect to the database.".faqe_db_error());
		}
		if($direction=="down")
...
<table align="center" width="100%" border="0" cellspacing="1" cellpadding="1">
<?php
$sql = "select * from ".$tableprefix."_kb_subcat where category=$input_catnr order by displaypos asc";
if(!$result = faqe_db_query($sql, $db)) {
...
<?php
		...
		if($admin_rights > 1)
		{
			...
			{
				$tempsql="select max(displaypos) as newdisplaypos from ".$tableprefix."_kb_subcat where category=".$myrow["category"];
				...
				$updatesql="update ".$tableprefix."_kb_subcat set displaypos=$newpos where catnr=".$myrow["catnr"];
			...
			if($mycount>1)
			{
				...
				echo "<a href=\"".do_url_session("$act_script_url?mode=move&input_catnr=$input_catnr&input_subcatnr=$act_id&$langvar=$act_lang&direction=up")."\">";
			...
			{
				echo "<td class=\"inputrow\" align=\"center\" width=\"16\">";
				echo "<a href=\"".do_url_session("$act_script_url?mode=move&$langvar=$act_lang&input_catnr=$input_catnr&input_subcatnr=$act_id&direction=down")."\">";
