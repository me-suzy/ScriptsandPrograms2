...
$page_title=$l_kbsubcategory_title;
$page="kb_subcats";
...
if(!isset($storefaqfilter) && ($admstorefaqfilters==1))
{
	...
	if($admcookievals)
	{
		if(faqe_array_key_exists($admcookievals,"kbsc_filterprog"))
			$filterprog=$admcookievals["kbsc_filterprog"];
		if(faqe_array_key_exists($admcookievals,"kbsc_filtercat"))
			$filtercat=$admcookievals["kbsc_filtercat"];
		if(faqe_array_key_exists($admcookievals,"kbsc_filterlang"))
			$filterlang=$admcookievals["kbsc_filterlang"];
		if(faqe_array_key_exists($admcookievals,"kbsc_sorting"))
			$sorting=$admcookievals["kbsc_sorting"];
	}
}
if(!isset($sorting))
	$sorting=11;
...
<?php
		$sql = "select * from ".$tableprefix."_kb_subcat where (catnr=$input_subcatnr)";
...
<tr class="displayrow"><td align="right" width="30%"><?php echo $l_subcatname?>:</td><td><?php echo display_encoded($myrow["catname"])?></td></tr>
...
<td>
<?php
	$sql = "select cat.catname, prog.programmname, prog.language from ".$tableprefix."_kb_cat cat, ".$tableprefix."_programm prog where prog.prognr=cat.programm and cat.catnr=".$myrow["category"];
	if(!$result = faqe_db_query($sql, $db))
		die("<tr bgcolor=\"#cccccc\"><td>Could not connect to the database (3).");
	if ($temprow = faqe_db_fetch_array($result))
		echo display_encoded($temprow["catname"])." (".display_encoded($temprow["programmname"])." [".$temprow["language"]."])";
...
?>
<table align="center" width="100%" border="0" cellspacing="1" cellpadding="1">
<tr><td class="headingrow" align="center" colspan="2"><b><?php echo $l_newsubcategory?></b></td></tr>
...
<?php
	if($admin_rights<3)
		$sql = "select pr.programmname, pr.language, cat.* from ".$tableprefix."_kb_cat cat, ".$tableprefix."_programm pr, ".$tableprefix."_programm_admins pa where pr.prognr = pa.prognr and pa.usernr=$act_usernr and cat.programm=pr.prognr ";
	else
		$sql = "select pr.programmname, pr.language, cat.* from ".$tableprefix."_kb_cat cat, ".$tableprefix."_programm pr where cat.programm=pr.prognr ";
	...
	if(bittst($admedoptions,BIT_2))
		$sql.=", pr.programmname asc, cat.catname asc";
	else
	...
	if (!$temprow = faqe_db_fetch_array($result))
	{
		echo "<a href=\"".do_url_session("kb_cats.php?mode=new&$langvar=$act_lang")."\" target=\"_blank\">$l_new</a>";
	}
	else
	{
?>
...
<?php
		...
		if($errors==0)
		{
			$sql = "select max(displaypos) as newdisplaypos from ".$tableprefix."_kb_subcat where category=$category";
			if(!$result = faqe_db_query($sql, $db))
			...
			$sql = "INSERT INTO ".$tableprefix."_kb_subcat (catname, heading, category, displaypos) ";
			$sql .="VALUES ('$subcategory', '$heading', '$category', $displaypos)";
			if(!$result = faqe_db_query($sql, $db))
...
<table align="center" width="100%" border="0" cellspacing="1" cellpadding="1">
<?php
		$deleteSQL = "delete from ".$tableprefix."_kb_subcat where (catnr=$input_subcatnr)";
	...
	if($mode=="edit")
	{
		$modsql="select pa.* from ".$tableprefix."_programm_admins pa, ".$tableprefix."_kb_cat cat, ".$tableprefix."_kb_subcat subcat where cat.catnr=subcat.category and subcat.catnr=$input_subcatnr and pa.prognr=cat.programm and pa.usernr=$act_usernr";
...
?>
<table align="center" width="100%" border="0" cellspacing="1" cellpadding="1">
<tr><td class="headingrow" align="center" colspan="2"><b><?php echo $l_editsubcats?></b></td></tr>
<form name="inputform" onsubmit="return checkform();" method="post" action="<?php echo $act_script_url?>"><input type="hidden" name="<?php echo $langvar?>" value="<?php echo $act_lang?>">
<?php
		if($sessid_url)
			echo "<input type=\"hidden\" name=\"$sesscookiename\" value=\"$url_sessid\">";
...
<tr class="inputrow"><td align="right" width="30%"><?php echo $l_subcatname?>:</td><td><input class="faqeinput" type="text" name="subcategory" size="40" maxlength="80" value="<?php echo display_encoded($myrow["catname"])?>"></td></tr>
...
<?php
	if($admin_rights<3)
		$sql = "select pr.programmname, pr.language, cat.* from ".$tableprefix."_programm pr, ".$tableprefix."_programm_admins pa, ".$tableprefix."_kb_cat cat where pr.prognr = pa.prognr and pa.usernr=$act_usernr and cat.programm=pr.prognr ";
	else
		$sql = "select pr.programmname, pr.language, cat.* from ".$tableprefix."_programm pr, ".$tableprefix."_kb_cat cat where cat.programm=pr.prognr ";
	if(bittst($admedoptions,BIT_1))
	{
		if(isset($filterprog) && ($filterprog>=0))
			$sql.="and pr.prognr=$filterprog ";
	...
	if(bittst($admedoptions,BIT_2))
		...
		$sql.=", pr.displaypos asc, cat.displaypos asc";
	if(!$result = faqe_db_query($sql, $db)) {
		die("<tr class=\"errorrow\"><td>Could not connect to the database (3).".faqe_db_error());
	}
	if (!$temprow = faqe_db_fetch_array($result))
	{
		echo "<a href=\"".do_url_session("kb_cats.php?mode=new&$langvar=$act_lang")."\" target=\"_blank\">$l_new</a>";
...
<?php
	if($mode=="update")
	{
		$modsql="select pa.* from ".$tableprefix."_programm_admins pa, ".$tableprefix."_kb_cat cat, ".$tableprefix."_kb_subcat subcat where subcat.catnr=$input_subcatnr and cat.catnr=subcat.category and pa.prognr=cat.programm and pa.usernr=$act_usernr";
		if(!$modresult = faqe_db_query($modsql, $db)) {
		    die("<tr class=\"errorrow\"><td>Could not connect to the database.");
		}
		if($modrow=faqe_db_fetch_array($modresult))
		...
		{
			$heading=addslashes($heading);
			$sql = "UPDATE ".$tableprefix."_kb_subcat SET catname='$subcategory', heading='$heading', category='$category' ";
...
$sql="select sc.*, cat.catname as maincatname, prog.programmname, prog.language from ".$tableprefix."_kb_subcat sc, ".$tableprefix."_kb_cat cat, ".$tableprefix."_programm prog where sc.category=cat.catnr and cat.programm=prog.prognr ";
...
$sql.="group by sc.catnr ";
switch($sorting)
{
	case 12:
		$sql.="order by sc.catnr desc";
		break;
	case 21:
		$sql.="order by sc.catname asc";
		break;
	case 22:
		$sql.="order by sc.catname desc";
		break;
	case 31:
...
else
{
	...
	do {
		...
		$catname=display_encoded($myrow["maincatname"]);
		$progname=display_encoded($myrow["programmname"]);
		$proglang=$myrow["language"];
		echo "<td>$catname ($progname)</td>";
		echo "<td align=\"center\">$proglang</td>";
		echo "<td>";
		$modsql="select * from ".$tableprefix."_programm_admins pa, ".$tableprefix."_kb_cat cat where pa.prognr=cat.programm and cat.catnr=".$myrow["category"]." and usernr=$act_usernr";
		if(!$modresult = faqe_db_query($modsql, $db))
			die("<tr class=\"errorrow\"><td>Could not connect to the database.");
		if($modrow=faqe_db_fetch_array($modresult))
			$ismod=1;
		...
		{
			echo "<a class=\"listlink2\" href=\"".do_url_session("$act_script_url?mode=delete&input_subcatnr=$act_id&$langvar=$act_lang&catname=".urlencode($myrow["catname"]))."\">";
			echo "<img src=\"gfx/delete.gif\" border=\"0\" title=\"$l_delete\" alt=\"$l_delete\"></a>";
			echo "&nbsp; ";
			echo "<a class=\"listlink2\" href=\"".do_url_session("$act_script_url?mode=edit&input_subcatnr=$act_id&$langvar=$act_lang")."\">";
			echo "<img src=\"gfx/edit.gif\" border=\"0\" title=\"$l_edit\" alt=\"$l_edit\"></a>";
			echo "&nbsp; ";
		}
		echo "<a class=\"listlink2\" href=\"".do_url_session("$act_script_url?mode=display&input_subcatnr=$act_id&$langvar=$act_lang&catname=".urlencode($myrow["catname"]))."\">";
