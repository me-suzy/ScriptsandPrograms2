...
<a class="clistlink" name="#cat_top<?php echo $catcount?>" href="#cat<?php echo $catcount?>"><?php echo $catcount.". ".display_encoded($myrow["categoryname"])?></a></span><br>
<?php
...
<a class="clistlink" name="#subcat_top<?php echo $catcount?>_<?php echo $subcatcount?>" href="#subcat<?php echo $catcount?>_<?php echo $subcatcount?>"><?php echo $catcount.".".$subcatcount.". ".$l_withoutsubcat?></a></span></li>
<?php
}
?>
...
else
{
	...
	echo "<ul class=\"faqe\">";
	do{
		echo "<li><a class=\"clistlink\" href=\"#".$myrow2["faqnr"]."\">$catcount.";
		if($numsubcats>0)
			echo "$subcatcount.";
		echo "$faqcount ".undo_html_ampersand(stripslashes($myrow2["heading"]))."</a></li>";
		$faqcount+=1;
...
<a class="clistlink" name="#subcat_top<?php echo $catcount?>_<?php echo $subcatcount?>" href="#subcat<?php echo $catcount?>_<?php echo $subcatcount?>"><?php echo $catcount.".".$subcatcount.". ".display_encoded($myrow2["categoryname"])?></a></span></li>
<span style="font-face: <?php echo $FontFace?>; font-size: <?php echo $FontSize1?>; color: <?php echo $FontColor?>;">
<?php
		...
		else
		{
			...
			echo "<ul class=\"faqe\">";
			do{
				echo "<li><a class=\"clistlink\" href=\"#".$myrow3["faqnr"]."\">$catcount.";
				if($numsubcats>0)
					echo "$subcatcount.";
				echo "$faqcount ".undo_html_ampersand(stripslashes($myrow3["heading"]))."</a></li>";
				$faqcount+=1;
...
<span style="font-face: <?php echo $FontFace?>; font-size: <?php echo $FontSize2?>; color: <?php echo $GroupFontColor?>;">
<?php
	echo "<a name=\"#cat".$catcount."\"";
	if(bittst($listoptions,BIT_3))
		echo " class=\"groupheadinglink\" href=\"#cat_top".$catcount."\"";
	echo ">";
	echo $catcount.". ".display_encoded($myrow["categoryname"]);
	echo "</a></span></td></tr>";
$sql = "select * from ".$tableprefix."_subcategory where (category=".$myrow["catnr"].")";
if(!$result2 = faqe_db_query($sql, $db))
   	die("Could not connect to the database.");
...
<span style="font-face: <?php echo $FontFace?>; font-size: <?php echo $subcatfontsize?>; color: <?php echo $subcatfontcolor?>; <?php echo $addstyle?>">
<?php
	echo "<a name=\"#subcat".$catcount."_".$subcatcount."\"";
	if(bittst($listoptions,BIT_3))
		echo " class=\"subcatlink\" href=\"#subcat_top".$catcount."_".$subcatcount."\"";
	echo ">";
	echo "$catcount.$subcatcount. $l_withoutsubcat";
?>
</a></span></td></tr>
<?php
}
...
else
{
	...
	do{
		...
		echo $answertext."<br>";
		$attachsql="select f.filename, f.filesize, f.description, f.mimetype, fa.* from ".$tableprefix."_faq_attachs fa, ".$tableprefix."_files f where f.entrynr=fa.attachnr and fa.faqnr=".$entrydata["faqnr"];
...
<?php
	...
	if(bittst($listoptions,BIT_3))
		echo " class=\"subcatlink\" href=\"#subcat_top".$catcount."_".$subcatcount."\"";
	echo ">";
	echo "$catcount.$subcatcount. ".display_encoded($myrow2["categoryname"]);
	echo "</a></span></td></tr>";
$sql = "select * from ".$tableprefix."_data where subcategory=".$myrow2["catnr"];
if($faqsortmethod==0)
	$sql.=" order by editdate desc";
...
else
{
	...
	do{
		...
		$answertext = str_replace("{bbc_code}",$l_bbccode,$answertext);
		$answertext = str_replace("{bbc_quote}",$l_bbcquote,$answertext);
		echo "$catcount.$subcatcount.$faqcount. <a name=\"#".$myrow3["faqnr"]."\"><b>".undo_html_ampersand(stripslashes($myrow3["heading"]))."</b></a><br>";
		echo "<i>$l_question:</i><br>";
		echo $questiontext."<br>";
		echo "<i>$l_answer:</i><br>";
		echo $answertext."<br>";
