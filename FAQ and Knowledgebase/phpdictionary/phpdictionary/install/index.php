...
elseif ($_POST['Submit'] == "Next") {
	if(!isset($_POST['host']) || !ereg($pattern, $_POST['host']))
		die("Please enter a valid MySQL host.");
	if(!isset($_POST['user']) || !ereg($pattern, $_POST['user']))
		die("Please enter a valid MySQL username.");
	if(!isset($_POST['pass']) || !ereg($pattern, $_POST['pass']))
		die("Please enter a valid MySQL password.");
	if(!isset($_POST['db']) || !ereg($pattern, $_POST['db']))
	...
	$contents= str_replace($tag.$dmatche[1][6], $tag.addslashes($_POST['title']), $contents);
...
elseif ($_POST['Submit'] == "Finish") {
	if(!isset($_POST['admin']) || !ereg($pattern, $_POST['admin']))
