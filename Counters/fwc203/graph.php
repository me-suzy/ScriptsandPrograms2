...
/******************************************************************************
File Name    : graph.php
Description  : generates a graph displaying the last 7 days
...
include("config.php");
include("incl/filevars.php");
$siteid = $_GET["siteid"];
$sessionid = $_GET["sessionid"];

$fsiteid = split("!", $fsiteid);
$num_fsiteid = count($fsiteid);
// prevent some from viewing a counter they dont own
for ($i = 0; $i < $num_fsiteid; ++$i) {
		...
		for ($i = $x; $i < $num_lfdata; ++$i) {
			$day = explode("!", $lfdata[$i]);
			$bar_data = $bar_data + array($day[1] => $day[2]);
		}
		fclose($lf);
    	}
    	if (($i==$num_fsiteid-1) && (!$lfdata[0])) { print "Error"; exit; }
}
...
/* Settings */
$graph_width=300; //Width of entire graph
$graph_height=290; //Height of entire graph
$graph_padding=10; //Padding for edges of graph
$graph_title="Last 7 Days"; //Graph title
$left_title="Total Hits"; //Y-axis title
$bottom_title="Date"; //X-axis title
$marks=10; //How many numbers to generate in Y-axis
$text_bars=1; //1=Show numbers above and below bars
              //2=Show numbers only below bars
              //3=Show numbers only above bars

$use_logo=0; //Add logo to the bottom of your graph? 1-Yes 0-No
$full_img_url=""; //You may change this to your own logo
...
function PrintGraph($bar_data) {
   ...
   $darkblue = ImageColorAllocate($graph, 63, 122, 176);
   ...
   ImageRectangle($graph, 0, 0, $graph_width-1, $graph_height-1, $darkblue);
   ...
   /* Print graph bars */
   GraphBars($graph,$bar_data,$box_left_start,$box_top_start,$box_left_end,$box_top_end,$darkblue,$color2,$text_bars);
   ...
   $copyright="Graph generated by AdeptiSoft Graph";
