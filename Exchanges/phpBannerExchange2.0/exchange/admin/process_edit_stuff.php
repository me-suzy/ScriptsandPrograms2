...
$pass=$get_userinfo[adminpass];
	...
	session_destroy();
		...
		$editresult=nl2br($editresult);
		$editresult=addslashes($editresult);
		if($obj=="cou"){
			$title=$LANG_menu_editcou;
			if(!mysql_query("update bannerconfig set data='$editresult' where name='cou'")){
				$error='1';
				$errordump=mysql_error();
			}
			$gotourl="$baseurl/cou.php";
		}	
			
		if($obj=="rules"){
			$title=$LANG_menu_editrules;
			if(!mysql_query("update bannerconfig set data='$editresult' where name='rules'")){
				$error='1';
				$errordump=mysql_error();
			}
			$gotourl="$baseurl/rules.php";
		}

		if($error=='1'){
			$result="$LANG_mysql_error<p>$errordump";
		}else{
			$result= "$LANG_editstuff_result <a href=\"$gotourl\" target=\"_blank\">$LANG_editstuff_url</a>";
		}

		$page = new Page('../template/admin_editstuff_confirm.php');
		$page->replace_tags(array(
			'css' => "$css",
			'session' => "$session",
			'baseurl' => "$baseurl",
