...
$pass=$get_userinfo[adminpass];
	...
	session_destroy();
		...
		if($_REQUEST['fid']){
			$fid=$_REQUEST['fid'];
			$fid_form= "&fid=$fid";	
			$faqs=mysql_query("select * from bannerfaq where id='$fid'");	
			$get_faqs=mysql_fetch_array($faqs);	
			$question=$get_faqs[question];	
			$answer=$get_faqs[answer];	
			$answer=ereg_replace("<p>", "\n\n", $answer);	
			$answer=ereg_replace("<br>", "\n", $answer);	
			$answer=ereg_replace("<br />", "", $answer);	
		}else{	
			$fid_form="";	
		}

	$page = new Page('../template/admin_faqedit.php');
	$page->replace_tags(array(
		'css' => "$css",
		'session' => "$session",
		'baseurl' => "$baseurl",
		...
		'question' => "$question",
		'answer' => "$answer",
		'question_txt' => "$LANG_question",
		'answer_txt' => "$LANG_answer",
		'submit' => "$LANG_submit",
