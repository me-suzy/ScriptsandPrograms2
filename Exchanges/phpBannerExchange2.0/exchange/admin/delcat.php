...
$pass=$get_userinfo[adminpass];
	...
	$catid=$_REQUEST[catid];
		...
		$get_count=="0";	
		$check=mysql_query("select * from bannerstats where category=$catid");
		$get_count=@mysql_num_rows($check);	
		
		if($catid == 0){
			$msg= "$LANG_delcat_defaultcat";
		}else{
			if($get_count > 0){	
				$msg= "$LANG_delcat_acctexist";	
				$siteyn="Y";
			}else{	
					$msg= "$LANG_delcat_sure";	
					$siteyn="N";
			}
		}	
			
			$page = new Page('../template/admin_delcat.php');
