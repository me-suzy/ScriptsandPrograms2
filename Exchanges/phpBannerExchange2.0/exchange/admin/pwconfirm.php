...
$pass=$get_userinfo[adminpass];
	...
	session_destroy();
		...
		if($pass1 != $pass2){	
			$err = 1;		
			$error_html .= "$LANG_pwconfirm_err_mismatch\n";	
		}	

		if(strlen($pass1) < 4){		
			$err = 1;		
			$error_html .= "$LANG_pwconfirm_err_short<br><br>\n";	
		}
		...
		if($err=="1"){			
			...
			$page->output();	
				...
				if($usemd5 == Y){		
					$encpass = md5($pass1);			
					$insert=mysql_query("update banneradmin set adminpass='$encpass' where id='$id'");					}else{			
						$insert=mysql_query("update banneradmin set adminpass='$pass1' where id='$id'");
				}		
						$message="<br><br>$LANG_pwconfirm_success<br><br>";		
						
						$page = new Page('../template/admin_pwconfirm.php');
						$page->replace_tags(array(
							'css' => "$css",
							'session' => "$session",
							'baseurl' => "$baseurl",
							'title' => "$exchangename - $LANG_editpass_title",
							'shorttitle' => "$LANG_editpass_title",
							'message' => "$message",
							'menu' => 'admin_menuing.php',
							'footer' => '../footer.php'));
						$page->output();
