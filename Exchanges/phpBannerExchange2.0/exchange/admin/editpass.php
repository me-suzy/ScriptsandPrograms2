...
$pass=$get_userinfo[adminpass];
	...
	session_destroy();
		...
		$id_stuff = mysql_query("select id from banneradmin where adminuser='$login' AND adminpass='$pass'");
		$get_id_stuff=mysql_fetch_array($id_stuff);	
		$id=$get_id_stuff[id];
		$page = new Page('../template/admin_editpass_form.php');
		...
		$page->replace_tags(array(
			...
			'id' => "$id",
			'newpass' => "$LANG_editpass_newpass",
			'newpass1' => "$LANG_editpass_newpass1",
			'button' => "$LANG_editpass_button",
			'reset' => "$LANG_editpass_reset",
