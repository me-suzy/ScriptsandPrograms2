...
$pass=$get_userinfo[adminpass];
	...
	session_destroy();
		...
		$get_path=$_SERVER['SCRIPT_FILENAME'];	
		$server_path = ereg_replace("dbrestore.php","",$get_path);	
		$filename=$server_path."db/$file_name";
		exec("mysql -h$dbhost -u$dbuser -p$dbpass $dbname < $filename");
		
		header("Location: dbtools.php?SID=$session");
	}	
?>