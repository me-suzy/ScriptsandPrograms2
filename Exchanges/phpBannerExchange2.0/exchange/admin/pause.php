...
$pass=$get_userinfo[adminpass];
	...
	session_destroy();
		...
		$get_status=mysql_fetch_array($status);	$status=$get_status[data];
		if($status=="0"){
			mysql_query("update bannerconfig set data='1' where name='exchangestate'");
		}	
		if($status=="1"){	
				mysql_query("update bannerconfig set data='0' where name='exchangestate'");	
		}	
		header("Location: stats.php?SID=$session");
	}
