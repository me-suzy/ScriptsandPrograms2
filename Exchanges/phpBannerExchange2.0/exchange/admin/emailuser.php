...
$id=$_REQUEST[id];
	...
	$user=mysql_query("select email from banneruser where id='$id'");
	$get_user=mysql_fetch_array($user);	
	$email=$get_user[email];
	
	$page = new Page('../template/admin_emailuser.php');
	$page->replace_tags(array(
		'css' => "$css",
		'session' => "$session",
		'baseurl' => "$baseurl",
		'title' => "$exchangename - $LANG_email_button_send",
		'shorttitle' => "$LANG_email_button_send",
