...
$pass=$get_userinfo[adminpass];
	...
	session_destroy();
		...
		$get_cat=mysql_query("select * from bannercats where id='$catid'");
		$get_rows=mysql_fetch_array($get_cat);	
		$catname=$get_rows[catname];

		$page = new Page('../template/admin_editcat_form.php');
		$page->replace_tags(array(
			'css' => "$css",
			'session' => "$session",
			'baseurl' => "$baseurl",
