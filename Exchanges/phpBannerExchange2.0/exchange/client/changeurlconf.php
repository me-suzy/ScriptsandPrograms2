...
if($login=="" AND $pass=="" OR $pass=="") {
	...
	mysql_query("update bannerurls set targeturl='$_REQUEST[targeturl]' where id='$bid'");
	mysql_query("update bannerstats set approved=0 where uid='$id'");
	$msg="$LANG_changeurl_msg";
	}

	$page = new Page('../template/client_changeurl_success.php');
	$page->replace_tags(array(
	'css' => "$css",
	'session' => "$session",
	'baseurl' => "$baseurl",
	'title' => "$exchangename - $LANG_changeurl_title",
	'shorttitle' => "$LANG_changeurl_title",
	'msg' => "$msg",
	'targeturl' => "$targeturl",
	'back' => "$LANG_back",
	'footer' => '../footer.php',
