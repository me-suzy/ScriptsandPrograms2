...
if($num_cats > 0){
	...
	$get_row_category=eregi_replace("_"," ",$get_rows[catname]);
		 if ($get_row_id==$catno) { 
	$catwhile.="<option selected value=\"".$get_row_id."\">".$get_row_category."</option>"; 
	}else{ 
	$catwhile.= "<option value=\"".$get_row_id."\">".$get_row_category."</option>"; 
	} 
	}
	}else{ 
	$catwhile= "$LANG_cat_nocats";
	}

	$page = new Page('../template/client_cat_edit.php');
	$page->replace_tags(array(
	'css' => "$css",
	'session' => "$session",
	'baseurl' => "$baseurl",
	'title' => "$exchangename - $LANG_menu_cat",
	'shorttitle' => "$LANG_menu_cat",
	'catwhile' => "$catwhile",
	'catchange' => "$LANG_menu_cat",
	'warning' => "$LANG_cat_reval_warn",
	'button' => "$LANG_cat_change_button",
	'footer' => '../footer.php',
