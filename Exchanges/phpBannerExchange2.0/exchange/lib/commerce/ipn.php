...
$timestamp=time();
$query = mysql_query("insert into bannersales values('$invoice','$custom','$item_number','$payment_status','$payment_gross','$payer_email','$timestamp')");
$query = mysql_query("UPDATE bannercommerce SET purchased = purchased +1 WHERE productid='$item_number'");
//payout
$creditdue=mysql_query("select credits from bannercommerce where productid='$item_number'");
$creditsdue=mysql_fetch_array($creditdue);
$paidcredits=$creditsdue[credits];
$query = mysql_query("update bannerstats set credits=credits+$paidcredits where uid='$custom'");
