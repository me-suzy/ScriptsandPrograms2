...
**************************************/
       ...
       // filter
        ...
        if ($affiliateID!=0){    $where.=" AND affiliateID='".$affiliateID."' ";       }
        else {    $where.=" AND affiliateID<>0 ";       }
	...
	$allLinkColumns = array(
		0=>array("name"=>"lc.affiliateID", "alias"=>"affiliateID", "title"=>"affiliate","select"=>"lc.affiliateID as affiliateID",),
	);
	...
	for ($i=0;$i<$detailsLink;$i++){
		$listingColumns[]=$allLinkColumns[$i];
	}
...
$ss=array();
	...
	for ($i=0;$i<sizeof($stats);$i++){
		...
		$member = getMember($stat["affiliateID"],"affiliate");
				
		$stat["m"] = $months[$stat["m"]];
		$stat["d"] = $stat["d"]==0?"all days":$stat["d"];
		$stat["y"] = $stat["y"]==0?"all":$stat["y"];

		$stat["affiliateID"] = $stat["affiliateID"]==0?"all affiliates":($member["info"]["firstName"]." ".$member["info"]["lastName"]);
    ...
    $tpl->display("admin/template.admin.affclickstats.php");
