...
/**
admin statistics 
$statsMode = clickstats, keywordstats, bidstats
*/
...
**************************************/
        ...
        // affiliates
        $dbSet->open("SELECT * FROM affiliates" );
        $affiliates = $affiliateIDs = $affiliateNames = array();
        while ($row=$dbSet->fetchArray()){
        	$affiliateIDs[] = $row["affiliateID"];
        	$affiliateNames[] = $row["firstName"]." ".$row["lastName"];
        }
        $tpl->assign("affiliateIDs",$affiliateIDs);
        $tpl->assign("affiliateNames",$affiliateNames);
       ...
       // filter
		...
		// filter member
		if (!isset($memberID)) $memberID=0;
        if ($memberID!=0){    $where.=" AND memberID='".$memberID."' ";       }
		...
		if ($logType=="banner" ){   $urlID=$linkID=0;		}
	...
	if ($logType=="banner"){
		if ($detailsLink>2) $detailsLink=2;
	...
	$allLinkColumns = array(
		0=>array("name"=>"u.memberID", "alias"=>"memberID", "title"=>"member","select"=>"u.memberID as memberID",),
		1=>array("name"=>"l.urlID", "alias"=>"urlID", "title"=>"url","select"=>"l.urlID as urlID",),
		2=>array("name"=>"l.linkID", "alias"=>"linkID", "title"=>"link","select"=>"l.linkID as linkID"),
		3=>array("name"=>"l.keywordID", "alias"=>"keywordID", "title"=>"keyword","select"=>"l.keywordID as keywordID"),
	);
	$allBannerColumns = array(
		0=>array("name"=>"b.memberID", "alias"=>"memberID", "title"=>"member","select"=>"b.memberID as memberID",),
		1=>array("name"=>"b.bannerID", "alias"=>"bannerID", "title"=>"banner","select"=>"b.bannerID as bannerID",),
	);
...
$ss=array();
	...
	for ($i=0;$i<sizeof($stats);$i++){
		...
		$member = getMember($stat["memberID"],"member");
		
		// map values
		// link
		if ($logType=="link"){
			//$urlID = getUrlOfLink($stat["linkID"]);
			...
			$stat["memberID"] = $stat["memberID"]==0?"all members":($member["info"]["firstName"]." ".$member["info"]["lastName"]);
		...
		//banner
			...
			//$stat["bannerID"] = $stat["bannerID"]==0?"all banners":pathToName($banner["path"]);
...
//    $tpl->assign("items",$items);
    ...
    $tpl->display("admin/template.admin.clickstats.php");
