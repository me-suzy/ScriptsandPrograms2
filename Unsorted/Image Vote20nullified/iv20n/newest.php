...
if (!isset($lastnumber)) $lastnumber  = 10;  // number of listings to display on the top list
...
if ($c == "all") 
$result=mysql_query("SELECT name, id, url, MAX(average) as average, total FROM $imagetable WHERE status = 'active' group by name order by id DESC LIMIT $n,$lastnumber") or die(mysql_error());
else 
$result=mysql_query("SELECT name, id, url, MAX(average) as average, total FROM $imagetable WHERE category = '$c' and status = 'active' group by name order by id DESC LIMIT $n,$lastnumber") or die(mysql_error());
$rows = mysql_num_rows($result);
$rows--;


for ($num = 0; $num <= $lastnumber; $num++) {
if ($rows >= $num) {
...
${$num."pic"} = "<img src=\"".${$num."pic"}."\" width=\"80\" border=0>\n";
}
else {
...
<? } ?>
        ...
        <p><? if ($n>=10) print "<a href=\"$gophp?go=newphp&c=$c&amp;w=$w&amp;n=$n1\">".PREVIOUS."</a> -";
         print " <a href=\"$gophp?go=newphp&c=$c&amp;w=$w&amp;n=$n2\">".NEXT."</a><br>";  ?></p>
       <? print CLICKIMG; ?><br>
        <a href="<?=$votephp?>"> <? print RETURNTO; ?></a>
<p> <? print ALSOVIEW;?><br>
<?
foreach ($categories as $a) {
print "<a href=\"$gophp?go=newphp&c=$a\">".LAST.OF.ucfirst($a)."</a><br>";
}
print "<a href=\"$gophp?go=newphp\">".LAST.OF.ALL."</a><br>";
?>
      </p>
        </div>
<? if (!$go) {?>
