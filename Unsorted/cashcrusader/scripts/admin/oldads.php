...
echo "<title>Old Email Ads Report</title><script>window.focus()</script>
<STYLE TYPE=\"text/css\">
  <!--
...
<center><h2>Old Email Ads Report</h2><hr></center><form method=post>List ads that have that have not been clicked in <input type=text name=days value=$days> days. <br><input type=submit name=report value=Report></form>";
if ($connum){
echo "<br><br>";
if ($connum!=$md5time){
...
echo "<table border=1 class=fsize2><tr><td><b>Delete</b></td><td><b>Ad ID</b></td><td><b>Description</b></td><td><b>Type</b></td><td><b>Expires at</b></td><th>Clicks</th><td><b>Last Click</b>";
...
echo "</td></tr><tr $bgcolor><td><input type=checkbox name=\"uid[$row[emailid]]\" value=1 $checked></td><td>$row[emailid]</a></td><td>$row[description]</td><td>$row[run_type]</td><td>$row[run_quantity]</td><td>$row[clicks]</td><td>".mytimeread($row[time]);
