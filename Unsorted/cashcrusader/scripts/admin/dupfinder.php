...
admin_login();
echo "<title>List accounts that DO belong to cheaters</title><script>window.focus()</script>
<STYLE TYPE=\"text/css\">
  <!--
...
<center><h2>List accounts that DO belong to cheaters</h2><hr></center>The list of accounts below match so exactly they can only belong to cheaters. But of course use your own judgement<br>";
if ($connum){
echo "<br><br>";
if ($connum!=$md5time){
...
$report=@mysql_query("select password,pay_account,referrer,signup_ip_host,first_name,last_name,count(*) as count from ".$mysql_prefix."users group by signup_ip_host,password,pay_account,referrer,first_name,last_name order by count desc");
...
$userreport=@mysql_query("select * from ".$mysql_prefix."users where password='$getlist[password]' and pay_account='$getlist[pay_account]' and referrer='$getlist[referrer]' and signup_ip_host='$getlist[signup_ip_host]' and first_name='$getlist[first_name]' and last_name='$getlist[last_name]'");
