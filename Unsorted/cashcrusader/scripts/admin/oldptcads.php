...
echo "<title>Old PTC Ads Report</title><script>window.focus()</script>
<STYLE TYPE=\"text/css\">
  <!--
...
<center><h2>Old PTC Ads Report</h2><hr></center><form method=post>List ads that have that have not been shown in <input type=text name=days value=$days> days. <br><input type=submit name=report value=Report></form>";
if ($connum){
echo "<br><br>";
if ($connum!=$md5time){
...
echo "</td></tr><tr $bgcolor><td><input type=checkbox name=\"uid[$row[ptcid]]\" value=1 $checked></td><td>$row[ptcid]</a></td><td>$row[description]</td><td>$row[run_type]</td><td>$row[run_quantity]</td><td>$row[views]</td><td>$row[clicks]</td><td>".mytimeread($row[time]);
