	...
	$result = queryDB( "SELECT uid,password,admin,status,logintime FROM $tbl_userinfo WHERE uid='$uidform' && status = '1' && (validity >= $now_date || validity = '0')" );
    	...
    	if ($nr > 0)
    	{
		   ...
		   if (md5($password) == $row[password])
		    ...
		    $redurl = "$Config_mainurl/user/index.php";
		    ...
		    $strLoginWelcome = $csr->LangConvert($strLoginWelcome, $row[uid]);
...
_HTML_END_;
		...
		{
			$result = queryDB( "SELECT uid,status,validity FROM $tbl_userinfo WHERE uid='$uidform'" );
	            $row = mysql_fetch_array($result);
