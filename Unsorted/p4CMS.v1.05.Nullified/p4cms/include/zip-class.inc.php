...
function mkdir_p($target){ 
 ...
 function ExtractFile($header,$to,$zip)
 {
   ...
   $fl = "";
   $la = 0;
   $ams = explode("/", $header['filename']);
   while (list($key,$val) = each($ams)) {
   		$la++;
   		if ($la != count($ams))
   			$fl .= "/" . $val;
   }
   ...
   if (!file_exists($fl))
   		$this->mkdir_p(substr($to,0,strlen($to)-1) . $fl);  
   
   $splot = explode("/", $header['filename']);
   $dir = $to . substr($header['filename'], 0, strlen($header['filename']) - strlen($splot[count($splot)-1]));
   $dr = "$to/";
   for ($i=0;$i<=count($splot)-1;$i++) {
   	$dr .= $splot[$i] . "/";
   	$ou = umask(0);
   	@chmod ($dr, 0777);
   	umask($ou);
  ...
  if (!($header['external']==0x41FF0010)&&!($header['external']==16))
  {
   ...
   $ou = umask(0);
   @chmod($to.$header['filename'],0777);
   @chmod($to.$header['filename'].'.gz',0777);
   umask($ou);
   gzclose($gzp);
