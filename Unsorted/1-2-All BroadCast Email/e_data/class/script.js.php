  ...
  {
    var found = false;
    for(i=0;i<visEdit_editors.lenght;i++)
    {
      if (visEdit_editors[i] == editor)
      {
        found = true;
        break;
      }
  ...
  // onsubmit
  function visEdit_UpdateFields()
  {
    for (i=0; i<visEdit_editors.length; i++)
    {
      visEdit_updateField(visEdit_editors[i], null);
    }
  }
  
  // adds event handler for the form to update hidden fields
  function visEdit_addOnSubmitHandler(editor)
  {
    thefield = visEdit_getFieldByEditor(editor, null);
    ...
    if (sTemp.indexOf("visEdit_UpdateFields();") == -1)
    {
      oForm.onsubmit = new Function("visEdit_UpdateFields();" + sTemp);
    }
  }

  // editor initialization
  function visEdit_editorInit(editor, css_stylesheet, direction)
  {
    // prevent from executing twice on the same editor
    if (!visEdit_editor_registered(editor))
    {
      // check if the editor completely loaded and schedule to try again if not
      if (document.readyState != 'complete')
      {
        setTimeout(function(){visEdit_editorInit(editor, css_stylesheet, direction);},20);
        return;
      }
      ...
      // register the editor 
      visEdit_editors[visEdit_editors.length] = editor;
      
      // add on submit handler
      visEdit_addOnSubmitHandler(editor);
	...
	visEdit_toggle_borders(editor,this[editor+'_rEdit'].document.body,null);
        ...
        this[editor+'_rEdit'].document.onkeyup = function() { visEdit_onkeyup(editor); }
        this[editor+'_rEdit'].document.onmouseup = function() { visEdit_update_toolbar(editor, true); }
        
        // initialize toolbar
        visEdit_context_html = "";
  ...
  function visEdit_showColorPicker(editor,curcolor) {
    return showModalDialog('e_data/dialogs/colorpicker.php?lang=' + document.all['visEdit_'+editor+'_lang'].value + '&theme=' + document.all['visEdit_'+editor+'_theme'].value, curcolor, 
  ...
  function visEdit_image_insert_click(editor, sender)
  {
    ...
    var imgSrc = showModalDialog('e_data/dialogs/img_library.php?lang=' + document.all['visEdit_'+editor+'_lang'].value + '&theme=' + document.all['visEdit_'+editor+'_theme'].value, '', 
  ...
  function visEdit_image_prop_click(editor, sender)
  {
    ...
    if (im)
    {
      ...
      var niProps = showModalDialog('e_data/dialogs/img.php?lang=' + document.all['visEdit_'+editor+'_lang'].value + '&theme=' + document.all['visEdit_'+editor+'_theme'].value, iProps, 
  ...
  function visEdit_table_create_click(editor, sender)
  {
    if (window.frames[editor+'_rEdit'].document.selection.type != "Control")
    {
      ...
      var nt = showModalDialog('e_data/dialogs/table.php?lang=' + document.all['visEdit_'+editor+'_lang'].value + '&theme=' + document.all['visEdit_'+editor+'_theme'].value, null, 
        'dialogHeight:250px; dialogWidth:366px; resizable:no; status:no');  
      ...
      if (nt)
      {
        ...
        try {
          ...
          visEdit_toggle_borders(editor, window.frames[editor+'_rEdit'].document.body, null);
          visEdit_update_toolbar(editor, true);    
        }
        catch (excp)
        {
  ...
  function visEdit_table_prop_click(editor, sender)
  {
    window.frames[editor+'_rEdit'].focus();     
    ...
    var ntProps = showModalDialog('e_data/dialogs/table.php?lang=' + document.all['visEdit_'+editor+'_lang'].value + '&theme=' + document.all['visEdit_'+editor+'_theme'].value, tProps, 
      'dialogHeight:250px; dialogWidth:366px; resizable:no; status:no');  
  ...
  function visEdit_table_cell_prop_click(editor, sender)
  {
    ...
    if (cd)
    {
      ...
      var ncProps = showModalDialog('e_data/dialogs/td.php?lang=' + document.all['visEdit_'+editor+'_lang'].value + '&theme=' + document.all['visEdit_'+editor+'_theme'].value, cProps, 
  ...
  function visEdit_table_column_insert_click(editor, sender)
  {
    var ct = visEdit_getTable(editor); // current table
    var cr = visEdit_getTR(editor); // current row
  ...
  function visEdit_table_cell_merge_down_click(editor, sender)
  {
    var ct = visEdit_getTable(editor); // current table
    var cr = visEdit_getTR(editor); // current row
    var cd = visEdit_getTD(editor); // current row
  ...
  function visEdit_table_column_delete_click(editor, sender)
  {
    var ct = visEdit_getTable(editor); // current table
    var cr = visEdit_getTR(editor); // current row
    var cd = visEdit_getTD(editor); // current cell
  ...
  function visEdit_confirm(editor,block,message) {
    return showModalDialog('e_data/dialogs/confirm.php?lang=' + document.all['visEdit_'+editor+'_lang'].value + '&theme=' + document.all['visEdit_'+editor+'_theme'].value + '&block=' + block + '&message=' + message, null, 'dialogHeight:100px; dialogWidth:300px; resizable:no; status:no');  
  }
  ...
  function visEdit_cleanup_click(editor, sender)
  {
    ...
    visEdit_update_toolbar(editor, true);    
  } // visEdit_cleanup_click
  
  // toggle borders worker function
  function visEdit_toggle_borders(editor, root, toggle)
  {
    // get toggle mode (on/off)
    var toggle_mode = toggle;
    if (toggle == null)
    {
      var tgl_borders = document.getElementById("visEdit_"+editor+"_borders");
      if (tgl_borders != null)
      {
  ...
  } // visEdit_toggle_borders
  
  // toggle borders click event 
  function visEdit_toggle_borders_click(editor, sender)
  {
    ...
    if (tgl_borders != null)
    {
      ...
      visEdit_toggle_borders(editor,this[editor+'_rEdit'].document.body, toggle_mode.value);
    }
    visEdit_update_toolbar(editor, true);    
  } // visEdit_toggle_borders_click
  
  // returns base toolbar image name
  function visEdit_base_image_name(ctrl)
  {
  ...
  function visEdit_onkeyup(editor)
  {
  ...
  function visEdit_update_toolbar(editor, force)
  {
    ...
    {
      visEdit_toggle_tbi_state(editor, standard_cmd_items[i][1], edoc.queryCommandState(standard_cmd_items[i][0]));
    }
    for (i=0; i<standard_cmd_items.length; i++)
    {
      visEdit_toggle_tbi(editor, standard_cmd_items[i][1], edoc.queryCommandEnabled(standard_cmd_items[i][0]));
    }
    
    // set state of toggle borders button
    if (document.all["visEdit_"+editor+"_borders"].value == "on")
    {
      visEdit_toggle_tbi_state(editor, "toggle_borders", true);
    }
    else
    {
      visEdit_toggle_tbi_state(editor, "toggle_borders", false);
    }
    
    // dropdowns
    for (i=0; i<standard_dropdowns.length; i++)
    {
      visEdit_toggle_tbi_dropdown(editor, standard_dropdowns[i][1], edoc.queryCommandValue(standard_dropdowns[i][0]));
    }
    // style dropdown
    var pt = visEdit_getParentTag(editor);
    visEdit_toggle_tbi_dropdown(editor, "style", pt.className);
  }
  
  // enable/disable toolbar item
  function visEdit_toggle_tb_items(editor, items, enable)
  {
    for (i=0; i<items.length; i++)
    {
      visEdit_toggle_tbi(editor, items[i], enable);
    }
  }
  
  // enable/disable toolbar item
  function visEdit_toggle_tbi(editor, item, enable)
  {
    if (document.all["visEdit_"+editor+"_tb_"+item])
    {
      var ctrl = document.all["visEdit_"+editor+"_tb_"+item];
      if (enable)
      {
        if (ctrl)
        {
          eval("visEdit_"+document.all["visEdit_"+editor+"_theme"].value+"_bt_out(ctrl);");
        }
      }
      else
      {
        if (ctrl)
        {
          eval("visEdit_"+document.all["visEdit_"+editor+"_theme"].value+"_bt_off(ctrl);");
        }
      }
    }
  }
  ...
  function visEdit_toggle_tbi_state(editor, item, state)
  {
    if (document.all["visEdit_"+editor+"_tb_"+item])
    {
      var ctrl = document.all["visEdit_"+editor+"_tb_"+item];
      ctrl.setAttribute("visEdit_state",state)
      eval("visEdit_"+document.all["visEdit_"+editor+"_theme"].value+"_bt_out(ctrl);");
    }
  }
  
  // set dropdown value
  function visEdit_toggle_tbi_dropdown(editor, item, value)
  {
    if (document.all["visEdit_"+editor+"_tb_"+item])
    {
      var ctrl = document.all["visEdit_"+editor+"_tb_"+item];
