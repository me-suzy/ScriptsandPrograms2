...
class Template {
  ...
  function pparse_js($block_name, $is_repeat)
  {
    $this->parse($block_name, $is_repeat);
    $parsed_values=$this->ParsedBlocks[$block_name];
    $parsed_values = str_replace("\r", "\n", $parsed_values);
    $parsed_values = str_replace("\\", "\\\\", $parsed_values);
    $parsed_values = str_replace('\'', '\\\'', $parsed_values);
    $parsed_values = str_replace("\"", "\\\"", $parsed_values);
    $parsed_values = str_replace("</script", "<' + '/script", $parsed_values);
    $parsed_values = str_replace("</SCRIPT", "<' + '/SCRIPT", $parsed_values);
    $parsed_values = str_replace("<script", "<' + 'script", $parsed_values);
    $parsed_values = str_replace("<SCRIPT", "<' + 'SCRIPT", $parsed_values);
    $parsed_values=split("\n", $parsed_values);
    for ($l=0;$l<sizeof($parsed_values);$l++){
    	echo "document.write('".$parsed_values[$l]."\\r\\n');\n";
    }
  }

  function preturn($block_name, $is_repeat)
  {
    $this->parse($block_name, $is_repeat);
