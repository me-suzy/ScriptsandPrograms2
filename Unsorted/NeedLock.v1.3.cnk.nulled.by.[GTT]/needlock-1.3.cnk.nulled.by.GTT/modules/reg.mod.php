  ...
  function Reg() {
     ...
     if ( !$needsecure->input['code'] || empty($needsecure->input['code']) ) {
      // Registration
	   ...
	   (id,name,password,plain_password,regdate,email,realname,lang,member_ip,member_browser,count_visits,access_dirs,suspended,approved,authcode)
	   VALUES
	   ('{$new_member['id']}','{$new_member['name']}','{$new_member['password']}','{$new_member['plain_password']}','{$new_member['regdate']}','{$new_member['email']}','{$new_member['realname']}','{$new_member['lang']}','{$new_member['member_ip']}','{$new_member['user_agent']}','1','','0','{$new_member['approved']}','{$new_member['authcode']}')";
	   ...
	   if ( $INFO['REG_AUTH_CODES'] == 1 ) {
	      ...
	      $tpl->set_var("words_reg_done",$needsecure->words['reg_done_without_authcode_without_approve']);
           ...
           // If notify admin set to 1, send admin mail
	      if ( $INFO['NOTIFY_NEW_REGS'] == '1' ) {

                 if ( $INFO['REG_AUTH_CODES'] == 1 ) {
                    $new_member['activation'] = "User need to activate it's account";
		 		} else {
                    $new_member['activation'] = "User don't need to activate it's account";
		 		}

		 		if ( $INFO['REG_ADMIN_APPROVE'] == 1 ) {
                    $new_member['approving'] = "Account is waiting for administrator approving";
		 		} else {
                    $new_member['approving'] = "Account isn't waiting for administrator approving";
		 		}

                 $new_member['admincp_link'] = "{$INFO['BASE_URL']}admin{$INFO['PHP_EXT']}";

		 		 //require ("{$needsecure->dirs['INC']}mail.class{$INFO['PHP_EXT']}");
                 //$mailer = new mailer($INFO['EMAIL_SENDER']);
                 $mailer->setFrom($INFO['EMAIL_OUT']);
                 $mailer->setTo($INFO['ADMIN_EMAIL']);
                 $mailer->setMessage("admin_reg_notify",$new_member );
                 $mailer->setSubj("New registration");
                 $mailer->send_mail();
