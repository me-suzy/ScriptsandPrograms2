...
function error_handling($error_page_name, $error_message)
{
	echo '<table width="500"  border="1" cellpadding="0" cellspacing="0" bordercolor="#000000">
		  ...
		  <tr>
		  <td height="20" bgcolor="#E3E8EF"><font face="Arial, Helvetica, sans-serif" size="2"><b>Îøèáêà: </b></font></td>
		  </tr>
		  <tr>
		  <td><font face="Arial, Helvetica, sans-serif" size="2">Âîçíèêëà îøèáêà â '.$error_page_name.':<br /><br /><i>'.$error_message.'</i></font></td>
		  </tr>
		  </table></td>
		  </tr>
		  </table>';
...
function get_config($where)
{
	...
	if (!$result)
	{
		$error_message = 'Îøèáêà â áàçå äàííûõ - '.mysql_errno().': '.mysql_error().' in get_config()';
...
function admin_login($username, $password)
{
	...
	if (!$result)
	{
		$error_message = 'Îøèáêà â áàçå äàííûõ - '.mysql_errno().': '.mysql_error().' in admin_login()';
		error_handling('includes/functions_admin.php', $error_message);
	}
...
function admin_license()
{
	...
	if (!$license)
	{
		echo '<p><b>Îøèáêà:</b></p>Íå ìîãó îòêðûòü ôàéë ñ ëèöåíçèåé ïî àäðåñó http://www.olate.com/legal/license_text.php.';
	}
}
...
function admin_update($config)
{
	...
	if (!$update)
	{
		echo '<p><b>Îøèáêà:</b></p>Íåâîçìîæíî ñîåäèíèòüñÿ ñ ñåðâåðîì ïî àäðåñó http://www.olate.com/scripts/updates.php?script=1&version='.urlencode($config['version']).'.';
	}
}
...
function admin_generalsettings_update($language, $urlpath, $topdownloadslink, $alldownloads, $searchlink, $sorting, $ratings, $notopdownloads, $pages)
{
	...
	if (!$result)
	{
		$error_message = 'Îøèáêà â áàçå äàííûõ - '.mysql_errno().': '.mysql_error().' in admin_generalsettings_update()';
		error_handling('includes/functions_admin.php', $error_message);
	}
...
function admin_languages_update($language)
{
	...
	if (!$result)
	{
		$error_message = 'Îøèáêà â áàçå äàííûõ - '.mysql_errno().': '.mysql_error().' in admin_languages_update()';
		error_handling('includes/functions_admin.php', $error_message);
	}
}
...
function admin_downloads_add_catmenu()
{
	...
	if (!$result)
	{
		$error_message = 'Îøèáêà â áàçå äàííûõ - '.mysql_errno().': '.mysql_error().' in admin_downloads_add_catmenu()';
		error_handling('includes/functions_admin.php', $error_message);
	}
...
function admin_downloads_edit_select()
{
	...
	if (!$result)
	{
		$error_message = 'Îøèáêà â áàçå äàííûõ - '.mysql_errno().': '.mysql_error().' in admin_downloads_edit_select()';
		error_handling('includes/functions_admin.php', $error_message);
	}
	...
	if ($num_results == 0)
	{
		echo 'Â áàçå äàííûõ íå íàéäåíî ôàéëîâ. Âîçìîæíî ñòîèò äîáàâèòü ïàðî÷êó.';
		return;
	}
...
function admin_downloads_edit_view($id)
{
	...
	if (!$result)
	{
		$error_message = 'Îøèáêà â áàçå äàííûõ - '.mysql_errno().': '.mysql_error().' in admin_downloads_edit_view()';
		error_handling('includes/functions_admin.php', $error_message);
	}
...
function admin_downloads_edit_view_cat($category)
{
	...
	if (!$result)
	{
		$error_message = 'Îøèáêà â áàçå äàííûõ - '.mysql_errno().': '.mysql_error().' in admin_downloads_edit_view_cat()';
		error_handling('includes/functions_admin.php', $error_message);
	}
...
function admin_downloads_edit($language, $id, $date, $name, $location, $size, $category, $description_brief, $description_full, $custom_1_l, $custom_1_v, $custom_2_l, $custom_2_v, $image)
{
	...
	if (!$result)
	{
		$error_message = 'Îøèáêà â áàçå äàííûõ - '.mysql_errno().': '.mysql_error().' in admin_downloads_edit()';
		error_handling('includes/functions_admin.php', $error_message);
	}
...
function admin_downloads_delete_select()
{
	...
	if (!$result)
	{
		$error_message = 'Îøèáêà â áàçå äàííûõ - '.mysql_errno().': '.mysql_error().' in admin_downloads_delete_select()';
		error_handling('includes/functions_admin.php', $error_message);
	}
...
function admin_downloads_delete($id)
{
	...
	if (!$result)
	{
		$error_message = 'Îøèáêà â áàçå äàííûõ - '.mysql_errno().': '.mysql_error().' in admin_downloads_delete()';
		error_handling('includes/functions_admin.php', $error_message);
	}
}
...
{	
	...
	if (!$result)
	{
		$error_message = 'Îøèáêà â áàçå äàííûõ - '.mysql_errno().': '.mysql_error().' in admin_users_add()';
		error_handling('includes/functions_admin.php', $error_message);
	}
...
function admin_users_delete_select()
{
	...
	if (!$result)
	{
		$error_message = 'Îøèáêà â áàçå äàííûõ - '.mysql_errno().': '.mysql_error().' in admin_users_delete_select()';
		error_handling('includes/functions_admin.php', $error_message);
	}
...
function admin_users_delete($language, $id)
{
	...
	if (!$result)
	{
		$error_message = 'Îøèáêà â áàçå äàííûõ - '.mysql_errno().': '.mysql_error().' in admin_users_delete()';
		error_handling('includes/functions_admin.php', $error_message);
	}
...
function admin_categories_add($language, $name)
{
	...
	if (!$result)
	{
		$error_message = 'Îøèáêà â áàçå äàííûõ - '.mysql_errno().': '.mysql_error().' in admin_categories_add()';
		error_handling('includes/functions_admin.php', $error_message);
	}
...
function admin_categories_edit_select()
{
	...
	if (!$result)
	{
		$error_message = 'Îøèáêà â áàçå äàííûõ - '.mysql_errno().': '.mysql_error().' in admin_categories_edit_select()';
		error_handling('includes/functions_admin.php', $error_message);
	}
	...
	if ($num_results == 0)
	{
		echo 'Â áàçå äàííûõ íå íàéäåíû ôàéëû. Ìîæåò ñòîèò äîáàâèòü ïàðî÷êó.';
		return;
	}
...
function admin_categories_edit_view($id)
{
	...
	if (!$result)
	{
		$error_message = 'Îøèáêà â áàçå äàííûõ - '.mysql_errno().': '.mysql_error().' in admin_categories_edit_view()';
		error_handling('includes/functions_admin.php', $error_message);
	}
...
function admin_categories_edit($language, $id, $name)
{
	...
	if (!$result)
	{
		$error_message = 'Îøèáêà â áàçå äàííûõ - '.mysql_errno().': '.mysql_error().' in admin_categories_edit()';
		error_handling('includes/functions_admin.php', $error_message);
	}
...
function admin_categories_delete_select()
{
	...
	if (!$result)
	{
		$error_message = 'Îøèáêà â áàçå äàííûõ - '.mysql_errno().': '.mysql_error().' in admin_categories_delete_select()';
		error_handling('includes/functions_admin.php', $error_message);
	}
...
function admin_categories_delete($id)
{
	...
	if (!$result)
	{
		$error_message = 'Îøèáêà â áàçå äàííûõ - '.mysql_errno().': '.mysql_error().' in admin_categories_delete()';
		error_handling('includes/functions_admin.php', $error_message);
	}
}
