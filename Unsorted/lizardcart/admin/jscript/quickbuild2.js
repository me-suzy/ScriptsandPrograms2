...
/********************** EXTRA CONFIGUARION ***************************/
var DFFACE=''; // 'times new roman'; // Default fontFamily of Editor
var DFSIZE=''; // '14px'; // Default fontSize
var DCOLOR=''; // 'blue'; // Default color
var DBGCOL=''; // 'green'; // Default backgroundColor
var DBGIMG=''; // '/imgedit/editorbg.jpg' ; // Default URL-backgroundImage 
var DCSS=''; // 'test.css'; // Default-Stylesheet-URL
var SYMBOLE='\n' ; // Symbole for end-of-line in clipboard-chipcard, may not "\n"
/*********************** END CONFIGUARION ****************************/
...
window.onbeforeunload=actualize
...
<TD nowrap>\
<SELECT name='QBCNTRL1' onchange='doFormatF(\"FontName,\"+this.value)' style='height:22; width:145; background:#a0a080; color:#FFFFFF; cursor:hand'>\
...
<INPUT name='QBCNTRL8' value='Options' onclick='doEditorOptions()' type=button style='height:22; width:74;background:#a0a080; border-color:#f0f0c0; color:#FFFFFF'>\
</TD></TR>"
   ...
   for(fidx=0; fidx<document.forms.length; fidx++)
    {
     ...
     for(var i=0; i<oForm.elements.length; i++)
     {
      ...
      strx1 += toUnicode(el.value) + SYMBOLE ;
     }
	}
...
function editorContents(frameID)
{
  ...
  if(DEFCSS[frameID] && DEFCSS[frameID].href) strx += '<style>@import url("'+DEFCSS[frameID].href+'");</style>';

  var defdiv="" ;
  if(DEFFFACE[frameID]) defdiv += "; FONT-FAMILY:"+ DEFFFACE[frameID] 
  if(DEFFSIZE[frameID]) defdiv += "; FONT-SIZE:"+ DEFFSIZE[frameID]
  if(DEFCOLOR[frameID]) defdiv += "; COLOR:"+ DEFCOLOR[frameID]
  if(DEFBCOLOR[frameID])defdiv += "; BACKGROUND-COLOR:"+ DEFBCOLOR[frameID]
  if(DEFBIMAGE[frameID])
	{
     DEFBIMAGE[frameID]= DEFBIMAGE[frameID].replace(/\\/g,"/"); 
	 defdiv += "; BACKGROUND-IMAGE:url("+ DEFBIMAGE[frameID]+")"
    }
  if(defdiv)
	{
...
if(VISUAL==0) changeAllTextareaToEditors();
