...
if($comments_in_comment  > $comments_replies_per_page){
	$nav_url = "$base_url/view_comment.php?c=$comment_id&";
...
$comment_user_name = get_username($comment_user_id);

if(check_approved_image($comment_user_id)){
$comment_user_name = <<<EOF
<a class="bold" href="$base_url/?i=$comment_user_id">$comment_user_name</a>
EOF;
}
...
<tr>
	<td class="bold">&nbsp;<a class="bold" href="$base_url/">$site_title</a> >> <a class="bold" href="$base_url/comments.php">Comments</a> >> $comment_subject</td>
</tr>
<tr>
...
if($array["author_id"] > 0){
	...
	if(strlen($author_name)){
		$comment_author_link = $author_name;
		if(check_approved_image($array["author_id"])){
$comment_author_link = <<<EOF
<a class="smallbold" href="$base_url/?i=$array[author_id]">$author_name</a>
EOF;
		}
	} else {

$comment_author_link = <<<EOF
<span class="bold">Username Not Found</span>
EOF;
...
$comments = get_author_comments_count($array["author_id"]);
...
if(comment_is_editable($array["id"], $userid)){
