...
for($x=0; $x<=30; $x++){
	...
	$sql = "
		...
		where
			user_id = '$_GET[i]'
		and
			timestamp < '$days_ago[$x]'
		and
			timestamp >= '$days_ago[$x2]'
	";
	$query = mysql_query($sql) or die(mysql_error());
	$sum = (int) mysql_result($query, 0, "sum");
	$datay[] = $sum;
	if($sum > $high_y) $high_y = $sum;
}

$graph = new Graph(640,480,"stats_im_user_points_month_" . $_GET['i'] . ".png",1440);
...
$graph->tabtitle->Set(' .: Points Past Month :. ' );
