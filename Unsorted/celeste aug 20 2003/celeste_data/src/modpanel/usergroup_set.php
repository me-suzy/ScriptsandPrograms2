...
if (empty($_GET['usergroupid']) || !isInt($_GET['usergroupid'])) mod_exception_in("Invalid Usergroup ID");

if ($_GET['usergroupid']<=3)
mod_exception_in("Permission Denied");

if (!empty($_POST['submit'])) {

  if ($DB->result("select count(*) from celeste_permission where forumid=".$forumid." and usergroupid=".$_GET['usergroupid'])) {
    ...
    where forumid=".$forumid." and userid=0 and usergroupid=".$_GET['usergroupid']);
    ...
    allowhtml='".($_POST['allowhtml'] ? 1 : 0)."',
    allowsmiles='".($_POST['allowsmiles'] ? 1 : 0)."',
    forumid=".$forumid.",userid=0,usergroupid=".$_GET['usergroupid']);
  }
  mod_success_redirect_in('Permission(s) has been set.', 'fid='.$forumid.'&prog=usergroup::permission' );
}

$username =& $DB->result('select title from celeste_usergroup where usergroupid=\''.$_GET['usergroupid'].'\'');

$oldresult =& $DB->result('select * from celeste_permission where forumid='.$forumid.' and usergroupid=\''.$_GET['usergroupid'].'\'');
...
<br><br><br>
Set User Permission :  &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; [ <?=$username?> ]<br>
<hr size=1>
</td>
<td>
</td>
...
<tr><td width=50%> User Group ID : </td>
<td width=50%>
<?=$_GET['usergroupid']?></td></tr>

<tr><td width=50%> User Group Title : </td>
<td width=50%>
<?=$username?></td></tr>
