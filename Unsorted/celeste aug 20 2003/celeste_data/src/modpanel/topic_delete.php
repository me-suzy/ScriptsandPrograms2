<?php
if (!getpermission('deltopic')) mod_exception('Permission denied.');
mod_header("Prune Topic - Moderator Control Panel");
  ...
  if ($conditions>0 && !empty($_POST['confirm'])) {
    ...
    if (strlen($idlist)>0) {
       $idlist = substr($idlist, 0, -1);
	     $DB->update('delete from celeste_post where topicid in ('.$idlist.')');
       $posts = $DB->affected_rows();
    } else $posts = 0;

  	$query = 'Delete from celeste_topic where forumid=\''.$forumid.'\'  '.$query;
	  $DB->update($query);
