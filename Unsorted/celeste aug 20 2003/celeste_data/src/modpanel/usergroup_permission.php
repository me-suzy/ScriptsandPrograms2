...
mod_header("Browse User Group Permissions - Moderator Control Panel");
...
$permissions = $DB->query('select p.*,u.title from celeste_permission p,celeste_usergroup u where u.usergroupid=p.usergroupid and forumid=\''.$forumid.'\' and p.userid=0 and u.usergroupid>3');

$content = '';

while($dataRow=&$permissions->fetch()) {
    
  $content .= '<tr align=center><td><a href=\'modpanel.php?fid='.$forumid.'&prog=user::set&uid='.$dataRow['userid'].'\'><img src="images/mod/edit.gif" border=0> '.$dataRow['title'].'</a></td><td><img src="images/mod/check_'.
...
<tr><td colspan=12>
You are now browsing user group permissions in :
<br><br><b>   Forum (<?=$forumid?>) - <?=$forum->getProperty('title')?> </b>
<br><br>
<br>
User Group Permissions In this Forum : 
<br>
<hr size=1>
</td></tr>
<tr align=center>
...
<br>
<form action="modpanel.php" method=get>
<input type=hidden name=fid value=<?=$forumid?>>
<input type=hidden name=prog value="usergroup::set">
<table width=90% cellspacing=2 cellpadding=4 border=0 align=center>
<tr><td>[ <img src="images/mod/next.gif" border=0> Set User Group Permission ]</td></tr>
<tr><td><select name=usergroupid><?=getAllGroups();?></select> <input type=submit name=submit value=" GO "></td></tr>
</table>
<?
