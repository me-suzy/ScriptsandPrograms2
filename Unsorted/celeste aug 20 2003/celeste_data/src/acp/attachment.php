...
if(!empty($_POST['delete_submit'])) {
  ...
  if(!is_array($_POST['delAtt'])) {
    acp_success_redirect('The selected attachments have been deleted successfully.', $_SERVER['PHP_SELF'].'?prog=att::list');
  }
  ...
  while($ut = $rs->fetch()) {
    ...
    $acp->newRow2(
      $ut['app_name'],
      $ut['userid']!=0 ?
        "<a href='".$_SERVER['PHP_SELF']."?prog=user::edit&uid=".$ut['userid']."' target=_blank>".$ut['username']."</a>" :
        'Guest',
      $ut['forumname'],
  ...
  if($matched_files > $filesPP) {
    ...
    $pagenav .= "&nbsp; <a href='".$_SERVER['PHP_SELF'].".php?prog=att::list&page=1&".$appendix."'>|&lt;&lt;</a>";

    for($i = max(1, $page - 5); $i <= min(ceil($matched_files / $filesPP), $page+5); $i++) {
      $pagenav .= " <a href='".$_SERVER['PHP_SELF']."?prog=att::list&page=$i&".$appendix."'><b>$i</b></a> ";
    }

    $pagenav .= "&nbsp; <a href='".$_SERVER['PHP_SELF']."?prog=att::list&page=".ceil($matched_files / $filesPP)."&".$appendix."'>|&gt;&gt;</a>";
