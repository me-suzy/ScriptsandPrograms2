...
//if (!$celeste->usergroup['allowcreatetopic'] || !$forum->permission['allowcreatetopic']) {
if (!$forum->permission['allowcreatetopic']) {
  if($usergroupid==5) {
    import('login');
...
if (isset($_POST['title'])) {
  ...
  $tp->setProperty('iconid', (isset($_POST['iconid']) ? $_POST['iconid'] : 1));
  ...
  $header->set('nav', $path.'&#187; '.SET_NEW_TOPIC_TITLE);
  $header->set('pagetitle', SET_NEW_TOPIC_TITLE);
  ...
  $root->set('pagetitle', SET_NEW_TOPIC_TITLE);
