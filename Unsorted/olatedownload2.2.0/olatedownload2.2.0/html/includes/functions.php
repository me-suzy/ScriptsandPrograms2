...
{			
    ...
    while (@ $row = mysql_fetch_assoc($result)) 
	{
		...
		echo '<tr>';
		echo '<td width="50%"><a href="details.php?id='.stripslashes($row['id']).'"><strong>'.stripslashes($row['name']).'</strong></a></td>';
		echo '<td width="35%"><div align="right"><a class="listing" href="details.php?id='.stripslashes($row['id']).'">'.$language['link_moreinfo'].'</a></div></td>';
		echo '<td width="35%"><div align="right"><a class="listing" href="download.php?id='.stripslashes($row['id']).'">'.$language['link_downloadnow'].'</a></div></td>';
		echo '</tr>';
		echo '</table></td>';
		echo '</tr>';
...
function get_categories()
{
    ...
    for ($i=0; $i < $num_rows; $i++) 
	{
		...
		echo '<li><a href="index.php?category='.stripslashes($row['id']).'">'.stripslashes($row['name']).'</a></li>';
	}
}
...
function rate($id, $user_rating, $language)
{
	...
	{ 
        $result = $language['rating_already'].'<a href="details.php?id='.$id.'">'.$language['link_clickheregoback'].'</a>.'; 
    } else { 
    	...
    	while(list($rating, $votes) = mysql_fetch_assoc($get_count)) 
		{
            ...
            $result = $language['rating_thanks'].$new_rating2.$language['rating_outof'].' <a href="details.php?id='.$id.'">'.$language['link_clickheregoback'].'</a>'; 
        } 
    } 
}
...
function search($search, $language)
{
	...
	{ 
        ...
        $str  =  '<li><a href="details.php?id='.stripslashes($row['id']).'">'.stripslashes($row['name']) . "</a>\n\n";
