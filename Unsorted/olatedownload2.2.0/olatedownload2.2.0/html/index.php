...
<title>Olate Download - www.olate.com</title>
...
<strong>Olate Download - www.olate.com</strong><br /><br />
...
if ($category != 'top') {
    ...
    if ($config['sorting'] == 1  && $config['ratings'] == 1)
        echo '<div align="right"><strong>'.$language['title_sortby'].'</strong> <a href="index.php?category='.$category.'&sort=name">'.$language['title_sorting_name'].'</a> | <a href="index.php?category='.$category.'&sort=rating">'.$language['title_sorting_rating'].'</a> | <a href="index.php?category='.$category.'">'.$language['title_sorting_downloads'].'</a></div><br />';

    // If sortings are disabled, do not show the option to sort by them!
    elseif ($config['sorting'] == 1)
 
	echo '<div align="right"><strong>'.$language['title_sortby'].'</strong> <a href="index.php?category='.$category.'&sort=name">'.$language['title_sorting_name'].'</a> | <a href="index.php?category='.$category.'">'.$language['title_sorting_downloads'].'</a></div><br />';
}
?>
			...
			<table class="categoriesbox_table" border="1" align="center" cellpadding="3" cellspacing="3">
				...
				<tr class="categoriesbox_transparentborder">
					<td class="categoriesbox_text">
						...
						{
							echo '<li><a href="index.php?category=top">'.$language['menu_top'].' '.$config['notopdownloads'].' '.$language['menu_downloads'].'</a></li>';
						}
						// Show 'All Downloads' link if enabled
						if ($config['alldownloads'] == 1)
						{
							echo '<li><a href="index.php?category=all">'.$language['menu_all'].'</a></li>';
						}
						// Show 'All Downloads' link if enabled
						if ($config['searchlink'] == 1)
						{
							echo '<li><a href="search.php">'.$language['title_search'].'</a></li>';
						}						
