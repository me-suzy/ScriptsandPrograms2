...
if(!is_array($right_list)){
        ...
        $sql="SELECT A.idarea, A.parent_id, B.location,A.name FROM ".$cfg["tab"]["area"]." as A LEFT JOIN ".$cfg["tab"]["nav_sub"]." as B ON  A.idarea = B.idarea WHERE A.idarea!='login' AND A.relevant='1' AND A.online='1' GROUP BY A.name ORDER BY A.idarea";
         $db->query($sql);
        ...
        if(is_array($arraysave)){
				...
				$datas[1] = 0;
                              ...
                              $sql="INSERT INTO ".$cfg["tab"]["rights"]."
                                    ...
                                    VALUES ('".$db->nextid($cfg["tab"]["rights"])."', '$groupid','$datas[0]','$datas[1]','$datas[2]','$rights_client','$rights_lang',1)";
		$db->query($sql);
                     }
