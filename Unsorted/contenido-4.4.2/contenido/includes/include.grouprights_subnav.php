...
if ( isset($HTTP_GET_VARS['groupid']) ) {
    ...
    while ( $db->next_record() ) {
        ...
        if ($perm->have_perm_area_action($tmp_area))
        {
            ...
            $tpl->set("d", "CLASS",     '');
            $tpl->set("d", "OPTIONS",   '');
            $tpl->set("d", "CAPTION",   '<a class="white" onclick="sub.click(this.offsetParent)" target="right_bottom" href="'.$sess->url("main.php?area=$tmp_area&frame=4&groupid=$groupid").'">'.$caption.'</a>');
            $tpl->next();
