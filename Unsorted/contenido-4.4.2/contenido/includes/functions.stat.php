...
function statsDisplayInfo($id, $type, $x, $y, $w, $h)
{
    ...
    $div = '<DIV ID="idElement14" class="text_medium" style="background: #E8E8EE;
             border: 1px; border-style: solid; border-color: #747488; position:absolute;
...
function statsOverviewAll($yearmonth)
{
    ...
    $sql = "SELECT
                    idtree, A.idcat, level, preid, C.name
                FROM
    ...
    while ($db->next_record()) {
        ...
        $icon = '<img src="'.$cfg['path']['images'].'folder.gif">';
        ...
        $online = $db->f("visible");
        if ($online == 1) {
            $offonline = '<img src="'.$cfg['path']['images'].'offline.gif" alt="Kategorie ist unsichtbar" title="Kategorie ist unsichtbar">';
        } else {
            $offonline = '<img src="'.$cfg['path']['images'].'online.gif" alt="Kategorie ist sichtbar" title="Kategorie ist sichtbar">';
        }
        ...
        while ($db2->next_record()) {
            ...
            if ($online == 0) {
                $offonline = '<img src="'.$cfg['path']['images'].'offline.gif" alt="Artikel ist offline" title="Artikel ist offline">';
            } else {
                $offonline = '<img src="'.$cfg['path']['images'].'online.gif" alt="Artikel ist online" title="Artikel ist online">';
            }

            $icon = '<img src="'.$cfg['path']['images'].'article.gif">';
...
function statsOverviewYear($year)
{
    ...
    while ($db->next_record()) {
        ...
        $online = $db->f("visible");
        if ($online == 1) {
            $offonline = '<img src="'.$cfg['path']['images'].'offline.gif" alt="Kategorie ist offline" title="Kategorie ist unsichtbar">';
        } else {
            $offonline = '<img src="'.$cfg['path']['images'].'online.gif" alt="Kategorie ist online" title="Kategorie ist sichtbar">';
        }
        $bgcolor = $cfg["color"]["table_dark"];
        $tpl->set('d', 'BGCOLOR', $bgcolor);
        $tpl->set('d', 'PADDING_LEFT', $padding_left);
...
function statWriteNewSatfile($filepath,$lang,$client){
        ...
        //************** Überschriften **************
        fwrite($file,"Kategorie / Seiten;Anzahl der Seiten;Total;In dieser Sprache\n");
        ...
        fwrite($file,"Summe;;$hitstot;$hitstotlang\n\n");




        $sql = "SELECT idtree, A.idcat, level, preid, C.name, idtpl
                FROM ".$cfg["tab"]["cat_tree"]." AS A, ".$cfg["tab"]["cat"]." AS B, ".$cfg["tab"]["cat_lang"]." AS C
                WHERE A.idcat=B.idcat AND B.idcat=C.idcat AND C.idlang='$lang' AND B.idclient='$client' ORDER BY idtree";
        $db->query($sql);
        while ($db->next_record()) {
                if ($db->f("level") == 0 && $db->f("preid") != 0) {
                         fwrite($file,"\n");
                ...
                $spaces = "";
                        for ($i=0; $i<$db->f("level")+1; $i++) {
                                $spaces = $spaces . "\t";
                        }
                $name="$spaces".$db->f("name");
                ...
                fwrite($file,"$name;$anz;$tot;$totlang\n");
...
fclose($file);
$sql = "UPDATE ".$cfg["tab"]["stat"]." SET visited=0";
$db->query($sql);
}
