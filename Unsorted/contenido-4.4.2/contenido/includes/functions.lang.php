...
/*****************************************
*
* $Id: functions.lang.php,v 1.11 2003/08/28 10:47:38 timo.hummel Exp $
*
...
* Modified  :   $Date: 2003/08/28 10:47:38 $
*
 ...
 */
function langEditLanguage($idlang, $langname, $encoding, $active) {
...
function langNewLanguage($name, $client) {
        ...
        global $db, $sess, $cfg, $cfgClient, $notification;

        $new_idlang = $db->nextid($cfg["tab"]["lang"]);

        $sql = "INSERT INTO ".$cfg["tab"]["lang"]." (idlang, name, active, encoding) VALUES ('$new_idlang', '$name', '0', 'iso-8859-1')";
        $db->query($sql);
        ...
        langDuplicateFromFirstLanguage($client, $new_idlang);
 ...
 * @copyright four for business AG <www.4fb.de>
 */
function langDeleteLanguage($idlang) {
        ...
        {
            $deleteok = 0;
            return $notification->returnNotification("error", i18n("Could not delete language"));
        }
        ...
        if ($deleteok == 1) {
                ...
                $last_language = 0;
                $sql = "SELECT COUNT(*) FROM ".$cfg["tab"]["clients_lang"]." WHERE idclient='$client'";
                $db->query($sql);
                $db->next_record();
                if ($db->f(0) == 1) {
                    $lastlanguage = 1;
                ...
                $sql = "SELECT A.idtplcfg AS idtplcfg, idartlang, A.idart FROM ".$cfg["tab"]["art_lang"]." AS A, ".$cfg["tab"]["art"]." AS B WHERE A.idart=B.idart AND B.idclient='$client' AND idlang!='0' AND idlang='$idlang'";
                $db->query($sql);
                while ($db->next_record()) {
                ...
                $sql = "DELETE FROM ".$cfg["tab"]["stat"]." WHERE idlang='$idlang' AND idclient='$client'";
                $db->query($sql);


                //********** delete from 'code'-table *************
                $sql = "DELETE FROM ".$cfg["tab"]["code"]." WHERE idlang='$idlang' AND idclient='$client'";
                $db->query($sql);
