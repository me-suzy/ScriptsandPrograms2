...
while ($db->next_record()) {
        ...
        foreach($right_list["mod"] as $value2)
              {
               ...
               if(is_array($value2["action"]))
                 foreach($value2["action"] as $key3 => $value3)
                 {
                          ...
                          $table->cell("<input type=\"checkbox\" name=\"rights_list[".$value2["perm"]."|$value3|".$db->f("idmod")."]\" value=\"x\" $checked>");
        ...
        $table->cell("
              <input type=\"checkbox\" name=\"checkall_".$value2["perm"]."_".$value3."_".$db->f("idmod")."\" value=\"\" onClick=\"setRightsFor('".$value2["perm"]."','$value3','".$db->f("idmod")."')\">");
