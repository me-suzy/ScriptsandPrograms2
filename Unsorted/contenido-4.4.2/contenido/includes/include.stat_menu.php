...
$currentLink = '<a target="right_bottom" href="'.$sess->url("main.php?area=stat&frame=4&displaytype=all&action=stat_show&yearmonth=current").'">'.i18n("Current Report").'</a>';
...
foreach ($availableYears as $yearIterator)
{
        ...
        $dark = !$dark;
        $yearLink = '<a target="right_bottom" href="'.$sess->url("main.php?area=stat&frame=4&action=stat_show&displaytype=all&showYear=1&year=".$yearIterator).'">'."$yearIterator".'</a>';
        if ($dark) {
        ...
        foreach ($availableMonths as $monthIterator)
        {
                ...
                $monthLink = '<a target="right_bottom" href="'.$sess->url("main.php?area=stat&frame=4&action=stat_show&displaytype=all&yearmonth=".$yearIterator . $monthIterator).'">'."$monthCanonical".'</a>';
