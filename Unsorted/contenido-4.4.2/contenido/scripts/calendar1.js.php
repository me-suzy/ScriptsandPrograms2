...
function calendar1(obj_target) {
	...
	if (!obj_target)
		return cal_error("<?php echo i18n("Error calling the calendar: no target control specified"); ?>");
	if (obj_target.value == null)
		return cal_error("<?php echo i18n("Error calling the calendar: parameter specified is not valid tardet control"); ?>");
...
function cal_popup1 (str_datetime, prefixpath, oldwindowobj) {

	this.dt_current = this.prs_tsmp(str_datetime ? str_datetime : this.target.value);
	if (!this.dt_current) return;

	if (!prefixpath) prefixpath = "";
	if (oldwindowobj) { oldwindowobj.close(); }
	var obj_calwindow = window.open(
		prefixpath + 'calendar.php?datetime=' + this.dt_current.valueOf()+ '&id=' + this.id,
		'', 'width=200,height='+(this.time_comp ? 215 : 190)+
