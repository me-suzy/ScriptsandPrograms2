...
class Form {
    ...
    /**
     * GetFormValues()
     * Extract the Form Data from the $HTTP_POST_VARS or $HTTP_GET_VARS
     * global arrays
     * @return void
     */
    function GetFormValues() {
        ...
        if (strtolower($this->method) == 'post') {
            // extract values from the $HTTP_POST_VARS global array
            foreach ($this->fields as $id => $element) {
                // check if value exists, extract it
                if (isset($HTTP_POST_VARS[$element['name']])) {
                    $this->fields[$id]['value'] = $HTTP_POST_VARS[$element['name']];
                    $this->isSend = true;
                } else {
                    $this->isSend = false;
                }
            ...
            // extract values from the $HTTP_GET_VARS global array
            foreach ($this->fields as $id => $element) {
                // check if value exists, extract it
                if (isset($HTTP_POST_VARS[$element['name']])) {
                    $this->fields[$id]['value'] = $HTTP_GET_VARS[$element['name']];
                    $this->isSend = true;
                } else {
                    $this->isSend = false;
                }
        ...
        if (!is_array($item)){
            ...
            switch (strtolower($item['type'])) {
                ...
                case 'suche':
                    ...
                    $tmp_ret_str .= '&nbsp;&nbsp;<a href="#" onclick="popUp(\''.$item['values'][1].'\')"><img src="images/button_hilfe.gif" border="0"></a>';
                    break;
