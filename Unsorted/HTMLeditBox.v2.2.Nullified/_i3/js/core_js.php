<script LANGUAGE="JavaScript">
	...
	if(($input_method == 3) || ($input_method == 4)) {
		// pool is filled by bridge feature
...
function Swap_it() {
	...
	function blockDefault() {
		...
		subHTML += "<tr><td>"
		subHTML += "<font style=\"font: 9.9px verdana; color: #000000;\">HTMLeditbox <? echo str_replace('_','.',$settings[version]); ?> (build <?=$settings[build]?>) &nbsp&nbsp - <?=$settings['copyright']?> All Rights Reserved&nbsp; - <?=$lang['language']?> translation: <a href=\"mailto:<?=$lang['translator_email']?>\" target=\"_blank\"><font color=\"#CE0000\"><?=$lang['translator']?></font></a>"
...
?>
	...
	function Help() {
		window.open("http://support.labs4.com/editbox/help.php?host=<?=$PHP_URI?>","wHelp", "toolbar=0, scrollbars=yes, width=666, height=460");
	}

	function Update() {
		window.open("http://www.labs4.com/update.php?version=<?=$version?>&build=<?=$build?>","wUpdate", "toolbar=yes, scrollbars=yes, statusbar=yes, width=640, height=480");
	}
