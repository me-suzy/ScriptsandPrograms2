/*	$Id: clseBayStaffPicksWidget.cpp,v 1.5 1999/03/07 08:15:17 josh Exp $	*/
//
//	File:	clseBayStaffPicksWidget.cpp
//
...
//
//	Function:
//			Widget that shows staff-picked items using clseBayItemWidget.
//
// Modifications:
...
clseBayStaffPicksWidget::clseBayStaffPicksWidget(clsMarketPlace *pMarketPlace) :
	clseBayItemWidget(pMarketPlace)
{
	mCatId = 0;
}

clseBayStaffPicksWidget::~clseBayStaffPicksWidget()
{
}

void clseBayStaffPicksWidget::SetParams(vector<char *> *pvArgs)
{
	int p;
	char *cArg;
	char cArgCopy[256];
...
void clseBayStaffPicksWidget::GetItemIds(vector<int> *pvItemIds)
{
	...
	if (mpLoggingStream)
	{
		...
		*mpLoggingStream << pDate << " " << pTime << " Start Getting Staff Pick Item Ids for Category " << mCatId << "\n";
	}

	if (pItems) pItems->GetStaffPicksItemIds(pvItemIds, CurrentTime, mCatId);

	if (mpLoggingStream)
	{
		t = time(0);
		clsUtilities::GetDateAndTime(t, pDate, pTime);
		*mpLoggingStream << pDate << " " << pTime << " End Getting Staff Pick Item Ids for Category " << mCatId << "\n\n";
	}
