/*	$Id: clseBayUserSellingDetailWidget.cpp,v 1.3 1998/12/06 05:23:24 josh Exp $	*/
//
//	File:	clseBayUserSellingDetailWidget.cpp
//
...
//
//	Function:
//			Shows items that a user is selling, in detail.
//
...
clseBayUserSellingDetailWidget::clseBayUserSellingDetailWidget(clsMarketPlace *pMarketPlace) :
...
clseBayUserSellingDetailWidget::~clseBayUserSellingDetailWidget()
{
	ItemList::iterator i;
...
void clseBayUserSellingDetailWidget::SetParams(vector<char *> *pvArgs)
{
	int p;
	char *cArg;
	char cArgCopy[256];
...
bool clseBayUserSellingDetailWidget::Initialize()
{
	// safety
	if (!mpMarketPlace)
		return false;
	...
	// get 'em (true means get more stuff)
	mpUser->GetListedItems(&mvItems, mDaysSince, true, mSortCode);
...
bool clseBayUserSellingDetailWidget::EmitCell(ostream *pStream, int n)
{
	ItemList::iterator		i;
	int						ii;
	...
	*pStream <<		"<TD>";
	...
	clseBayItemDetailWidget dW(mpMarketPlace);
	dW.SetItem(pItem);
	dW.SetColor(mHeaderColor);
	dW.SetMode(clseBayItemDetailWidget::Seller);
	dW.EmitHTML(pStream);
...
bool clseBayUserSellingDetailWidget::EmitPreTable(ostream *pStream)
{
	// Items I'm Selling
