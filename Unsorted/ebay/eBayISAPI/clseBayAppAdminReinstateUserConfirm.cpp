/*	$Id: clseBayAppAdminReinstateUserConfirm.cpp,v 1.2.198.1.34.1 1999/08/01 02:51:49 barry Exp $	*/
//
//	File:		clseBayAppAdminReinstateUserConfirm.cpp
//
//	Class:		clseBayApp
//
...
// if not.
//
bool clseBayApp::ValidateReinstateInput(char *pUser,
										char *pPass,
										char *pTarget,
										int type,
										char *pText)
...
void clseBayApp::ReinstateUserConfirm(char *pUser,
									  char *pPass,
									  char *pTarget,
									  int type,
									  char *pText)
{
	...
	*mpStream <<	" validated! Complete the e-mail text below and hit "
					"\"Reinstate\" to reinstate this user."
					"</font>"
					"<br>"
					"<br>";
	...
	*mpStream <<	"<form method=post action="
					"\""
			  <<	mpMarketPlace->GetCGIPath(PageAdminReinstateUserResult)
			  <<	"eBayISAPI.dll"
					"\""
					">"
					"<INPUT TYPE=HIDDEN "
					"NAME=\"MfcISAPICommand\" "
					"VALUE=\"AdminReinstateUser\">";
	...
	*mpStream <<	"</textarea>";
    ...
    subjectlength = strlen(pEmailSubject) + EBAY_MAX_USERID_SIZE;
	subject = new char [subjectlength + 1];
	sprintf(subject, pEmailSubject, pTargetUser->GetUserId());
	...
	*mpStream <<	"<tr>"
					...
					"</td>"
					"<td width=\"430\">"
					"<input type=text name=\"emailsubject\" size=\"56\" value=\""
			  <<	subject
			  <<	"\">"
					"<br>";
	...
	*mpStream <<	"<font size=\"2\" color=\"#006600\">"
	...
	// XXX what the heck do we do here if pEmailTemplate is NULL???
...
void clseBayApp::AdminReinstateUserConfirm(char *pUserId,
										  char *pPass,
										  char *pTarget,
										  int type,
										  char *pText,
										  eBayISAPIAuthEnum authLevel)
{
	...
	*mpStream <<	mpMarketPlace->GetHeader()
	...
	ReinstateUserConfirm(pUserId, pPass, pTarget, type, pText);
