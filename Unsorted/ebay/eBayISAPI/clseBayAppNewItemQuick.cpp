/*	$Id: clseBayAppNewItemQuick.cpp,v 1.18.2.11.14.3 1999/08/09 17:23:54 nsacco Exp $	*/
//
...
//
//	Function:	clseBayApp::NewItemQuick
//
//
//	Modifications:
//				- 06/14/97 michael/yp	- Created
//				- 09/17/97 poon - new layout
//				- 29/29/97 wen	- added codes for free relisting
//				- 06/03/98 vicki - new layout
...
//				- 06/09/99 nsacco - added examples for countries
//				- 07/19/99 nsacco	- Switched from mpMarketPlace->GetName() to GetCurrentPartnerName()

// NOTE: The page is now redirect to ListItemForSale

// TODO - nsacco - remove this file, it is not used anymore
...
void clseBayApp::NewItemQuick(CEBayISAPIExtension *pServer, char *pItemNo, char *pCatNo)
{
	...
	if ((clsUtilities::CompareTimeToGivenDate(time(NULL), 5, 19, 99, 0, 0, 0) >= 0) &&
		(clsUtilities::CompareTimeToGivenDate(time(NULL), 6, 3, 99, 23, 59, 59) <= 0))
		FreeGallery = true;
	...
	*mpStream <<	"<table border=\"1\" cellspacing=\"0\" width=\"590\" "
					...
					"<b>";
	*mpStream <<	(itemNo ? "Relist Your Item" : "Sell Your Item (quick form)");
	...
	*mpStream <<	"size=" << EBAY_MAX_LOCATION_SIZE << " "
	...
	// nsacco
	// TODO - should this be site? yes but the file is not used anymore
	switch(countryId)
	{
	case Country_UK:
	...
	*mpStream <<	"size=" << EBAY_MAX_ZIP_SIZE << " "
	...
	if (verbose)
	{
		*mpStream <<	"<p><font size=\"2\">Please specify the location zip code of the item(s) "
						"you are offering.</font><br>";
	}

	*mpStream <<	"</td>"
					"</tr>";

	//row for country entry:
	...
	*mpStream <<	"</table>\n";
	...
	else
	{
		...
		*mpStream << "<input type=hidden name=category1 value=\""
		...
		{
			*mpStream <<	"<br><b> Fees in this category are fixed. "
							"eBay charges sellers a $25 listing fee and a $25 final "
							"value fee, regardless of the listing or selling price. "  
							"Please see "
							"<a href=\""
							<<	mpMarketPlace->GetHTMLPath()
							<<	"help/sellerguide/selling-fees.html\">"
		...
		{
			*mpStream <<	"<br><b> Fees in this category are fixed. "
							"eBay charges sellers a $50 listing fee and no final "
							"value fee, regardless of the listing or selling price. "  
							"Please see "
							"<a href=\""
							<<	mpMarketPlace->GetHTMLPath()
...
//  Lena - Ts and Cs
	...
	*mpStream <<	"<br><table border=\"1\" cellpadding=\"3\" cellspacing=\"0\">"
					...
					"color=\"#006600\"> (choose as many as apply) </font>"
					"</td>"
					"<td width=\"430\">"
	...
	*mpStream <<	"</table>\n"
					...
					"<tr>\n"
					"<td width=\"160\" bgcolor=\"#EFEFEF\" valign=\"top\"><font size=\"3\" color=\"#006600\">"
					"<strong>Payment Terms</strong></font>"
					"</td>\n"
					"<td width=\"430\">\n"
	...
	if (currencyId == Currency_USD)
	{
		if ((itemNo) && (mpItem->IsShippingInternationally()))
			*mpStream <<	"<td width=\"50%\"> <input type=\"radio\" name=\"shippingInternationally\" value=\"off\">"
							...
							"<font size=\"2\"> Ship Within Home Country Only</font>"
							"</td>\n"
							"<td width=\"50%\"> <input type=\"radio\" name=\"shippingInternationally\" checked value=\"on\">"
							"<font size=\"2\"> Will Ship Internationally</font>"
							"</td>\n";
		else
...
// kakiyama 07/16/99
	...
	*mpStream <<	"size=" << maxSize << " "
	...
	if (verbose)
	{
		...
		*mpStream <<	"<p><font size=\"2\">This is the price at which the auction will start, and "
		...
		// nsacco 06/11/99
		// tell the user to leave out some currency formatting
		switch (currencyId)
		{
		case Currency_AUD:
			*mpStream << "dollar signs ('$') and commas (',')";
			break;
		case Currency_USD:
			*mpStream << "dollar signs ('$') and commas (',')>";
			break;
		case Currency_GBP:
			*mpStream << "pound signs ('&pound;') and commas (',')";
			break;
		default:
			*mpStream << "dollar signs ('$') and commas (',')";
			break;
	...
	*mpStream <<	"<tr>"
	...
	else	// 10 and 14 days auctions added by Soc 4/12/99
	{
		if (itemNo)
		{
			*mpStream <<	"<select name=duration>\n"
	...
	*mpStream <<	"size=" << (maxSize + 2)<< " "
	...
	if (verbose)
	{
		...
		*mpStream <<	"<p><font size=\"2\">If you want to start the bidding at a price lower than "
				  ...
				  <<	"Prices are not allowed for <a href=\""
				  <<	mpMarketPlace->GetHTMLPath()
	...
	*mpStream <<	"<tr>\n"
	...
	if (clsUtilities::CompareTimeToGivenDate(when, 02, 15, 99, 0, 0, 0) < 0) 
	{
		...
		*mpStream <<	" charge)</font>"
						"<font size=\"2\" color=\"#800000\">&nbsp;(optional)</font> "
						"<font size=\"2\" color=\"red\">As part of Featured Auction "
						"changes, prices for Featured Auctions will be increased to ";

		currencyWidget.SetNativeAmount(objFees.GetFee(NewFeaturedFee));
		currencyWidget.EmitHTML(mpStream);

		*mpStream <<	" on 2/15/99.</font>";
	}
	else
	{
		*mpStream <<	">"
	...
	*mpStream <<	"<tr>\n"
	...
	if (clsUtilities::CompareTimeToGivenDate(when, 02, 15, 99, 0, 0, 0) < 0) 
	{
		...
		*mpStream <<	"charge)</font>"
						"<font size=\"2\" color=\"#800000\">&nbsp;(optional)</font> "
						"<font size=\"2\" color=\"red\">Prices for Category Featured "
						"Auctions will be increased to ";
	...
	if (verbose)
	{
		if (clsUtilities::CompareTimeToGivenDate(when, 02, 15, 99, 0, 0, 0) < 0) 
		{
			...
			*mpStream <<    "</b>, you may have your listing title appear at the top of the "
					  ...
					  <<	"10 to place a featured category auction.</font><br>";
		}
		else
		{
			*mpStream <<	"<p><font size=\"2\">For "
...
// mutiple gift icon
	 *mpStream <<	"<tr>\n"
					"<td width=\"160\" bgcolor=\"#EFEFEF\" valign=\"top\">"
					"<font size=\"3\">Great Gift icon?<img border=\"0\" height=\"11\" width=\"28\" alt=\"[NEW!]\" src=\"" 
	...
	//need change to dropdown box , E114 we only need support one icon --vicki
...
/*	 *mpStream <<	"<input type=\"checkbox\" name=\"giftIcon\"";
	 if (itemNo && mpItem != NULL && mpItem->GetGiftIconType() == GiftIcon)
	 {
		 *mpStream <<	" checked";
	 } */
     *mpStream	<< "<font size=\"2\">(";
	 ...
	 *mpStream	 << " charge)&nbsp;</font>"
					"<font size=\"2\" color=\"#800000\">  (optional)</font>";
/*		 " <a HREF=\""
		 <<	mpMarketPlace->GetHTMLPath() // remove it due to mutiple icons
		 <<	"help/sellerguide/gift-icon.html\">"
					" <img border=\"0\" height=\"14\" width=\"16\" alt=\"[GIFT!]\" src=\"" 
					<<	mpMarketPlace->GetImagePath()
					<<	"gift-vday.gif\"></a>" */
	 ...
	 if (verbose)
	 {
		...
		*mpStream <<    ", you may choose to have your listing title appear with our "
						"new \"Great Gift\" icon in the listing pages, making it stand out among the other listings.</font><br>";
	 }
	  *mpStream <<	"</td>\n"
					"</tr>\n";

// Lena - end of gift icon
	...
	*mpStream   << "charge)";
	...
	// FREEGALLERY
	if (FreeGallery)
		*mpStream << " Free through June 3rd!";
