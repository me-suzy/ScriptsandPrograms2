/*	$Id: ebay_items_blocked.sql,v 1.2 1999/05/19 02:35:08 josh Exp $	*/
/*
...
storage(initial 60M next 5m);
 ...
 create index ebay_items_last_modified_index
   on ebay_items_blocked(last_modified)
   tablespace bidi01
...
alter table ebay_items_blocked 
	add constraint			items_blocked_pk
		primary key		(marketplace, id)
...
/* 1:50 - 1:53 */
 ...
 create index ebay_items_blocked_category_index 
	on ebay_items_blocked(category)
	tablespace ritemi01
	storage(initial 70m next 30m) unrecoverable parallel (degree 3);
/* 2:07 - 2:11 */

alter table ebay_items_blocked add ( ORIG_SALE_END date);
alter table ebay_items_blocked add ( CONTENT_MODIFIED date);
alter table ebay_items_blocked add ( ICON_FLAGS VARCHAR2(3));
/* Where the item is located by country. */
alter table ebay_items_blocked
	add (country_id number(10));
alter table ebay_items_blocked add ( GALLERY_URL VARCHAR2(255));
alter table ebay_items_blocked add ( GALLERY_THUMB_X_SIZE NUMBER(4,0) );
alter table ebay_items_blocked add ( GALLERY_THUMB_Y_SIZE NUMBER(4,0) );
alter table ebay_items_blocked add ( GALLERY_TYPE NUMBER(2,0));
...
alter index ebay_items_blocked_seller_index rebuild parallel 2 tablespace qitemsi01;

-----------
CREATE INDEX EBAY_ITEMS_blocked_SELLER_IDX ON EBAY_ITEMS (SELLER)
...
alter table ebay_items_blocked
	add	constraint		items_blocked_country_id_fk
		not null (country_id)
...
rename ebay_items_blocked to ebay_items_old;
