/*	$Id: space_summary.sql,v 1.2 1999/02/21 02:55:02 josh Exp $	*/
rem
rem space_summary.sql
rem  parameter 1:  check date
rem  parameter 2:  ratio of Oracle block size to operating system block size
rem
rem  to call this report from within sqlplus:
rem  @space_summary check_date block_ratio
rem
rem  Example:
rem  @space_summary 07-AUG-94 4
rem
...
select
   ...
   blocks*&&2 diskblocks  /*operating system blocks in the file*/
from files
where check_date = '&&1'
and db_nm = upper('ebay')
order by ts, file_nm
/
...
select
   ...
   spaces.sum_free_blocks*&&2 dfrb, /*disk blocks free*/
   sum_file_blocks*&&2 sum_file_blocks /*disk blocks allocated*/
from spaces, files_ts_view ftv
where spaces.db_nm = ftv.db_nm
and spaces.ts = ftv.ts
and spaces.check_date = ftv.check_date
and spaces.db_nm = upper ('ebay')
and spaces.check_date = '&&1'
/
spool off
undefine 1
undefine 2
