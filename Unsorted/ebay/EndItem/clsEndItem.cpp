...
//	Function:
//
//		Move items from ebay_items to ebay_items_ended
//


#include "eBayDebug.h"
#include "eBayTypes.h"
#include "clsEndItem.h"
...
void clsEndItemApp::Run(char *startDate, char *endDate)
{
	// logging 
	int				itemId = 0;
	...
	mpDatabase->GetItemsToEnded(0, &vItems, startDate, endDate);
	if (vItems.size() == 0)
		return;
	for (i = vItems.begin();
		 i != vItems.end();
		...
		// Get the item
	{
			currentItem = mpMarketPlace->GetItems()->GetItem((*i), true);
			if (currentItem != NULL)
			{
				mpDatabase->EndItem(currentItem);
				delete currentItem;
			}
	}
...
static clsEndItemApp *pTestApp = NULL;
