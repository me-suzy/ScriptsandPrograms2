/*	$Id: function.h,v 1.3 1998/12/17 04:13:35 josh Exp $	*/
/*
 *
 * Copyright (c) 1994
...
// should be harmless using namespaces
...
class unary_negate : public unary_function<typename __UNARY_ARG(Predicate,argument_type), bool> {
protected:
    Predicate pred;
public:
...
class binary_negate 
    : public binary_function<typename __BINARY_ARG(Predicate,first_argument_type),
			     typename __BINARY_ARG(Predicate,second_argument_type), bool> {
protected:
    Predicate pred;
public:
...
template <class Operation> 
class binder1st : public unary_function<typename __BINARY_ARG(Operation,second_argument_type),
					typename __BINARY_ARG(Operation,result_type) > {
protected:
    Operation op;
    typename __BINARY_ARG(Operation,first_argument_type) value;
public:
    binder1st(const Operation& x, const typename __BINARY_ARG(Operation,first_argument_type)& y)
	: op(x), value(y) {}
...
binder1st<Operation> bind1st(const Operation& op, const T& x) {
    return binder1st<Operation>(op, __BINARY_ARG(Operation,first_argument_type)(x));
}

template <class Operation> 
class binder2nd : public unary_function<typename __BINARY_ARG(Operation,first_argument_type),
					typename __BINARY_ARG(Operation,result_type)> {
protected:
    Operation op;
    typename __BINARY_ARG(Operation,second_argument_type) value;
public:
    binder2nd(const Operation& x, const typename __BINARY_ARG(Operation,second_argument_type)& y) 
	: op(x), value(y) {}
    result_type operator()(const argument_type& x) const {
	return op(x, value); 
    }
...
binder2nd<Operation> bind2nd(const Operation& op, const T& x) {
    return binder2nd<Operation>(op, __BINARY_ARG(Operation,second_argument_type)(x));
}

template <class Operation1, class Operation2>
class unary_compose : public unary_function<typename __UNARY_ARG(Operation2,argument_type),
...
class binary_compose : public unary_function<typename __UNARY_ARG(Operation2,argument_type),
