/*	$Id: deque.h,v 1.3 1998/12/17 04:13:34 josh Exp $	*/
/*
 *
 * Copyright (c) 1994
...
public:
	...
	STL_NAMESPACE::swap(length, x.length);
	STL_NAMESPACE::swap(map, x.map);
	STL_NAMESPACE::swap(map_size, x.map_size);
    }
...
#  define iterator               __deque_iterator<T>
...
template <class T, class Alloc>
__iterator__ deque<T, Alloc>::insert(iterator position, const T& x) {
    if (position == begin()) {
	push_front(x);
	return begin();
...
template <class T, class Alloc>
void 
deque<T, Alloc>::insert(iterator position, size_t n, const T& x) {
    difference_type index = position - begin();
	...
	if (n > (size_t)remainder) {
	    ...
	    difference_type i = 0;
	    while ((size_t)i < n) push_back(*(begin() + orig_len - n + i++));
