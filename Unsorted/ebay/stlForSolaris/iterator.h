/*	$Id: iterator.h,v 1.3 1998/12/17 04:13:40 josh Exp $	*/
/*
 *
 * Copyright (c) 1994
...
protected:
    Container& container;
public:
    back_insert_iterator(Container& x) THROWS(()): container(x) {}
...
protected:
    Container& container;
public:
    front_insert_iterator(Container& x) THROWS(()): container(x) {}
...
public:
    insert_iterator(Container& x, typename Container::iterator i) THROWS(()) 
	: container(x), iter(i) {}
...
public:
    reverse_bidirectional_iterator() THROWS(()) {}
    reverse_bidirectional_iterator(BidirectionalIterator x) THROWS(()) : current(x) {}
...
public:
    reverse_iterator() THROWS(()) {}
    reverse_iterator(RandomAccessIterator x) THROWS(()) : current(x) {}
...
public:
    explicit raw_storage_iterator(ForwardIterator x) THROWS(()) : iter(x) {}
...
public:
    istream_iterator() THROWS(()) : stream(&cin), end_marker(false) {}
    istream_iterator(istream& s) THROWS(()) : stream(&s) { read(); }
...
protected:
    ...
    char* string;
public:
    ostream_iterator(ostream& s) THROWS(()) : stream(&s), string(0) {}
    ostream_iterator(ostream& s, char* c) THROWS(()) : stream(&s), string(c)  {}
