	...
	/**********************************************************/
 	...
 	function view_msg() {
        ...
        $this->parser = new post_parser();
 		...
 		$DB->query("SELECT g.g_title as member_group, m.id, m.joined, m.name, m.title, m.posts, m.mgroup, m.email, m.signature, m.avatar, m.avatar_size, m.hide_email, m.aim_name, m.icq_number ".
		...
		{
			$member['signature'] = $ibforums->vars[SIG_SEP] . $this->parser->convert( array( TEXT    => $member['signature'],
																					   SMILIES => 0,
 	...
 	function send_form($preview=0, $errors="") {
    	...
    	if ($preview == 1)
    	{
    		...
    		$old_msg = $this->parser->convert( array( TEXT    => $ibforums->input['Post'],
													  SMILIES => 1,
													  CODE    => $ibforums->vars['msg_allow_code'],
													  HTML    => $ibforums->vars['msg_allow_html']
													)
		...
		if (!empty($ibforums->input['MID']))
		{
 		...
 		if ($preview == 1)
 		{
 			$old_message = stripslashes($HTTP_POST_VARS['Post']);
 	...
 	function edit_saved() {
    	...
    	//--------------------------------------
 		...
 		$this->output .= $this->post_html->postbox_buttons($msg['message']);
 	...
 	function send_msg() {
		...
		if ( (($to_msg_stats['msg_total']) >= $to_msg_stats['g_max_messages']) and ($to_msg_stats['g_max_messages'] != "") )
		{
 		...
 		if ($ibforums->input['add_sent'] and $ibforums->member['g_max_messages'])
 		{
 	...
 	function msg_list() {
 		...
 		if ($ibforums->member['g_max_messages'] > 0)
 		{
 			...
 			$info['img_width']    = $info['full_percent'] > 0 ? intval($info['full_percent']) * 3 : 1;
 		...
 		if ($this->vid == 'sent')
 		{
 			...
 			$DB->query("SELECT m.*, mp.name as from_name FROM ibf_messages m, ibf_members mp WHERE member_id='".$this->member['id']."' AND vid='".$this->vid."' and mp.id=m.recipient_id ORDER BY $sort_key");
 		
 		}
 		else
 		{
 			$DB->query("SELECT m.*, mp.name as from_name FROM ibf_messages m, ibf_members mp WHERE member_id='".$this->member['id']."' AND vid='".$this->vid."' and mp.id=m.from_id ORDER BY $sort_key");
 		}
 		
 		$this->output .= $this->html->inbox_table_header( $this->msg_stats['current_dir'], $info, $this->jump_html );
 		...
 		$this->output .= $this->html->end_inbox($this->jump_html, $amount_info);
...
//+------------------------------------------------------------------------------------
	...
	/**********************************************************/
 	...
 	function show_tracking() {
 		...
 		$DB->query("SELECT m.*, mp.name as to_name FROM ibf_messages m, ibf_members mp WHERE m.tracking=1 AND m.read_state=1 AND m.from_id='".$this->member['id']."' AND m.member_id=mp.id ORDER BY msg_date DESC");
 		...
 		$DB->query("SELECT m.*, mp.name as to_name FROM ibf_messages m, ibf_members mp WHERE m.tracking=1 AND m.read_state=0 AND m.from_id='".$this->member['id']."' AND m.member_id=mp.id ORDER BY msg_date DESC");
