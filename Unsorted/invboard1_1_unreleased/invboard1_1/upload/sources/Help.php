...
class Help {
    ...
    function Help() {
 	...
 	function show_section() {
 		...
 		if (! preg_match( "/^(\d+)$/" , $id ) ) {
 			$std->Error( array( LEVEL => 1, MSG => 'no_search_id') );
 		}
 	...
 	function do_search() {
 		...
 		if (empty( $ibforums->input['search_q'] ) ) {
 			$std->Error( array( LEVEL => 1, MSG => 'search_no_input') );
 		}
 		...
 		$this->output = $this->html->start( $ibforums->lang['search_results'], $ibforums->lang['results_txt'], $ibforums->lang['search_results'] );
 		
 		$DB->query("SELECT id, title from ibf_faq WHERE LOWER(title) LIKE '%$search_string%' or LOWER(text) LIKE '%$search_string%' ORDER BY title ASC");
