...
class ad_cat {
	...
	function edit_form() {
		...
		$ADMIN->html .= $SKIN->add_td_row( array( "<b>Category State</b>" ,
		...
		foreach ($cats as $c)
		{
			...
			{	
				...
				if ($r['category'] == $last_cat_id)
				{
					...
					if ($r['subwrap'] == 1)
					{
						...
						if ($r['sub_can_post'])
						{
							$ADMIN->html .= $SKIN->add_td_row( array(
																	   ...
																	   $r['posts'],
																	   $r['topics'],
																	   "<a href='{$ADMIN->base_url}&act=forum&code=subedit&f={$r['id']}'>Edit Settings</a><br><br>".
																	   "<a href='{$ADMIN->base_url}&act=forum&code=pedit&f={$r['id']}'>Edit Permissions</a>",
																	   ...
																	   "<a href='{$ADMIN->base_url}&act=forum&code=subdelete&f={$r['id']}'>Delete</a><br><br>".
																	   "<a href='{$ADMIN->base_url}&act=forum&code=empty&f={$r['id']}'>Empty Forum</a>",
															 )  , 'catrow2'    );
						}
						else
						{
					...
					{
						$ADMIN->html .= $SKIN->add_td_row( array(
																   array("<b>".$r['name']."</b>$skin_stuff<br>".$r['description'], 2),
																   $r['posts'],
																   $r['topics'],
																   "<a href='{$ADMIN->base_url}&act=forum&code=edit&f={$r['id']}'>Edit Settings</a><br><br>".
																   "<a href='{$ADMIN->base_url}&act=forum&code=pedit&f={$r['id']}'>Edit Permissions</a>",
																   ...
																   "<a href='{$ADMIN->base_url}&act=forum&code=delete&f={$r['id']}'>Delete</a><br><br>".
																   "<a href='{$ADMIN->base_url}&act=forum&code=empty&f={$r['id']}'>Empty Forum</a>",
														 )      );
					}
					...
					if ( ( isset($children[ $r['id'] ]) ) and ( count ($children[ $r['id'] ]) > 0 ) )
					{
						foreach($children[ $r['id'] ] as $idx => $rd)
						{
							...
							$ADMIN->html .= $SKIN->add_td_row( array(
															   ...
															   $rd['posts'],
															   $rd['topics'],
															   "<a href='{$ADMIN->base_url}&act=forum&code=edit&f={$rd['id']}'>Edit Settings</a><br><br>".
															   "<a href='{$ADMIN->base_url}&act=forum&code=pedit&f={$rd['id']}'>Edit Permissions</a>",
															   ...
															   "<a href='{$ADMIN->base_url}&act=forum&code=delete&f={$rd['id']}'>Delete</a><br><br>".
															   "<a href='{$ADMIN->base_url}&act=forum&code=empty&f={$rd['id']}'>Empty Forum</a>",
													 ) , 'subforum' );
						}
					}					 
