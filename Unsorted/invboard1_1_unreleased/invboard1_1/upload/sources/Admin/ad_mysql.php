...
class ad_mysql {
	...
	function ad_mysql() {
		global $IN, $root_path, $INFO, $DB, $SKIN, $ADMIN, $std, $MEMBER, $GROUP, $HTTP_POST_VARS, $HTTP_GET_VARS;
	...
	function get_table_sql($tbl, $create_tbl, $skip=0)
	{
		...
		if ($skip == 1)
		{
			if ($tbl == $INFO['sql_database'].'admin_sessions'
				OR $tbl == $INFO['sql_database'].'sessions'
				OR $tbl == $INFO['sql_database'].'reg_anti_spam'
				OR $tbl == $INFO['sql_database'].'search_results'
			   )
			{
				return $ret;
			}
	...
	function list_index()
	{
		...
		if ( $this->mysql_version >= 32303 )
		{
			...
			$ADMIN->html .= $SKIN->start_table( "Invision Board Tables" );
