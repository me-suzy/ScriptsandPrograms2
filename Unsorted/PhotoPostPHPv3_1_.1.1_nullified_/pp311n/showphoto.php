...
if ( !empty($photo) ) {
    ...
    $prevlink = "<font size=\"".$Globals{'fontmedium'}."\" face=\"".$Globals{'mainfonts'}."\"><A href=\"".$Globals{'maindir'}."/showphoto.php?photo=$previous_image&amp;papass=$papass&amp;sort=$sort&amp;size=$size\"><img border=\"0\" src=\"".$Globals{'idir'}."/previmg.gif\" alt=\"Previous image in category\"></a></font>$botbuster";
    $nextlink = "<font size=\"".$Globals{'fontmedium'}."\" face=\"".$Globals{'mainfonts'}."\"><A href=\"".$Globals{'maindir'}."/showphoto.php?photo=$next_image&amp;papass=$papass&amp;sort=$sort&amp;size=$size\"><img border=\"0\" src=\"".$Globals{'idir'}."/nextimg.gif\" alt=\"Next image in category\"></a></font>";
    ...
    if ( $slideshow == 1 ) {
        ...
        $slideurl = $Globals{'maindir'}."/showphoto.php?photo=$next_image&amp;slideshow=1&amp;papass=$papass&amp;sort=$sort&amp;slidedelay=$slidedelay";
    ...
    if ($approved == "1") {
        if ($medsize > 0) {
            if ($size != "big") {
                ...
                $altlink = "<center><font size=\"".$Globals{'fontsmall'}."\" face=\"".$Globals{'mainfonts'}."\"><B><a href=\"".$Globals{'maindir'}."/showphoto.php?photo=$photo&amp;papass=$papass&amp;sort=$sort\">View Smaller Image</a></b></font></center><Br>";
            }
        }
        ...
        if ($Globals{'bigsave'} == "yes") {
            if ($dispmed > 0) {
                ...
                $filesize = "<A href=\"".$Globals{'maindir'}."/showphoto.php?photo=$id&amp;papass=$papass&amp;sort=$sort\">$medsize</a>, <A
                    href=\"".$Globals{'maindir'}."/showphoto.php?photo=$id&amp;size=big&amp;papass=$papass&amp;sort=$sort\">$filesize</a>";

                if ( $Globals{'onthefly'} == 1 ) {
                    $imgdisp = "<a href=\"".$Globals{'maindir'}."/showphoto.php?photo=$photo&amp;size=big&amp;papass=$papass&amp;sort=$sort\"><img
                ...
                if ( is_multimedia($bigimage) == 1 ) {
                     ...
                     $imgdisp = "<a href=\"".$Globals{'datadir'}."/$cat/$iuserid$filename$theext&amp;sort=$sort\"><img src=\"$mmthumb\" border=\"0\" alt=\"\"></a>
                    ...
                    if ( $cuserid > 0 ) {
                        if ($Globals{'vbversion'} == "2.0.3" || $Globals{'vbversion'} == "2.2.0") {
                            $posts .= "<a href=\"$profilelink\" target=\"_blank\"><img src=\"".$Globals{'vbulletin'}."/images/profile.gif\"
                                ...
                                border=\"0\"
                                alt=\"Find more posts by $cuser\"></a><!-- CyKuH [WTN]--> <a
    ...
    if ( $posts != ""  ) {
        ...
        print "$output$comq<p>".$Globals{'cright'}."$footer";
    }
