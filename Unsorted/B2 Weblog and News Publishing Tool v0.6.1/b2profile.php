...
case "update":
	...
	if ($use_ljupdate) {
		$newuser_lj=addslashes($HTTP_POST_VARS["newuser_lj"]);
		if ($HTTP_POST_VARS["ljpass1"] == "") {
			if ($HTTP_POST_VARS["ljpass2"] != "")
				die ("<strong>ERROR</strong>: you typed your new LiveJournal password only once. Go back to type it twice.");
			$updateljpassword = "";
		} else {
			if ($HTTP_POST_VARS["ljpass2"] == "")
				die ("<strong>ERROR</strong>: you typed your new LiveJournal password only once. Go back to type it twice.");
			if ($HTTP_POST_VARS["ljpass1"] != $HTTP_POST_VARS["ljpass2"])
				die ("<strong>ERROR</strong>: you typed two different LiveJournal passwords. Go back to correct that.");
			$newljuser_pass = $HTTP_POST_VARS["ljpass1"];
			$updateljpassword = ", user_pass='$newljuser_pass'";
		}
		$query = "REPLACE $tableljusers SET ID = $user_ID, user_login='$newuser_lj'".$updateljpassword;
		$result = mysql_query($query);
		if ($result==false) {
			die ("<strong>ERROR</strong>: couldn't update your LiveJournal profile... ".mysql_error()."<br />please contact the <a href=\"mailto:$admin_email\">webmaster</a> !<br /><br />$query<br /><br />".mysql_error());
		}
	}
...
*/	$profiledata=get_userdata($user);
	...
	<?php if ($use_ljupdate) {
		$ljprofiledata=get_ljuserdata($user); ?>
		<tr>
		<td align="right"><strong>LiveJournal</strong></td>
		<td><?php echo $ljprofiledata["user_login"] ?></td>
		</tr>
	<?php } ?>
	</table>
...
default:
	...
	<?php if ($use_ljupdate) { ?>
		...
		<td>
		<br />
		new <strong>LJ password</strong> (twice)<br>
		<input type="password" name="ljpass1" size="16" value="" class="postform" /><br>
		<input type="password" name="ljpass2" size="16" value="" class="postform" />
		</td>
		</tr>
	<?php } ?>
<?php
