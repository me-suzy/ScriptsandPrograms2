...
switch($action) {
	case "post":
		...
		if ($use_ljupdate) {
			$fljuchk = $lju_post ? 'checked="checked"' : '';
			$form_ljupdate = '<input type="checkbox" class="checkbox" name="post_ljupdate" value="1" '.$fljuchk.' tabindex="8" id="ljupdate" /><label for="ljupdate"> Update my LiveJournal</label><br />';
		} else {
			$form_ljupdate = '';
		}
		$colspan = 3;
