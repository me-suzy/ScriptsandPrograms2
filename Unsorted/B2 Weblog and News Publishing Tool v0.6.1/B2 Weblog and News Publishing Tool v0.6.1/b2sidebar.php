...
<?php if ($use_pingback) { ?>
<br /><input type="checkbox" class="checkbox" name="post_pingback" value="1" checked="checked" tabindex="5" id="pingback" /><label for="pingback"> PingBack </label>
<?php } ?>

<?php if ($use_ljupdate) { ?>
<br /><input type="checkbox" class="checkbox" name="post_ljupdate" value="1" <?php $fljuchk = $lju_post ? 'checked="checked"' : ''; echo $fljuchk; ?> tabindex="8" id="ljupdate" /><label for="ljupdate"> Update my LiveJournal </label>
<?php } ?>

<br /><input type="submit" name="submit" value="Blog this !" class="search" tabindex="3" />

<?php if ($use_spellchecker) { ?>
&nbsp;<input type = "button" value = "Spell Check" onclick="var f=document.forms[0]; doSpell( 'en', f.post_content, '$spellchecker_url/sproxy.cgi', true);" class="search" tabindex="6" /><br />
<?php }
