...
function check_permission($type, $cat_id = 0) {
  ...
  elseif ($cat_cache[$cat_id][$type] == AUTH_USER && $user_info['user_level'] == USER) {
    return true;
  }
  elseif ($cat_cache[$cat_id][$type] == AUTH_ACL && $user_info['user_level'] == USER) {
...
function get_permission() {
  ...
  $user_access = array();
  if ($user_info['user_id'] != GUEST && $user_info['user_id'] != USER_AWAITING) {
    $current_time = time();
    /*
