...
if ($action == "editcomment") {
  ...
  show_input_row($lang['field_date'].$lang['date_format'].$lang['date_desc'], "comment_date", $comment['comment_date'], $textinput_size);
...
if ($action == "findcomments") {
  ...
  if ($countcomments['comments'] > 0) {
    ...
    while ($comment_row = $site_db->fetch_array($result)) {
      ...
      $show_comment = "<b>".htmlspecialchars($comment_row['comment_headline'])."</b><br />";
      ...
      $show_user_name = htmlspecialchars($comment_row['user_name']);
