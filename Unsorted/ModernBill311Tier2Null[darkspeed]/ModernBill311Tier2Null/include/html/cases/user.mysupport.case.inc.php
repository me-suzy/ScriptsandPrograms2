...
if(isset($submit)&&isset($check_id)&&isset($new_status_id))
{
   for($i=0;$i<=count($check_id)-1;$i++){
       $old_status = mysql_one_data("SELECT call_status FROM support_desk WHERE client_id=$this_user[0] AND call_id = '$check_id[$i]'");
       $result     = mysql_query("UPDATE support_desk SET call_status = '$new_status_id', client_id = '$this_user[0]' WHERE call_id = '$check_id[$i]'");
       $details_view = 1;
...
if(isset($submit)&&$mytype == "insert_call") {
    ...
    $this_client    = load_client_array($this_user[0]);
    $email_to       = array($this_user[0]);
    $email_priority = trim($call_priority);
    $email_subject  = trim(SUPPORTDESK.": ".ID."#$call_id: ".$call_subject);
    ...
    $email_body    .= PHONE.": ".$this_client[client_phone1]."\n\n";
    $email_body    .= str_pad(" ".ID."#$call_id: ".SUBJECT." ",40,"=",STR_PAD_BOTH)."\n";
    $email_body    .= $call_subject."\n\n";
    $email_body    .= str_pad(" ".QUESTION." ",40,"=",STR_PAD_BOTH)."\n";
    $email_body    .= $call_question."\n\n";
    $email_body    .= str_pad(" ".ERROR." ",40,"=",STR_PAD_BOTH)."\n";
    $email_body    .= $call_error."\n\n";
    $email_from     = "$this_client[client_fname] $this_client[client_lname] <$this_client[client_email]>";
    send_email($email_to,$email_cc,$email_priority,$email_subject,$email_body,$email_from);
}
if(isset($submit)&&$mytype == "update_call") {
    list($old_call_response) = mysql_fetch_row(mysql_query("SELECT call_response FROM support_desk WHERE client_id=$this_user[0] AND call_id='$call_id'"));
    ...
    $update_sql = "UPDATE support_desk SET call_response = '$new_call_response' WHERE client_id=$this_user[0] AND call_id='$call_id'";
    ...
    $email_body    .= $new_call_response."\n\n";
    $email_from     = "$this_client[client_fname] $this_client[client_lname] <$this_client[client_email]>";
    send_email($email_to,$email_cc,$email_priority,$email_subject,$email_body,$email_from);
}

$result = mysql_query("SELECT call_status, COUNT(*) FROM support_desk WHERE client_id=$this_user[0] GROUP BY call_status");
...
<tr>
  <td>
    <table cellpadding=0 cellspacing=0 border=0 align=center width=100%>
     <tr><td><?=LFH?><b><?=SUPPORTDESK?>:</b><?=EF?> [<a href=<?=$page?>?op=menu&tile=mysupport&type=add><b><?=ADD?></b></a>]</td>
         <td><?=LFH?><b><?=HELPDOCS?>:</b><?=EF?></td>
     <tr>
       <td width=50% valign=top>
         <table>
          <tr><td>
              ...
              <td align=right>
               <?=MFB?>
               [<a href=<?=$page?>?op=menu&tile=mysupport&type=view&id=1><?=($call_stats[1])?$call_stats[1]:0;?></a>]<br>
               [<a href=<?=$page?>?op=menu&tile=mysupport&type=view&id=2><?=($call_stats[2])?$call_stats[2]:0;?></a>]<br>
               [<a href=<?=$page?>?op=menu&tile=mysupport&type=view&id=3><?=($call_stats[3])?$call_stats[3]:0;?></a>]<br>
               <?=EF?>
              </td>
              <td align=right>
               <?=MFB?>
...
<tr>
  ...
  switch ($type) {
          case view:
               ...
               start_box($num." ".$title);
               $sql = "SELECT * FROM support_desk WHERE client_id=$this_user[0] AND call_status = $call_status";
               $result = mysql_query($sql,$dbh);
               ?>
                <table cellpadding=0 cellspacing=0 border=0 width=100%><tr><td bgcolor=DDDDDD>
                <table cellpadding=2 cellspacing=1 border=0 width=100%>
                  <tr>
                     <td bgcolor=DDDDDD align=center width=15%><b><?=SFB.CALLID.EF?></b></td>
                     <td bgcolor=DDDDDD align=left width=40%><b><?=SFB.SUBJECT.EF?></b></td>
                     <td bgcolor=DDDDDD align=center width=15%><b><?=SFB.STATUS.EF?></b></td>
               ...
               while($this_support_call = mysql_fetch_array($result))
               {
                 ...
                 ?>
                 <tr>
                     <td bgcolor=FFFFFF align=center><?=SFB.$this_support_call[call_id].EF?></td>
                     <td bgcolor=FFFFFF align=left><?=SFB?><a href=<?=$page?>?op=menu&tile=mysupport&type=details&id=<?=$this_support_call[call_id]?>><?=$this_support_call[call_subject]?></a><?=EF?></td>
          ...
          case details:
               ...
               $sql = "SELECT * FROM support_desk WHERE client_id=$this_user[0] AND call_id = $id";
               $result = mysql_query($sql,$dbh);
               ...
               <tr><td bgcolor=DDDDDD>
                <table cellpadding=2 cellspacing=1 border=0 width=100%>
                  <tr>
                     ...
                     <td bgcolor=DDDDDD align=left width=40%><b><?=SFB.SUBJECT.EF?></b></td>
                     <td bgcolor=DDDDDD align=center width=15%><b><?=SFB.STATUS.EF?></b></td>
                     <td bgcolor=DDDDDD align=center width=15%><b><?=SFB.TECH.EF?></b></td>
                     <td bgcolor=DDDDDD align=center width=15%><b><?=SFB.TIMESTAMP.EF?></b></td>
                 </tr>
                 <tr>
                     <td bgcolor=FFFFFF align=center valign=top><?=SFB.$this_support_call[call_id].EF?></td>
                     <td bgcolor=FFFFFF align=left valign=top><?=SFB?><a href=<?=$page?>?op=menu&tile=mysupport&type=details&id=<?=$this_support_call[call_id]?>><?=$this_support_call[call_subject]?></a><?=EF?></td>
               ...
               <tr><td bgcolor=DDDDDD>
                 ...
                 <tr>
                  <td bgcolor=FFFFFF align=right valign=top><b><?=SFB.RESPONSE.":".EF?></b></td>
                  <td bgcolor=FFFFFF valign=top>
                      <?=nl2br(htmlspecialchars(stripslashes($this_support_call[call_response])))?>
                      <? if ($this_support_call[call_status]!=3) { ?>
                            <form method=post action=<?=$page?>?op=menu&tile=mysupport&type=details&id=<?=$id?>>
                            <input type=hidden name=mytype value=update_call>
                            <input type=hidden name=call_id value=<?=$id?>>
                            <textarea name=call_response rows=5 cols=60 maxlength=5000 wrap=VIRTUAL></textarea><br>
          ...
          case add:
               ...
               <td><textarea name=call_question rows=12 cols=80 maxlength=2000 wrap=VIRTUAL><?=$call_question?></textarea></td>
               </tr>
               <tr>
