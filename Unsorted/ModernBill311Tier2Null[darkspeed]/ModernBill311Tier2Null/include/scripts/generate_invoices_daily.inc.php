...
while($client_info=mysql_fetch_array($result))
{
     ...
     $new_client_row = "<tr><td colspan=8>&nbsp;".SF."<b>(".$client_info['client_id'].")".$client_info['client_fname']." ".$client_info['client_lname']."</b>".E;

     #------------------------------------------------------#
     #  Loop through all "active" packages for this client  #
     #------------------------------------------------------#
     $sql="SELECT * FROM client_package WHERE
                    client_id = ".$client_info['client_id']." AND
                    cp_status = 2 AND
                    cp_renew_stamp > ".mktime(0,0,0,date("m"),date("d"),date("Y")-1).""; # <-- 2 = active
     ...
     while($client_package=mysql_fetch_array($result2)) {
          ...
          #------------------------------------------------------#
                ...
                list($renew_on_this_year,$renew_on_this_month,$renew_on_this_day) = explode("/",$start_date);
                ...
                switch($client_package['cp_billing_cycle'])
                {
                     case 1:   #"1"   => MONTHLY,
                               ...
                               $next_renew_stamp   = date_to_stamp($next_renew_date);
                     ...
                     case 100: #"100" => ONETIME." ".NORENEWAL);
                               ...
                               $next_renew_stamp   = 1;//date_to_stamp(0);
                               $this_pack_price    = ($client_package[pack_price]>0) ? $client_package[pack_price] : split_price($package_type['pack_price'],"price",1) ;
                     ...
                     default:
                               ...
                               $full_pay           = ($this_pack_price * $client_package['cp_qty'] * $client_package['cp_billing_cycle']) ;
                               $is_domain          = FALSE;
                     break;
                }
                ...
                echo"
                <pre>
                if ( ( ".date("Yz")." == ".date("Yz",$client_package['cp_renew_stamp'])." ) ||
                     ( ( $today_day_of_year_stamp >= ".$client_package['cp_renew_stamp']." ) &&
                       ( ( ".date("Yz",$client_package['cp_renewed_on'])." != ".date("Yz",$next_renew_stamp)." ) ||
                         ( ".$client_package['cp_renewed_on']." == 0 ) ) ) )
                </pre>";
                }

                /* --- DID WE RENEW YET? --- */
                     # today_day == renew_day OR
                if ( ( date("Yz") == date("Yz",$client_package['cp_renew_stamp']) ) ||
                     # today_stamp >= renew_stamp AND renewed_day != next_renew_day OR renewed == 0
                     ( ( $today_day_of_year_stamp >= $client_package['cp_renew_stamp'] ) &&
                       ( ( date("Yz",$client_package['cp_renewed_on']) != date("Yz",$next_renew_stamp) ) ||
                         ( $client_package['cp_renewed_on'] == 0 ) ) ) )
                {
                ...
                $update_sql = "UPDATE client_package SET cp_renew_stamp='$raw_cp_renew_stamp', cp_renewed_on='".$client_package['cp_renew_stamp']."' WHERE cp_id='".$client_package['cp_id']."'";
                ...
                $total_cp = $full_pay + $setup_pay_amount;
                ...
                */
                ...
                $rows .= /* Pack Name  */  S.$package_type['pack_name'].": ".$list_domains.
                         ...
                         /* PayPeriod  */ MC.$client_package['cp_billing_cycle'].
          ...
          } # <-- END RENEWED THIS MONTH

          ## --- DEBUGGING DATA --- ##
          if ($debug) echo "-n-e-x-t--p-a-c-k-a-g-e--$debug_br$debug_br";

    } # <-- Go to next package in loop for this client!
    ...
    if ($invoice_insert)
    {
        ...
        while(list($credit_id,$credit_amount,$credit_comments,$credit_stamp)=mysql_fetch_row($credit_result))
        {
          ...
          {
             $update_sql = "UPDATE client_credit SET credit_amount='0', credit_comments='$credit_comments\n\n$credit_amount ~ ".date("$date_format: h:i:s")."', credit_stamp='".stamp_to_date(mktime())."' WHERE credit_id='$credit_id' ";
    ...
    if ($tax_enabled)
    {
       $rows .= "<tr><td colspan=9 align=right>".SF." (".CREDIT."): ".MR.display_currency($this_credit).E;
       $rows .= "<tr><td colspan=9 align=right>".SF.SUBTOTAL.": ".MR."<b>".display_currency($this_client_owes_pre_tax)."</b>".E;
       $tax_due = $this_client_owes_pre_tax * $tax_amount;
       $this_tax = $tax_amount * 100;
       if ($tax_number) $this_tax_number = " (ID: $tax_number)";
       $rows .= "<tr><td colspan=9 align=right>".SF."$this_tax%".TAXDUE." $this_tax_number: ".MR."<b>".display_currency($tax_due)."</b>".E;
       $total_due = $this_client_owes_pre_tax + $tax_due;
       $rows .= "<tr><td colspan=9 align=right>".SF.TOTALDUE.": ".MR."<b>".display_currency($total_due)."</b>".E;
    ...
    {
       $rows .= "<tr><td colspan=9 align=right bgcolor=FFFFFF>".SF.SUBTOTAL.": ".MR.display_currency($this_client_owes).E;
       $rows .= "<tr><td colspan=9 align=right bgcolor=FFFFFF>".SF." (".CREDIT."): ".MR.display_currency($this_credit).E;
       $rows .= "<tr><td colspan=9 align=right bgcolor=FFFFFF>".SF.TOTALDUE.": ".MR."<b>".display_currency($this_client_owes_pre_tax)."</b>".E;
    ...
    if ($debug) {
        echo "Sub Total: ".display_currency($this_client_owes)."$debug_br
              ...
              $this_tax%".TAXDUE." $this_tax_number: ".display_currency($total_due)."
