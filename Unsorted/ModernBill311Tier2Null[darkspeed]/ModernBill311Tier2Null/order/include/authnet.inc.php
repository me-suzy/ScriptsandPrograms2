...
include($DIR."include/misc/authnet.php");

$string  = "x_Invoice_Num=". urlencode(getenv("REMOTE_ADDR"))."&";
...
if ($allow_credit_card&&$pay_method=="creditcard")
{
    ...
    $string .= "x_Card_Name=".   urlencode($x_Card_Name)."&";
    $string .= "x_Card_Bank=".   urlencode($x_Card_Bank)."&";
    if ($require_cvvc_code)
    {
        $string .= "x_Card_Code=".   urlencode($x_Card_Code)."&";
    }
}
if ($echeck_enabled&&$allow_echeck&&$pay_method=="echeck")
{
    $string .= "x_Bank_Name=".            urlencode($x_Bank_Name)."&";
    $string .= "x_Bank_ABA_Code=".        urlencode($x_Bank_ABA_Code)."&";
    $string .= "x_Bank_Acct_Num=".        urlencode($x_Bank_Acct_Num)."&";
    $string .= "x_Drivers_License_Num=".  urlencode($x_Drivers_License_Num)."&";
    $string .= "x_Drivers_License_State=".urlencode($x_Drivers_License_State)."&";
    $string .= "x_Drivers_License_DOB=".  urlencode($x_Drivers_License_DOB)."&";
}

$string .= "x_Cust_ID=$x_Email&";
$string .= "x_First_Name=".  urlencode($x_First_Name)."&";
$string .= "x_Last_Name=".   urlencode($x_Last_Name)."&";
$string .= "x_Company=".     urlencode($x_Company)."&";
$string .= "x_Address=".     urlencode($x_Address)."&";
$string .= "x_Address_2=".   urlencode($x_Address_2)."&";
