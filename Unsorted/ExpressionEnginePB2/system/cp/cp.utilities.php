...
class Utilities {
	...
	function plugin_manager()
	{
        ...
        if (count($plugins) > 0)
        {
            foreach ($plugins as $plugin)
            {
                ...
                $r .= $DSP->tr();
                $r .= $DSP->table_qcell($style, $i.$DSP->nbs(2).$DSP->qspan('defaultBold', $name), '60%');
                $r .= $DSP->table_qcell($style, $DSP->anchor(BASE.AMP.'C=admin'.AMP.'M=utilities'.AMP.'P=plugin_info'.AMP.'name='.$plugin, $LANG->line('view_info')), '40%');      
                $r .= $DSP->tr_c();
            }
        }
        ...
        $DSP->title  = $LANG->line('installed_plugins');
        $DSP->crumb  = $LANG->line('installed_plugins');
        $DSP->body   = &$r;  
	}
	// END
    ...
    {  
        ...
        {            
            ...
            $qtypes = array('CREATE', 'DROP', 'FLUSH');
                ...
                if ( ! preg_match("/LIMIT\s+[0-9]/i", $sql))
                {
                    $result = $DB->query($sql); 
                     ...
                     { 
                        ...
                        $url = 'C=admin'.AMP.'M=utilities'.AMP.'P=run_query'.AMP.'thequery='.urlencode($sql);
    ...
    {  
        ...
        $DSP->body = $DSP->heading($LANG->line('search_and_replace')).                    
   ...
   // END
    ...
    {  
        ...
        $r  = $DSP->heading($LANG->line('translation_tool'));
        $r .= $DSP->form(BASE.AMP.'C=admin'.AMP.'M=utilities'.AMP.'P=save_translation');
