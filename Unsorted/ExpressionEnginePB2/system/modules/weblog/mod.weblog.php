    ...
    function add_pagination_data()
    {
        ...
        if ($this->paginate == TRUE)
        {
        	...
        	if (preg_match("/".LD."if previous_page".RD."(.+?)".LD.SLASH."if".RD."/s", $this->paginate_data, $match))
        	{
        		...
        		{
					$match['1'] = str_replace(LD.'path'.RD, $this->page_previous, $match['1']);
        	...
        	if (preg_match("/".LD."if next_page".RD."(.+?)".LD.SLASH."if".RD."/s", $this->paginate_data, $match))
        	{
        		...
        		{
					$match['1'] = str_replace(LD.'path'.RD, $this->page_next, $match['1']);
    ...
    function build_sql_query()
    {
		...
		{		
			...
			else
			{
				...
				if ($cat_id == '' AND $current_page == '' AND $year == '')
				{
        ...
        $f_sql = "SELECT DISTINCT(exp_weblog_titles.url_title),
        					 exp_weblog_titles.*,
                             exp_weblog_data.*,
                             exp_member_data.*,
                             exp_weblogs.blog_title, exp_weblogs.blog_url, exp_weblogs.weblog_html_formatting, exp_weblogs.weblog_allow_img_urls, exp_weblogs.weblog_auto_link_urls, exp_weblogs.enable_trackbacks, exp_weblogs.trackback_field, exp_members.username, exp_members.email, exp_members.url, exp_members.screen_name, exp_members.location, exp_members.interests, exp_members.aol_im, exp_members.yahoo_im, exp_members.msn_im, exp_members.icq, exp_members.group_id, exp_members.member_id ";
              
		$p_sql = "SELECT COUNT(exp_weblog_titles.entry_id) AS count ";
        ...
        $this->sql .= "exp_weblog_titles.entry_date < ".$timestamp." ";
                   
        $this->sql .= "AND (exp_weblog_titles.expiration_date = 0 || exp_weblog_titles.expiration_date > ".$timestamp.") ";
		...
		if ($not_entry_id = $TMPL->fetch_param('not_entry_id'))
		{
			...
			{
            	$this->sql .= "AND exp_weblog_titles.entry_id != '$not_entry_id' ";
        	}
        }
        ...
        if ($TMPL->fetch_param('year') || $TMPL->fetch_param('month') || $TMPL->fetch_param('day'))
        {
            ...
            $this->sql .= "AND exp_weblog_titles.year = '".$DB->escape_str($p_year)."' ";
            ...
            if ($p_month)
            {
                ...
                $this->sql .= "AND exp_weblog_titles.month = '".$DB->escape_str($p_month)."' ";
            } 
            ...
            if ($day = $TMPL->fetch_param('day'))
            {
                ...
                $this->sql .= "AND exp_weblog_titles.day = '".$DB->escape_str($day)."' ";
            } 
        }
        else
        {
            ...
            if ($year != '' AND $month != '')
            {
				if ($dynamic)
				{
					$this->sql .= "AND exp_weblog_titles.year = '".$DB->escape_str($year)."' AND exp_weblog_titles.month = '".$DB->escape_str($month)."'";
            ...
            else
            {
                ...
                {            
                    ...
                    $this->sql .= "AND exp_weblog_titles.entry_date >= '".$DB->escape_str($time)."' ";
                }
        ...
        {    
			$this->sql .= "AND exp_weblog_titles.url_title = '".$DB->escape_str($qtitle)."' ";
        }
        ...
        else
        {
            ...
            {           
                $this->sql .= " AND exp_categories.cat_id = '".$DB->escape_str($cat_id)."' ";
            }
        }
		...
		// We'll run the modified version of the query to find out
        ...
        {        
			if ($display_by == '')
			{
				...
				{
					$this->sql .= ($current_page == '') ? " LIMIT ".$offset.', '.$limit : " LIMIT ".$current_page.', '.$limit;;  
				}
				else
				{
					$this->sql .= ($current_page == '') ? " LIMIT ".$this->limit : " LIMIT ".$current_page.', '.$this->limit;
				}
			}			
        }
