...
class Template {
    ...
    {
        global $PREFS, $REGX, $LANG, $IN, $FNS;
        
		// -------------------------------------
		//  Sub-template loop prevention
		// -------------------------------------
        ...
        if ($this->parse_php == TRUE AND $this->php_parse_location == 'input')
        {
			...
			echo $FNS->evaluate($this->template);
    ...
    */
    ...
    function assign_variables($str = '', $slash = SLASH)
    {
        ...
        {  
            ...
            if (ereg('\|', $val) AND ! eregi("^switch", $val) AND ! eregi("^multi_field", $val))
            {
		...
		while (FALSE !== ($in_point = strpos($float, $CB)))
		{	
			$depth = 1;
			
			$float = substr($float, $in_point);
						
			if ( ! preg_match("#".$CB."(.+?)".RD."#s", $float, $matches))
				break;
				
			$opening_tag = substr($float, 0, strlen($matches['0']));
		
			$float = substr($float, strlen($matches['0']));
			
			$next_close = strpos($float, $CE);
		
			if (strpos($float, $CB) < $next_close)
			{		
				$temp = substr($float, 0, $next_close);
				
				if (preg_match_all("#".$CB."(.+?)".RD."#s", $temp, $matches))
				{
					$depth += count($matches['0']);
				}
			}
			
			$conds[] = array($opening_tag, $depth);	
		}
		...
		{	
			...
			{		
				...
				{		
					...
					$this->var_cond[] = array($tag, $outer, $inner, $field);
    ...
    {    
        global $LANG, $PREFS, $FNS, $IN, $DB, $LOC;
		...
		$this->template =& str_replace(LD.'ip_address'.RD, $IN->IP, $this->template); 
         
        $this->template =& str_replace(LD.'ip_address'.RD, @gethostbyaddr($IN->IP), $this->template); 
        ...
        $this->template =& str_replace(LD.'site_name'.RD, stripslashes($PREFS->ini('site_name')), $this->template);
        ...
        // --------------------------------------------------
   		...
   		if (preg_match_all("/".LD."current_time\s+format=[\"\'](.+?)[\"\']".RD."/", $this->template, $matches))
   		{	
        	for ($j = 0; $j < count($matches['0']); $j++)
        	{				
				$this->template = preg_replace("/".$matches['0'][$j]."/", $LOC->decode_date($matches['1'][$j], $LOC->now), $this->template, 1);				
        	}
		}  		
