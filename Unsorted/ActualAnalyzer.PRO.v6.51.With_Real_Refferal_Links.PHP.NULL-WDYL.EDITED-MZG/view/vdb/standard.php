<?php
  ...
  if($flag==0||$flag==1) {
      if($sort['column']==1) $ordert='lname ASC';
      elseif(($sort['table']==1&&$sort['column']==2)||($sort['table']==2&&$sort['column']==2)) $ordert='v DESC,lname ASC';
      elseif(($sort['table']==1&&$sort['column']==3)||($sort['table']==3&&$sort['column']==2)) $ordert='hs DESC,lname ASC';
      elseif(($sort['table']==1&&$sort['column']==4)||($sort['table']==4&&$sort['column']==2)) $ordert='r DESC,lname ASC';
      elseif(($sort['table']==1&&$sort['column']==5)||($sort['table']==5&&$sort['column']==2)) $ordert='ht DESC,lname ASC';
  }
  elseif($flag==2||$flag>6) {
      if($sort['column']==1) $ordert='stid ASC';
      elseif(($sort['table']==1&&$sort['column']==2)||($sort['table']==2&&$sort['column']==2)) $ordert='v DESC,stid ASC';
      elseif(($sort['table']==1&&$sort['column']==3)||($sort['table']==3&&$sort['column']==2)) $ordert='hs DESC,stid ASC';
      elseif(($sort['table']==1&&$sort['column']==4)||($sort['table']==4&&$sort['column']==2)) $ordert='r DESC,stid ASC';
      elseif(($sort['table']==1&&$sort['column']==5)||($sort['table']==5&&$sort['column']==2)) $ordert='ht DESC,stid ASC';
  }
  else {
      if($sort['column']==1) $ordert='stname ASC';
      elseif(($sort['table']==1&&$sort['column']==2)||($sort['table']==2&&$sort['column']==2)) $ordert='v DESC,stname ASC';
      elseif(($sort['table']==1&&$sort['column']==3)||($sort['table']==3&&$sort['column']==2)) $ordert='hs DESC,stname ASC';
      elseif(($sort['table']==1&&$sort['column']==4)||($sort['table']==4&&$sort['column']==2)) $ordert='r DESC,stname ASC';
      elseif(($sort['table']==1&&$sort['column']==5)||($sort['table']==5&&$sort['column']==2)) $ordert='ht DESC,stname ASC';
  }
  //total results
  $vsumt=0;
  $hssumt=0;
  ...
  if($flag==0) $request='LOCK TABLES aa_lang_base READ,aa_coun_base READ,aa_lang_total READ';
  elseif($flag==1) $request='LOCK TABLES aa_coun_base READ,aa_coun_total READ';
  else $request='LOCK TABLES aa_st_base READ,aa_st_total READ';
  $result=mysql_query($request,$conf->link);
  if(!$result) {$err->reason('vdb.php|standard|blocking of tables has failed -- '.mysql_error());return;}

  if($flag==0) $request='SELECT lname AS name,counid,'.$values.' FROM aa_lang_total LEFT OUTER JOIN aa_lang_base ON aa_lang_total.langid=aa_lang_base.langid WHERE id='.$page_id.$where.' ORDER BY '.$ordert.' LIMIT '.$begstr.','.$numstr;
  elseif($flag==1) $request='SELECT lname AS name,sname,'.$svalues.' FROM aa_coun_total LEFT OUTER JOIN aa_coun_base ON aa_coun_total.counid=aa_coun_base.counid WHERE id='.$page_id.$where.' GROUP BY name ORDER BY '.$ordert.' LIMIT '.$begstr.','.$numstr;
  elseif($flag==2) $request='SELECT stid AS name,'.$values.' FROM aa_st_total WHERE id='.$page_id.' AND stid<=1000'.$where.' ORDER BY '.$ordert.' LIMIT '.$begstr.','.$numstr;
  elseif($flag==3) $request='SELECT stname AS name,fname,'.$values.' FROM aa_st_total LEFT OUTER JOIN aa_st_base ON aa_st_total.stid=aa_st_base.stid WHERE id='.$page_id.' AND aa_st_total.stid>1000 AND aa_st_total.stid<=2000'.$where.' ORDER BY '.$ordert.' LIMIT '.$begstr.','.$numstr;
  elseif($flag==4) $request='SELECT stname AS name,fname,'.$values.' FROM aa_st_total LEFT OUTER JOIN aa_st_base ON aa_st_total.stid=aa_st_base.stid WHERE id='.$page_id.' AND aa_st_total.stid>2000 AND aa_st_total.stid<=3000'.$where.' ORDER BY '.$ordert.' LIMIT '.$begstr.','.$numstr;
  elseif($flag==5) $request='SELECT stname AS name,'.$values.' FROM aa_st_total LEFT OUTER JOIN aa_st_base ON aa_st_total.stid=aa_st_base.stid WHERE id='.$page_id.' AND aa_st_total.stid>3000 AND aa_st_total.stid<=4000'.$where.' ORDER BY '.$ordert.' LIMIT '.$begstr.','.$numstr;
  elseif($flag==6) $request='SELECT stname AS name,'.$values.' FROM aa_st_total LEFT OUTER JOIN aa_st_base ON aa_st_total.stid=aa_st_base.stid WHERE id='.$page_id.' AND aa_st_total.stid>4000 AND aa_st_total.stid<=5000'.$where.' ORDER BY '.$ordert.' LIMIT '.$begstr.','.$numstr;
  elseif($flag==7) $request='SELECT stid AS name,'.$values.' FROM aa_st_total WHERE id='.$page_id.' AND stid IN (5001,5002,5003)'.$where.' ORDER BY '.$ordert.' LIMIT '.$begstr.','.$numstr;
  elseif($flag==8) $request='SELECT stid AS name,'.$values.' FROM aa_st_total WHERE id='.$page_id.' AND stid IN (6001,6002,6003)'.$where.' ORDER BY '.$ordert.' LIMIT '.$begstr.','.$numstr;
  $result=mysql_query($request,$conf->link);
  if(!$result) {$err->reason('vdb.php|standard|the request \''.$request.'\' has failed -- '.mysql_error());return;}

  if($flag==1) {
      $request='SELECT lname AS name,'.$svalues.' FROM aa_coun_total LEFT OUTER JOIN aa_coun_base ON aa_coun_total.counid=aa_coun_base.counid WHERE id='.$page_id.$where.' GROUP BY name ORDER BY v ASC';
      $result1=mysql_query($request,$conf->link);
      if(!$result1) {$err->reason('vdb.php|standard|the request \''.$request.'\' has failed -- '.mysql_error());return;}
      if(mysql_num_rows($result1)) {
          $row=mysql_fetch_object($result1);
          $vmint=$row->v;
          mysql_data_seek($result1,mysql_num_rows($result1)-1);
          $row=mysql_fetch_object($result1);
          $vmaxt=$row->v;
      }
      mysql_free_result($result1);
      $request='SELECT lname AS name,'.$svalues.' FROM aa_coun_total LEFT OUTER JOIN aa_coun_base ON aa_coun_total.counid=aa_coun_base.counid WHERE id='.$page_id.$where.' GROUP BY name ORDER BY hs ASC';
      $result1=mysql_query($request,$conf->link);
      if(!$result1) {$err->reason('vdb.php|standard|the request \''.$request.'\' has failed -- '.mysql_error());return;}
      if(mysql_num_rows($result1)) {
          $row=mysql_fetch_object($result1);
      ...
      mysql_free_result($result1);
      $request='SELECT lname AS name,'.$svalues.' FROM aa_coun_total LEFT OUTER JOIN aa_coun_base ON aa_coun_total.counid=aa_coun_base.counid WHERE id='.$page_id.$where.' GROUP BY name ORDER BY r ASC';
      $result1=mysql_query($request,$conf->link);
      if(!$result1) {$err->reason('vdb.php|standard|the request \''.$request.'\' has failed -- '.mysql_error());return;}
      if(mysql_num_rows($result1)) {
          $row=mysql_fetch_object($result1);
      ...
      mysql_free_result($result1);
      $request='SELECT lname AS name,'.$svalues.' FROM aa_coun_total LEFT OUTER JOIN aa_coun_base ON aa_coun_total.counid=aa_coun_base.counid WHERE id='.$page_id.$where.' GROUP BY name ORDER BY ht ASC';
      $result1=mysql_query($request,$conf->link);
      if(!$result1) {$err->reason('vdb.php|standard|the request \''.$request.'\' has failed -- '.mysql_error());return;}
      if(mysql_num_rows($result1)) {
          $row=mysql_fetch_object($result1);
      ...
      $nrect=mysql_num_rows($result1);
      mysql_free_result($result1);
  }
  if($flag==0) $request='SELECT '.$minvalues.','.$maxvalues.','.$sumvalues.',COUNT(*) AS nrect FROM aa_lang_total WHERE id='.$page_id.$where;
  elseif($flag==1) $request='SELECT '.$sumvalues.' FROM aa_coun_total WHERE id='.$page_id.$where;
  elseif($flag==2) $request='SELECT '.$minvalues.','.$maxvalues.','.$sumvalues.',COUNT(*) AS nrect FROM aa_st_total WHERE id='.$page_id.' AND aa_st_total.stid<=1000'.$where;
  elseif($flag==3) $request='SELECT '.$minvalues.','.$maxvalues.','.$sumvalues.',COUNT(*) AS nrect FROM aa_st_total WHERE id='.$page_id.' AND aa_st_total.stid>1000 AND aa_st_total.stid<=2000'.$where;
  elseif($flag==4) $request='SELECT '.$minvalues.','.$maxvalues.','.$sumvalues.',COUNT(*) AS nrect FROM aa_st_total WHERE id='.$page_id.' AND aa_st_total.stid>2000 AND aa_st_total.stid<=3000'.$where;
  elseif($flag==5) $request='SELECT '.$minvalues.','.$maxvalues.','.$sumvalues.',COUNT(*) AS nrect FROM aa_st_total WHERE id='.$page_id.' AND aa_st_total.stid>3000 AND aa_st_total.stid<=4000'.$where;
  elseif($flag==6) $request='SELECT '.$minvalues.','.$maxvalues.','.$sumvalues.',COUNT(*) AS nrect FROM aa_st_total WHERE id='.$page_id.' AND aa_st_total.stid>4000 AND aa_st_total.stid<=5000'.$where;
  elseif($flag==7) $request='SELECT '.$minvalues.','.$maxvalues.','.$sumvalues.',COUNT(*) AS nrect FROM aa_st_total WHERE id='.$page_id.' AND stid IN (5001,5002,5003)'.$where;
  elseif($flag==8) $request='SELECT '.$minvalues.','.$maxvalues.','.$sumvalues.',COUNT(*) AS nrect FROM aa_st_total WHERE id='.$page_id.' AND stid IN (6001,6002,6003)'.$where;
  $resultt=mysql_query($request,$conf->link);
  if(!$resultt) {$err->reason('vdb.php|standard|the request \''.$request.'\' has failed -- '.mysql_error());return;}
  if($flag==0) {
      $mascoun=array();
      $idcm=array();
      $idc='';
      while($row=mysql_fetch_object($result)) {
          if(isset($idcm[$row->counid])||!$row->counid) continue;
      ...
      if(!empty($idc)) {
          ...
          if(!$resultc) {$err->reason('vdb.php|standard|the request \''.$request.'\' has failed -- '.mysql_error());return;}
  ...
  if(!$resultu) {$err->reason('vdb.php|standard|the request \''.$request.'\' has failed -- '.mysql_error());return;}

  while($row=mysql_fetch_object($resultt)) {
      if($flag!=1) $nrect=$row->nrect;
      if($nrect) {
          $vsumt=$row->sumv;
          $hssumt=$row->sumhs;
          $htsumt=$row->sumht;
  ...
  if($flag==0) { $vars['ITEM']=_LANGUAGE; $vars['RHEADER']=_LANGUAGES; }
  elseif($flag==1) { $vars['ITEM']=_COUNTRY; $vars['RHEADER']=_COUNTRIES; }
  elseif($flag==2) { $vars['ITEM']=_COLORDEPTH; $vars['RHEADER']=_COLORDEPTH; }
  elseif($flag==3) { $vars['ITEM']=_BROWSER; $vars['RHEADER']=_BROWSERS; }
  elseif($flag==4) { $vars['ITEM']=_OS; $vars['RHEADER']=_OSS; }
  elseif($flag==5) { $vars['ITEM']=_JAVASCRIPT; $vars['RHEADER']=_JAVASCRIPT; }
  elseif($flag==6) { $vars['ITEM']=_SRESOLUTION; $vars['RHEADER']=_SRESOLUTIONS; }
  elseif($flag==7) { $vars['ITEM']=_COOKIE; $vars['RHEADER']=_COOKIE; }
  elseif($flag==8) { $vars['ITEM']=_JAVA; $vars['RHEADER']=_JAVA; }
  ...
  if(mysql_num_rows($result)) {
      ...
      while($row=mysql_fetch_object($result)) {
          ...
          $rd='';
          if($flag==0) {
              if(isset($mascoun[$row->counid])) $nm.=' ('.$mascoun[$row->counid]['lname'].')';
              $vars['NAME']=$nm;
              tparse($center,$vars);
          }
          elseif($flag==1) {
              if(!strcmp($nm,'undefined')) $nm=_UNDEFINED;
              $vars['CAT']='flags';
              $vars['IMG']=$row->sname;
              $vars['NAME']=$nm;
              $rd=$row->sname;
          ...
          elseif($flag==2) {
              if($row->name==1000) $nm=_UNDEFINED;
              else $nm.=' bit';
              $vars['NAME']=$nm;
              tparse($center,$vars);
          }
          ...
          elseif($flag==7) {
              if($row->name==5001) { $nm=_ENABLED; $vars['IMG']='enabled'; $rd='enabled'; }
              elseif($row->name==5002) { $nm=_DISABLED;  $vars['IMG']='disabled'; $rd='disabled'; }
              elseif($row->name==5003) $nm=_UNDEFINED;
              $vars['NAME']=$nm;
              if($row->name<5003) tparse($icenter,$vars);
              else tparse($center,$vars);
          }
          elseif($flag==8) {
              if($row->name==6001) { $nm=_ENABLED; $vars['IMG']='enabled'; $rd='enabled'; }
              elseif($row->name==6002) { $nm=_DISABLED;  $vars['IMG']='disabled'; $rd='disabled'; }
              elseif($row->name==6003) $nm=_UNDEFINED;
              $vars['NAME']=$nm;
              if($row->name<6003) tparse($icenter,$vars);
              else tparse($center,$vars);
          }
          else {
              if(!strcmp($nm,'undefined')) $nm=_UNDEFINED;
              else if($flag==6) { $nm=preg_split("/_/",$nm); $nm=$nm[1]; }
              $vars['NAME']=$nm;
              tparse($center,$vars);
          }
          //frames parameters
          ...
          $request='INSERT INTO aa_rdata (id,added,num,name,addpar,vi,vp,v,hsi,hsp,hs,ri,rp,r,hti,htp,ht) VALUES ('.$rdid.','.$conf->ctime.','.$rdnum.',"'.$nm.'","'.$rd.'",0,'.($vsumt?sprintf("%.2f",$row->v/$vsumt*100):'0.00').','.$row->v.',0,'.($hssumt?sprintf("%.2f",$row->hs/$hssumt*100):'0.00').','.$row->hs.',0,'.($rsumt?sprintf("%.2f",$row->r/$rsumt*100):'0.00').','.$row->r.',0,'.($htsumt?sprintf("%.2f",$row->ht/$htsumt*100):'0.00').','.$row->ht.')';
          $result1=mysql_query($request,$conf->link);
          if(!$result1) {$err->reason('vdb.php|standard|the request \''.$request.'\' has failed -- '.mysql_error());return;}
          $rdnum++;
      }
  ...
  if(mysql_num_rows($result)) {
      ...
      while($row=mysql_fetch_object($result)) {
          ...
          elseif($flag==7) {
              if($row->name==5001) { $nm=_ENABLED; $vars['IMG']='enabled'; }
              elseif($row->name==5002) { $nm=_DISABLED;  $vars['IMG']='disabled'; }
              elseif($row->name==5003) $nm=_UNDEFINED;
              $vars['NAME']=$nm;
              if($row->name<5003) tparse($icenter,$vars);
              else tparse($center,$vars);
          ...
          elseif($flag==8) {
              if($row->name==6001) { $nm=_ENABLED; $vars['IMG']='enabled'; }
              elseif($row->name==6002) { $nm=_DISABLED;  $vars['IMG']='disabled'; }
              elseif($row->name==6003) $nm=_UNDEFINED;
              $vars['NAME']=$nm;
              if($row->name<6003) tparse($icenter,$vars);
              else tparse($center,$vars);
