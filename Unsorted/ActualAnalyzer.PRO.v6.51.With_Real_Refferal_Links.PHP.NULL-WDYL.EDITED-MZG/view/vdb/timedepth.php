<?php
  ...
  if($flag<3) {
      if($sort['column']==1) $ordert='rangeid ASC';
      elseif(($sort['table']==1&&$sort['column']==2)||($sort['table']==2&&$sort['column']==2)) $ordert='v DESC,rangeid ASC';
      elseif(($sort['table']==1&&$sort['column']==3)||($sort['table']==3&&$sort['column']==2)) $ordert='hs DESC,rangeid ASC';
      elseif(($sort['table']==1&&$sort['column']==4)||($sort['table']==4&&$sort['column']==2)) $ordert='r DESC,rangeid ASC';
      elseif(($sort['table']==1&&$sort['column']==5)||($sort['table']==5&&$sort['column']==2)) $ordert='ht DESC,rangeid ASC';
  }
  else {
      if($sort['column']==1) $ordert='pages ASC';
      elseif(($sort['table']==1&&$sort['column']==2)||($sort['table']==2&&$sort['column']==2)) $ordert='v DESC,pages ASC';
      elseif(($sort['table']==1&&$sort['column']==3)||($sort['table']==3&&$sort['column']==2)) $ordert='hs DESC,pages ASC';
      elseif(($sort['table']==1&&$sort['column']==4)||($sort['table']==4&&$sort['column']==2)) $ordert='r DESC,pages ASC';
      elseif(($sort['table']==1&&$sort['column']==5)||($sort['table']==5&&$sort['column']==2)) $ordert='ht DESC,pages ASC';
  }
  //total results
  $vsumt=0;
  $hssumt=0;
  ...
  if($flag<3) $request='LOCK TABLES aa_times READ';
  else $request='LOCK TABLES aa_depthes READ';
  $result=mysql_query($request,$conf->link);
  if(!$result) {$err->reason('vdb.php|timedepth|blocking of tables has failed -- '.mysql_error());return;}

  if($flag<3) $request='SELECT rangeid,'.$values.' FROM aa_times WHERE id='.$page_id.$where.' AND flag='.$flag.' ORDER BY '.$ordert.' LIMIT '.$begstr.','.$numstr;
  else $request='SELECT pages,'.$values.' FROM aa_depthes WHERE id='.$page_id.$where.' ORDER BY '.$ordert.' LIMIT '.$begstr.','.$numstr;
  $result=mysql_query($request,$conf->link);
  if(!$result) {$err->reason('vdb.php|timedepth|the request \''.$request.'\' has failed -- '.mysql_error());return;}

  if($flag<3) $request='SELECT MAX(rangeid) AS mr,'.$minvalues.','.$maxvalues.','.$sumvalues.',COUNT(*) AS nrect FROM aa_times WHERE id='.$page_id.' AND flag='.$flag.$where;
  else $request='SELECT '.$minvalues.','.$maxvalues.','.$sumvalues.',COUNT(*) AS nrect FROM aa_depthes WHERE id='.$page_id.$where;
  $resultt=mysql_query($request,$conf->link);
  if(!$resultt) {$err->reason('vdb.php|timedepth|the request \''.$request.'\' has failed -- '.mysql_error());return;}

  $request='UNLOCK TABLES';
  $resultu=mysql_query($request,$conf->link);
  if(!$resultu) {$err->reason('vdb.php|timedepth|the request \''.$request.'\' has failed -- '.mysql_error());return;}
  ...
  if($flag==1) $vars['RHEADER']=_TIMEONGRPG;
  elseif($flag==2) $vars['RHEADER']=_RETBACK;
  elseif($flag==3) $vars['RHEADER']=_DEPTHOFVIEW;
  if($flag==1) $vars['ITEM']=_TIMEINT;
  elseif($flag==2) $vars['ITEM']=_TIMEINT;
  elseif($flag==3) $vars['ITEM']=_DEPTH;
  ...
  if($flag==3) $vars['SORTBYN']=_SORTBYN;
  else $vars['SORTBYN']=_SORTBYT;
  ...
  if(!$result1) {$err->reason('vdb.php|timedepth|the request \''.$request.'\' has failed -- '.mysql_error());return;}
  ...
  if(mysql_num_rows($result)) {
      ...
      while($row=mysql_fetch_object($result)) {
          $cid++;
          if($flag<3&&$sort['column']==1) {
              for(;$cid<$row->rangeid;$cid++) {
                  $vars['NUM']=$num++;
                  $vars['VISITORS']=0;
                  $vars['HOSTS']=0;
                  $vars['HITS']=0;
                  $vars['RELOADS']=0;
                  $vars['NAME']=$rettime[$cid];
                  tparse($center,$vars);

                  $request='INSERT INTO aa_rdata (id,added,num,name,addpar,vi,vp,v,hsi,hsp,hs,ri,rp,r,hti,htp,ht) VALUES ('.$rdid.','.$conf->ctime.','.$rdnum.',"'.$rettime[$cid].'","",0,0.00,0,0,0.00,0,0,0.00,0,0,0.00,0)';
                  $result1=mysql_query($request,$conf->link);
                  if(!$result1) {$err->reason('vdb.php|timedepth|the request \''.$request.'\' has failed -- '.mysql_error());return;}
                  $rdnum++;
                  $vmin=0; $hsmin=0; $htmin=0; $rmin=0; $vmint=0; $hsmint=0; $htmint=0; $rmint=0;
              }
          }
          $vars['NUM']=$num++;
          $vars['VISITORS']=$row->v;
          ...
          $vars['HITS']=$row->ht;
          $vars['RELOADS']=$row->r;
          if($flag<3) { $vars['NAME']=$rettime[$row->rangeid]; $present[$row->rangeid]=1; }
          else $vars['NAME']=$row->pages.' '._PAGE_S;
          tparse($center,$vars);
          //frames parameters
          $vmax=max($vmax,$row->v);
  ...
  if(mysql_num_rows($result)) {
      ...
      while($row=mysql_fetch_object($result)) {
          ...
          $vars['NUM']=$num++;
          if($flag<3) $vars['NAME']=$rettime[$row->rangeid];
          else $vars['NAME']=$row->pages.' '._PAGE_S;
          $vars['TOTAL']=$row->v;
