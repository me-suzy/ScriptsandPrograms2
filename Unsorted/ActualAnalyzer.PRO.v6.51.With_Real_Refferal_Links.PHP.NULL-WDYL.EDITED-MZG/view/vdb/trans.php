<?php
  ...
  if($flag!=1) { $field='sourid'; $nfield='destid'; }
  else { $field='destid'; $nfield='sourid'; }
  ...
  $request='LOCK TABLES aa_vectors READ,aa_pages READ,aa_groups READ';
  $result=mysql_query($request,$conf->link);
  if(!$result) {$err->reason('vdb.php|trans|blocking of tables has failed -- '.mysql_error());return;}

  if($page_id<201) $request='SELECT '.$values.',aa_pages.id AS id,aa_pages.name AS name,aa_pages.url AS url FROM aa_vectors LEFT OUTER JOIN aa_pages ON aa_vectors.'.$nfield.'=aa_pages.id WHERE aa_vectors.'.$field.'='.$page_id.$where.' ORDER BY '.$ordert.' LIMIT '.$begstr.','.$numstr;
  else $request='SELECT '.$values.',aa_groups.id AS id,aa_groups.name AS name FROM aa_vectors LEFT OUTER JOIN aa_groups ON aa_vectors.'.$nfield.'=aa_groups.id WHERE aa_groups.id IS NOT NULL AND aa_vectors.'.$field.'='.$page_id.$where.' ORDER BY '.$ordert.' LIMIT '.$begstr.','.$numstr;
  $result=mysql_query($request,$conf->link);
  if(!$result) {$err->reason('vdb.php|trans|the request \''.$request.'\' has failed -- '.mysql_error());return;}

  $request='SELECT '.$minvalues.','.$maxvalues.','.$sumvalues.',COUNT(*) AS nrect FROM aa_vectors WHERE '.$field.'='.$page_id.$where;
  $resultt=mysql_query($request,$conf->link);
  if(!$resultt) {$err->reason('vdb.php|trans|the request \''.$request.'\' has failed -- '.mysql_error());return;}

  $request='UNLOCK TABLES';
  $resultu=mysql_query($request,$conf->link);
  if(!$resultu) {$err->reason('vdb.php|trans|the request \''.$request.'\' has failed -- '.mysql_error());return;}
  ...
  if($flag==1) $vars['RHEADER']=_TRANSTO;
  elseif($flag==2) $vars['RHEADER']=_TRANSFROM;
  if($page_id<201) $vars['GRPG']=($flag==1)?_FROMPG:_TOPG;
  else $vars['GRPG']=($flag==1)?_FROMGR:_TOGR;
  ...
  if($page_id>200) { $vars['FPG']=_FORGR.' \'<strong><i>'.$name.'</i></strong>\''; }
  ...
  if(!$result1) {$err->reason('vdb.php|trans|the request \''.$request.'\' has failed -- '.mysql_error());return;}
  ...
  if(mysql_num_rows($result)) {
      ...
      while($row=mysql_fetch_object($result)) {
          ...
          $rd='';
          if($page_id<201) { $vars['PGURL']=$row->url; $rd=$row->url; }
          $vars['VISITORS']=$row->v;
          $vars['HOSTS']=$row->hs;
          $vars['HITS']=$row->ht;
          $vars['RELOADS']=$row->r;
  ...
  if(mysql_num_rows($result)) {
      ...
      while($row=mysql_fetch_object($result)) {
          ...
          $vars['GRPG']=$fname;
          $vars['GRPGSHORT']=$sname;
          if($page_id<201) $vars['PGURL']=$row->url;
          $vars['TOTAL']=$row->v;
