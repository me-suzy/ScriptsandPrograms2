<?php
  ...
  $request='LOCK TABLES aa_points READ,aa_pages READ,aa_groups READ';
  $result=mysql_query($request,$conf->link);
  if(!$result) {$err->reason('vdb.php|points_grpg|blocking of tables has failed -- '.mysql_error());return;}
  if($page_id!=221) {
      //get list of pages which contained in group
      $emptyid=0;
      $mid=array();
      getpgs($page_id,$mid);
      if($err->flag) {$err->reason('vdb.php|points_grpg|\'getpgs\' function has failed');return;}
      $ids='';
      reset($mid);
      while($e=each($mid)) {
  ...
  if($page_id!=221) $request='SELECT '.$values.',aa_pages.id AS id,aa_pages.name AS name,aa_pages.url AS url FROM aa_points LEFT OUTER JOIN aa_pages ON aa_points.id=aa_pages.id WHERE aa_pages.id IN'.$ids.$where.' AND aa_points.flag='.$flag.' ORDER BY '.$ordert.' LIMIT '.$begstr.','.$numstr;
  else $request='SELECT '.$values.',aa_groups.id AS id,aa_groups.name AS name FROM aa_points LEFT OUTER JOIN aa_groups ON aa_points.id=aa_groups.id WHERE aa_groups.id IS NOT NULL AND aa_points.flag='.$flag.$where.' ORDER BY '.$ordert.' LIMIT '.$begstr.','.$numstr;
  $result=mysql_query($request,$conf->link);
  if(!$result) {$err->reason('vdb.php|points_grpg|the request \''.$request.'\' has failed -- '.mysql_error());return;}

  if($page_id!=221) $request='SELECT '.$minvalues.','.$maxvalues.',COUNT(*) AS nrect FROM aa_points WHERE id IN'.$ids.' AND flag='.$flag.$where;
  else $request='SELECT '.$minvalues.','.$maxvalues.',COUNT(*) AS nrect FROM aa_points WHERE id>200 AND flag='.$flag.$where;
  $resultt=mysql_query($request,$conf->link);
  if(!$resultt) {$err->reason('vdb.php|points_grpg|the request \''.$request.'\' has failed -- '.mysql_error());return;}

  if($page_id!=221) $request='SELECT '.$sumvalues.' FROM aa_points WHERE id='.$page_id.' AND flag='.$flag.$where;
  else $request='SELECT '.$sumvalues.' FROM aa_points WHERE id=201 AND flag='.$flag.$where;
  $results=mysql_query($request,$conf->link);
  if(!$results) {$err->reason('vdb.php|points_grpg|the request \''.$request.'\' has failed -- '.mysql_error());return;}

  $request='UNLOCK TABLES';
  $resultu=mysql_query($request,$conf->link);
  if(!$resultu) {$err->reason('vdb.php|points_grpg|the request \''.$request.'\' has failed -- '.mysql_error());return;}
  ...
  if($page_id<221) $vars['GRPG']=_PAGE;
  else $vars['GRPG']=_GROUP;
  if($nrect) $vars['RANGE']=($begstr+1).' - '.($begstr+$nrec).' '._OUTOF.' '.$nrect;
  else $vars['RANGE']='0 - 0 '._OUTOF.' '.$nrect;
  $vars['SHOWING']=_SHOWING.' '.$nrec.' '._ITEM_S;
  if($page_id<221) $vars['FPG']=_FORGR.' \'<strong><i>'.$name.'</i></strong>\'';
  else $vars['FPG']=_FORALLGRS;
  ...
  if(!$result1) {$err->reason('vdb.php|points_grpg|the request \''.$request.'\' has failed -- '.mysql_error());return;}
  ...
  if(mysql_num_rows($result)) {
      ...
      while($row=mysql_fetch_object($result)) {
          ...
          $rd='';
          if($page_id<221) { $vars['PGURL']=$row->url; $rd=$row->url; }
          $vars['VISITORS']=$row->v;
          $vars['HOSTS']=$row->hs;
          $vars['HITS']=$row->ht;
          $vars['RELOADS']=$row->r;
          ...
          $request='INSERT INTO aa_rdata (id,added,num,name,addpar,vi,vp,v,hsi,hsp,hs,ri,rp,r,hti,htp,ht) VALUES ('.$rdid.','.$conf->ctime.','.$rdnum.',"'.$fname.'","'.$rd.'",0,'.($vsumt?sprintf("%.2f",$row->v/$vsumt*100):'0.00').','.$row->v.',0,'.($hssumt?sprintf("%.2f",$row->hs/$hssumt*100):'0.00').','.$row->hs.',0,'.($rsumt?sprintf("%.2f",$row->r/$rsumt*100):'0.00').','.$row->r.',0,'.($htsumt?sprintf("%.2f",$row->ht/$htsumt*100):'0.00').','.$row->ht.')';
          $result1=mysql_query($request,$conf->link);
          if(!$result1) {$err->reason('vdb.php|points_grpg|the request \''.$request.'\' has failed -- '.mysql_error());return;}
          $rdnum++;
      }
  ...
  if(mysql_num_rows($result)) {
      ...
      while($row=mysql_fetch_object($result)) {
          ...
          $vars['GRPG']=$fname;
          $vars['GRPGSHORT']=$sname;
          if($page_id<221) $vars['PGURL']=$row->url;
          $vars['TOTAL']=$row->v;
