<?php
  ...
  $request='LOCK TABLES aa_frm_base READ,aa_frm_total READ';
  $result=mysql_query($request,$conf->link);
  if(!$result) {$err->reason('vdb.php|frames|blocking of tables has failed -- '.mysql_error());return;}

  $request='SELECT name,'.$values.' FROM aa_frm_total LEFT OUTER JOIN aa_frm_base ON aa_frm_total.frmid=aa_frm_base.frmid WHERE id='.$page_id.$where.' ORDER BY '.$ordert.' LIMIT '.$begstr.','.$numstr;
  $result=mysql_query($request,$conf->link);
  if(!$result) {$err->reason('vdb.php|frames|the request \''.$request.'\' has failed -- '.mysql_error());return;}

  $request='SELECT '.$minvalues.','.$maxvalues.','.$sumvalues.',COUNT(*) AS nrect FROM aa_frm_total WHERE id='.$page_id.$where;
  $resultt=mysql_query($request,$conf->link);
  if(!$resultt) {$err->reason('vdb.php|frames|the request \''.$request.'\' has failed -- '.mysql_error());return;}

  $request='UNLOCK TABLES';
  $resultu=mysql_query($request,$conf->link);
  if(!$resultu) {$err->reason('vdb.php|frames|the request \''.$request.'\' has failed -- '.mysql_error());return;}
  ...
  $vars['ITEM']=_FRAMEADDR;
  $vars['RHEADER']=_PGINFRAMES;
  ...
  if(!$result1) {$err->reason('vdb.php|frames|the request \''.$request.'\' has failed -- '.mysql_error());return;}
  ...
  if(mysql_num_rows($result)) {
      ...
      while($row=mysql_fetch_object($result)) {
          ...
          $request='INSERT INTO aa_rdata (id,added,num,name,addpar,vi,vp,v,hsi,hsp,hs,ri,rp,r,hti,htp,ht) VALUES ('.$rdid.','.$conf->ctime.','.$rdnum.',"'.$rd.'","",0,'.($vsumt?sprintf("%.2f",$row->v/$vsumt*100):'0.00').','.$row->v.',0,'.($hssumt?sprintf("%.2f",$row->hs/$hssumt*100):'0.00').','.$row->hs.',0,'.($rsumt?sprintf("%.2f",$row->r/$rsumt*100):'0.00').','.$row->r.',0,'.($htsumt?sprintf("%.2f",$row->ht/$htsumt*100):'0.00').','.$row->ht.')';
          $result1=mysql_query($request,$conf->link);
