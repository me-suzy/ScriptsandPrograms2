<?php
  ...
  if($sort['column']==1) $ordert='zoneid ASC';
  elseif(($sort['table']==1&&$sort['column']==2)||($sort['table']==2&&$sort['column']==2)) $ordert='v DESC,zoneid ASC';
  elseif(($sort['table']==1&&$sort['column']==3)||($sort['table']==3&&$sort['column']==2)) $ordert='hs DESC,zoneid ASC';
  elseif(($sort['table']==1&&$sort['column']==4)||($sort['table']==4&&$sort['column']==2)) $ordert='r DESC,zoneid ASC';
  elseif(($sort['table']==1&&$sort['column']==5)||($sort['table']==5&&$sort['column']==2)) $ordert='ht DESC,zoneid ASC';
  //total results
  $vsumt=0;
  $hssumt=0;
  $htsumt=0;
  ...
  $request='LOCK TABLES aa_zones READ';
  $result=mysql_query($request,$conf->link);
  if(!$result) {$err->reason('vdb.php|zones|blocking of tables has failed -- '.mysql_error());return;}

  $request='SELECT zoneid,'.$values.' FROM aa_zones WHERE id='.$page_id.$where.' ORDER BY '.$ordert.' LIMIT '.$begstr.','.$numstr;
  $result=mysql_query($request,$conf->link);
  if(!$result) {$err->reason('vdb.php|zones|the request \''.$request.'\' has failed -- '.mysql_error());return;}

  $request='SELECT '.$minvalues.','.$maxvalues.','.$sumvalues.',COUNT(*) AS nrect FROM aa_zones WHERE id='.$page_id.$where;
  $resultt=mysql_query($request,$conf->link);
  if(!$resultt) {$err->reason('vdb.php|zones|the request \''.$request.'\' has failed -- '.mysql_error());return;}

  $request='UNLOCK TABLES';
  $resultu=mysql_query($request,$conf->link);
  if(!$resultu) {$err->reason('vdb.php|zones|the request \''.$request.'\' has failed -- '.mysql_error());return;}
  ...
  while($row=mysql_fetch_object($resultt)) {
      ...
      else { $vmint=0; $hsmint=0; $htmint=0; $rmint=0; }
  }
  ...
  if(!$result1) {$err->reason('vdb.php|zones|the request \''.$request.'\' has failed -- '.mysql_error());return;}
