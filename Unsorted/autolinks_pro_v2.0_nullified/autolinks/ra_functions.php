...
/////////////////////////////////////////////////////////////
  ...
  function multicats( $login="_all_" )
  {
    if( $login=="_all_" )
	{
	  $res_cat = mysql_query( "SELECT * FROM al_cat WHERE name!='' AND selectable=1" );
  ...
  function checklogin( $activeonly=true )
  {
    // get session variables
	global $sesslogin, $sesspass, $REQUEST_URI;

	// must the referrer be fully active?
    if( $activeonly ) $status .= " AND status=1 "; 

    $res_ref = mysql_query( "SELECT * FROM al_ref WHERE login='$sesslogin' AND password='$sesspass' $status LIMIT 1" );
    if( mysql_num_rows($res_ref)==0 ) header( "Location: signin.php?from=$REQUEST_URI" );
	$ref = mysql_fetch_array( $res_ref );
  ...
  function shownotice( $notice )
  {
    if( isset($notice) )
	{
	  ...
	  echo( "<tr><td class='highlight'>$notice</td></tr>" );
  ...
  function showinfo( $message )
  {
    if( isset($message) )
	{
	  ...
	  echo( "<tr><td height='15'></td></tr></table>" );
	}
  }
  
  function showmenu( $currentpage )
  {
    global $CONF, $sitelogin, $ref, $PHPSESSID;
  
    echo( "<table width='100%' border='0' cellspacing='0' cellpadding='2' class='menu'><tr>" );
    echo( "<td>&nbsp;<a href='editinfo.php?PHPSESSID=$PHPSESSID' class='menu'>Edit Info</a></td>" );
	echo( "<td align='center'><a href='getcode.php?PHPSESSID=$PHPSESSID' class='menu'>Get Code</a></td>" );
	
	// check if we need advanced linking option
	$res_site = mysql_query( "SELECT * FROM al_site WHERE status=1 AND FIND_IN_SET('{$ref['category']}', categories)>0 ORDER BY name" );
	if( $CONF[link_banners] || $CONF[link_buttons] || $CONF[link_thumbs] || mysql_num_rows($res_site)>1 )
	{
	  echo( "<td align='center'><a href='advanced.php?PHPSESSID=$PHPSESSID' class='menu'>Advanced Linking</a></td>" );
    }
	 
	echo( "<td align='center'><a href='statistics.php?PHPSESSID=$PHPSESSID' class='menu'>Statistics</a></td>" );
    echo( "<td align='right'><a href='signout.php?PHPSESSID=$PHPSESSID' class='menu'>Sign out</a>&nbsp;</td>" );
	echo( "</tr></table><table width='100%' border='0' cellspacing='0' cellpadding='0' height='10'><tr><td></td></tr></table>" );
  }
  ...
  function showheader()
  {
	...
	echo( "<html><head><title>Referrers Area</title><style type='text/css'><!--\n" );
	
	echo( "body { font-family: {$STYLE[mainfontface]}; font-size: {$STYLE[mainfontsize]}; color: {$STYLE[mainfontcol]}; background-color: {$STYLE[bodybackcol]}; }\n" ); 
	echo( "table { font-family: {$STYLE[mainfontface]}; font-size: {$STYLE[mainfontsize]}; color: {$STYLE[mainfontcol]}; }\n" );
	echo( "input, select, textarea { font-family: {$STYLE[mainfontface]}; font-size: {$STYLE[mainfontsize]}; }\n" );
	echo( "a:link { color: {$STYLE[mainfontcol]}; }\n" );
	echo( "a:visited { color: {$STYLE[mainfontcol]}; }\n" );
	echo( "a:active { color: {$STYLE[mainfontcol]}; }\n" );
	echo( "a:hover { color: {$STYLE[mainfontcol]}; }\n" );
	echo( ".menu { background-color: {$STYLE[formfrontcol]}; }\n" );
	echo( "a.menu:link { text-decoration: none; color: {$STYLE[headerfontcol]}; font-weight: bold; }\n" );
	echo( "a.menu:visited { text-decoration: none; color: {$STYLE[headerfontcol]}; font-weight: bold; }\n" );
	echo( "a.menu:active { text-decoration: none; color: {$STYLE[headerfontcol]}; font-weight: bold; }\n" );
	echo( "a.menu:hover { text-decoration: none; color: {$STYLE[headerfontcol]}; font-weight: bold; }\n" );
	echo( ".formfront { background-color: {$STYLE[formfrontcol]}; color: {$STYLE[headerfontcol]}; text-transform: uppercase; font-size: 10px; }\n" );
	echo( ".formback { background-color: {$STYLE[formbackcol]}; }\n" );
	echo( ".area { background-color: {$STYLE[areabackcol]}; }\n" );
	echo( ".highlight { color: {$STYLE[highlightfontcol]}; }\n" );
	echo( "a.highlight:link { color: {$STYLE[highlightfontcol]}; }\n" );
	echo( "a.highlight:visited { color: {$STYLE[highlightfontcol]}; }\n" );
	echo( "a.highlight:active { color: {$STYLE[highlightfontcol]}; }\n" );
	echo( "a.highlight:hover { color: {$STYLE[highlightfontcol]}; }\n" );
	
	echo( "\n--></style><head><body leftmargin='25' topmargin='25' marginwidth='25' marginheight='25'>" );
  ...
  function showfooter()
  {
    ...
    // DO NOT REMOVE OR EDIT THE AUTOLINKS LINK BELOW -- If you do, this will void your AutoLinks license.
    echo( "<table width='100%' height='30' border='0' cellspacing='0' cellpadding='0'>" );
	echo( "<tr><td align='center' valign='bottom'>" );
    echo( "<!--CyKuH--><font size='1'><b>Powered by AutoLinks Pro 2.0</b></font>" );
    echo( "</td></tr></table>" );
	// DO NOT REMOVE OR EDIT THE AUTOLINKS LINK ABOVE -- If you do, this will void your AutoLinks license.
