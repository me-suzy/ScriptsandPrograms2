...
Generating keys and encode-decode actions 
for symbos less then ANSI 129 (English letters, special symbols and numbers)
...
This script was written for using with Hacker Hunter authentication system
http://www.polar-lights.com/hackerhunter/

Also it is distributed separately in the hope that it will be useful, but
...
srand ((double) microtime() * 1000000000000000);
...
function generate_keys ($show_debug=0){
	...
	/* $pi_ (we need it to calculate D and E) */
	$pi = ($p - 1) * ($q - 1);
...
/* standard fuction for calculating D
D = E-1 (mod N)
supposed that D must be found in less then 16 loops */
...
/* This function return Greatest Common Divisor for $e_ and $pi_ numbers */
function GCD($e,$pi) {
	$y = $e;
	$x = $pi;
	while ($y != 0) {
...
I think that it is the easiest examples around with binary RSA calculations */
function rsa_encrypt ($m, $e, $n) {

	/* Each letters in the message is represented as its ASCII code number - 30
	3 letters in each block with 1 in the beginning and end.
	For example string
	AAA
	will become
	13535351 (ansi number for A is 65 and 65-30=35)
	we can build such numbers because the least prime from table - 4507
	4507^2 = 20313049 
	it means that 
	1. modulo N will be always > 19999991
	2. letters with ansi number more then 129 will not be encoded
	*/
	$asci = array ();
...
function rsa_decrypt ($c, $d, $n) {
	...
	for ($z=0; $z< count($decryptarray); $z++) {
		$resultmod = powmod($decryptarray[$z], $d, $n);
		/*removing leading and trailing 1-s*/
		$deencrypt.= substr ($resultmod,1,strlen($resultmod)-2);
	}
	/*Each ASCII code number + 30 in the message is represented as its letter*/
	for ($x=0; $x<strlen($deencrypt); $x+=2) {
		$resultd .= chr(substr ($deencrypt, $x, 2) + 30);
	}
	return $resultd;
}
...
echo "Encoded: <b>$encoded</b><br>";
echo "Decoded: <b>$decoded</b><br>";
*/
?>