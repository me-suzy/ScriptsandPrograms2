...
if (empty($id) or empty($marker) or empty($section)) {header("Location: test_access_denied.php");}
...
if (($comments_registered==1) AND !isset($_COOKIE["website_member"])) {header("Location: test_access_denied.php");}
...
?>
	...
	<!-- articles start -->
		...
		<form name="add_comment" method="post" action="test_comments_insert.php">
          <tr>
		...
		echo  '<input type="hidden" name="marker" value="'.$marker.'">';
...
?>
                Add comment:<br>
                <textarea name="comment" cols="50" rows="8" class="formfields" id="comment"></textarea>
                <br>
                <br>
                <input type="submit" name="submit" value="Add comment ->" style="width: 100px; height: 24px;" class="formfields2">              
            </td>
          </tr>
		</form>
        </table>
