...
//
// Modified to be used with 100janCMS Content Management System
...
"justifynone": ['JustifyNone','Remove Alignment','editor_action(this.id)','ed_align_none.gif'], 
    ...
    "orderedlist":    ['InsertOrderedList',    'Number Ordered List',       'editor_action(this.id)',  'ed_list_num.gif'],
    ...
    "insertimage":    ['InsertImage',          'Insert External Image',       'editor_action(this.id)',  'ed_image.gif'],
    ...
    "popupeditor":    ['popupeditor',          'Fullscreen Editor',     'editor_action(this.id)',  'fullscreen_maximize.gif'],
    "about":          ['about',                'About this editor',  'editor_about(\''+objname+'\')',  'ed_about.gif'],
//Hyperlinks and Images 
	"insertlink": ['insertlink', 'Hyperlink in Text & Image', 'editor_action(this.id)', 'ed_link.gif'], 
	"unlink": ['Unlink', 'Remove Hyperlink', 'editor_action(this.id)', 'ed_unlink.gif'], 
...
"print":    	   ['Print',          'Print',         'editor_action(this.id)',   'ed_print.gif'],
    "100janinsertimage":           ['100janInsertImage',         'Insert Internal Image',  'editor_action(this.id)',  'ed_image_100jan.gif'],
...
function editor_action(button_id) {
  ...
  if (cmdID == 'popupeditor') {
    window.open(_editor_url + "popups/fullscreen.html?"+objname,
                'FullScreen',
//              'toolbar=no,location=no,directories=no,status=yes,menubar=no,scrollbars=yes,resizable=yes,width=1010,height=680');
                'fullscreen=yes');
    return;
  }
...
//***************************************************************
  ...
  // 100janinsertimage
    else if (cmdID == '100janInsertImage') {
      showModalDialog(_editor_url + "popups/insert_image_internal.php", editdoc, "resizable: no; help: no; status: no; scroll: no; ");
    }
...
function editor_updateToolbar(objname,action) {
  ...
  var IDList = Array('Bold','Italic','Underline','StrikeThrough','SubScript','SuperScript','JustifyLeft','JustifyCenter','JustifyRight','JustifyFull','JustifyNone','InsertOrderedList','InsertUnorderedList');
