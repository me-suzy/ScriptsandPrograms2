...
{ 
    ...
    { 
        ...
        // delete record
		$query="DELETE FROM ".$db_table_prefix."articles_category WHERE idCat=".$todelete[$i]; 
		mysql_query($query);
...
//ending message
$message = '
		<span class="maintext"><strong>&nbsp;&nbsp;Status:</strong> Category(s) has been deleted!</span>&nbsp;&nbsp;<img src="images/app/all_good.jpg" width="16" height="16">
		';
		$action = "true";
...
$text1='
<form name="delform" action="articles_cat_delete.php" method="post">
...
$text3='<span class="maintext">Are you sure you want to DELETE the following category(s)? <span class="maintext2"></span><br>
...
for ($i=0;$i<count($todelete);$i++) {
			...
			//title
			$query="SELECT * FROM ".$db_table_prefix."articles_category WHERE idCat=".$todelete[$i]." ORDER BY category"; 
			$result=mysql_query($query);
			$row = mysql_fetch_array($result); //wich row

			//
			$item_display_1=$row["category"];
...
{
			//title
			$query="SELECT * FROM ".$db_table_prefix."articles_category WHERE idCat=".$id." ORDER BY category"; 
			$result=mysql_query($query);
			$row = mysql_fetch_array($result); //wich row
