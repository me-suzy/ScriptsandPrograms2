...
{ 
    ...
    { 
        ...
        //delete record 
		$query="DELETE FROM ".$db_table_prefix."visitors WHERE idVis=".$todelete[$i]; 
		mysql_query($query);
...
$message = '
		<span class="maintext"><strong>&nbsp;&nbsp;Status:</strong> Visitor(s) has been deleted!</span>&nbsp;&nbsp;<img src="images/app/all_good.jpg" width="16" height="16">
		';
		$action = "true";
		echo'<meta http-equiv="Refresh" content="3; url=visitors_search.php">' ;
	}
	else 
	{			
//
...
$text1='
<form name="delform" action="visitors_delete.php" method="post">
...
$text3='<span class="maintext">Are you sure you want to delete the following visitor(s)? <span class="maintext2"></span><br>
...
for ($i=0;$i<count($todelete);$i++) {
			...
			//ititle
			$query="SELECT * FROM ".$db_table_prefix."visitors WHERE idVis=".$todelete[$i]." ORDER BY full_name"; 
...
{
			//title
			$query="SELECT * FROM ".$db_table_prefix."visitors WHERE idVis=".$id." ORDER BY full_name"; 
			$result=mysql_query($query);
			$row = mysql_fetch_array($result); //wich row
