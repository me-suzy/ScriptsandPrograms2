...
class Comments
{
	function Comments()
	{
		...
		/* -- congfiguration -- */
		$this->conf_file = OUT_FOLDER."comment_vb3.php";
		$this->data_file = OUT_FOLDER."comment_vb3_data.php";
	}
	...
	function process_addarticle($artlink,$category,$subject,$artid)
	{
		...
		$vdb = new sqldb;
		$vdb->scriptname = "vBulletin 3 Comments System";
		//$vdb->report = 1;
		...
		if ($vdb->report_error == '')
		{
			...
			$vdb->query("INSERT INTO thread 
							(title,lastpost,forumid,open,replycount,postusername,postuserid,lastposter,dateline,iconid,visible,attach)
						VALUES							('".$subject."',".$time.",".$cat.",1,0,'".$forumuser['username']."',$config[userid],'".$forumuser['username']."',".$time.",$config[icons],1,0)
			");
			
			$threadid = $vdb->insert_id();

			$vdb->query("INSERT INTO post (threadid,parentid,title,username,userid,dateline,pagetext,allowsmilie,showsignature,ipaddress,iconid,visible) VALUES ($threadid,0,'".$subject."','".$forumuser['username']."',$config[userid],$time,'".$message."',1,1,'',1,1)");
		
			$vdb->query("UPDATE user SET posts=posts+1,lastvisit='".$time."',lastactivity='".$time."',lastpost='".$time."' WHERE userid='".$config['userid']."'");

			$vdb->query("UPDATE forum SET replycount=replycount+1,threadcount=threadcount+1,lastpost='".$time."',lastposter='".$forumuser['username']."',lastthread='".$subject."',lastthreadid='$threadid',lasticonid='".$config['icons']."' WHERE forumid='".$forum['parentlist']."'");
	...
	function manage()
	{
		...
		$admin->row_width = "30%";
		$content .= "Please check your vBulletin 3 config file if you are not sure about these database settings. <br /><br />";
