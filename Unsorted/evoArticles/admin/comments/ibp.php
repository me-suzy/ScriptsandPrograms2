...
class Comments
{
	function Comments()
	{
		...
		/* -- congfiguration -- */
		$this->conf_file = OUT_FOLDER."comment_ibp.php";
		$this->data_file = OUT_FOLDER."comment_ibp_data.php";
	...
	function process_addarticle($artlink,$category,$subject,$artid)
	{
		...
		$vdb = new sqldb;
		$vdb->scriptname = "Invision Power Board Comments System";
		//$vdb->report = 1;
		...
		if ($vdb->report_error == '')
		{
			...
			$forumuser = $vdb->query_once("SELECT * FROM ibf_members WHERE id=".$config['userid']);
			//$forum = $vdb->query_once("SELECT * FROM forum WHERE forumid='".$cat."'");
			
			$subject = addslashes($subject);
			$forumuser['name'] = addslashes($forumuser['name']);
			$message = addslashes($message);

			//echo $vdb->report_error;
			$vdb->query("INSERT INTO ibf_topics (title,description,state,posts,starter_id,starter_name,start_date,last_poster_id,last_poster_name,last_post,icon_id,author_mode,poll_state,last_vote,views,forum_id,approved,pinned) VALUES ('".$subject."','','open','0','".$config['userid']."','".$forumuser['name']."','".$time."','".$config['userid']."','".$forumuser['name']."','".$time."','0','1','0','0','0','".$cat."','1','0')");
			
			$threadid = $vdb->insert_id();

			$vdb->query("INSERT INTO ibf_posts 
					SET author_id = '".$config['userid']."',
						use_sig  = '1',
						use_emo  = '1',
						ip_address = '',
						post_date = '".$time."',
			...
			$vdb->query("UPDATE ibf_members SET posts=posts+1,last_post='".$time."' WHERE id='".$config['userid']."'");

			$vdb->query("UPDATE ibf_forums SET last_title='".$subject."',last_id='".$threadid."',last_post='".$time."',last_poster_name='".$forumuser['name']."',last_poster_id='".$config['userid']."',posts=posts+1,topics=topics+1 WHERE id='".$cat."'");

			$vdb->query("UPDATE ibf_stats SET TOTAL_TOPICS=TOTAL_TOPICS+1");
	...
	function write_data($artid,$threadid)
	{
		global $admin;
		//thread url
		$thread_file = "index.php?showtopic=".$threadid;
	...
	function manage()
	{
		...
		$admin->row_width = "30%";
		$content .= "Please check your Invision Power Board config file if you are not sure about these database settings. <br /><br />";
		
		$content .= $admin->form_start("",$_SERVER['PHP_SELF']);
		$table .= $admin->add_spacer("Database Configuration");
		$table .= $admin->add_row("Invision Power Board's mySQL Host",$admin->form_input("opt[db_host]",$config['db_host']));
		$table .= $admin->add_row("Invision Power Board's mySQL Username",$admin->form_input("opt[db_user]",$config['db_user']));
		$table .= $admin->add_row("Invision Power Board's mySQL Password",$admin->form_input("opt[db_pass]",$config['db_pass']));
		$table .= $admin->add_row("Invision Power Board's mySQL Database Name",$admin->form_input("opt[db_name]",$config['db_name']));
		...
		if ($this->loaded)
		{
			...
			else
			{
				...
				$ctable .= $admin->add_row("Invision Power Board URL",$admin->form_input("conf[url]",$config['url']),"No trailing slash");
