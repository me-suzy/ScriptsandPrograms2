...
class Comments
{
	function Comments()
	{
		...
		/* -- congfiguration -- */
		$this->conf_file = OUT_FOLDER."comment_phpbb2.php";
		$this->data_file = OUT_FOLDER."comment_phpbb2_data.php";
	}
	...
	function process_addarticle($artlink,$category,$subject,$artid)
	{
		...
		$vdb = new sqldb;
		$vdb->scriptname = "phpBB 2 Comments System";
		//$vdb->report = 1;
		...
		if ($vdb->report_error == '')
		{
			...
			$forumuser = $vdb->query_once("SELECT * FROM $config[db_num]users WHERE user_id=".$config['userid']);
			...
			$vdb->query("INSERT INTO $config[db_num]topics 
							(topic_title,topic_poster,topic_time,forum_id,topic_status,topic_type,topic_vote)
					VALUES
							('$subject','".$config['userid']."','$time',$cat,'0','0','0')
						");
			
			$threadid = $vdb->insert_id();

			$vdb->query("INSERT INTO $config[db_num]posts
						(topic_id,forum_id,poster_id,post_username,post_time,poster_ip,enable_bbcode,enable_html,enable_smilies,enable_sig)
							
						VALUES
						
						($threadid,$cat,'".$config['userid']."','$forumuser[username]','$time','','1','1','1','1')
						");
			$postid = $vdb->insert_id();
			
			$vdb->query("INSERT INTO $config[db_num]posts_text 
			...
			$vdb->query("UPDATE $config[db_num]forums SET forum_posts=forum_posts+1,forum_topics=forum_topics+1,forum_last_post_id='$postid' WHERE forum_id='".$cat."'");

			$vdb->query("UPDATE $config[db_num]topics SET topic_first_post_id='$postid',topic_last_post_id='$postid' WHERE topic_id='$threadid'");

			$vdb->query("UPDATE $config[db_num]users SET user_posts=user_posts+1 WHERE user_id='$config[userid]'");
	...
	function write_data($artid,$threadid)
	{
		global $admin;
		//thread url
		$thread_file = "viewtopic.php?t=".$threadid;
	...
	function manage()
	{
		...
		$admin->row_width = "30%";
		$content .= "Please check your phpBB 2 config file if you are not sure about these database settings. <br /><br />";
		
		$content .= $admin->form_start("",$_SERVER['PHP_SELF']);
		$table .= $admin->add_spacer("Database Configuration");
		$table .= $admin->add_row("phpBB 2's mySQL Host",$admin->form_input("opt[db_host]",$config['db_host']));
		$table .= $admin->add_row("phpBB 2's mySQL Username",$admin->form_input("opt[db_user]",$config['db_user']));
		$table .= $admin->add_row("phpBB 2's mySQL Password",$admin->form_input("opt[db_pass]",$config['db_pass']));
		$table .= $admin->add_row("phpBB 2's mySQL Database Name",$admin->form_input("opt[db_name]",$config['db_name']));
		$table .= $admin->add_row("phpBB 2's Table Prefix",$admin->form_input("opt[db_num]",$config['db_num'] != '' ? $config['db_num']:'phpbb_'),"Leave \"phpbb_\" as default if you are unsure.");
		...
		if ($this->loaded)
		{
			...
			else
			{
				...
				$ctable .= $admin->add_row("phpBB 2 URL",$admin->form_input("conf[url]",$config['url']),"No trailing slash");
