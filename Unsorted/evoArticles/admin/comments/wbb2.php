...
class Comments
{
	function Comments()
	{
		...
		/* -- congfiguration -- */
		$this->conf_file = OUT_FOLDER."comment_wbb2.php";
		$this->data_file = OUT_FOLDER."comment_wbb2_data.php";
	}
	...
	function process_addarticle($artlink,$category,$subject,$artid)
	{
		...
		$vdb = new sqldb;
		$vdb->scriptname = "Woltlab Burning Board 2 Comments System";
		//$vdb->report = 1;
		...
		if ($vdb->report_error == '')
		{
			...
			$forumuser = $vdb->query_once("SELECT * FROM bb$config[db_num]_users WHERE userid=".$config['userid']);
			$forum = $vdb->query_once("SELECT * FROM bb$config[db_num]_boards WHERE boardid='".$cat."'");
			...
			$vdb->query("INSERT INTO bb$config[db_num]_threads (threadid,boardid,prefix,iconid,starttime,starterid,starter,lastposttime,lastposterid,lastposter,attachments,pollid,important,visible,topic) VALUES ('','".$cat."','','0','".$time."','".$config['userid']."','".$forumuser['username']."','".$time."','".$config['userid']."','".$forumuser['username']."','0','0','0','1','$subject')");
			
			$threadid = $vdb->insert_id();

			$vdb->query("INSERT INTO bb$config[db_num]_posts (postid,threadid,userid,username,iconid,posttopic,posttime,message,attachmentid,allowsmilies,showsignature,ipaddress,visible) VALUES ('','".$threadid."','".$config['userid']."','".$forumuser['username']."','0','".$subject."','".$time."','".$message."','0','1','1','','1')");
			
			$vdb->query("UPDATE bb$config[db_num]_users SET userposts=userposts+1 WHERE userid='".$config['userid']."'");

			$vdb->query("UPDATE bb$config[db_num]_boards SET threadcount=threadcount+1, postcount=postcount+1, lastthreadid='$threadid', lastposttime='$time',lastposterid='$config[userid]',lastposter='$forumuser[username]' WHERE boardid IN ($forum[parentlist],$cat)");
	...
	function manage()
	{
		...
		$admin->row_width = "30%";
		$content .= "Please check your Woltlab Burning Board 2 config file if you are not sure about these database settings. <br /><br />";
		
		$content .= $admin->form_start("",$_SERVER['PHP_SELF']);
		$table .= $admin->add_spacer("Database Configuration");
		$table .= $admin->add_row("Woltlab Burning Board 2's mySQL Host",$admin->form_input("opt[db_host]",$config['db_host']));
		$table .= $admin->add_row("Woltlab Burning Board 2's mySQL Username",$admin->form_input("opt[db_user]",$config['db_user']));
		$table .= $admin->add_row("Woltlab Burning Board 2's mySQL Password",$admin->form_input("opt[db_pass]",$config['db_pass']));
		$table .= $admin->add_row("Woltlab Burning Board 2's mySQL Database Name",$admin->form_input("opt[db_name]",$config['db_name']));
		$table .= $admin->add_row("Woltlab Burning Board 2's Board Number",$admin->form_input("opt[db_num]",$config['db_num'] != '' ? $config['db_num']:'1'),"Leave \"1\" as default if you are unsure.");
		...
		if ($this->loaded)
		{
			...
			else
			{
				...
				/*$icon_sql = $vdb->query("SELECT * FROM bb$config[db_num]_icons");
				$posticons .= "0|No Icon,";
				while ($irow = $vdb->fetch_array($icon_sql))
				{
					$posticons .= $irow['iconid']."|".$irow['icontitle'].",";
					if ($irow['iconid'] == $config['icons'])
					{
						$image = " <img src=\"".$config['url']."/".$irow['iconpath']."\" alt=\"$irow[icontitle]\" />";
					}
				}
				...
				$ctable .= $admin->add_row("Woltlab Burning Board 2 URL",$admin->form_input("conf[url]",$config['url']),"No trailing slash");
