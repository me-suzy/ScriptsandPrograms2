...
if ($_POST['submitted'] == "true") {
	...
	function ban_file($filename) {
		...
		$now = date("U");
		$add_log	= mysql_query("INSERT INTO $config->logs VALUES ('', '$now', '".$_SERVER['REMOTE_ADDR']."', '".$_SESSION['uid']."', 'import ban', 'imported bans: $j')") or die (mysql_error());
		return $import_array;
	}
...
if (isset($acceptable_file_types) && trim($acceptable_file_types)) {
	$submit = "This form only accepts <b>".str_replace("|", " or ", $acceptable_file_types)."</b> files &nbsp;&nbsp;<input type='submit' name='importit' value=' import ' style='font-family: verdana, tahoma, arial; font-size: 10px;'>";
} else {
	$submit = "No acceptable filetypes set.&nbsp;&nbsp;<input type='submit' name='importit' value=' import ' style='font-family: verdana, tahoma, arial; font-size: 10px;' disabled>";
}
...
$smarty->assign("import",$import_array);
