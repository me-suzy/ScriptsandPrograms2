...
if (($sektion == "levels") && ($_SESSION['permissions_edit'] == "yes")) {
	...
	if ($action == " add ") {
		...
		$now = date("U");
		$add_log	= mysql_query("INSERT INTO $config->logs VALUES ('', '$now', '".$_SERVER['REMOTE_ADDR']."', '".$_SESSION['uid']."', 'lvl management', 'Added new level')") or die (mysql_error());
		...
		if ($lvls->get_lvls == 0) {
			...
			$now = date("U");
			$add_log	= mysql_query("INSERT INTO $config->logs VALUES ('', '$now', '".$_SERVER['REMOTE_ADDR']."', '".$_SESSION['uid']."', 'lvl management', 'Removed level ".$_POST['ex_lvl']."')") or die (mysql_error());

		} else {
			echo "Some admins are using this level. Make sure no admin(s) are using this level before trying to remove it.";

			$now = date("U");
			$add_log	= mysql_query("INSERT INTO $config->logs VALUES ('', '$now', '".$_SERVER['REMOTE_ADDR']."', '".$_SESSION['uid']."', 'lvl management', 'Attempted to remove level ".$_POST['ex_lvl']." (level still used)')") or die (mysql_error());
		}
...
/*
		...
		$now = date("U");
		$add_log	= mysql_query("INSERT INTO $config->logs VALUES ('', '$now', '".$_SERVER['REMOTE_ADDR']."', '".$_SESSION['uid']."', 'lvl management', 'Updated one or more levels')") or die (mysql_error());
	}
	...
	if ($action == " remove ") {
		...
		$now = date("U");
		$add_log	= mysql_query("INSERT INTO $config->logs VALUES ('', '$now', '".$_SERVER['REMOTE_ADDR']."', '".$_SESSION['uid']."', 'webadmins management', 'Removed admin ".$_POST['username']."')") or die (mysql_error());
		...
		$now = date("U");
		$add_log	= mysql_query("INSERT INTO $config->logs VALUES ('', '$now', '".$_SERVER['REMOTE_ADDR']."', '".$_SESSION['uid']."', 'webadmins management', 'Edited admin ".$_POST['username']."')") or die (mysql_error());

	} else if ($action == " insert ") {
		//display_post_get();
		$username	= htmlentities($_POST['username'], ENT_QUOTES); 
		$password = md5($_POST['password']);
		$resource	= mysql_query("INSERT INTO $config->webadmins VALUES('', '$username', '$password', '".$_POST['level']."', '')") or die (mysql_error());

		$now = date("U");
		$add_log	= mysql_query("INSERT INTO $config->logs VALUES ('', '$now', '".$_SERVER['REMOTE_ADDR']."', '".$_SESSION['uid']."', 'webadmins management', 'Added admin $username')") or die (mysql_error());
	}
	...
	if ($action == " remove ") {
		...
		$now = date("U");
		$add_log	= mysql_query("INSERT INTO $config->logs VALUES ('', '$now', '".$_SERVER['REMOTE_ADDR']."', '".$_SESSION['uid']."', 'amxadmins management', 'Removed admin ".$_POST['username']."')") or die (mysql_error());

	} else if ($action == " apply ") {
		$username		= htmlentities($_POST['username'], ENT_QUOTES); 
		$password		= htmlentities($_POST['password'], ENT_QUOTES);
		$nickname		= htmlentities($_POST['nickname'], ENT_QUOTES);
		$resource2	= mysql_query("UPDATE $config->amxadmins SET username = '$username', password = '$password', access = '".$_POST['access']."', flags = '".$_POST['flags']."', steamid = '".$_POST['steamid']."', nickname = '$nickname' WHERE id = '".$_POST['id']."'") or die (mysql_error());

		$now = date("U");
		$add_log	= mysql_query("INSERT INTO $config->logs VALUES ('', '$now', '".$_SERVER['REMOTE_ADDR']."', '".$_SESSION['uid']."', 'amxadmins management', 'Edited admin $username')") or die (mysql_error());

	} else if ($action == " insert ") {
		$username		= htmlentities($_POST['username'], ENT_QUOTES); 
		$password		= htmlentities($_POST['password'], ENT_QUOTES);
		$nickname		= htmlentities($_POST['nickname'], ENT_QUOTES);
		$resource2	= mysql_query("INSERT INTO $config->amxadmins VALUES('', '$username', '$password', '".$_POST['access']."', '".$_POST['flags']."', '".$_POST['steamid']."', '$nickname')") or die (mysql_error());

		$now = date("U");
		$add_log	= mysql_query("INSERT INTO $config->logs VALUES ('', '$now', '".$_SERVER['REMOTE_ADDR']."', '".$_SESSION['uid']."', 'amxadmins management', 'Added admin $username')") or die (mysql_error());
	}

	//get all amxadmins
	$resource = mysql_query("SELECT id, username, password, access, flags, steamid, nickname FROM $config->amxadmins ORDER BY username ASC") or die (mysql_error());
...
$smarty->assign("level",$level_array);
$smarty->assign("existing_levels",$existing_levels);
$smarty->assign("available_levels",$available_levels);
$smarty->assign("webadmin",$webadmins_array);
$smarty->assign("amxadmin",$amxadmins_array);
