...
if (isset($_POST['action'])) {
	...
	if ($_POST['action'] == "delete") {
		...
		$add_log		= mysql_query("INSERT INTO $config->logs VALUES ('', '$now', '".$_SERVER['REMOTE_ADDR']."', '".$_SESSION['uid']."', 'delete ban', 'Ban with BanID ".$_POST['bid']." deleted')") or die (mysql_error());
			...
			if(mysql_num_rows($resource) == 0) {
				...
				if(!empty($result->player_ip)) {
					if((isset($_SESSION["user_authenticated"]) && $_SESSION["user_authenticated"] == TRUE) && ($_SESSION["user_level"] == "OWNER" || $_SESSION["user_level"] == "ADMIN")) {
						...
						$player_ip = "<i><font color='#677882'>hidden</font></i>";
					}
				} else {
					$player_ip = "<i><font color='#677882'>Not applicable</font></i>";
				}
		...
		$now = date("U");
		$add_log	= mysql_query("INSERT INTO $config->logs VALUES ('', '$now', '".$_SERVER['REMOTE_ADDR']."', '".$_SESSION['uid']."', 'edit ban', 'Ban with BanID ".$_POST['bid']." edited')") or die (mysql_error());
		...
		while ($myban	= mysql_fetch_array($list_ban)) {
			...
			$insert_ban = mysql_query("INSERT INTO $config->ban_history VALUES ('', '$myban[player_ip]', '$myban[player_id]', '$myban[player_nick]', '$myban[admin_ip]', '$myban[admin_id]', '$myban[admin_nick]', '$myban[ban_type]', '$myban[ban_reason]', '$myban[ban_created]', '$myban[ban_length]', '$myban[server_ip]', '$myban[server_name]', '$unban_created', '".$_POST['unban_reason']."', '".$_SESSION['uid']."')") or die (mysql_error());
			$remove_ban = mysql_query("DELETE FROM $config->bans WHERE bid = '".$_POST['bid']."'") or die (mysql_error());

			$now = date("U");
			$add_log	= mysql_query("INSERT INTO $config->logs VALUES ('', '$now', '".$_SERVER['REMOTE_ADDR']."', '".$_SESSION['uid']."', 'unban ban', 'Ban with BanID ".$_POST['bid']." unbanned (SteamID $myban[player_id])')") or die (mysql_error());
		}
