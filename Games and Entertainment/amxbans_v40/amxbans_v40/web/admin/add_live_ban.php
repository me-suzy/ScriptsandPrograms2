...
if ((isset($_POST['submit'])) && ($_POST['submit'] == " kickban ")) {
	...
	// ban! ban! ban!
	$insert_ban		= mysql_query("INSERT INTO $config->bans VALUES ('', '".$_POST['player_ip']."', '".$_POST['player_id']."', '".$_POST['player_nick']."', '".$_SERVER["REMOTE_ADDR"]."', '$admin_id', '".$_SESSION['uid']."', '".$_POST['ban_type']."', '".$_POST['ban_reason']."', '$ban_created', '".$_POST['ban_length']."', '', '$server_name')") or die (mysql_error());
...
if ($_POST['live_player_ban'] == "true") {

	//create array of admin steamIDs
	$resource = mysql_query("SELECT DISTINCT steamid FROM $config->amxadmins") or die (mysql_error());
	...
	foreach ($lists as $list) {
		...
		if (in_array($list_2['1'], $admin_steamids_array)) {
			$is_admin = 1;
		} else {
			$is_admin = 0;
		}
...
$smarty->assign("live_player_ban", $_POST['live_player_ban']);
