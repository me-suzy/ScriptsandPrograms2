...
*/
class SSView extends SSTableObject
{
	...
	*/
	function _addProperties () {
		...
		$this->_addProperty ('view_date', 0);
		$this->_addProperty ('view_ip', 0);
	...
	function getDBKeyValueArray ($includeDBKey) {
		...
		$fields = array
				(
					...
					$GLOBALS[$tableConstant]['fields']['VIEW_DATE'] => $this->get ('view_date'),
					$GLOBALS[$tableConstant]['fields']['VIEW_IP'] => $this->get ('view_ip'),
	...
	*/
	function requiredFieldsValid ($checkKey = false) {
		...
		if ($this->get('target_type') == '') {
			$this->addError (STR_248, ERROR_TYPE_SERIOUS);
			$invalidField = true;
		}
		if ($this->get('target_id') == '') {
			$this->addError (STR_249, ERROR_TYPE_SERIOUS);
			$invalidField = true;
		}
		if ($this->get('view_date') == '') {
			$this->addError (STR_250, ERROR_TYPE_SERIOUS);
			$invalidField = true;
		}
		
		if ($checkKey) {
			if ($this->get (PROP_ID) == '') {
				$this->addError (STR_251, ERROR_TYPE_SERIOUS);
				$invalidField = true;
			}
		}
