...
class SSBBCode extends SSObject
{
	var $tags;
	var $settings;
	...
	function SSBBCode(){
		...
		$this->add_tag(array('Name'=>'fork','HtmlBegin'=>'<a href="'.READING_ROOT.'/read.php?t=3&i=%%P%%">','HtmlEnd'=>'</a>','HasParam'=>true,'ParamRegex'=>'[0-9]+'));		
	}
	...
	function onparam($param,$regexarray){
		...
		$param = $this->_replace_pcre_array($param,$regexarray);
	...
	function add_tag($params){
		...
		if(!array_key_exists('HtmlEnd',$params)){
			 ...
			 if(preg_match('/^(<[A-Za-z]>)+$/',$params['HtmlBegin'])){
			 	...
			 	$params['HtmlEnd'] = $this->_begtoend($params['HtmlBegin']);
			 }else{
	...
	function _begtoend($htmltag){
		
		return preg_replace('/<([A-Za-z]+)>/','</$1>',$htmltag);
	}
	
	function _replace_pcre_array($text,$array) {
