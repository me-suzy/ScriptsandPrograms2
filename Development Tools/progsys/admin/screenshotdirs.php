...
$page_title=$l_screenshots." - ".$l_directories;
...
?>
<table align="center" width="100%" border="0" cellspacing="1" cellpadding="1">
<tr><td class="headingrow" align="center" colspan="2"><b><?php echo $l_newdirectory?></b></td></tr>
...
<?php
		$sql="select * from ".$tableprefix."_programm prog, ".$tableprefix."_programm_admins pa where pa.prognr=prog.prognr and pa.usernr=$act_usernr order by prog.prognr asc";
		if(!$result = mysql_query($sql, $db))
			die("<tr class=\"errorrow\"><td>Could not connect to the database.".mysql_error());
		while($myrow = mysql_fetch_array($result))
		{
			echo "<option value=\"".$myrow["prognr"]."\">";
			echo htmlentities($myrow["programmname"])." [".$myrow["language"]."]";
			echo "</option>";
		}
?>
</select>
</td></tr>
<tr class="inputrow"><td align="right" width="30%"><?php echo $l_picdir?>:</td><td><input class="psysinput" type="text" name="picdir" size="40" maxlength="255"></td></tr>
<tr class="inputrow"><td align="right" width="30%"><?php echo $l_thumbdir?>:</td><td><input class="psysinput" type="text" name="thumbdir" size="40" maxlength="255"></td></tr>
<tr class="inputrow" valign="top"><td align="right"><?php echo $l_addheader?>:</td><td><textarea class="psysinput" name="addheader" rows="8" cols="40"></textarea></td></tr>
...
<a href="<?php echo do_url_session("$act_script_url?mode=new&lang=$lang")?>"><?php echo $l_newdirectory?></a>
</table></td></tr></table>
<table align="center" width="80%" CELLPADDING="1" CELLSPACING="0" border="0" valign="top">
<tr><TD BGCOLOR="#000000">
<table align="center" width="100%" border="0" cellspacing="1" cellpadding="1">
...
$sql = "select * from ".$tableprefix."_screenshotdirs dirs, ".$tableprefix."_programm prog where prog.prognr=dirs.program order by prog.prognr";
if(!$result = mysql_query($sql, $db))
	die("<tr class=\"errorrow\"><td>Could not connect to the database.");
if (!$myrow = mysql_fetch_array($result))
{
...
{
?>
<div class="bottombox" align="center"><a href="<?php echo do_url_session("$act_script_url?mode=new&lang=$lang")?>"><?php echo $l_newdirectory?></a></div>
<?php
}
}
include('trailer.php');
