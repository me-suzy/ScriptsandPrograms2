...
<b><?php echo $l_top10requests?></b></td></tr>
<?php
do {
	$progname=htmlentities($myrow["programmname"]);
	$proglang=$myrow["language"];
	$prognr=$myrow["prognr"];
	$datasql="select * from ".$tableprefix."_feature_requests where programm=$prognr and ratingcount>0 order by (rating/ratingcount) desc limit 10";
	...
	if ($datarow = mysql_fetch_array($dataresult))
	{
		...
		do {
			echo "<tr class=\"displayrow\"><td align=\"center\">";
			echo "<a class=\"listlink\" href=\"".do_url_session("requests.php?lang=$lang&mode=display&input_requestnr=".$datarow["requestnr"])."\">";
			echo $datarow["requestnr"];
			echo "</a></td><td align=\"center\">";
			echo $datarow["request"];
			echo "</td>";
...
<tr class="inforow"><td align="center" colspan="3">
<b><?php echo $l_last10requests?></b></td></tr>
<?php
$sql = "select * from ".$tableprefix."_programm order by language, prognr";
if(!$result = mysql_query($sql, $db))
    die("Could not connect to the database.");
while($myrow = mysql_fetch_array($result))
{
	...
	$proglang=$myrow["language"];
	$prognr=$myrow["prognr"];
	$datasql="select * from ".$tableprefix."_feature_requests where programm=$prognr and ratingcount>0 order by (rating/ratingcount) asc limit 10";
	if(!$dataresult = mysql_query($datasql, $db))
	    die("<tr class=\"errorrow\"><td colspan=\"3\" align=\"center\">Could not connect to the database.".mysql_error());
	if ($datarow = mysql_fetch_array($dataresult))
	{
