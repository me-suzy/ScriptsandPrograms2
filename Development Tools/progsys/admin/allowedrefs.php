...
<?php
		...
		if($errors==0)
		{
			$sql = "INSERT INTO ".$tableprefix."_allowed_referers (address) ";
			$sql .="VALUES ('$address')";
	...
	if(isset($delete))
	{
		$deleteSQL = "delete from ".$tableprefix."_allowed_referers where (entrynr=$input_entrynr)";
	...
	if(isset($update))
	{
		...
		if($errors==0)
		{
			$sql = "UPDATE ".$tableprefix."_allowed_referers SET address='$address' ";
...
$sql = "select * from ".$tableprefix."_allowed_referers order by address asc";
if(!$result = mysql_query($sql, $db)) {
