  ...
  function query($sql = "", $dbCon =0) {
    ...
    if ($this->max_rows) {
     ...
     // $qpos = " limit ".$this->pos.", ".$this->max_rows ; PL oposite of MySQL
      $qpos = " limit ".$this->max_rows.", ".$this->pos ;
    } else {
      if (!$this->pos) { $this->pos = "" ; }
      $qpos = $this->pos;
    }
//    echo "$this->sql_query $this->sql_order $qpos ";
    $rquery = pg_Exec($this->dbCon->id,"$this->sql_query $this->sql_order $qpos");
        ...
        if (eregi("^alter", $this->sql_query)
         ...
         || eregi("^drop", $this->sql_query)) {
            if ($this->dbCon->getUseDatabase()) {
                ...
                $rquery = pg_Exec($this->dbCon->db,$qInsSync, $this->dbCon->id);
            } else {
  ...
  function getInsertId($table="",  $field="") {
    ...
    if (strlen($table) > 0 && strlen($field) > 0) {
        ...
        $rquery = pg_Exec($this->dbCon->id, "SELECT currval('".$sequence."')") ;
  ...
  function getTableField($table="") {
    ...
    if (is_array($atable)) {
      ...
      while(list($key, $table) = each($atable)) {
        ...
        $table_def = pg_exec($this->dbCon->id, $q_getfield) ;
  ...
  function getTables($dbc=0) {
    ...
    $result = pg_exec($dbc->id, $q_getTables) ;
    $this->result = $result ;
    $this->cursor = 0 ;
    $this->num_rows = pg_numrows($result) ;
    return $result ;
  ...
  function getDatabases($dbc=0) {
    ...
    $q_getDatabase = "
       ...
       ORDER BY \"Database\"
";
    $result = pg_exec($dbc->id, $q_getDatabase);
    $this->result = $result ;
    $this->cursor = 0 ;
    $this->num_rows = pg_numrows($result) ;
    return $result ;
  ...
  function createDatabase($name) {
    $b_success = pg_exec("CREATE DATABASE $name");
    return $b_success;
  }
