...
class PntPropertyDescriptorsTest extends PntTestCase {
	...
	function test_getSetDerived1Value() 
	{
		...
		$result =& $prop->_getValueFor( $value, $this->obj1 );
		// assume PntError
		$this->assertEquals(
		...
		// assume PntError
		$this->assertEquals(
			'TestPropsObject>>derived1 attempt to set value on readOnly property'
			, $result->getlabel()
	...
	function test_getSetDerived2Value() 
	{
		...
		// assume PntError
		$this->assertEquals(
			'TestPropsObject>>derived2 Unable to derive value: no getter and no id-property'
			, $result->getlabel()
			, 'derived2 getValueFor error message'
		);
	...
	function test_getSetMulti1Value() 
	{
		...
		// assume PntError
		$this->assertEquals(
			'TestPropsObject>>multi1 attempt to set value on readOnly property'
			, $result->getlabel()
